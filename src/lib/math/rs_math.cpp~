...
// x^3 + ce[0] x^2 + ce[1] x + ce[2] = 0
{
    ...
    // depressed cubic, Tschirnhaus transformation, x= t - b/3
    // t^3 + p t +q =0
    ...
    if(discriminant>0) {
        ...
        if (! ret ) { //should not happen
            std::cerr<<"RS_Math::cubicSolver()::Error cubicSolver("<<ce[0]<<' '<<ce[1]<<' '<<ce[2]<<")\n";
        }
        ret=1;
        double u,v;
...
// x^4 + ce[0] x^3 + ce[1] x^2 + ce[2] x + ce[3] = 0
{
    ...
    // depressed quartic transformed to two quadratic equations
    ...
    if (ret==1) { //one real root from cubic
        if (croots[0]< 0.) {//this should not happen
            std::cerr<<"RS_Math::quarticSolver():: Found one real root for cubic, but negative. This should not happen.\n";
            return 0;
        }
