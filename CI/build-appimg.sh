...
convert -resize 256x256 CI/librecad.svg appdir/usr/share/icons/hicolor/256x256/apps/librecad.png
	for q_lib in libLLVM-15 libOpenGL libelf libxcb libsensors libQt6 libQt6Core5Compat libQt6QuickControls2 libQt6StateMachine libQt6WebView libQt6Xml libQt6Svg libQt6DBus libQt6WaylandClient libQt6WaylandEglClientHwIntegration libxcb
	do
	for f in $(find .. /usr/lib64 /lib64 -iname "${q_lib}*.so*" -print)
	do
