...
#if QT_VERSION < 0x040500
    int data = emu_qt45_QInputDialog_getInt( mainWidget, tit, msg, 0, -2147483647, 2147483647, 1, &ok);
#else
    int data = QInputDialog::getInt( mainWidget, tit, msg, 0, -2147483647, 2147483647, 1, &ok);
#endif
...
bool Doc_plugin_interface::getReal(qreal *num, const QString& mesage, const QString& title){
    ...
    double data = QInputDialog::getDouble( mainWidget, tit, msg, 0, -2147483647, 2147483647, 4, &ok);
    if (ok )
        *num = data;
    return ok;
}
bool Doc_plugin_interface::getString(QString *txt, const QString& mesage, const QString& title){
    ...
    QString text = QInputDialog::getText( mainWidget, tit,msg, QLineEdit::Normal,
                                         QString(), &ok);
    if (ok && !text.isEmpty()) {
        txt->clear();
        txt->append(text);
