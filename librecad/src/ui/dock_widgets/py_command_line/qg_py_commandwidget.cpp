 ...
 */
QG_Py_CommandWidget::QG_Py_CommandWidget(QWidget* parent, const char* name, Qt::WindowFlags fl)
    : QWidget(parent, fl)
    , actionHandler(nullptr)
{
    ...
    connect(leCommand, &QG_Py_CommandEdit::spacePressed, this, &QG_Py_CommandWidget::spacePressed);
    ...
    connect(leCommand, &QG_Py_CommandEdit::keycode, this, &QG_Py_CommandWidget::handleKeycode);

    auto a1 = new QAction(QObject::tr("Keycode mode"), this);
    a1->setObjectName("keycode_action");
    a1->setCheckable(true);
    connect(a1, &QAction::toggled, this, &QG_Py_CommandWidget::setKeycodeMode);
    ...
    connect(a3, &QAction::triggered, leCommand, &QG_Py_CommandEdit::modifiedPaste);
 ...
 */
QG_Py_CommandWidget::~QG_Py_CommandWidget()
{
    QSettings settings;
 ...
 *  language.
 */
void QG_Py_CommandWidget::languageChange()
{
    retranslateUi(this);
}

bool QG_Py_CommandWidget::eventFilter(QObject */*obj*/, QEvent *event)
{
...
void QG_Py_CommandWidget::setFocus()
{
    if (!isActiveWindow())
        activateWindow();
...
void QG_Py_CommandWidget::setCommand(const QString& cmd) {
    if (cmd.isEmpty()) {
        lCommand->setText(tr("Enter Python Command:"));
    }
    else {
...
void QG_Py_CommandWidget::setInput(const QString& cmd) {
    leCommand->setText(cmd);
    leCommand->setFocus();
}

void QG_Py_CommandWidget::appendHistory(const QString& msg) {
    teHistory->append(msg);
}

void QG_Py_CommandWidget::handleCommand(QString cmd)
{
    cmd = cmd.simplified();
    bool isAction=false;
    if (!cmd.isEmpty()) {
...
void QG_Py_CommandWidget::spacePressed() {
    if (actionHandler)
...
void QG_Py_CommandWidget::tabPressed() {
    if (actionHandler) {
...
void QG_Py_CommandWidget::escape() {
...
void QG_Py_CommandWidget::setActionHandler(QG_ActionHandler* ah) {
    actionHandler = ah;
}

void QG_Py_CommandWidget::setCommandMode() {
    QPalette palette;
    palette.setColor(lCommand->foregroundRole(), Qt::blue);
    lCommand->setPalette(palette);
}

void QG_Py_CommandWidget::setNormalMode() {
    QPalette palette;
    palette.setColor(lCommand->foregroundRole(), Qt::black);
    lCommand->setPalette(palette);
}

QString QG_Py_CommandWidget::getRootCommand( const QStringList & cmdList, const QString & typed ) {
...
void QG_Py_CommandWidget::handleKeycode(QString code)
{
...
void QG_Py_CommandWidget::setKeycodeMode(bool state)
{
    leCommand->keycode_mode = state;
}

void QG_Py_CommandWidget::dockingButtonTriggered(bool /*docked*/)
{
