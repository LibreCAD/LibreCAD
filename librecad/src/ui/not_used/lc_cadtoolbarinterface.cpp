...
LC_CadToolBarInterface::LC_CadToolBarInterface(QG_CadToolBar* _parentTB, Qt::WindowFlags fl) : QWidget(_parentTB, fl),
    cadToolBar(_parentTB), actionHandler(nullptr), m_pHidden(new QAction("ActionHidden", this)), m_pGrid0(new QToolBar),
    m_pGrid1(new QToolBar), m_pActionGroup(new QActionGroup(this)) {
}
...
void LC_CadToolBarInterface::finishCurrentAction(bool resetToolBar) {
    ...
    if (currentAction != nullptr) {
        currentAction->finish(); //finish the action, but do not update toolBar
    }
}
