...
namespace {
    QString getExtension(const RS2::FormatType type) {
        switch (type) {
            case RS2::FormatLFF:
                return QString(".lff");
            case RS2::FormatJWW:
    ...
    bool hasExtension(const QString& fileName, RS2::FormatType ftype) {
        // QString extension = getExtension(ftype); // fixme - reivew, why extension is not used there???
        const QStringList supported = {".cxf", ".dxf", ".lff"};
...
RS2::FormatType QG_FileDialog::getType(const QString& filter) const {
    ...
    if (filter == fDxfrw2004) {
        return RS2::FormatDXFRW2004;
    }
    if (filter == fDxfrw2000) {
...
QG_FileDialog::QG_FileDialog(QWidget* parent, Qt::WindowFlags f, FileType type)
    ...
    switch (type) {
        case BlockFile:
            m_name = tr("Block", "block file");
            break;
        default:
            m_name = tr("Drawing", "drawing file");
    }
}
...
QString QG_FileDialog::getSaveFile(RS2::FormatType* type, const QString& currentName) {
    ...
    if (!currentName.isEmpty()) {
        const QFileInfo currentFileInfo(currentName);
        const QString currentNameWithoutExtension = currentFileInfo.baseName();
        defaultFileName = currentNameWithoutExtension;
    }
    const auto extension = getExtension(m_type);
    const QString fileNameBase = defDir + defaultFileName;
    const QString fileNameGuess = fileNameBase + extension;
    ...
    setWindowTitle(tr("Save %1 As").arg(m_name));
    ...
    if (!hasExtension(firstSelectedFile.fileName(), m_type)) {
...
QString QG_FileDialog::getSaveFileName(QWidget* parent, RS2::FormatType* type) {
    ...
    auto fileDlg = QFileDialog(parent, "Save as");
    QStringList filters;
    bool done = false;
    bool cancel = false;
    QString fn = "";
    ...
    do {
        ...
        if (fileDlg.exec() == QDialog::Accepted) {
            ...
            if (QFileInfo(fn).exists()) {
                int choice = QMessageBox::warning(parent, QObject::tr("Save Drawing As"),
...
QString QG_FileDialog::getOpenFileName(QWidget* parent, RS2::FormatType* type) {
    ...
    const QString fDxfrw(QObject::tr("Drawing Exchange %1").arg("(*.dxf)"));

#ifdef DWGSUPPORT
    const QString fDwg(QObject::tr("dwg Drawing %1").arg("(*.dwg)"));
#endif
    const QString fDxf1(QObject::tr("QCad 1.x file %1").arg("(*.dxf *.DXF)"));
    ...
    auto fileDlg = QFileDialog(parent, "File Dialog");
