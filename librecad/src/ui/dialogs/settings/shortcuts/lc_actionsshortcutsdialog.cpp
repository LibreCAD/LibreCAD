...
LC_ActionsShortcutsDialog::LC_ActionsShortcutsDialog(QWidget* parent, LC_ActionGroupManager* pManager)
    ...
    // Enter = start recording (Issue #2333)
    connect(ui->tvMappingsTree, &QTreeView::activated, this, &LC_ActionsShortcutsDialog::onTreeDoubleClicked);
...
void LC_ActionsShortcutsDialog::initTreeView() const {
    ...
    // Issue #2333: restore full keyboard navigation
    ...
    treeView->setStyleSheet("QTreeView {" "    background-color: white;" "    outline: none;" // removes ugly dotted focus rectangle
...
void LC_ActionsShortcutsDialog::onExportClicked() {
    QString fileName;
    // Fixed: use save dialog for export
...
void LC_ActionsShortcutsDialog::reportLoadResult(const int loadResult) const {
    switch (loadResult) {
        ...
        case LC_ShortcutsStorage::ERROR_WRONG_DTD: {
            showIOInfoDialog(true, false, tr(
                                 "Shortcuts mappings were not imported.\n\n "
...
bool LC_ActionsShortcutsDialog::checkHasCollisions(LC_ShortcutInfo* shortcutInfo) const {
    ...
    if (hasCollisions) {
        ...
        if (shortcutInfo == nullptr) {
            ...
            msg = tr("Key sequence has potential conflicts. <a href=\"show\">Show.</a>");
        }
