...
LC_PropertiesEditingWidgetHyperbola::LC_PropertiesEditingWidgetHyperbola(QWidget *parent)
    : LC_EntityPropertiesEditorWidget(parent), ui(new Ui::LC_HyperbolaPropertiesEditingWidget){
  ui->setupUi(this);
}

LC_PropertiesEditingWidgetHyperbola::~LC_PropertiesEditingWidgetHyperbola(){
  delete ui;
}
...
void LC_PropertiesEditingWidgetHyperbola::updateUI() const {
  ...
  auto setValue = [](QLineEdit* le, const double v) { le->setText(QString::number(v, 'f', 10)); };
  ...
  const RS_Vector f1 = m_entity->getFocus1();
  const RS_Vector f2 = m_entity->getFocus2();
  setValue(ui->leFocus1X, f1.x); setValue(ui->leFocus1Y, f1.y);
  setValue(ui->leFocus2X, f2.x); setValue(ui->leFocus2Y, f2.y);

  const RS_Vector v = m_entity->getPrimaryVertex();
...
void LC_PropertiesEditingWidgetHyperbola::updateEntityData(){
  ...
  auto& d = const_cast<LC_HyperbolaData&>(m_entity->getData());
  ...
  if (evalDouble(ui->leFocus1X, f1x) && evalDouble(ui->leFocus1Y, f1y) &&
    ...
    if ((newF1 - newF2).squared() >= RS_TOLERANCE2) {
      const LC_HyperbolaData test(newF1, newF2, point);
      if (test.isValid()) {
  ...
  if (!applied) {
    ...
    const double angleRad = RS_Math::deg2rad(angleDeg);
    d.center = RS_Vector(cx, cy);
    d.majorP = RS_Vector(a * std::cos(angleRad), a * std::sin(angleRad));
    d.reversed = ui->cbReversed->isChecked();

    double a1, a2;
    if (evalDouble(ui->leAngle1, a1)) {
        d.angle1 = a1;
    }
    if (evalDouble(ui->leAngle2, a2)) {
        d.angle2 = a2;
    }
