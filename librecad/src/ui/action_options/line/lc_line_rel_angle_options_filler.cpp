...
void LC_LineRelAngleOptionsFiller::fillToolOptionsContainer(LC_PropertyContainer* container) {
    auto action = static_cast<LC_ActionDrawLineRelAngle*>(m_action);

    const bool fixedAngle = action->hasFixedAngle();
    if (!fixedAngle) {
