...
void LC_StretchOptionsFiller::fillToolOptionsContainer(LC_PropertyContainer* container) {
    auto action = static_cast<LC_ActionModifyStretch*>(m_action);

    addBoolean({"a_keepOriginals", tr("Keep originals"), tr("If checked, original entities will survive, otherwise they will be removed")},
              [action]()-> bool {
                  return !action->isRemoveOriginals();
