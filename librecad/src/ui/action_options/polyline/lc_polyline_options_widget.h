...
class LC_ActionDrawPolyline;

namespace Ui {
    class LC_PolylineOptionsWidget;
}

class LC_PolylineOptionsWidget : public LC_ActionOptionsWidget {
    Q_OBJECT
public:
    LC_PolylineOptionsWidget();
    ~LC_PolylineOptionsWidget() override;

public slots :
    void close() const; // fixme - rename
    void undo() const;
    void updateUIBySegmentMode(int m) const;
...
protected:
    LC_ActionDrawPolyline* m_action = nullptr;
    void doUpdateByAction(RS_ActionInterface* a) override;
private:
    std::unique_ptr<Ui::LC_PolylineOptionsWidget> ui;
};
