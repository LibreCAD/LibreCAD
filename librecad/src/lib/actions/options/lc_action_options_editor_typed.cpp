...
void LC_ActionOptionsEditorTyped::updateOptionsUIInToolOptionsWidget(int mode,const QVariant *value) const {
    if (m_optionWidget != nullptr) {
        m_optionWidget->updateUI(mode, value);
    }
}

void LC_ActionOptionsEditorTyped::hideOptionsInToolOptionsWidget() const {
     if (m_optionWidget != nullptr) {
         m_optionWidget->hideOptions();
         const auto actionContext = m_action->getActionContext();
...
void LC_ActionOptionsEditorTyped::showOptionsInPropertySheetWidget() {
    ...
    const auto propertySheetWidget = obtainPropertySheetWidget();
...
LC_PropertySheetInterface* LC_ActionOptionsEditorTyped::obtainPropertySheetWidget() {
    // fixme sand - any way to achieve more elegant depenency?
    return QC_ApplicationWindow::getAppWindow()->getPropertySheetWidget();
}
