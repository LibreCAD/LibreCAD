...
public:
    LC_Highlight();
    void addEntity([[maybe_unused]] const RS_Entity* entity) override {}
    void addEntity(const RS_Entity* entity, bool selected = false);
    bool removeEntity(RS_Entity *entity) override;
    void clear() override;
    void addEntitiesToContainer(RS_EntityContainer* container);
protected:
   QMap<long long, RS_Entity*> m_entitiesMap;
};
