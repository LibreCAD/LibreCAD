...
void RS_Arc::calculateBorders() {
    if(data.ratio != 1.0)
        ...
        RS_Vector const startpoint = data.center + RS_Vector::polar(data.majorP.x, data.angle1);
        RS_Vector const endpoint = data.center + RS_Vector::polar(data.majorP.x, data.angle2);
        ...
        if ( RS_Math::isAngleBetween(0.5*M_PI,a1,a2,false) ) {
            maxY = data.center.y + data.majorP.x;
        }
        if ( RS_Math::isAngleBetween(1.5*M_PI,a1,a2,false) ) {
            minY = data.center.y - data.majorP.x;
        }
        if ( RS_Math::isAngleBetween(M_PI,a1,a2,false) ) {
            minX = data.center.x - data.majorP.x;
        }
        if ( RS_Math::isAngleBetween(0.,a1,a2,false) ) {
            maxX = data.center.x + data.majorP.x;
        }
...
RS_Vector RS_Arc::getNearestPointOnEntity(const RS_Vector& coord,
    ...
    if(data.ratio != 1.0)
        ...
        if ( ! onEntity || RS_Math::isAngleBetween(angle,
            ...
            vec.setPolar(data.majorP.x, angle);
            vec+=data.center;
        } else {
...
RS_Vector RS_Arc::getNearestDist(double distance,
    ...
    if(data.ratio != 1.0)
        ...
        double aDist = distance / data.majorP.x;
        ...
        RS_Vector ret = RS_Vector::polar(data.majorP.x, a);
        ret += getCenter();
					...
					bool onEntity ) const
{
    ...
    printf("RS_Arc::getNearestOrthTan  \n");
    
    if (data.ratio != 1.0)
        return RS_Ellipse::getNearestOrthTan(coord,normal,onEntity);
    else {
        if ( !coord.valid ) {
                return RS_Vector(false);
        }
...
/**
  ...
  *Author: Dongxu Li
  */
#ifdef NEVER        // what to do about ellipse ?
#endif
...
RS2::Ending RS_Arc::getTrimPoint(const RS_Vector& trimCoord,
    ...
    if (data.ratio != 1.0)
        return RS_Ellipse::getTrimPoint(trimCoord,RS_Vector(0,0,0));
    else {
...
RS_Vector RS_Arc::prepareTrim(const RS_Vector& trimCoord,
    ...
    if (data.ratio != 1.0)
        return RS_Ellipse::prepareTrim(trimCoord,trimSol);
    else {
...
void RS_Arc::scale(const RS_Vector& center, const RS_Vector& factor) {
    if (data.ratio != 1.0)
        ...
        data.majorP.x *= factor.x;
        data.majorP.x = fabs( data.majorP.x );
...
void RS_Arc::mirror(const RS_Vector& axisPoint1, const RS_Vector& axisPoint2) {
    if (data.ratio != 1.0)
        RS_Ellipse::mirror(axisPoint1,axisPoint2);
    else {
...
void RS_Arc::draw(RS_Painter* painter, RS_GraphicView* view,
    ...
    if (data.ratio != 1.0)
        RS_Ellipse::draw(painter,view,patternOffset);
    else {
...
//    double styleFactor = getStyleFactor(view);
	...
	if (!pat || ra<0.5) {//avoid division by zero from small ra
        ...
        painter->drawArc(cp, ra, getRatio()*ra,
                         getAngle1(),getAngle2(),
                         isReversed());
        return;
    }
...
//    patternOffset=remainder(patternOffset - length -0.5*pat->totalLength,pat->totalLength)+0.5*pat->totalLength;
	...
	for(int j=0; fabs(total-a1) < limit; j=(j+1)%pat->num) {
		...
		if(pat->pattern[j] > 0.0 && fabs(t2-a2) < limit) {
			...
			painter->drawArc(cp, ra, getRatio()*ra,a11, a21, isReversed());
		}
...
double RS_Arc::getLength() const {
    if (data.ratio != 1.0)
        ...
        return getAngleLength()*data.majorP.x;
    }
}
...
LC_Quadratic RS_Arc::getQuadratic() const
{
    if (data.ratio != 1.0)
        ...
        ce[0]=1.;
        ce[2]=1.;
        ce[5]=-data.majorP.x*data.majorP.x;
        LC_Quadratic ret(ce);
