 ...
 * (one of the vertices)
 */
RS_Vector RS_EntityContainer::getNearestEndpoint(
...
RS_Vector RS_EntityContainer::getNearestPointOnEntity(
    const RS_Vector &coord,
...
RS_Vector RS_EntityContainer::getNearestCenter(
    const RS_Vector &coord,
    double *dist) const {
    double minDist = RS_MAXDOUBLE;  // minimum measured distance
...
RS_Vector RS_EntityContainer::getNearestMiddle(
    const RS_Vector &coord,
    double *dist,
    int middlePoints
) const {
...
RS_Vector RS_EntityContainer::getNearestDist(
    double distance,
    const RS_Vector &coord,
    double *dist) const {
 ...
 */
RS_Vector RS_EntityContainer::getNearestIntersection(
    const RS_Vector &coord,
    double *dist)
{
    double minDist = RS_MAXDOUBLE;  // minimum measured distance
    ...
    if (closestEntity) {
        for (RS_Entity *en = firstEntity(RS2::ResolveAllButTextImage);
            ...
            RS_VectorSolutions sol = RS_Information::getIntersection(closestEntity,
                                                  en,
                                                  true);
...
RS_Vector RS_EntityContainer::getNearestVirtualIntersection(
    const RS_Vector &coord,
    const double &angle,
    double *dist)
{
...
RS_Vector RS_EntityContainer::getNearestRef(
    const RS_Vector &coord,
    double *dist) const {
...
RS_Vector RS_EntityContainer::getNearestSelectedRef(
    const RS_Vector &coord,
    double *dist) const
{
...
RS_EntityContainer::RefInfo RS_EntityContainer::getNearestSelectedRefInfo(
    const RS_Vector &coord,
    double *dist) const
{
    double minDist = RS_MAXDOUBLE;  // minimum measured distance
...
double RS_EntityContainer::getDistanceToPoint(
    const RS_Vector &coord,
    RS_Entity **entity,
...
RS_Entity *RS_EntityContainer::getNearestEntity(
    const RS_Vector &coord,
    double *dist,
...
bool RS_EntityContainer::optimizeContours() {
    ...
    // Pre-process: Handle closed loops, remove unsupported/zero-length
    LC_LoopUtils::LoopOptimizer optimizer{*this};  // Feeds all edges recursively
    auto optimizedContainer = optimizer.GetResults();  // Owned result with loops

    // Clear original (deletes if autoDelete)
    clear();

    // Add optimized loops as sub-containers
    for (RS_Entity* loopEntity : *optimizedContainer) {
        if (RS_EntityContainer* loop = dynamic_cast<RS_EntityContainer*>(loopEntity)) {
            RS_EntityContainer* newLoop = new RS_EntityContainer(this, false);
            for (RS_Entity* edge : *loop) {
                newLoop->addEntity(edge->clone());  // Clone to transfer ownership
            }
            addEntity(newLoop);
        }
    }

    // Post-process: Sort for nesting if needed
    auto loops = getLoops();  // From parent
    LC_LoopUtils::LoopSorter sorter{std::move(loops)};
    auto sortedRoots = sorter.getResults();
    // Rebuild hierarchy if nested (optional; current is flat, so flatten back if not needed)

    // Validate closure (utils ensure, but check gaps)
    bool closed = true;
    for (RS_Entity* e : *this) {
        RS_EntityContainer* loop = dynamic_cast<RS_EntityContainer*>(e);
        if (loop != nullptr && !LC_LoopUtils::isLoopClosed(*loop)) {  // Custom isClosed() check via endpoints
            closed = false;
            RS_DEBUG->print(RS_Debug::D_ERROR, "optimizeContours: Open loop found");
        }
    }

    RS_DEBUG->print(closed ? "optimizeContours: OK" : "optimizeContours: bad");
    return closed;
}
...
void RS_EntityContainer::stretch(
...
void RS_EntityContainer::moveRef(
    const RS_Vector &ref,
    const RS_Vector &offset) {
...
void RS_EntityContainer::moveSelectedRef(
    const RS_Vector &ref,
    const RS_Vector &offset) {
...
//        LC_ERR << e->getId() << ": int = " << lineIntegral << ": " << startPoint.x << " - " << endPoint.x;
        ...
        if (previousPoint.valid && endPoint.squaredTo(previousPoint) <= RS_TOLERANCE15) {
            ...
            if (!previousPoint.valid && i + 1 < count()) {
                useEndPoint = endPointDistance(endPoint, *m_entities.at(i + 1))
                              < endPointDistance(startPoint, *m_entities.at(i + 1));
            }
