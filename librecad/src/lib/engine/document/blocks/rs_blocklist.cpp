 ...
 */
bool RS_BlockList::add(RS_Block* block, const bool notify) {
...
QString RS_BlockList::newName(const QString& suggestion) {
    ...
    for (const RS_Block* b : std::as_const(m_blocks)) {
        ...
        const QString part1 = b->getName().mid(0, index);
        if (part1 != name) {
            continue;
        }
 ...
 */
RS_Block* RS_BlockList::at(const int i) {
    return m_blocks.at(i);
}

RS_Block* RS_BlockList::at(const int i) const {
    return m_blocks.at(i);
}
 ...
 */
void RS_BlockList::setModified(const bool m) {
    m_modified = m;
