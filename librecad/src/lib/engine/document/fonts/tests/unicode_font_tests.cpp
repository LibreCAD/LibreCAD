...
// Greek Extended (polytonic): all 233 assigned codepoints (0x1F00-0x1FFE) -
// every systematic vowel+breathing+accent+iota-subscript combination, the
// uppercase "prosgegrammeni" (trailing adscript iota) forms, and the
// standalone spacing modifier marks (koronis, psili, dasia, perispomeni
// etc.) used in isolation. Real ongoing use: Ancient/Koine Greek is
// actively taught and published, not merely historical - a polytonic
// document needs this whole systematic set, not a subset. Same
// tractability as accented Latin, just a larger combination space -
// standalone precomposed letters/symbols, no combining marks, no
// positioning caveat. The block has real internal gaps (e.g. epsilon and
// omicron have no circumflex forms, upsilon has no smooth-breathing
// uppercase forms - short vowels and a breathing rule respectively) -
// this list follows them exactly, same reasoning as kEthiopic.
const char16_t kGreekExtended[] = {
    ...
    0x1FF7, 0x1FF8, 0x1FF9, 0x1FFA, 0x1FFB, 0x1FFC, 0x1FFD, 0x1FFE, 0};

// Modern minority-language Cyrillic extensions: a long tail of distinct
// communities sharing the extended Cyrillic range beyond what's already
// shipped - Abkhaz, Komi, Khakas, Kildin Sami, Aleut, Nanai, Even, Khanty,
// Mansi, several Caucasian languages in Azerbaijan/Dagestan (via the
// "*we"/"*sse"/"*che" letters), plus genuine Old Cyrillic paleographic
// letters (Zemlya, Dzelo, the Yus/Yat/Omega variants, Multiocular O and
// its relatives) that round out the adjacent Cyrillic Extended-B block.
// All standalone precomposed letters, no combining marks, no positioning
// caveat, same tier as the base Cyrillic already shipped. Two exact-
// duplicate-stroke pairs confirmed as genuine font design choices, not
// tracer bugs, the same way as Greek's koronis/psili (see kGreekExtended):
// palochka (U+04C0) has no real visual case distinction, so its "small"
// form U+04CF is drawn identically in the reference font.
const char16_t kCyrillicMinority[] = {
    0x048A, 0x048B, 0x048C, 0x048D, 0x048E, 0x048F, 0x0494, 0x0495, 0x049C,
    0x049D, 0x049E, 0x049F, 0x04A4, 0x04A5, 0x04A6, 0x04A7, 0x04A8, 0x04A9,
    0x04AC, 0x04AD, 0x04B4, 0x04B5, 0x04B8, 0x04B9, 0x04BC, 0x04BD, 0x04BE,
    0x04BF, 0x04C0, 0x04C1, 0x04C2, 0x04C3, 0x04C4, 0x04C5, 0x04C6, 0x04C7,
    0x04C8, 0x04C9, 0x04CA, 0x04CB, 0x04CC, 0x04CD, 0x04CE, 0x04CF, 0x04D2,
    0x04D3, 0x04DA, 0x04DB, 0x04DC, 0x04DD, 0x04DE, 0x04DF, 0x04E0, 0x04E1,
    0x04E4, 0x04E5, 0x04E6, 0x04E7, 0x04EA, 0x04EB, 0x04EC, 0x04ED, 0x04F0,
    0x04F1, 0x04F4, 0x04F5, 0x04F6, 0x04F7, 0x04F8, 0x04F9, 0x04FA, 0x04FB,
    0x04FC, 0x04FD, 0x04FE, 0x04FF, 0x0500, 0x0501, 0x0502, 0x0503, 0x0504,
    0x0505, 0x0506, 0x0507, 0x0508, 0x0509, 0x050A, 0x050B, 0x050C, 0x050D,
    0x050E, 0x050F, 0x0510, 0x0511, 0x0512, 0x0513, 0x0514, 0x0515, 0x0516,
    0x0517, 0x0518, 0x0519, 0x051A, 0x051B, 0x051C, 0x051D, 0x051E, 0x051F,
    0x0520, 0x0521, 0x0522, 0x0523, 0x0524, 0x0525, 0x0526, 0x0527, 0xA640,
    ...
    0xA653, 0xA654, 0xA655, 0xA656, 0xA657, 0xA658, 0xA659, 0xA65A, 0xA65B,
    0xA65C, 0xA65D, 0xA65E, 0xA65F, 0xA660, 0xA661, 0xA662, 0xA663, 0xA664,
    0xA665, 0xA666, 0xA667, 0xA668, 0xA669, 0xA66A, 0xA66B, 0xA66C, 0xA66D,
    0xA66E, 0xA67F, 0xA680, 0xA681, 0xA682, 0xA683, 0xA684, 0xA685, 0xA686,
    0xA687, 0xA688, 0xA689, 0xA68A, 0xA68B, 0xA68C, 0xA68D, 0xA68E, 0xA68F,
    0xA690, 0xA691, 0xA692, 0xA693, 0xA694, 0xA695, 0xA696, 0xA697, 0xA698,
    0xA699, 0xA69A, 0xA69B, 0xA69C, 0xA69D, 0};
...
TEST_CASE("unicode.lff covers Greek Extended (polytonic)",
          "[font][unicode][i18n]") {
    ensureApp();
    checkRange(kGreekExtended, /*minX=*/-2.0, /*maxX=*/14.0, /*minY=*/-3.5,
               /*maxY=*/13.0);
}

TEST_CASE("unicode.lff covers the modern minority-language Cyrillic "
          "extensions",
          "[font][unicode][i18n]") {
    ensureApp();
    checkRange(kCyrillicMinority, /*minX=*/-2.0, /*maxX=*/16.5, /*minY=*/-3.5,
               /*maxY=*/12.5);
}
