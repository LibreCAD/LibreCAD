...
public:
  enum class ReplayState { ReplayAllowed, ReplayInvalidated, ReplayReplaced };
  ...
  enum class ModelerPayloadKind { Unknown, Sat, Sab };

  enum class ModelerPayloadSection { Unknown, Body, HandleStream, DataStorage };
  ...
  enum class AdvancedEntityWriterOdaCoverage { Complete, Partial, Absent };

  enum class TableFallbackRole { GridLine, CellText, Placeholder, Boundary };
  ...
  enum class MeshRawRangeStatus { Unknown, Complete, Missing, Incomplete };
  ...
  enum class DocumentMappingSource { View, Ucs, Vport };

  enum class VisualMetadataSource { View, Vport, VisualStyle, Light, Sun };
  ...
  struct RawObjectRecord {
    ...
    // Raw bytes are the validated DWG object body, not the enclosing
    // frame. The writer reconstructs the frame prefix and CRC.
    std::vector<std::uint8_t> rawBytes;
    bool typedPayloadValidated = false;
    ReplayState replayState = ReplayState::ReplayAllowed;
    bool hasDataStorage = false;
    // Preserve the source CLASSES tuple for raw custom-object replay.
    // These fields are file-local metadata and must not be replaced by
    // writer defaults when a document is saved unchanged.
    bool hasClassDefinition = false;
    std::uint16_t classProxyFlag = 0x401;
    std::string classAppName;
    bool classWasProxy = false;
    std::uint16_t classEntityFlagRaw = 0x1F3;
  ...
  enum class DwgSourceGraphStatus { Complete, Refused };

  // Writer-facing proof for a FIELDLIST that was read from DWG. This is
  // intentionally narrower than the diagnostic source graph: an unrelated
  // source family must not decide whether this counted relation is writable.
  enum class DwgFieldListSourceAuthorizationStatus {
    NotDwgSource,
    Authorized,
    OrphanMembership,
    FrameCoverageIncomplete,
    DuplicateSourceFrame,
    InvalidSourceFrame,
    MissingSourceCoverage,
    MismatchedSourceCoverage,
    MissingMembership,
    DuplicateMembership,
    IncompleteMembership,
    InvalidMembership,
    MismatchedMembership,
    MissingFieldEndpoint,
    DuplicateFieldEndpoint,
    InvalidFieldEndpoint
  };
  ...
  struct DwgFieldListSourceAuthorization {
    ...
    bool authorized() const noexcept {
      return status == DwgFieldListSourceAuthorizationStatus::Authorized;
    }
  };

  // Writer-facing proof for an AcDbField object read from DWG. FIELD has a
  // semantic payload in addition to FIELDLIST's counted links, so replacing
  // its raw frame requires an exact immutable payload receipt.
  enum class DwgFieldSourceAuthorizationStatus {
    NotDwgSource,
    Authorized,
    OrphanPayload,
    FrameCoverageIncomplete,
  ...
  struct DwgFieldSourceAuthorization {
    DwgFieldSourceAuthorizationStatus status =
        DwgFieldSourceAuthorizationStatus::NotDwgSource;

    bool isDwgSource() const noexcept {
      return status != DwgFieldSourceAuthorizationStatus::NotDwgSource;
    }
    bool authorized() const noexcept {
      return status == DwgFieldSourceAuthorizationStatus::Authorized;
    }
  };

  // Writer-facing proof for a fixed DICTIONARY read from DWG. A dictionary
  // list is an ordered, ownership-bearing relation, so it is authorized
  // independently of the broader diagnostic source graph.
  enum class DwgDictionarySourceAuthorizationStatus {
    NotDwgSource,
    Authorized,
    OrphanMembership,
    FrameCoverageIncomplete,
  ...
  struct DwgDictionarySourceAuthorization {
    DwgDictionarySourceAuthorizationStatus status =
        DwgDictionarySourceAuthorizationStatus::NotDwgSource;

    bool isDwgSource() const noexcept {
      return status != DwgDictionarySourceAuthorizationStatus::NotDwgSource;
    }
    bool authorized() const noexcept {
      return status == DwgDictionarySourceAuthorizationStatus::Authorized;
    }
  };

  // Writer-facing proof for an AcDbDictionaryWithDefault object read from
  // DWG. Its inherited entries and required default pointer must agree with
  // one source receipt before the raw custom frame may be replaced.
  enum class DwgDictionaryWithDefaultSourceAuthorizationStatus {
    NotDwgSource,
    Authorized,
    OrphanMembership,
    FrameCoverageIncomplete,
  ...
  struct DwgDictionaryWithDefaultSourceAuthorization {
    DwgDictionaryWithDefaultSourceAuthorizationStatus status =
        DwgDictionaryWithDefaultSourceAuthorizationStatus::NotDwgSource;
    ...
    bool authorized() const noexcept {
      return status ==
             DwgDictionaryWithDefaultSourceAuthorizationStatus::Authorized;
    }
  };

  enum class DwgSourceGraphRefusal {
    None,
    FrameCoverageIncomplete,
    FramePublicationConflict,
    InvalidFrameHandle,
    DuplicateFrameHandle,
    DuplicateFrameOrdinal,
    MissingFramePublication,
    ExtraFramePublication,
    MismatchedFramePublication,
    MissingDictionaryMembership,
    ExtraDictionaryMembership,
    MismatchedDictionaryMembership,
    IncompleteDictionaryMembership,
    InvalidDictionaryMembership,
    DuplicateDictionaryKey,
    DuplicateDictionaryMember,
    MissingGroupMembership,
    ExtraGroupMembership,
    MismatchedGroupMembership,
    IncompleteGroupMembership,
    InvalidGroupMembership,
    DuplicateGroupMembership,
    MissingSortEntsMembership,
    ExtraSortEntsMembership,
    MismatchedSortEntsMembership,
    IncompleteSortEntsMembership,
    InvalidSortEntsMembership,
    DuplicateSortEntsMembership,
    MissingFieldListMembership,
    ExtraFieldListMembership,
    MismatchedFieldListMembership,
    IncompleteFieldListMembership,
    InvalidFieldListMembership,
    DuplicateFieldListMembership,
    MissingTypedReference,
    ExtraTypedReference,
    MismatchedTypedReference,
    IncompleteTypedReference,
    InvalidTypedReference,
    DuplicateTypedReference,
    MissingBlockReachability,
    ExtraBlockReachability,
    MismatchedBlockReachability,
    IncompleteBlockReachability,
    InvalidBlockReachability,
    DuplicateBlockReachability,
    InconsistentCommonLinkEvidence,
    ClassCoverageIncomplete,
    MissingCustomClassOrdinal,
    InvalidCustomClassOrdinal,
    UnpublishedClass,
    CustomClassMismatch,
    UnknownCommonLinks,
    InvalidValidatedAbsentLinks,
    ReactorCountMismatch,
    InvalidControlReceipt,
    MissingLinkEndpoint,
    DuplicateLink,
    HierarchyCycle,
    AllocationFailure
  ...
  enum class DwgSourceGraphNodeKind { Frame, Class };

  enum class DwgSourceGraphEdgeKind {
    Owner,
    Reactor,
    ExtensionDictionary,
    DictionaryMember,
  ...
  struct DwgSourceGraphSource {
    std::uint32_t handle = DRW::NoHandle;
    std::uint32_t offset = 0;
    std::uint64_t mapOrdinal = 0;
    DRW_DwgFrameOffsetSpace offsetSpace = DRW_DwgFrameOffsetSpace::Unknown;
  };

  struct DwgSourceGraphNode {
    DwgSourceGraphNodeKind kind = DwgSourceGraphNodeKind::Frame;
    DwgSourceGraphSource source;
    std::uint64_t classStreamOrdinal = 0;
    bool hasClassStreamOrdinal = false;
  ...
  struct DwgSourceGraphEdge {
    std::size_t sourceNode = 0;
    std::size_t targetNode = 0;
    DwgSourceGraphEdgeKind kind = DwgSourceGraphEdgeKind::Owner;
    DRW_DwgTypedReferenceField typedReferenceField =
        DRW_DwgTypedReferenceField::Unknown;
    std::uint32_t membershipOrdinal = 0;
  };

  struct DwgSourceGraphResult {
    DwgSourceGraphStatus status = DwgSourceGraphStatus::Refused;
    DwgSourceGraphRefusal refusal = DwgSourceGraphRefusal::None;
    std::optional<DwgSourceGraphSource> source;
    std::optional<std::uint64_t> classStreamOrdinal;
    std::vector<DwgSourceGraphNode> nodes;
    std::vector<DwgSourceGraphEdge> edges;

    bool complete() const {
      return status == DwgSourceGraphStatus::Complete &&
             refusal == DwgSourceGraphRefusal::None;
    }
  };
  ...
  struct RawObjectFamilyCounts {
    ...
    size_t total() const {
      return unknown + associative + evaluationGraph + dynamicBlock +
             objectContext;
    }
  };
  ...
  struct MeshWriterBlockerCounts {
    ...
    size_t totalBlockers() const {
      return missingCreaseData + unsupportedSubdivisionData +
             fallbackOnlyPreview + editedFallback + missingOwnerOrClassHandle +
             malformedCountRelationships + invalidated + replaced;
    }
  };

  struct ExternalReferencePathDiagnostic {
    std::string path;
    ExternalReferencePathStatus status = ExternalReferencePathStatus::Unknown;
  ...
  struct ImageDefinitionRecord {
    ...
    std::vector<std::uint32_t> commonReactorHandles;
    std::uint32_t xDictHandle = 0;
    std::int32_t numReactors = 0;
    std::uint8_t xDictFlag = 0;
  ...
  struct PointCloudDefinitionRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
  ...
  struct NavisworksModelDefinitionRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
    ...
    DRW_Coord minExtent;
    DRW_Coord maxExtent;
    bool hostDrawingVisibility = false;
    std::vector<std::uint32_t> reactorHandles;
    std::uint32_t xDictHandle = 0;
    std::int32_t numReactors = 0;
    std::uint8_t xDictFlag = 0;
  ...
  struct PointCloudColorMapRampRecord {
    std::int32_t classVersion = 0;
    std::uint32_t declaredColorCount = 0;
    std::vector<std::string> colorSchemes;
  };

  struct PointCloudColorMapRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
    std::int32_t classVersion = 0;
    std::string defaultIntensityColorScheme;
    std::string defaultElevationColorScheme;
    std::string defaultClassificationColorScheme;
    std::uint32_t declaredColorRampCount = 0;
    std::vector<PointCloudColorMapRampRecord> colorRamps;
    std::uint32_t declaredClassificationColorRampCount = 0;
    std::vector<PointCloudColorMapRampRecord> classificationColorRamps;
    std::vector<std::uint32_t> reactorHandles;
    std::uint32_t xDictHandle = 0;
    std::int32_t numReactors = 0;
    std::uint8_t xDictFlag = 0;
  ...
  struct ShapeOleWriterBlockerCounts {
    ...
    size_t totalBlockers() const {
      return missingStyleHandle + unresolvedShapeStyle + missingOlePayload +
             truncatedOlePayload + oversizedOlePayload + editedPreviewFrame +
  ...
  struct VportRecord {
    ...
    std::vector<std::uint32_t> reactorHandles;
    std::uint32_t xDictHandle = 0;
    bool importedFromDwg = false;
    ReplayState replayState = ReplayState::ReplayAllowed;
  };

  struct ViewportEntityHeaderRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::string name;
    int flags = 0;
    bool isOn = false;
    std::uint32_t xrefBlockHandle = 0;
    std::uint32_t viewportEntityHandle = 0;
    std::uint32_t previousViewportEntityHeaderHandle = 0;
    std::vector<std::uint32_t> reactorHandles;
    std::uint32_t xDictHandle = 0;
    std::uint32_t numReactors = 0;
    std::uint8_t xDictFlag = 0;
    ...
    std::uint32_t rawDataBitSize = 0;
    DRW::Version rawDataVersion = DRW::UNKNOWNV;
    bool rawDataValid = false;
    ReplayState replayState = ReplayState::ReplayAllowed;
  };

  struct VxControlRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
    ...
    std::vector<std::uint32_t> recordHandles;
    std::vector<std::uint8_t> rawData;
    std::uint32_t rawDataBitSize = 0;
    DRW::Version rawDataVersion = DRW::UNKNOWNV;
    bool rawDataValid = false;
  ...
  struct DocumentMappingRecord {
    ...
    bool isMappedToDocumentItem() const { return documentItemIndex >= 0; }
  };
  ...
  struct VisualStyleRecord {
    ...
    std::int32_t type =
        0; // mirrors DRW_VisualStyle::type widening BS->BL (P3.3)
    DRW_VisualStyleBody body;
  ...
  struct VisualMetadataWriterBlockerCounts {
    ...
    size_t totalBlockers() const {
      return unresolvedUcs + unresolvedBaseUcs + unresolvedVisualStyle +
             unresolvedSun + unresolvedBackground + unresolvedLiveSection +
             missingOwnerOrLayout + invalidatedRawPayload +
             replacedNativeUnavailablePayload + unsupportedVisualStyleWriter;
    }
  };
  ...
  struct LightListEntryRecord {
    std::uint32_t handle = 0;
    std::string name;
  };

  struct LightListRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::uint32_t classVersion = 0;
    std::uint32_t lightCount = 0;
    std::vector<LightListEntryRecord> lights;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
    std::vector<std::uint32_t> reactorHandles;
    std::uint32_t xDictHandle = 0;
  ...
  struct DataLinkCustomDataRecord {
    std::uint32_t targetHandle = 0;
    std::string text;
  };

  struct DataLinkRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    ...
    std::int16_t milliseconds = 0;
    std::int16_t pathOption = 0;
    std::int32_t value93 = 0;
    std::string updateStatus;
    std::int32_t customDataCount = 0;
    std::vector<DataLinkCustomDataRecord> customData;
    std::uint32_t hardOwnerHandle = 0;
    ReplayState replayState = ReplayState::ReplayAllowed;
  };

  struct GeoMapImageRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::int32_t classVersion = 0;
    DRW_Coord insertionPoint;
    DRW_Coord imageSize;
    std::uint16_t displayProps = 0;
    bool clipping = false;
  ...
  struct GeoPositionMarkerRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::uint32_t classVersion = 0;
    DRW_Coord position;
    ...
    std::shared_ptr<const DRW_MText> embeddedMText;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
    ReplayState replayState = ReplayState::ReplayAllowed;
  };

  struct SectionObjectRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
  ...
  struct TvDevicePropertiesRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::int32_t flags = 0;
    std::int32_t maxRegenThreads = 0;
    std::int32_t useLutPalette = 0;
    std::uint64_t alternateHighlight = 0;
    std::uint64_t alternateHighlightColor = 0;
    std::uint64_t geometryShaderUsage = 0;
    std::int32_t blendingMode = 0;
    double antialiasingLevel = 0.0;
    double valueBd2 = 0.0;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
    std::vector<std::uint32_t> reactorHandles;
  ...
  struct BackgroundRecord {
    ...
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
    DRW_Background::Kind kind = DRW_Background::Solid;
    std::int32_t classVersion = 0;
    int solidColor = 0;
    int colorTop = 0;
    int colorMiddle = 0;
    ...
    double height = 0.0;
    double rotation = 0.0;
    int colorSkyZenith = 0;
    int colorSkyHorizon = 0;
    int colorUndergroundHorizon = 0;
    int colorUndergroundAzimuth = 0;
    int colorNear = 0;
    int colorFar = 0;
    std::string fileName;
  ...
  struct RenderSettingsRecord {
    ...
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
    DRW_RenderSettings::Kind kind = DRW_RenderSettings::Settings;
    std::int32_t classVersion = 0;
    std::string name;
    ...
    bool fogBackgroundEnabled = false;
    bool backfacesEnabled = false;
    bool environmentImageEnabled = false;
    std::string environmentImageFilename;
    std::string description;
    ...
    int fogColorG = 0;
    int fogColorB = 0;
    double fogDensityNear = 0.0;
    double fogDensityFar = 0.0;
    double fogDistanceNear = 0.0;
    double fogDistanceFar = 0.0;
    std::int32_t procedure = 0;
  ...
  struct SunStudyRecord {
    ...
    std::string description;
    std::string sheetSetName;
    std::string sheetSubsetName;
    std::int32_t outputType = 0;
    bool useSubset = false;
    bool selectDatesFromCalendar = false;
    bool selectRangeOfDates = false;
    bool lockViewports = false;
    bool labelViewports = false;
    std::int32_t startTime = 0;
    std::int32_t endTime = 0;
    std::int32_t interval = 0;
    std::int32_t shadePlotType = 0;
    ...
    std::uint32_t pageSetupWizardHandle = 0;
    std::uint32_t viewHandle = 0;
    std::uint32_t visualStyleHandle = 0;
  ...
  struct MotionPathRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
    std::int32_t classVersion = 0;
    std::uint32_t cameraPathHandle = 0;
    std::uint32_t targetPathHandle = 0;
    std::uint32_t viewTableHandle = 0;
  ...
  struct PartialViewingIndexRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
  ...
  struct DimensionAssociationRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
    ...
    std::uint32_t associativityFlags = 0;
    bool isTransSpace = false;
    std::uint8_t rotatedDimensionType = 0;
    std::vector<DRW_DimensionAssociationOsnapRef> osnapRefs;
    bool hasUnrepresentableDetail = false;
    ReplayState replayState = ReplayState::ReplayAllowed;
  };

  struct EvaluationGraphRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
  ...
  struct BlockRepresentationRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
  ...
  struct SectionRecord {
    ...
    DRW::Version rawDataVersion = DRW::UNKNOWNV;
    bool rawDataValid = false;
    DRW_Section::Kind kind = DRW_Section::Manager;
    bool isLive = false;
    std::int32_t sectionCount = 0;
    std::vector<std::uint32_t> sectionHandles;
    std::int32_t classVersion = 0;
    std::int32_t sectionType = 0;
    std::int32_t generationOptions = 0;
    std::int32_t currentType = 0;
    std::int32_t typeCount = 0;
    std::vector<DRW_SectionTypeSettings> types;
    std::uint32_t sourceHandle = 0;
    std::uint32_t destinationBlockHandle = 0;
    std::uint32_t destinationFileHandle = 0;
  ...
  struct ModelerGeometryRecord {
    ...
    std::string dataStorageHandleKey;
  ...
  struct TableWriterBlockerCounts {
    ...
    size_t totalBlockers() const {
      return fallbackRendered + incompleteSemanticParse + unresolvedStyle +
             fieldContent + blockContent + attributeContent + overrideCells +
             geometryCells + unknownRanges + incompleteRanges + overrideMasks +
             breakRanges + tableGeometryTailRanges + editedFallbackEntities +
  ...
  struct TableNativeWriterEligibility {
    ...
    TableContentStorageMode storageMode = TableContentStorageMode::Unsupported;
  ...
  struct TableNativeWriterBlockerCounts {
    ...
    size_t totalBlockers() const {
      return noSemanticTableContent + missingFallbackAttachment +
             editedFallback + unresolvedTableStyle + unresolvedCellStyleMap +
             unknownSubrecordRange + incompleteSubrecordRange + overrideMask +
             breakData + geometryTail + mergedCell + fieldContent +
             blockContent + attributeContent + unknownCellContent +
             ...
             unresolvedLineType + rawReplayInvalidated + rawReplayReplaced +
             nonPositiveDimension;
    }
  ...
  struct TableRecord {
    ...
    std::uint32_t m_tableUnknownHandle = 0;
    ...
    // Complete legacy TABLESTYLE carrier. Keeping the parsed object here
    // lets the writer use the typed AC1015-AC1021 path without rebuilding
    // its positional row-style data from summaries.
    std::optional<DRW_TableStyle> nativeTableStyle;
  ...
  struct AssociativePrefixStatusRecord {
    ...
    AssociativePrefixParseStatus status = AssociativePrefixParseStatus::Missing;
  ...
  struct AssociativeRecord {
    ...
    std::uint32_t persistentSubentityCount = 0;
    std::vector<std::uint32_t> persistentSubentityHandles;
    std::uint32_t persistentUnknown0 = 0;
    std::uint32_t persistentUnknown2 = 0;
    std::uint32_t persistentAssocStepCount = 0;
    std::uint32_t persistentAssocSubentityCount = 0;
    std::vector<std::uint32_t> persistentSteps;
    std::vector<std::uint32_t> persistentSubentityIds;
  ...
  struct AssociativeEdgeRecord {
    ...
    AssociativeEdgeKind edgeKind = AssociativeEdgeKind::UnknownHandleReference;
    int rawRangeIndex = -1;
    AssociativeEdgeConfidence confidence = AssociativeEdgeConfidence::Unknown;
    ReplayState replayState = ReplayState::ReplayAllowed;
  };
  ...
  struct DictionaryRecord {
    ...
    // Decode diagnostic only. Writers reconstruct the count from entries.
    std::optional<DRW_DictionaryCountCap> countCap;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
    std::vector<std::uint32_t> reactorHandles;
    std::uint32_t xDictHandle = 0;
    ...
    std::uint8_t xDictFlag = 0;
    bool hasDsData = false;
    bool typedPayloadComplete = true;
    ReplayState replayState = ReplayState::ReplayAllowed;
  };
  ...
  struct XRecordRecord {
    ...
    std::vector<DRW_Variant> dataEntries;
    std::vector<std::uint8_t> rawData;
    bool rawDataValid = false;
    std::list<std::list<DRW_Variant>> appData;
    std::vector<std::shared_ptr<DRW_Variant>> extData;
  ...
  struct LayoutRecord {
    ...
    // Handle refs.
    std::uint32_t plotViewHandle = 0;
    std::uint32_t shadePlotHandle = 0;
  ...
  struct ObjectContextDataRecord {
    ...
    std::int32_t columnType = 0;
    std::int32_t columnHeightCount = 0;
    std::int32_t fragmentCount = 0;
    double columnWidth = 0.0;
    double columnGutter = 0.0;
    bool columnAutoHeight = false;
    bool columnFlowReversed = false;
    std::vector<double> columnHeights;
    bool embeddedScaleEnabled = false;
    std::uint16_t embeddedScaleFlag = 0;
    std::string embeddedScaleName;
    double embeddedScalePaperUnits = 1.0;
    double embeddedScaleDrawingUnits = 1.0;
    bool embeddedScaleUnitScale = false;
  ...
  /// INDEX (AcDbIndex) — generic index last-updated TIMEBLL.
  struct IndexRecord {
    std::uint32_t handle = 0;
    std::uint32_t parentHandle = 0;
  ...
  /// every evalexpr-bearing class, plus the AcDbBlockElement /
  /// AcDbBlockParameter element data for the parameter/grip/action classes, and
  /// the full body for BLOCKPOINTPARAMETER / BLOCKBASEPOINTPARAMETER /
  /// BLOCKLOOKUPPARAMETER / BLOCKFLIPPARAMETER / BLOCKROTATIONPARAMETER /
  /// BLOCKLINEARPARAMETER / BLOCKPOLARPARAMETER / BLOCKALIGNMENTPARAMETER /
  /// BLOCKXYPARAMETER /
  /// BLOCKALIGNEDCONSTRAINTPARAMETER / BLOCKHORIZONTALCONSTRAINTPARAMETER /
  ...
  struct DynamicBlockRecord {
    ...
    std::uint16_t purgeFlag = 0;
    std::uint32_t purgeBlockHandle = 0;
    ...
    double gripLocationX = 0.0;
    double gripLocationY = 0.0;
    double gripLocationZ = 0.0;
    bool gripValue280 = false;
    std::int32_t gripValue93 = 0;
    std::int32_t gripCombinedState = 0;
    bool gripHasOrientation = false;
    double gripOrientationX = 0.0;
    double gripOrientationY = 0.0;
    double gripOrientationZ = 0.0;
    // Shared AcDbBlock1PtParameter body.
    double defPointX = 0.0;
    double defPointY = 0.0;
    double defPointZ = 0.0;
    std::int32_t propertyInfo1Count = 0;
    std::vector<std::int32_t> propertyInfo1Codes;
    std::vector<std::string> propertyInfo1Names;
    std::int32_t propertyInfo2Count = 0;
    std::vector<std::int32_t> propertyInfo2Codes;
    std::vector<std::string> propertyInfo2Names;
    std::int32_t propertyInfoCount = 0;
    std::int16_t parameterValue170 = 0;
    std::int16_t parameterValue171 = 0;
    std::int32_t parameterValue93 = 0;
    // Shared AcDbBlock2PtParameter body.
    double firstPointX = 0.0;
    double firstPointY = 0.0;
    double firstPointZ = 0.0;
    double secondPointX = 0.0;
    double secondPointY = 0.0;
    double secondPointZ = 0.0;
    std::array<std::vector<std::int32_t>, 4> twoPointPropertyCodes;
    std::array<std::vector<std::string>, 4> twoPointPropertyNames;
    std::array<std::int32_t, 4> propertyStates{};
    std::int16_t parameterBaseLocation = 0;
    // BLOCKFLIPPARAMETER body.
    std::string flipCaption;
    std::string flipDescription;
    std::string flipBaseStateName;
    std::string flipFlippedStateName;
    double flipCaptionLocationX = 0.0;
    double flipCaptionLocationY = 0.0;
    double flipCaptionLocationZ = 0.0;
    std::string flipTooltip;
    std::int32_t flipValue96 = 0;
    // BLOCKROTATIONPARAMETER body.
    double rotationBaseAnglePointX = 0.0;
    double rotationBaseAnglePointY = 0.0;
    double rotationBaseAnglePointZ = 0.0;
    std::string rotationAngleName;
    std::string rotationAngleDescription;
    double rotationAngle = 0.0;
    std::int32_t rotationValueSetFlags = 0;
    double rotationMinimum = 0.0;
    double rotationMaximum = 0.0;
    double rotationIncrement = 0.0;
    std::uint16_t rotationDeclaredValueCount = 0;
    std::vector<double> rotationValues;
    // BLOCKLINEARPARAMETER body.
    std::string linearDistanceName;
    std::string linearDistanceDescription;
    double linearDistance = 0.0;
    std::int32_t linearValue96 = 0;
    double linearValue141 = 0.0;
    double linearValue142 = 0.0;
    double linearValue143 = 0.0;
    std::uint16_t linearDeclaredValueCount = 0;
    std::vector<double> linearValues;
    // BLOCKPOLARPARAMETER body.
    std::string polarAngleName;
    std::string polarAngleDescription;
    std::string polarDistanceName;
    std::string polarDistanceDescription;
    double polarOffset = 0.0;
    std::int32_t polarAngleValueSetFlags = 0;
    double polarAngleMinimum = 0.0;
    double polarAngleMaximum = 0.0;
    double polarAngleIncrement = 0.0;
    std::uint16_t polarAngleDeclaredValueCount = 0;
    std::vector<double> polarAngleValues;
    std::int32_t polarDistanceValueSetFlags = 0;
    double polarDistanceMinimum = 0.0;
    double polarDistanceMaximum = 0.0;
    double polarDistanceIncrement = 0.0;
    std::uint16_t polarDistanceDeclaredValueCount = 0;
    std::vector<double> polarDistanceValues;
    // BLOCKALIGNMENTPARAMETER body.
    bool alignPerpendicular = false;
    // BLOCKXYPARAMETER body.  The DWG body stores the X value set first.
    std::string xyXLabel;
    std::string xyXDescription;
    std::string xyYLabel;
    std::string xyYDescription;
    double xyXValue = 0.0;
    double xyYValue = 0.0;
    std::int32_t xyXValueSetFlags = 0;
    double xyXMinimum = 0.0;
    double xyXMaximum = 0.0;
    double xyXIncrement = 0.0;
    std::uint16_t xyXDeclaredValueCount = 0;
    std::vector<double> xyXValues;
    std::int32_t xyYValueSetFlags = 0;
    double xyYMinimum = 0.0;
    double xyYMaximum = 0.0;
    double xyYIncrement = 0.0;
    std::uint16_t xyYDeclaredValueCount = 0;
    std::vector<double> xyYValues;
    // BLOCK*CONSTRAINTPARAMETER body.
    std::uint32_t constraintDependencyHandle = 0;
    std::string constraintExpressionName;
    std::string constraintExpressionDescription;
    double constraintValue = 0.0;
    std::int32_t constraintValueSetFlags = 0;
    double constraintMinimum = 0.0;
    double constraintMaximum = 0.0;
    double constraintIncrement = 0.0;
    std::uint16_t constraintDeclaredValueCount = 0;
    std::vector<double> constraintValues;
    double angularCenterPointX = 0.0;
    double angularCenterPointY = 0.0;
    double angularCenterPointZ = 0.0;
    double angularEndPointX = 0.0;
    double angularEndPointY = 0.0;
    double angularEndPointZ = 0.0;
    bool angularOrientationOnBothGrips = false;
    // BLOCKPOINTPARAMETER body.
    std::string positionName;
    std::string positionDescription;
    double labelPointX = 0.0;
    double labelPointY = 0.0;
    double labelPointZ = 0.0;
    // BLOCKBASEPOINTPARAMETER body.
    double pointX = 0.0;
    double pointY = 0.0;
    double pointZ = 0.0;
    double basePointX = 0.0;
    double basePointY = 0.0;
    double basePointZ = 0.0;
    // BLOCKLOOKUPPARAMETER body.
    std::int32_t lookupIndex = 0;
    std::string lookupName;
    std::string lookupDescription;
    std::string lookupUnknownText;
    ...
    std::vector<std::int32_t> connectionCodes;
    double rotateActionOffsetX = 0.0;
    double rotateActionOffsetY = 0.0;
    double rotateActionOffsetZ = 0.0;
    std::vector<std::string> rotateBaseConnectionNames;
    std::vector<std::int32_t> rotateBaseConnectionCodes;
    bool rotateActionDependent = false;
    double rotateActionBasePointX = 0.0;
    double rotateActionBasePointY = 0.0;
    double rotateActionBasePointZ = 0.0;
    std::int32_t rotateConnectionCode = 0;
    std::string rotateConnectionName;
    double arrayColumnOffset = 0.0;
    double arrayRowOffset = 0.0;
    int lookupRowCount = 0;
    int lookupColumnCount = 0;
    std::vector<std::string> lookupExpressions;
    std::vector<std::vector<std::int32_t>> lookupConnectionCodes;
    std::vector<std::vector<std::string>> lookupConnectionNames;
    std::vector<bool> lookupEntryFlags282;
    std::vector<bool> lookupEntryFlags281;
    bool lookupFlag280 = false;
    int polarStretchPointCount = 0;
    std::vector<double> polarStretchPointCoordinates;
    int polarStretchHandleCount = 0;
    std::vector<std::uint32_t> polarStretchHandles;
    std::vector<std::int16_t> polarStretchHandleShorts;
    int polarStretchCodeCount = 0;
    std::vector<std::int32_t> polarStretchCodes;
    double scaleOffsetX = 0.0;
    double scaleOffsetY = 0.0;
    double scaleOffsetZ = 0.0;
    std::vector<std::string> baseConnectionNames;
    std::vector<std::int32_t> baseConnectionCodes;
    bool scaleDependent = false;
    double scaleBasePointX = 0.0;
    double scaleBasePointY = 0.0;
    double scaleBasePointZ = 0.0;
    std::vector<std::string> scaleConnectionNames;
    std::vector<std::int32_t> scaleConnectionCodes;
    int stretchPointCount = 0;
    std::vector<double> stretchPointCoordinates;
    int handleReferenceCount = 0;
    std::vector<std::uint32_t> handleReferenceHandles;
    std::vector<std::int32_t> handleReferenceIndexCounts;
    std::vector<std::vector<std::int32_t>> handleReferenceIndexes;
    int codeReferenceCount = 0;
    std::vector<std::int32_t> codeReferenceCodes;
    std::vector<std::int32_t> codeReferenceIndexCounts;
    std::vector<std::vector<std::int32_t>> codeReferenceIndexes;
    int adbVersion = 0;
    int dimensionBaseVersion = 0;
  ...
  void clear() {
    ...
    m_rawObjects.clear();
    m_dwgFramePublications.clear();
    m_dwgDictionaryMemberships.clear();
    m_dwgGroupMemberships.clear();
    m_dwgSortEntsMemberships.clear();
    m_dwgFieldListMemberships.clear();
    m_dwgFieldPayloadReceipts.clear();
    m_dwgTypedReferences.clear();
    m_dwgBlockReachabilities.clear();
    m_dwgFramePublicationCoverageComplete = false;
    m_dwgFramePublicationConflicts.clear();
    m_dwgFrameCoverageReport = {};
    m_dwgClassCoverageReport = {};
    m_rawDwgSections.clear();
    m_dxfClasses.clear();
    m_lineTypeNames.clear();
    m_textStyleNames.clear();
    m_lineTypes.clear();
    m_textStyles.clear();
    m_lineTypeTableEntries.clear();
    m_textStyleTableEntries.clear();
    ...
    m_ucsRecords.clear();
    m_vports.clear();
    m_vportTableEntries.clear();
    m_viewportEntityHeaders.clear();
    m_lights.clear();
    m_cameras.clear();
    m_lightLists.clear();
    m_dataLinks.clear();
    m_geoMapImages.clear();
    m_layerFilters.clear();
    m_geoPositionMarkers.clear();
    m_sectionObjects.clear();
    m_tvDeviceProperties.clear();
    m_vxControls.clear();
    m_vxTableRecords.clear();
    m_suns.clear();
    m_backgrounds.clear();
    m_materials.clear();
    ...
    m_pointPaths.clear();
    m_objectPtrs.clear();
    m_partialViewingIndexes.clear();
    m_dbColors.clear();
    m_dimensionAssociations.clear();
    m_evaluationGraphs.clear();
    m_blockRepresentationData.clear();
    m_sections.clear();
    m_modelerGeometry.clear();
    m_surfaceGeometry.clear();
    m_tables.clear();
    ...
    m_pointCloudDefinitions.clear();
    m_navisworksModelDefinitions.clear();
    m_navisworksModels.clear();
    m_pointCloudColorMaps.clear();
    ...
    m_csacDocumentOptions.clear();
    m_contextDataManagers.clear();
    m_proxyEntities.clear();
    m_vbaProjects.clear();
    m_plotSettings.clear();
  ...
  void addUnsupportedObject(const DRW_UnsupportedObject &object) {
    ...
    record.parentHandle = object.m_parentHandle;
    record.commonLinkEvidence = object.m_commonLinkEvidence;
    record.commonHandleDataValidated =
        drwDwgCommonLinkEvidenceValidated(record.commonLinkEvidence);
    record.reactorHandles = object.m_reactorHandles;
    record.xDictHandle = object.m_xDictHandle;
    record.numReactors = object.m_numReactors;
    record.xDictFlag = object.m_xDictFlag;
    record.blockOwnerHandle = object.m_blockOwnerHandle;
    ...
    record.family =
        rawObjectFamilyFromNames(record.recordName, record.className);
    record.rawBytes = object.m_rawBytes;
    record.typedPayloadValidated = object.m_typedPayloadValidated;
    record.hasDataStorage = object.m_hasDataStorage;
    record.hasClassDefinition = object.m_hasClassDefinition;
    record.classProxyFlag = object.m_classProxyFlag;
    record.classAppName = object.m_classAppName;
    record.classWasProxy = object.m_classWasProxy;
    record.classEntityFlagRaw = object.m_classEntityFlagRaw;
    record.classDwgVersion = object.m_classDwgVersion;
    record.classMaintenanceVersion = object.m_classMaintenanceVersion;
    record.classUnknown1 = object.m_classUnknown1;
    record.classUnknown2 = object.m_classUnknown2;
    m_rawObjects.push_back(std::move(record));
  }

  void addDwgFramePublication(const DRW_DwgFramePublication &publication) {
    const auto existing = std::find_if(
        m_dwgFramePublications.begin(), m_dwgFramePublications.end(),
        [&publication](const DRW_DwgFramePublication &candidate) {
          return candidate.m_handle == publication.m_handle;
        });
    if (existing == m_dwgFramePublications.end()) {
      m_dwgFramePublications.push_back(publication);
      return;
    }
    if (sameDwgFrameIdentity(*existing, publication) &&
        canMergeDwgFrameCarriers(existing->m_carrier, publication.m_carrier)) {
      existing->m_carrier =
          mergedDwgFrameCarrier(existing->m_carrier, publication.m_carrier);
      return;
    }
    m_dwgFramePublicationCoverageComplete = false;
    if (std::find(m_dwgFramePublicationConflicts.cbegin(),
                  m_dwgFramePublicationConflicts.cend(),
                  publication.m_handle) ==
        m_dwgFramePublicationConflicts.cend()) {
      m_dwgFramePublicationConflicts.push_back(publication.m_handle);
    }
    m_dwgFramePublications.push_back(publication);
  }

  void
  addDwgDictionaryMembership(const DRW_DwgDictionaryMembership &membership) {
    m_dwgDictionaryMemberships.push_back(membership);
  }

  void addDwgDictionaryWithDefaultMembership(
      const DRW_DwgDictionaryWithDefaultMembership &membership) {
    m_dwgDictionaryWithDefaultMemberships.push_back(membership);
  }

  void addDwgGroupMembership(const DRW_DwgGroupMembership &membership) {
    m_dwgGroupMemberships.push_back(membership);
  }

  void addDwgSortEntsMembership(const DRW_DwgSortEntsMembership &membership) {
    m_dwgSortEntsMemberships.push_back(membership);
  }

  void addDwgFieldListMembership(const DRW_DwgFieldListMembership &membership) {
    m_dwgFieldListMemberships.push_back(membership);
  }

  void addDwgFieldPayloadReceipt(const DRW_DwgFieldPayloadReceipt &receipt) {
    m_dwgFieldPayloadReceipts.push_back(receipt);
  }

  void addDwgTypedReference(const DRW_DwgTypedReference &reference) {
    m_dwgTypedReferences.push_back(reference);
  }

  void addDwgBlockReachability(const DRW_DwgBlockReachability &reachability) {
    m_dwgBlockReachabilities.push_back(reachability);
  }

  void addDwgFrameCoverageReport(const DRW_DwgFrameCoverageReport &report) {
    m_dwgFrameCoverageReport = report;
    m_dwgFramePublicationCoverageComplete =
        report.m_status == DRW_DwgFrameCoverageStatus::FinalizedComplete &&
        report.m_complete && m_dwgFramePublicationConflicts.empty();
  }

  void addDwgClassCoverageReport(const DRW_DwgClassCoverageReport &report) {
    m_dwgClassCoverageReport = report;
  }
  ...
  void addRawDwgSection(const DRW_RawDwgSection &section) {
    ...
    record.encoding = section.m_encoding;
    record.encrypted = section.m_encrypted;
    record.maxSize = section.m_maxSize;
    for (const DataStorageRecord &storage : m_dataStorages) {
      if (storage.name == record.name && storage.version == record.version &&
          (storage.parseFailed || !storage.structurallyValid ||
           !storage.replayAllowed)) {
  ...
  void addDataStorage(const DRW_DataStorageSection &storage) {
    ...
    record.orphanRecordCount = storage.orphanRecordCount;
    record.payloadsRetained = storage.payloadsRetained;
    record.parseFailed = storage.parseFailed;
    record.structurallyValid = storage.structurallyValid;
    record.replayAllowed = storage.replayAllowed;
    ...
    if (storage.parseFailed || !storage.structurallyValid ||
        !storage.replayAllowed) {
      for (RawDwgSectionRecord &raw : m_rawDwgSections) {
        if (raw.name == storage.m_name && raw.version == storage.m_version) {
          raw.replayState = ReplayState::ReplayInvalidated;
        }
      }
    }
  }

  // Retain complete typed modern carriers alongside their raw shells. The
  // typed values are metadata-only until LibreCAD has native object models;
  // same-version raw replay remains authoritative for writing.
  void addCsacDocumentOptions(const DRW_CsacDocumentOptions &data) {
    m_csacDocumentOptions.push_back(data);
    noteFamilyExposure("CSACDOCUMENTOPTIONS", data.handle,
                       static_cast<std::uint32_t>(data.parentHandle),
                       "typed metadata-only carrier");
  }

  void addContextDataManager(const DRW_ContextDataManager &data) {
    m_contextDataManagers.push_back(data);
    noteFamilyExposure("CONTEXTDATAMANAGER", data.handle,
                       static_cast<std::uint32_t>(data.parentHandle),
                       "typed metadata-only carrier");
  }

  void addProxyEntity(const DRW_ProxyEntity &data) {
    m_proxyEntities.push_back(data);
    noteFamilyExposure("ACAD_PROXY_ENTITY", data.handle, data.parentHandle,
                       "typed metadata-only carrier");
  }

  void addVbaProject(const DRW_VbaProject &data) {
    m_vbaProjects.push_back(data);
    noteFamilyExposure("VBA_PROJECT", data.handle,
                       static_cast<std::uint32_t>(data.parentHandle),
                       "typed metadata-only carrier");
  }

  void addPlotSettings(const DRW_PlotSettings &data) {
    m_plotSettings.push_back(data);
    noteFamilyExposure("PLOTSETTINGS", data.handle,
                       static_cast<std::uint32_t>(data.parentHandle),
                       "typed metadata-only carrier");
  }

  void addProxyObject(const DRW_ProxyObject &data) {
    m_proxyObjects.push_back(data);
    noteFamilyExposure("ACAD_PROXY_OBJECT", data.handle,
                       static_cast<std::uint32_t>(data.parentHandle),
                       "typed metadata-only carrier");
  }

  // Slice A2: lossless DXF passthrough store (group-text records captured by
  // the DXF reader's processRawObject/processRawEntity). Kept on the graphic so
  // a read filter and a later write filter share them across a DXF round-trip.
  ...
  void addRawDxfSection(const DRW_RawDxfSection &section) {
    m_rawDxfSections.push_back(section);
  }

  void addDxfClass(const DRW_Class &cls) { m_dxfClasses.push_back(cls); }

  void addLineTypeName(const DRW_LType &data) {
    if (data.name.empty())
      return;
    m_lineTypeTableEntries[data.name] = data;
    if (data.handle != 0) {
      m_lineTypeNames[data.handle] = data.name;
      m_lineTypes[data.handle] = data;
    }
  }

  void addTextStyleName(const DRW_Textstyle &data) {
    if (data.name.empty())
      return;
    m_textStyleTableEntries[data.name] = data;
    if (data.handle != 0) {
      m_textStyleNames[data.handle] = data.name;
      m_textStyles[data.handle] = data;
    }
  }

  void addLayerTableEntry(const DRW_Layer &data) {
    if (!data.name.empty())
      m_layerTableEntries[data.name] = data;
  }

  void addDimStyleTableEntry(const DRW_Dimstyle &data) {
    if (!data.name.empty())
      m_dimStyleTableEntries[data.name] = data;
  }
  ...
  void addView(const DRW_View &view) {
    ...
    record.hasUcsHandleRefs =
        record.namedUcsHandle != 0 || record.baseUcsHandle != 0;
    record.hasVisualHandleRefs =
        record.backgroundHandle != 0 || record.visualStyleHandle != 0 ||
        record.sunHandle != 0 || record.liveSectionHandle != 0;
    record.sunResolved =
        record.sunHandle != 0 && findSunByHandle(record.sunHandle) != nullptr;
    ...
    record.appData = view.appData;
    record.extData = copyTableEntryExtData(view.extData);
    record.reactorHandles = view.reactorHandles;
    record.xDictHandle = view.xDictHandle;
  ...
  void addUcs(const DRW_UCS &ucs) {
    ...
    record.appData = ucs.appData;
    record.extData = copyTableEntryExtData(ucs.extData);
    record.reactorHandles = ucs.reactorHandles;
    record.xDictHandle = ucs.xDictHandle;
  ...
  void addVport(const DRW_Vport &vport) {
    ...
    record.appData = vport.appData;
    record.extData = copyTableEntryExtData(vport.extData);
    record.reactorHandles = vport.reactorHandles;
    record.xDictHandle = vport.xDictHandle;
    m_vports.push_back(record);
    if (!vport.name.empty())
      m_vportTableEntries[vport.name] = vport;
    upsertDocumentMapping(documentMappingFromVport(record, -1));
  }

  void addVxControl(const DRW_VxControl &control) {
    VxControlRecord record;
    record.handle = control.handle;
    record.parentHandle = static_cast<std::uint32_t>(control.parentHandle);
    record.appData = control.appData;
    record.extData = copyTableEntryExtData(control.extData);
    record.classVersion = control.classVersion;
    record.flags = control.flags;
    record.recordHandles = control.recordHandles;
    record.rawData = control.dwgRawData();
    record.rawDataBitSize = control.dwgRawDataBitSize();
    record.rawDataVersion = control.dwgRawDataVersion();
    record.rawDataValid = control.hasDwgRawData();
    record.reactorHandles = control.reactorHandles;
    record.xDictHandle = control.xDictHandle;
    record.numReactors = control.reactorCount();
    record.xDictFlag = control.extensionDictionaryFlag();
    record.hasDsData = control.hasDataStorageBinaryData();
    m_vxControls.push_back(std::move(record));
  }

  void addViewportEntityHeader(const DRW_ViewportEntityHeader &header) {
    ViewportEntityHeaderRecord record;
    record.handle = header.handle;
    record.parentHandle = static_cast<std::uint32_t>(header.parentHandle);
    record.name = header.name;
    record.flags = header.flags;
    record.isOn = header.isOn;
    record.xrefBlockHandle = header.xrefBlockHandle;
    record.viewportEntityHandle = header.viewportEntityHandle;
    record.previousViewportEntityHeaderHandle =
        header.previousViewportEntityHeaderHandle;
    record.reactorHandles = header.reactorHandles;
    record.xDictHandle = header.xDictHandle;
    record.numReactors = header.reactorCount();
    record.xDictFlag = header.extensionDictionaryFlag();
    record.hasDsData = header.hasDataStorageBinaryData();
    m_viewportEntityHeaders.push_back(std::move(record));
  }

  void addVxTableRecord(const DRW_VxTableRecord &tableRecord) {
    VxTableRecordRecord record;
    record.handle = tableRecord.handle;
    record.parentHandle = static_cast<std::uint32_t>(tableRecord.parentHandle);
    record.appData = tableRecord.appData;
    record.extData = copyTableEntryExtData(tableRecord.extData);
    record.name = tableRecord.name;
    record.classVersion = tableRecord.classVersion;
    record.flags = tableRecord.flags;
    record.rawData = tableRecord.dwgRawData();
    record.rawDataBitSize = tableRecord.dwgRawDataBitSize();
    record.rawDataVersion = tableRecord.dwgRawDataVersion();
    record.rawDataValid = tableRecord.hasDwgRawData();
    record.reactorHandles = tableRecord.reactorHandles;
    record.xDictHandle = tableRecord.xDictHandle;
    record.numReactors = tableRecord.reactorCount();
    record.xDictFlag = tableRecord.extensionDictionaryFlag();
    record.hasDsData = tableRecord.hasDataStorageBinaryData();
    m_vxTableRecords.push_back(std::move(record));
  }
  ...
  void addVisualStyle(const DRW_VisualStyle &visualStyle) {
    ...
    record.appData = visualStyle.appData;
    record.extData = copyTableEntryExtData(visualStyle.extData);
    record.name = visualStyle.name;
    record.description = visualStyle.desc;
    record.type = visualStyle.type;
    record.body = visualStyle.m_body;
    record.bodyDecoded = visualStyle.m_bodyDecoded;
    record.reactorHandles = visualStyle.reactorHandles;
    record.xDictHandle = visualStyle.xDictHandle;
    record.numReactors = visualStyle.reactorCount();
    record.xDictFlag = visualStyle.extensionDictionaryFlag();
    record.hasDsData = visualStyle.hasDataStorageBinaryData();
  ...
  void addCamera(const DRW_Camera &camera) {
    CameraRecord record;
    record.handle = camera.handle;
    record.parentHandle = camera.parentHandle;
    record.viewHandle = camera.m_viewHandle;
    record.extData = camera.extData;
    m_cameras.push_back(record);
  }

  void addLightList(const DRW_LightList &lightList) {
    LightListRecord record;
    record.handle = lightList.handle;
    record.parentHandle = static_cast<std::uint32_t>(lightList.parentHandle);
    record.classVersion = lightList.m_classVersion;
    record.lightCount = lightList.m_lightCount;
    record.lights.reserve(lightList.m_lights.size());
    for (const DRW_LightListEntry &light : lightList.m_lights) {
      LightListEntryRecord entry;
      entry.handle = light.m_handle;
      entry.name = light.m_name;
      record.lights.push_back(std::move(entry));
    }
    record.appData = lightList.appData;
    record.extData = copyTableEntryExtData(lightList.extData);
    record.reactorHandles = lightList.reactorHandles;
    record.xDictHandle = lightList.xDictHandle;
    record.numReactors = lightList.reactorCount();
    record.xDictFlag = lightList.extensionDictionaryFlag();
    record.hasDsData = lightList.hasDataStorageBinaryData();
    m_lightLists.push_back(std::move(record));
  }

  void addLayerFilter(const DRW_LayerFilter &layerFilter) {
    LayerFilterRecord record;
    record.handle = layerFilter.handle;
    record.parentHandle = static_cast<std::uint32_t>(layerFilter.parentHandle);
    record.nameCount = layerFilter.m_nameCount;
    record.names.reserve(layerFilter.m_names.size());
    for (const std::string &name : layerFilter.m_names)
      record.names.push_back(name);
    m_layerFilters.push_back(std::move(record));
  }

  void addDataLink(const DRW_DataLink &dataLink) {
    DataLinkRecord record;
    record.handle = dataLink.handle;
    record.parentHandle = static_cast<std::uint32_t>(dataLink.parentHandle);
    record.dataAdapter = dataLink.m_dataAdapter;
    record.description = dataLink.m_description;
    record.tooltip = dataLink.m_tooltip;
    record.connectionString = dataLink.m_connectionString;
    record.option = dataLink.m_option;
    record.updateOption = dataLink.m_updateOption;
    record.value92 = dataLink.m_value92;
    record.year = dataLink.m_year;
    record.month = dataLink.m_month;
    record.day = dataLink.m_day;
    record.hour = dataLink.m_hour;
    record.minute = dataLink.m_minute;
    record.seconds = dataLink.m_seconds;
    record.milliseconds = dataLink.m_milliseconds;
    record.pathOption = dataLink.m_pathOption;
    record.value93 = dataLink.m_value93;
    record.updateStatus = dataLink.m_updateStatus;
    record.customDataCount = dataLink.m_customDataCount;
    record.customData.reserve(dataLink.m_customData.size());
    for (const DRW_DataLinkCustomData &custom : dataLink.m_customData) {
      DataLinkCustomDataRecord entry;
      entry.targetHandle = custom.m_targetHandle;
      entry.text = custom.m_text;
      record.customData.push_back(std::move(entry));
    }
    record.hardOwnerHandle = dataLink.m_hardOwnerHandle;
    m_dataLinks.push_back(std::move(record));
  }

  void addGeoMapImage(const DRW_GeoMapImage &image) {
    GeoMapImageRecord record;
    record.handle = image.handle;
    record.parentHandle = static_cast<std::uint32_t>(image.parentHandle);
    record.classVersion = image.m_classVersion;
    record.insertionPoint = image.m_insertionPoint;
    record.imageSize = image.m_imageSize;
    record.displayProps = image.m_displayProps;
    record.clipping = image.m_clipping;
    record.brightness = image.m_brightness;
    record.contrast = image.m_contrast;
    record.fade = image.m_fade;
    m_geoMapImages.push_back(std::move(record));
  }

  void addGeoPositionMarker(const DRW_GeoPositionMarker &marker) {
    GeoPositionMarkerRecord record;
    record.handle = marker.handle;
    record.parentHandle = marker.parentHandle;
    record.classVersion = marker.m_classVersion;
    record.position = marker.m_position;
    record.radius = marker.m_radius;
    record.notes = marker.m_notes;
    record.landingGap = marker.m_landingGap;
    record.mtextVisible = marker.m_mtextVisible;
    record.textAlignment = marker.m_textAlignment;
    record.enableFrameText = marker.m_enableFrameText;
    if (marker.mtext != nullptr) {
      record.embeddedMText = std::make_shared<DRW_MText>(*marker.mtext);
    }
    record.extData = marker.extData;
    m_geoPositionMarkers.push_back(std::move(record));
  }

  void addSectionObject(const DRW_SectionObject &section) {
    SectionObjectRecord record;
    record.handle = section.handle;
    record.parentHandle = section.parentHandle;
    record.state = section.m_state;
    record.flags = section.m_flags;
    record.name = section.m_name;
    record.vertDir = section.m_vertDir;
    record.topHeight = section.m_topHeight;
    record.bottomHeight = section.m_bottomHeight;
    record.indicatorAlpha = section.m_indicatorAlpha;
    record.indicatorColor = section.m_indicatorColor;
    record.verts = section.m_verts;
    record.blVerts = section.m_blVerts;
    record.sectionSettingsHandle = section.m_sectionSettingsHandle;
    record.extData = section.extData;
    m_sectionObjects.push_back(std::move(record));
  }

  void addTvDeviceProperties(const DRW_TvDeviceProperties &properties) {
    TvDevicePropertiesRecord record;
    record.handle = properties.handle;
    record.parentHandle = static_cast<std::uint32_t>(properties.parentHandle);
    record.flags = properties.flags;
    record.maxRegenThreads = properties.maxRegenThreads;
    record.useLutPalette = properties.useLutPalette;
    record.alternateHighlight = properties.alternateHighlight;
    record.alternateHighlightColor = properties.alternateHighlightColor;
    record.geometryShaderUsage = properties.geometryShaderUsage;
    record.blendingMode = properties.blendingMode;
    record.antialiasingLevel = properties.antialiasingLevel;
    record.valueBd2 = properties.valueBd2;
    record.appData = properties.appData;
    record.extData = copyTableEntryExtData(properties.extData);
    record.reactorHandles = properties.reactorHandles;
    record.xDictHandle = properties.xDictHandle;
    record.numReactors = properties.reactorCount();
    record.xDictFlag = properties.extensionDictionaryFlag();
    record.hasDsData = properties.hasDataStorageBinaryData();
    m_tvDeviceProperties.push_back(std::move(record));
  }
  ...
  void addSun(const DRW_Sun &sun) {
    ...
    record.appData = sun.appData;
    record.extData = copyTableEntryExtData(sun.extData);
    ...
    record.reactorHandles = sun.reactorHandles;
    record.xDictHandle = sun.xDictHandle;
    record.numReactors = sun.reactorCount();
    record.xDictFlag = sun.extensionDictionaryFlag();
    record.hasDsData = sun.hasDataStorageBinaryData();
  ...
  void addBackground(const DRW_Background &background) {
    BackgroundRecord record;
    record.handle = background.handle;
    record.parentHandle = background.parentHandle;
    record.appData = background.appData;
    record.extData = copyTableEntryExtData(background.extData);
    record.kind = background.m_kind;
    record.classVersion = background.m_classVersion;
    record.solidColor = background.m_solidColor;
    record.colorTop = background.m_colorTop;
    record.colorMiddle = background.m_colorMiddle;
    record.colorBottom = background.m_colorBottom;
    record.horizon = background.m_horizon;
    record.height = background.m_height;
    record.rotation = background.m_rotation;
    record.colorSkyZenith = background.m_colorSkyZenith;
    record.colorSkyHorizon = background.m_colorSkyHorizon;
    record.colorUndergroundHorizon = background.m_colorUndergroundHorizon;
    record.colorUndergroundAzimuth = background.m_colorUndergroundAzimuth;
    record.colorNear = background.m_colorNear;
    record.colorFar = background.m_colorFar;
    record.fileName = background.m_fileName;
    record.fitToScreen = background.m_fitToScreen;
    record.maintainAspect = background.m_maintainAspect;
    record.useTiling = background.m_useTiling;
    record.offset = background.m_offset;
    record.scale = background.m_scale;
    record.iblName = background.m_iblName;
    record.enabled = background.m_enabled;
    record.displayImage = background.m_displayImage;
    record.secondaryBackgroundHandle = background.m_secondaryBackgroundHandle;
    record.sunHandle = background.m_sunHandle;
    record.reactorHandles = background.reactorHandles;
    record.xDictHandle = background.xDictHandle;
    record.numReactors = background.reactorCount();
    record.xDictFlag = background.extensionDictionaryFlag();
    record.hasDsData = background.hasDataStorageBinaryData();
    m_backgrounds.push_back(std::move(record));
  }

  void addMaterial(const DRW_Material &material) {
    MaterialRecord record;
    record.handle = material.handle;
    record.parentHandle = material.parentHandle;
    record.name = material.m_name;
    record.description = material.m_description;
    record.appData = material.appData;
    record.extData = copyTableEntryExtData(material.extData);
    record.reactorHandles = material.reactorHandles;
    record.xDictHandle = material.xDictHandle;
    record.numReactors = material.reactorCount();
    record.xDictFlag = material.extensionDictionaryFlag();
    record.hasDsData = material.hasDataStorageBinaryData();
    record.rawData = material.dwgRawData();
    record.rawDataBitSize = material.dwgRawDataBitSize();
    record.rawDataVersion = material.dwgRawDataVersion();
    record.rawDataValid = material.hasDwgRawData();
    m_materials.push_back(std::move(record));
  }

  void addRenderSettings(const DRW_RenderSettings &settings) {
    RenderSettingsRecord record;
    record.handle = settings.handle;
    record.parentHandle = settings.parentHandle;
    record.appData = settings.appData;
    record.extData = copyTableEntryExtData(settings.extData);
    record.kind = settings.m_kind;
    record.classVersion = settings.m_classVersion;
    record.name = settings.m_name;
    record.longs = settings.m_longs;
    record.strings = settings.m_strings;
    record.bools = settings.m_bools;
    record.shorts = settings.m_shorts;
    record.bytes = settings.m_bytes;
    record.doubles = settings.m_doubles;
    record.fogEnabled = settings.m_fogEnabled;
    record.fogBackgroundEnabled = settings.m_fogBackgroundEnabled;
    record.backfacesEnabled = settings.m_backfacesEnabled;
    record.environmentImageEnabled = settings.m_environmentImageEnabled;
    record.environmentImageFilename = settings.m_environmentImageFilename;
    record.description = settings.m_description;
    record.displayIndex = settings.m_displayIndex;
    record.hasPredefined = settings.m_hasPredefined;
    record.hasPredefinedPresent = settings.m_hasPredefinedPresent;
    record.fogColorR = settings.m_fogColorR;
    record.fogColorG = settings.m_fogColorG;
    record.fogColorB = settings.m_fogColorB;
    record.fogDensityNear = settings.m_fogDensityNear;
    record.fogDensityFar = settings.m_fogDensityFar;
    record.fogDistanceNear = settings.m_fogDistanceNear;
    record.fogDistanceFar = settings.m_fogDistanceFar;
    record.procedure = settings.m_procedure;
    record.destination = settings.m_destination;
    record.reactorHandles = settings.reactorHandles;
    record.xDictHandle = settings.xDictHandle;
    record.numReactors = settings.reactorCount();
    record.xDictFlag = settings.extensionDictionaryFlag();
    record.hasDsData = settings.hasDataStorageBinaryData();
    record.rawData = settings.dwgRawData();
    record.rawDataBitSize = settings.dwgRawDataBitSize();
    record.rawDataVersion = settings.dwgRawDataVersion();
    record.rawDataValid = settings.hasDwgRawData();
    m_renderSettings.push_back(std::move(record));
  }

  void addSunStudy(const DRW_SunStudy &study) {
    SunStudyRecord record;
    record.handle = study.handle;
    record.parentHandle = study.parentHandle;
    record.appData = study.appData;
    record.extData = copyTableEntryExtData(study.extData);
    record.classVersion = study.m_classVersion;
    record.setupName = study.m_setupName;
    record.description = study.m_description;
    record.sheetSetName = study.m_sheetSetName;
    record.sheetSubsetName = study.m_sheetSubsetName;
    record.outputType = study.m_outputType;
    record.useSubset = study.m_useSubset;
    record.selectDatesFromCalendar = study.m_selectDatesFromCalendar;
    record.selectRangeOfDates = study.m_selectRangeOfDates;
    record.lockViewports = study.m_lockViewports;
    record.labelViewports = study.m_labelViewports;
    record.startTime = study.m_startTime;
    record.endTime = study.m_endTime;
    record.interval = study.m_interval;
    record.shadePlotType = study.m_shadePlotType;
    record.viewportCount = study.m_viewportCount;
    record.rowCount = study.m_rowCount;
    record.columnCount = study.m_columnCount;
    record.spacing = study.m_spacing;
    record.dates = study.m_dates;
    record.hours = study.m_hours;
    record.pageSetupWizardHandle = study.m_pageSetupWizardHandle;
    record.viewHandle = study.m_viewHandle;
    record.visualStyleHandle = study.m_visualStyleHandle;
    record.textStyleHandle = study.m_textStyleHandle;
    record.reactorHandles = study.reactorHandles;
    record.xDictHandle = study.xDictHandle;
    record.numReactors = study.reactorCount();
    record.xDictFlag = study.extensionDictionaryFlag();
    record.hasDsData = study.hasDataStorageBinaryData();
    m_sunStudies.push_back(std::move(record));
  }

  void addMotionPath(const DRW_MotionPath &path) {
    MotionPathRecord record;
    record.handle = path.handle;
    record.parentHandle = path.parentHandle;
    record.appData = path.appData;
    record.extData = copyTableEntryExtData(path.extData);
    record.classVersion = path.m_classVersion;
    record.cameraPathHandle = path.m_cameraPathHandle;
    record.targetPathHandle = path.m_targetPathHandle;
    record.viewTableHandle = path.m_viewTableHandle;
    record.frames = path.m_frames;
    record.frameRate = path.m_frameRate;
    record.cornerDeceleration = path.m_cornerDeceleration;
    record.reactorHandles = path.reactorHandles;
    record.xDictHandle = path.xDictHandle;
    record.numReactors = path.reactorCount();
    record.xDictFlag = path.extensionDictionaryFlag();
    record.hasDsData = path.hasDataStorageBinaryData();
    m_motionPaths.push_back(std::move(record));
  }

  void addCurvePath(const DRW_CurvePath &path) {
    CurvePathRecord record;
    record.handle = path.handle;
    record.parentHandle = path.parentHandle;
    record.appData = path.appData;
    record.extData = copyTableEntryExtData(path.extData);
    record.classVersion = path.m_classVersion;
    record.entityHandle = path.m_entityHandle;
    record.reactorHandles = path.reactorHandles;
    record.xDictHandle = path.xDictHandle;
    record.numReactors = path.reactorCount();
    record.xDictFlag = path.extensionDictionaryFlag();
    record.hasDsData = path.hasDataStorageBinaryData();
    m_curvePaths.push_back(std::move(record));
  }

  void addPointPath(const DRW_PointPath &path) {
    PointPathRecord record;
    record.handle = path.handle;
    record.parentHandle = path.parentHandle;
    record.appData = path.appData;
    record.extData = copyTableEntryExtData(path.extData);
    record.classVersion = path.m_classVersion;
    record.point = path.m_point;
    record.reactorHandles = path.reactorHandles;
    record.xDictHandle = path.xDictHandle;
    record.numReactors = path.reactorCount();
    record.xDictFlag = path.extensionDictionaryFlag();
    record.hasDsData = path.hasDataStorageBinaryData();
    m_pointPaths.push_back(std::move(record));
  }

  void addObjectPtr(const DRW_ObjectPtr &object) {
    ObjectPtrRecord record;
    record.handle = object.handle;
    record.parentHandle = object.parentHandle;
    record.appData = object.appData;
    record.extData = copyTableEntryExtData(object.extData);
    record.reactorHandles = object.reactorHandles;
    record.xDictHandle = object.xDictHandle;
    record.numReactors = object.reactorCount();
    record.xDictFlag = object.extensionDictionaryFlag();
    record.hasDsData = object.hasDataStorageBinaryData();
    m_objectPtrs.push_back(std::move(record));
  }

  void addPartialViewingIndex(const DRW_PartialViewingIndex &index) {
    PartialViewingIndexRecord record;
    record.handle = index.handle;
    record.parentHandle = index.parentHandle;
    record.appData = index.appData;
    record.extData = copyTableEntryExtData(index.extData);
    record.entryCount = index.m_entryCount;
    record.hasEntries = index.m_hasEntries;
    record.entries = index.m_entries;
    record.reactorHandles = index.reactorHandles;
    record.xDictHandle = index.xDictHandle;
    record.numReactors = index.reactorCount();
    record.xDictFlag = index.extensionDictionaryFlag();
    record.hasDsData = index.hasDataStorageBinaryData();
    m_partialViewingIndexes.push_back(std::move(record));
  }

  void addDbColor(const DRW_DbColor &color) {
    DbColorRecord record;
    record.handle = color.handle;
    record.parentHandle = color.parentHandle;
    record.appData = color.appData;
    record.extData = copyTableEntryExtData(color.extData);
    record.reactorHandles = color.reactorHandles;
    record.xDictHandle = color.xDictHandle;
    record.numReactors = color.reactorCount();
    record.xDictFlag = color.extensionDictionaryFlag();
    record.hasDsData = color.hasDataStorageBinaryData();
    record.name = color.name;
    record.rgb = color.rgb;
    record.colorIndex = color.colorIndex;
    record.colorMethod = color.colorMethod;
    record.bookName = color.bookName;
    m_dbColors.push_back(std::move(record));
  }

  void addDimensionAssociation(const DRW_DimensionAssociation &association) {
    DimensionAssociationRecord record;
    record.handle = association.handle;
    record.parentHandle = association.parentHandle;
    record.appData = association.appData;
    record.extData = copyTableEntryExtData(association.extData);
    record.reactorHandles = association.reactorHandles;
    record.xDictHandle = association.xDictHandle;
    record.numReactors = association.reactorCount();
    record.xDictFlag = association.extensionDictionaryFlag();
    record.hasDsData = association.hasDataStorageBinaryData();
    record.dimensionHandle = association.m_dimensionHandle;
    record.associativityFlags = association.m_associativityFlags;
    record.isTransSpace = association.m_isTransSpace;
    record.rotatedDimensionType = association.m_rotatedDimensionType;
    record.osnapRefs = association.m_osnapRefs;
    record.hasUnrepresentableDetail = association.m_hasUnrepresentableDetail;
    m_dimensionAssociations.push_back(std::move(record));
  }

  void addEvaluationGraph(const DRW_EvaluationGraph &graph) {
    EvaluationGraphRecord record;
    record.handle = graph.handle;
    record.parentHandle = graph.parentHandle;
    record.appData = graph.appData;
    record.extData = copyTableEntryExtData(graph.extData);
    record.reactorHandles = graph.reactorHandles;
    record.xDictHandle = graph.xDictHandle;
    record.numReactors = graph.reactorCount();
    record.xDictFlag = graph.extensionDictionaryFlag();
    record.hasDsData = graph.hasDataStorageBinaryData();
    record.value96 = graph.m_value96;
    record.value97 = graph.m_value97;
    record.nodes = graph.m_nodes;
    record.edges = graph.m_edges;
    m_evaluationGraphs.push_back(std::move(record));
  }

  void addBlockRepresentationData(const DRW_BlockRepresentationData &data) {
    BlockRepresentationRecord record;
    record.handle = data.handle;
    record.parentHandle = data.parentHandle;
    record.appData = data.appData;
    record.extData = copyTableEntryExtData(data.extData);
    record.reactorHandles = data.reactorHandles;
    record.xDictHandle = data.xDictHandle;
    record.numReactors = data.reactorCount();
    record.xDictFlag = data.extensionDictionaryFlag();
    record.hasDsData = data.hasDataStorageBinaryData();
    record.flag = data.m_flag;
    record.blockHandle = data.m_blockHandle;
    m_blockRepresentationData.push_back(std::move(record));
  }

  void addSection(const DRW_Section &section) {
    SectionRecord record;
    record.handle = section.handle;
    record.parentHandle = section.parentHandle;
    record.appData = section.appData;
    record.extData = copyTableEntryExtData(section.extData);
    record.reactorHandles = section.reactorHandles;
    record.xDictHandle = section.xDictHandle;
    record.xDictFlag = section.extensionDictionaryFlag();
    record.numReactors = section.reactorCount();
    record.hasDsData = section.hasDataStorageBinaryData();
    record.rawData = section.dwgRawData();
    record.rawDataBitSize = section.dwgRawDataBitSize();
    record.rawDataVersion = section.dwgRawDataVersion();
    record.rawDataValid = section.hasDwgRawData();
    record.kind = section.m_kind;
    record.isLive = section.m_isLive;
    record.sectionCount = section.m_sectionCount;
    record.sectionHandles = section.m_sectionHandles;
    record.classVersion = section.m_classVersion;
    record.sectionType = section.m_sectionType;
    record.generationOptions = section.m_generationOptions;
    record.currentType = section.m_currentType;
    record.typeCount = section.m_typeCount;
    record.types = section.m_types;
    record.sourceHandle = section.m_sourceHandle;
    record.destinationBlockHandle = section.m_destinationBlockHandle;
    record.destinationFileHandle = section.m_destinationFileHandle;
    record.destinationFile = section.m_destinationFile;
    m_sections.push_back(std::move(record));
  }
  ...
  void addModelerGeometry(const DRW_ModelerGeometry &geometry) {
    ...
    record.hasDataStorageRecord = geometry.hasDataStorageRecord;
    record.hasDsData = geometry.hasDataStorageBinaryData();
    record.dataStorageHandle = geometry.dataStorageHandle;
    record.dataStorageHandleKey = geometry.dataStorageHandleKey;
    record.dataStorageData = geometry.dataStorageData;
    record.dataStorageSegmentIndex = geometry.dataStorageSegmentIndex;
    record.dataStorageSchemaIndex = geometry.dataStorageSchemaIndex;
    const ModelerPayloadMarker marker =
        geometry.hasDataStoragePayloadMarker
            ? ModelerPayloadMarker{ModelerPayloadKind::Sab,
                                   geometry.dataStoragePayloadMarkerOffset,
                                   geometry.dataStoragePayloadMarkerLength,
                                   "ACIS BinaryFile"}
            : scanModelerPayloadMarker(geometry.m_rawBytes);
    ...
    record.markerSection = geometry.hasDataStoragePayloadMarker
                               ? ModelerPayloadSection::DataStorage
                               : ModelerPayloadSection::Unknown;
    record.historyHandle = geometry.m_historyHandle;
    record.rawByteCount = geometry.m_rawBytes.size();
    const ModelerRawByteSplit split =
        splitModelerRawBytes(geometry.m_bodyBitSize, record.rawByteCount);
    ...
    if (!geometry.hasDataStoragePayloadMarker) {
      record.markerSection =
          modelerPayloadSectionForMarker(marker, split, record.rawByteCount);
    }
    record.payloadRanges = scanModelerPayloadRanges(
        geometry.m_rawBytes, geometry.m_bodyBitSize, geometry.m_hasWireframe);
    record.rawBytes = geometry.m_rawBytes;
    record.extData = geometry.extData;
    m_modelerGeometry.push_back(std::move(record));
  }

  void addSurface(const DRW_Surface &surface) {
    SurfaceGeometryRecord record;
    record.handle = surface.handle;
    record.parentHandle = surface.parentHandle;
    record.type = surface.eType;
    record.uIsolines = surface.uIsolines;
    record.vIsolines = surface.vIsolines;
    record.modelerFormatVersion = surface.modelerFormatVersion;
    record.acisEmpty = surface.acisEmpty;
    record.acisVersion = surface.acisVersion;
    record.hasDataStorageRecord = surface.hasDataStorageRecord;
    record.hasDsData = surface.hasDataStorageBinaryData();
    record.dataStorageHandle = surface.dataStorageHandle;
    record.dataStorageHandleKey = surface.dataStorageHandleKey;
    record.dataStorageData = surface.dataStorageData;
    record.dataStorageSegmentIndex = surface.dataStorageSegmentIndex;
    record.dataStorageSchemaIndex = surface.dataStorageSchemaIndex;
    const ModelerPayloadMarker marker =
        surface.hasDataStoragePayloadMarker
            ? ModelerPayloadMarker{ModelerPayloadKind::Sab,
                                   surface.dataStoragePayloadMarkerOffset,
                                   surface.dataStoragePayloadMarkerLength,
                                   "ACIS BinaryFile"}
            : scanModelerPayloadMarker(surface.rawAcisData);
    record.payloadKind = marker.kind;
    record.markerOffset = marker.offset;
    record.markerLength = marker.length;
    record.markerText = marker.text;
    if (surface.hasDataStoragePayloadMarker) {
      record.markerSection = ModelerPayloadSection::DataStorage;
    } else if (marker.kind != ModelerPayloadKind::Unknown) {
      record.markerSection = ModelerPayloadSection::Body;
    }
    record.rawAcisData = surface.rawAcisData;
    m_surfaceGeometry.push_back(std::move(record));
  }
  ...
  void addTableStyle(const DRW_TableStyle &style) {
    ...
    record.nativeTableStyle = style;
    record.hasDsData = style.hasDataStorageBinaryData();
    ...
    record.m_tableUnknownHandle = style.m_unknownHandle;
  ...
  void addTable(const DRW_Table &table, bool fallbackRendered) {
    ...
    record.semanticParsed =
        table.m_hasSemanticContent && table.m_semanticContentComplete;
  ...
  void
  updateTableFallbackRenderSummary(const TableFallbackRenderSummary &summary) {
    ...
    for (TableRecord &record : m_tables) {
      ...
      record.m_fallbackPlaceholderEntityCount = summary.placeholderEntityCount;
      record.m_fallbackUnresolvedTextStyleCount =
          summary.unresolvedTextStyleCount;
      record.m_fallbackClampedDimensionCount = summary.clampedDimensionCount;
      return;
    }
  }
  ...
  void addAssociativeObject(const DRW_AssociativeObject &object) {
    ...
    record.persistentSubentityCount = object.m_persistentSubentityCount;
    record.persistentSubentityHandles = object.m_persistentSubentityHandles;
    record.persistentUnknown0 = object.m_persistentUnknown0;
    record.persistentUnknown2 = object.m_persistentUnknown2;
    record.persistentAssocStepCount = object.m_persistentAssocStepCount;
    record.persistentAssocSubentityCount =
        object.m_persistentAssocSubentityCount;
    record.persistentSteps = object.m_persistentSteps;
    record.persistentSubentityIds = object.m_persistentSubentityIds;
    ...
    for (const DRW_AssociativePrefixStatus &prefix : object.m_prefixStatuses) {
  ...
  void addAcShObject(const DRW_AcShHistoryObject &object) {
    ...
    record.blobBytes =
        object.m_binaryBlob1.size() + object.m_binaryBlob2.size();
    for (const DRW_AssociativePrefixStatus &prefix : object.m_prefixStatuses) {
      record.prefixStatuses.push_back(makeAssociativePrefixStatus(prefix));
    }
  ...
  void addMLeader(const DRW_MLeader &mleader) {
    ...
    for (const DRW_MLeader::BlockLabelEntry &blockLabel : mleader.blockLabels) {
      record.blockAttributeDefinitionHandles.push_back(
          blockLabel.attDefHandle.ref);
  ...
  void addMLeaderStyle(const DRW_MLeaderStyle &style) {
    ...
    record.appData = style.appData;
    record.extData = copyTableEntryExtData(style.extData);
    record.reactorHandles = style.reactorHandles;
    record.xDictHandle = style.xDictHandle;
    record.numReactors = style.reactorCount();
    record.xDictFlag = style.extensionDictionaryFlag();
    record.hasDsData = style.hasDataStorageBinaryData();
    ...
    resolveDictionaryMLeaderStyleNames();
  ...
  void addGroup(const DRW_Group &group) {
    ...
    record.appData = group.appData;
    record.extData = copyTableEntryExtData(group.extData);
    record.reactorHandles = group.reactorHandles;
    record.xDictHandle = group.xDictHandle;
    record.numReactors = group.reactorCount();
    record.xDictFlag = group.extensionDictionaryFlag();
    record.hasDsData = group.hasDataStorageBinaryData();
  ...
  void addImageDefinitionReactor(const DRW_ImageDefinitionReactor &reactor) {
    ...
    record.appData = reactor.appData;
    record.extData = copyTableEntryExtData(reactor.extData);
    record.reactorHandles = reactor.reactorHandles;
    record.xDictHandle = reactor.xDictHandle;
    record.numReactors = reactor.reactorCount();
    record.xDictFlag = reactor.extensionDictionaryFlag();
    record.hasDsData = reactor.hasDataStorageBinaryData();
  ...
  void addSpatialFilter(const DRW_SpatialFilter &filter) {
    ...
    record.appData = filter.appData;
    record.extData = copyTableEntryExtData(filter.extData);
    record.reactorHandles = filter.reactorHandles;
    record.xDictHandle = filter.xDictHandle;
    record.numReactors = filter.reactorCount();
    record.xDictFlag = filter.extensionDictionaryFlag();
    record.hasDsData = filter.hasDataStorageBinaryData();
  ...
  void addRasterVariables(const DRW_RasterVariables &rasterVariables) {
    ...
    record.appData = rasterVariables.appData;
    record.extData = copyTableEntryExtData(rasterVariables.extData);
    ...
    record.reactorHandles = rasterVariables.reactorHandles;
    record.xDictHandle = rasterVariables.xDictHandle;
    record.numReactors = rasterVariables.reactorCount();
    record.xDictFlag = rasterVariables.extensionDictionaryFlag();
    record.hasDsData = rasterVariables.hasDataStorageBinaryData();
  ...
  void addImageDefinition(const DRW_ImageDef &imageDefinition) {
    ...
    record.appData = imageDefinition.appData;
    record.extData = copyTableEntryExtData(imageDefinition.extData);
    record.commonReactorHandles = imageDefinition.reactorHandles;
    record.xDictHandle = imageDefinition.xDictHandle;
    record.numReactors = imageDefinition.reactorCount();
    record.xDictFlag = imageDefinition.extensionDictionaryFlag();
    record.hasDsData = imageDefinition.hasDataStorageBinaryData();
  ...
  void addRasterImage(const DRW_Image &image, bool isWipeout) {
    ...
    record.clipBoundaryType = image.m_clipBoundaryType;
    record.clipPath = image.clipPath;
    record.clipStatus =
        clipBoundaryStatus(image.clip, image.clipPath.size(), isWipeout);
    record.invertedClip = image.clipMode;
    record.frameVisible = image.clip != 0;
    record.hasDefinitionHandle = image.ref != 0;
    record.reactorHandle = image.m_imageDefReactorHandle;
    record.hasReactorHandle = record.reactorHandle != 0;
  ...
  void addUnderlayDefinition(const DRW_UnderlayDefinition &definition) {
    ...
    record.appData = definition.appData;
    record.extData = copyTableEntryExtData(definition.extData);
    record.kind = static_cast<int>(definition.kind);
    record.path = definition.filename;
    record.sheetName = definition.sheetName;
    record.reactorHandles = definition.reactorHandles;
    record.xDictHandle = definition.xDictHandle;
    record.numReactors = definition.reactorCount();
    record.xDictFlag = definition.extensionDictionaryFlag();
    record.hasDsData = definition.hasDataStorageBinaryData();
    record.pathDiagnostic = externalReferencePathDiagnostic(record.path);
  ...
  void addUnderlay(const DRW_Underlay &underlay,
    ...
    record.clipStatus =
        underlayClipBoundaryStatus(underlay.clipBoundary.size());
    record.frameVisible = (underlay.flags & 2u) != 0;
    record.fallbackPreviewGenerated = fallbackPreviewGenerated;
    m_underlays.push_back(record);
  }

  void addPointCloudDef(const DRW_PointCloudDef &definition) {
    PointCloudDefinitionRecord record;
    record.handle = definition.handle;
    record.parentHandle = static_cast<std::uint32_t>(definition.parentHandle);
    record.appData = definition.appData;
    record.extData = copyTableEntryExtData(definition.extData);
    record.kind = static_cast<int>(definition.m_kind);
    record.classVersion = definition.m_classVersion;
    record.sourceFilename = definition.m_sourceFilename;
    record.isLoaded = definition.m_isLoaded;
    record.pointCount = definition.m_pointCount;
    record.extentsMin = definition.m_extentsMin;
    record.extentsMax = definition.m_extentsMax;
    record.reactorHandles = definition.reactorHandles;
    record.xDictHandle = definition.xDictHandle;
    record.numReactors = definition.reactorCount();
    record.xDictFlag = definition.extensionDictionaryFlag();
    record.hasDsData = definition.hasDataStorageBinaryData();
    m_pointCloudDefinitions.push_back(std::move(record));
  }

  void addNavisworksModelDef(const DRW_NavisworksModelDef &definition) {
    NavisworksModelDefinitionRecord record;
    record.handle = definition.handle;
    record.parentHandle = static_cast<std::uint32_t>(definition.parentHandle);
    record.appData = definition.appData;
    record.extData = copyTableEntryExtData(definition.extData);
    record.flags = definition.m_flags;
    record.path = definition.m_path;
    record.status = definition.m_status;
    record.minExtent = definition.m_minExtent;
    record.maxExtent = definition.m_maxExtent;
    record.hostDrawingVisibility = definition.m_hostDrawingVisibility;
    record.reactorHandles = definition.reactorHandles;
    record.xDictHandle = definition.xDictHandle;
    record.numReactors = definition.reactorCount();
    record.xDictFlag = definition.extensionDictionaryFlag();
    record.hasDsData = definition.hasDataStorageBinaryData();
    m_navisworksModelDefinitions.push_back(std::move(record));
  }

  void addNavisworksModel(const DRW_NavisworksModel &model) {
    NavisworksModelRecord record;
    record.handle = model.handle;
    record.parentHandle = model.parentHandle;
    record.flags = model.flags;
    record.definitionHandle = model.definitionHandle;
    record.transform = model.transform;
    record.unitFactor = model.unitFactor;
    m_navisworksModels.push_back(std::move(record));
  }

  void addPointCloudColorMap(const DRW_PointCloudColorMap &colorMap) {
    PointCloudColorMapRecord record;
    record.handle = colorMap.handle;
    record.parentHandle = static_cast<std::uint32_t>(colorMap.parentHandle);
    record.appData = colorMap.appData;
    record.extData = copyTableEntryExtData(colorMap.extData);
    record.classVersion = colorMap.m_classVersion;
    record.defaultIntensityColorScheme = colorMap.m_defaultIntensityColorScheme;
    record.defaultElevationColorScheme = colorMap.m_defaultElevationColorScheme;
    record.defaultClassificationColorScheme =
        colorMap.m_defaultClassificationColorScheme;
    record.declaredColorRampCount = colorMap.m_colorRampCount;
    record.declaredClassificationColorRampCount =
        colorMap.m_classificationColorRampCount;
    const auto copyRamps = [](const auto &source, auto &destination) {
      destination.reserve(source.size());
      for (const auto &ramp : source) {
        PointCloudColorMapRampRecord copied;
        copied.classVersion = ramp.m_classVersion;
        copied.declaredColorCount = ramp.m_rampCount;
        copied.colorSchemes = ramp.m_colorSchemes;
        destination.push_back(std::move(copied));
      }
    };
    copyRamps(colorMap.m_colorRamps, record.colorRamps);
    copyRamps(colorMap.m_classificationColorRamps,
              record.classificationColorRamps);
    record.reactorHandles = colorMap.reactorHandles;
    record.xDictHandle = colorMap.xDictHandle;
    record.numReactors = colorMap.reactorCount();
    record.xDictFlag = colorMap.extensionDictionaryFlag();
    record.hasDsData = colorMap.hasDataStorageBinaryData();
    m_pointCloudColorMaps.push_back(std::move(record));
  }
  ...
  void addShape(const DRW_Shape &shape) {
    ...
    record.rawRangeStatus = shape.m_rawBytes.empty()
                                ? MeshRawRangeStatus::Missing
  ...
  void addOle2Frame(const DRW_Ole2Frame &ole2Frame) {
    ...
    record.lockAspect = ole2Frame.m_lockAspect;
  ...
  void addOleFrame(const DRW_OleFrame &oleFrame) {
    OleFrameRecord record;
    record.handle = oleFrame.handle;
    record.parentHandle = oleFrame.parentHandle;
    record.flags = oleFrame.m_flags;
    record.mode = oleFrame.m_mode;
    record.declaredPayloadLength = oleFrame.m_declaredPayloadLength;
    record.payloadByteCount = oleFrame.m_payloadByteCount;
    record.payloadPresent = oleFrame.m_payloadPresent;
    record.payloadTruncated = oleFrame.m_payloadTruncated;
    record.payloadTooLarge = oleFrame.m_payloadTooLarge;
    record.rawByteCount = oleFrame.m_rawBytes.size();
    record.payloadBytes = oleFrame.m_payloadBytes;
    m_oleFrames.push_back(std::move(record));
  }
  ...
  void addGeoData(const DRW_GeoData &geoData) {
    ...
    record.appData = geoData.appData;
    record.extData = copyTableEntryExtData(geoData.extData);
    record.reactorHandles = geoData.reactorHandles;
    record.xDictHandle = geoData.xDictHandle;
    record.numReactors = geoData.reactorCount();
    record.xDictFlag = geoData.extensionDictionaryFlag();
    record.hasDsData = geoData.hasDataStorageBinaryData();
  ...
  void addAcDbPlaceholder(const DRW_AcDbPlaceholder &placeholder) {
    ...
    record.appData = placeholder.appData;
    record.extData = copyTableEntryExtData(placeholder.extData);
    record.reactorHandles = placeholder.reactorHandles;
    record.xDictHandle = placeholder.xDictHandle;
    record.numReactors = placeholder.reactorCount();
    record.xDictFlag = placeholder.extensionDictionaryFlag();
    record.hasDsData = placeholder.hasDataStorageBinaryData();
  ...
  void addDictionary(const DRW_Dictionary &dictionary) {
    ...
    record.countCap = dictionary.countCap;
    record.appData = dictionary.appData;
    record.extData = copyTableEntryExtData(dictionary.extData);
    record.reactorHandles = dictionary.reactorHandles;
    record.xDictHandle = dictionary.xDictHandle;
    record.numReactors = dictionary.reactorCount();
    record.xDictFlag = dictionary.extensionDictionaryFlag();
    record.hasDsData = dictionary.hasDataStorageBinaryData();
    record.typedPayloadComplete = dictionary.hasCompleteDwgEntries();
  ...
  void addXRecord(const DRW_XRecord &xrecord) {
    ...
    record.dataEntries = xrecord.m_dataEntries;
    record.rawData = xrecord.m_rawData;
    record.rawDataValid = xrecord.m_rawDataValid;
    record.appData = xrecord.appData;
    record.extData = copyTableEntryExtData(xrecord.extData);
    record.reactorHandles = xrecord.reactorHandles;
    record.xDictHandle = xrecord.xDictHandle;
    record.numReactors = xrecord.reactorCount();
    record.xDictFlag = xrecord.extensionDictionaryFlag();
    record.hasDsData = xrecord.hasDataStorageBinaryData();
  ...
  void addLayout(const DRW_Layout &layout) {
    ...
    record.appData = layout.appData;
    record.extData = copyTableEntryExtData(layout.extData);
    record.reactorHandles = layout.reactorHandles;
    record.xDictHandle = layout.xDictHandle;
    record.numReactors = layout.reactorCount();
    record.xDictFlag = layout.extensionDictionaryFlag();
    record.hasDsData = layout.hasDataStorageBinaryData();
    // PlotSettings prefix.
    ...
    record.shadePlotHandle = layout.shadePlotHandle.ref;
  ...
  void addScale(const DRW_Scale &scale) {
    ...
    record.appData = scale.appData;
    record.extData = copyTableEntryExtData(scale.extData);
    record.reactorHandles = scale.reactorHandles;
    record.xDictHandle = scale.xDictHandle;
    record.numReactors = scale.reactorCount();
    record.xDictFlag = scale.extensionDictionaryFlag();
    record.hasDsData = scale.hasDataStorageBinaryData();
  ...
  void addObjectContextData(const DRW_ObjectContextData &context) {
    ...
    record.columnType = context.m_columnType;
    record.columnHeightCount = context.m_columnHeightCount;
    record.fragmentCount = context.m_fragmentCount;
    record.columnWidth = context.m_columnWidth;
    record.columnGutter = context.m_columnGutter;
    record.columnAutoHeight = context.m_columnAutoHeight;
    record.columnFlowReversed = context.m_columnFlowReversed;
    record.columnHeights = context.m_columnHeights;
    record.embeddedScaleEnabled = context.m_embeddedScaleEnabled;
    record.embeddedScaleFlag = context.m_embeddedScaleFlag;
    record.embeddedScaleName = context.m_embeddedScaleName;
    record.embeddedScalePaperUnits = context.m_embeddedScalePaperUnits;
    record.embeddedScaleDrawingUnits = context.m_embeddedScaleDrawingUnits;
    record.embeddedScaleUnitScale = context.m_embeddedScaleUnitScale;
  ...
  void addMLineStyle(const DRW_MLineStyle &style) {
    ...
    for (const DRW_MLineElement &e : style.elements) {
      ...
      er.offset = e.offset;
      er.color = e.color;
      er.color24 = e.color24;
      er.linetypeIndex = e.linetypeIndex;
      er.linetypeHandle = e.linetypeHandle;
  ...
  void addWipeoutVariables(const DRW_WipeoutVariables &wv) {
    ...
    record.appData = wv.appData;
    record.extData = copyTableEntryExtData(wv.extData);
    record.displayFrame = wv.m_displayFrame;
    record.reactorHandles = wv.reactorHandles;
    record.xDictHandle = wv.xDictHandle;
    record.numReactors = wv.reactorCount();
    record.xDictFlag = wv.extensionDictionaryFlag();
    record.hasDsData = wv.hasDataStorageBinaryData();
  ...
  void addIDBuffer(const DRW_IDBuffer &idBuffer) {
    ...
    record.appData = idBuffer.appData;
    record.extData = copyTableEntryExtData(idBuffer.extData);
    record.reactorHandles = idBuffer.reactorHandles;
    record.xDictHandle = idBuffer.xDictHandle;
    record.numReactors = idBuffer.reactorCount();
    record.xDictFlag = idBuffer.extensionDictionaryFlag();
    record.hasDsData = idBuffer.hasDataStorageBinaryData();
    record.classVersion = idBuffer.classVersion;
    record.objIds = idBuffer.objIds;
    m_idBuffers.push_back(std::move(record));
  }

  void addIndex(const DRW_Index &index) {
    IndexRecord record;
    record.handle = index.handle;
    record.parentHandle = static_cast<std::uint32_t>(index.parentHandle);
    record.timestamp1 = index.timestamp1;
    record.timestamp2 = index.timestamp2;
    m_indexes.push_back(std::move(record));
  }
  ...
  void addLayerIndex(const DRW_LayerIndex &layerIndex) {
    ...
    record.appData = layerIndex.appData;
    record.extData = copyTableEntryExtData(layerIndex.extData);
    ...
    record.reactorHandles = layerIndex.reactorHandles;
    record.xDictHandle = layerIndex.xDictHandle;
    record.numReactors = layerIndex.reactorCount();
    record.xDictFlag = layerIndex.extensionDictionaryFlag();
    record.hasDsData = layerIndex.hasDataStorageBinaryData();
  ...
  void addSpatialIndex(const DRW_SpatialIndex &spatialIndex) {
    ...
    record.appData = spatialIndex.appData;
    record.extData = copyTableEntryExtData(spatialIndex.extData);
    record.timestamp1 = spatialIndex.timestamp1;
    record.timestamp2 = spatialIndex.timestamp2;
    record.reactorHandles = spatialIndex.reactorHandles;
    record.xDictHandle = spatialIndex.xDictHandle;
    record.numReactors = spatialIndex.reactorCount();
    record.xDictFlag = spatialIndex.extensionDictionaryFlag();
    record.hasDsData = spatialIndex.hasDataStorageBinaryData();
  ...
  void addDictionaryVar(const DRW_DictionaryVar &dictionaryVar) {
    ...
    record.appData = dictionaryVar.appData;
    record.extData = copyTableEntryExtData(dictionaryVar.extData);
    record.schema = dictionaryVar.m_schema;
    record.value = dictionaryVar.m_value;
    record.reactorHandles = dictionaryVar.reactorHandles;
    record.xDictHandle = dictionaryVar.xDictHandle;
    record.numReactors = dictionaryVar.reactorCount();
    record.xDictFlag = dictionaryVar.extensionDictionaryFlag();
    record.hasDsData = dictionaryVar.hasDataStorageBinaryData();
  ...
  void addSortEntsTable(const DRW_SortEntsTable &sortEntsTable) {
    ...
    record.appData = sortEntsTable.appData;
    record.extData = copyTableEntryExtData(sortEntsTable.extData);
    ...
    record.reactorHandles = sortEntsTable.reactorHandles;
    record.xDictHandle = sortEntsTable.xDictHandle;
    record.numReactors = sortEntsTable.reactorCount();
    record.xDictFlag = sortEntsTable.extensionDictionaryFlag();
    record.hasDsData = sortEntsTable.hasDataStorageBinaryData();
  ...
  void addFieldList(const DRW_FieldList &fieldList) {
    ...
    record.appData = fieldList.appData;
    record.extData = copyTableEntryExtData(fieldList.extData);
    record.reactorHandles = fieldList.reactorHandles;
    record.xDictHandle = fieldList.xDictHandle;
    record.numReactors = fieldList.reactorCount();
    record.xDictFlag = fieldList.extensionDictionaryFlag();
    record.hasDsData = fieldList.hasDataStorageBinaryData();
  ...
  void addDynamicBlockObject(const DRW_DynamicBlockObject &obj) {
    ...
    record.valueHandle = obj.m_valueHandle;
    record.nodeId = obj.m_nodeId;
    record.purgeFlag = obj.m_purgeFlag;
    record.purgeBlockHandle = obj.m_purgeBlockHandle;
    ...
    record.gripValue91 = obj.m_gripValue91;
    record.gripValue92 = obj.m_gripValue92;
    record.gripLocationX = obj.m_gripLocation.x;
    record.gripLocationY = obj.m_gripLocation.y;
    record.gripLocationZ = obj.m_gripLocation.z;
    record.gripValue280 = obj.m_gripValue280;
    record.gripValue93 = obj.m_gripValue93;
    record.gripCombinedState = obj.m_gripCombinedState;
    record.gripHasOrientation = obj.m_gripHasOrientation;
    record.gripOrientationX = obj.m_gripOrientation.x;
    record.gripOrientationY = obj.m_gripOrientation.y;
    record.gripOrientationZ = obj.m_gripOrientation.z;
    record.defPointX = obj.m_defPoint.x;
    record.defPointY = obj.m_defPoint.y;
    record.defPointZ = obj.m_defPoint.z;
    record.propertyInfo1Count = obj.m_propertyInfo1Count;
    record.propertyInfo1Codes = obj.m_propertyInfo1Codes;
    record.propertyInfo1Names.reserve(obj.m_propertyInfo1Names.size());
    for (const auto &n : obj.m_propertyInfo1Names)
      record.propertyInfo1Names.push_back(n);
    record.propertyInfo2Count = obj.m_propertyInfo2Count;
    record.propertyInfo2Codes = obj.m_propertyInfo2Codes;
    record.propertyInfo2Names.reserve(obj.m_propertyInfo2Names.size());
    for (const auto &n : obj.m_propertyInfo2Names)
      record.propertyInfo2Names.push_back(n);
    record.propertyInfoCount = obj.m_propertyInfoCount;
    record.parameterValue170 = obj.m_parameterValue170;
    record.parameterValue171 = obj.m_parameterValue171;
    record.parameterValue93 = obj.m_parameterValue93;
    record.firstPointX = obj.m_firstPoint.x;
    record.firstPointY = obj.m_firstPoint.y;
    record.firstPointZ = obj.m_firstPoint.z;
    record.secondPointX = obj.m_secondPoint.x;
    record.secondPointY = obj.m_secondPoint.y;
    record.secondPointZ = obj.m_secondPoint.z;
    record.twoPointPropertyCodes = obj.m_twoPointPropertyCodes;
    for (std::size_t i = 0; i < obj.m_twoPointPropertyNames.size(); ++i) {
      record.twoPointPropertyNames[i].reserve(
          obj.m_twoPointPropertyNames[i].size());
      for (const auto &n : obj.m_twoPointPropertyNames[i])
        record.twoPointPropertyNames[i].push_back(n);
    }
    record.propertyStates = obj.m_propertyStates;
    record.parameterBaseLocation = obj.m_parameterBaseLocation;
    record.flipCaption = obj.m_flipCaption;
    record.flipDescription = obj.m_flipDescription;
    record.flipBaseStateName = obj.m_flipBaseStateName;
    record.flipFlippedStateName = obj.m_flipFlippedStateName;
    record.flipCaptionLocationX = obj.m_flipCaptionLocation.x;
    record.flipCaptionLocationY = obj.m_flipCaptionLocation.y;
    record.flipCaptionLocationZ = obj.m_flipCaptionLocation.z;
    record.flipTooltip = obj.m_flipTooltip;
    record.flipValue96 = obj.m_flipValue96;
    record.rotationBaseAnglePointX = obj.m_rotationBaseAnglePoint.x;
    record.rotationBaseAnglePointY = obj.m_rotationBaseAnglePoint.y;
    record.rotationBaseAnglePointZ = obj.m_rotationBaseAnglePoint.z;
    record.rotationAngleName = obj.m_rotationAngleName;
    record.rotationAngleDescription = obj.m_rotationAngleDescription;
    record.rotationAngle = obj.m_rotationAngle;
    record.rotationValueSetFlags = obj.m_rotationValueSetFlags;
    record.rotationMinimum = obj.m_rotationMinimum;
    record.rotationMaximum = obj.m_rotationMaximum;
    record.rotationIncrement = obj.m_rotationIncrement;
    record.rotationDeclaredValueCount = obj.m_rotationDeclaredValueCount;
    record.rotationValues = obj.m_rotationValues;
    record.linearDistanceName = obj.m_linearDistanceName;
    record.linearDistanceDescription = obj.m_linearDistanceDescription;
    record.linearDistance = obj.m_linearDistance;
    record.linearValue96 = obj.m_linearValue96;
    record.linearValue141 = obj.m_linearValue141;
    record.linearValue142 = obj.m_linearValue142;
    record.linearValue143 = obj.m_linearValue143;
    record.linearDeclaredValueCount = obj.m_linearDeclaredValueCount;
    record.linearValues = obj.m_linearValues;
    record.polarAngleName = obj.m_polarAngleName;
    record.polarAngleDescription = obj.m_polarAngleDescription;
    record.polarDistanceName = obj.m_polarDistanceName;
    record.polarDistanceDescription = obj.m_polarDistanceDescription;
    record.polarOffset = obj.m_polarOffset;
    record.polarAngleValueSetFlags = obj.m_polarAngleValueSetFlags;
    record.polarAngleMinimum = obj.m_polarAngleMinimum;
    record.polarAngleMaximum = obj.m_polarAngleMaximum;
    record.polarAngleIncrement = obj.m_polarAngleIncrement;
    record.polarAngleDeclaredValueCount = obj.m_polarAngleDeclaredValueCount;
    record.polarAngleValues = obj.m_polarAngleValues;
    record.polarDistanceValueSetFlags = obj.m_polarDistanceValueSetFlags;
    record.polarDistanceMinimum = obj.m_polarDistanceMinimum;
    record.polarDistanceMaximum = obj.m_polarDistanceMaximum;
    record.polarDistanceIncrement = obj.m_polarDistanceIncrement;
    record.polarDistanceDeclaredValueCount =
        obj.m_polarDistanceDeclaredValueCount;
    record.polarDistanceValues = obj.m_polarDistanceValues;
    record.alignPerpendicular = obj.m_alignPerpendicular;
    record.xyXLabel = obj.m_xyXLabel;
    record.xyXDescription = obj.m_xyXDescription;
    record.xyYLabel = obj.m_xyYLabel;
    record.xyYDescription = obj.m_xyYDescription;
    record.xyXValue = obj.m_xyXValue;
    record.xyYValue = obj.m_xyYValue;
    record.xyXValueSetFlags = obj.m_xyXValueSetFlags;
    record.xyXMinimum = obj.m_xyXMinimum;
    record.xyXMaximum = obj.m_xyXMaximum;
    record.xyXIncrement = obj.m_xyXIncrement;
    record.xyXDeclaredValueCount = obj.m_xyXDeclaredValueCount;
    record.xyXValues = obj.m_xyXValues;
    record.xyYValueSetFlags = obj.m_xyYValueSetFlags;
    record.xyYMinimum = obj.m_xyYMinimum;
    record.xyYMaximum = obj.m_xyYMaximum;
    record.xyYIncrement = obj.m_xyYIncrement;
    record.xyYDeclaredValueCount = obj.m_xyYDeclaredValueCount;
    record.xyYValues = obj.m_xyYValues;
    record.constraintDependencyHandle = obj.m_constraintDependencyHandle;
    record.constraintExpressionName = obj.m_constraintExpressionName;
    record.constraintExpressionDescription =
        obj.m_constraintExpressionDescription;
    record.constraintValue = obj.m_constraintValue;
    record.constraintValueSetFlags = obj.m_constraintValueSetFlags;
    record.constraintMinimum = obj.m_constraintMinimum;
    record.constraintMaximum = obj.m_constraintMaximum;
    record.constraintIncrement = obj.m_constraintIncrement;
    record.constraintDeclaredValueCount = obj.m_constraintDeclaredValueCount;
    record.constraintValues = obj.m_constraintValues;
    record.angularCenterPointX = obj.m_angularCenterPoint.x;
    record.angularCenterPointY = obj.m_angularCenterPoint.y;
    record.angularCenterPointZ = obj.m_angularCenterPoint.z;
    record.angularEndPointX = obj.m_angularEndPoint.x;
    record.angularEndPointY = obj.m_angularEndPoint.y;
    record.angularEndPointZ = obj.m_angularEndPoint.z;
    record.angularOrientationOnBothGrips = obj.m_angularOrientationOnBothGrips;
    record.positionName = obj.m_positionName;
    record.positionDescription = obj.m_positionDescription;
    record.labelPointX = obj.m_labelPoint.x;
    record.labelPointY = obj.m_labelPoint.y;
    record.labelPointZ = obj.m_labelPoint.z;
    record.pointX = obj.m_point.x;
    record.pointY = obj.m_point.y;
    record.pointZ = obj.m_point.z;
    record.basePointX = obj.m_basePoint.x;
    record.basePointY = obj.m_basePoint.y;
    record.basePointZ = obj.m_basePoint.z;
    record.lookupIndex = obj.m_lookupIndex;
    record.lookupName = obj.m_lookupName;
    record.lookupDescription = obj.m_lookupDescription;
    record.lookupUnknownText = obj.m_lookupUnknownText;
    ...
    record.blockHandles = obj.m_blockHandles;
    record.stateBlockHandles = obj.m_stateBlockHandles;
    record.stateParameterHandles = obj.m_stateParameterHandles;
    record.dependencyCount = obj.m_dependencyCount;
    record.actionCount = obj.m_actionCount;
    record.dependencyHandles = obj.m_dependencyHandles;
    ...
    record.connectionCodes = obj.m_connectionCodes;
    record.rotateActionOffsetX = obj.m_rotateActionOffset.x;
    record.rotateActionOffsetY = obj.m_rotateActionOffset.y;
    record.rotateActionOffsetZ = obj.m_rotateActionOffset.z;
    record.rotateBaseConnectionNames.reserve(
        obj.m_rotateBaseConnectionNames.size());
    for (const auto &n : obj.m_rotateBaseConnectionNames)
      record.rotateBaseConnectionNames.push_back(n);
    record.rotateBaseConnectionCodes = obj.m_rotateBaseConnectionCodes;
    record.rotateActionDependent = obj.m_rotateActionDependent;
    record.rotateActionBasePointX = obj.m_rotateActionBasePoint.x;
    record.rotateActionBasePointY = obj.m_rotateActionBasePoint.y;
    record.rotateActionBasePointZ = obj.m_rotateActionBasePoint.z;
    record.rotateConnectionCode = obj.m_rotateConnectionCode;
    record.rotateConnectionName = obj.m_rotateConnectionName;
    record.arrayColumnOffset = obj.m_arrayColumnOffset;
    record.arrayRowOffset = obj.m_arrayRowOffset;
    record.lookupRowCount = obj.m_lookupRowCount;
    record.lookupColumnCount = obj.m_lookupColumnCount;
    for (const auto &expression : obj.m_lookupExpressions)
      record.lookupExpressions.push_back(expression);
    record.lookupConnectionCodes = obj.m_lookupConnectionCodes;
    for (const auto &names : obj.m_lookupConnectionNames) {
      std::vector<std::string> converted;
      converted.reserve(names.size());
      for (const auto &name : names)
        converted.push_back(name);
      record.lookupConnectionNames.push_back(std::move(converted));
    }
    record.lookupEntryFlags282 = obj.m_lookupEntryFlags282;
    record.lookupEntryFlags281 = obj.m_lookupEntryFlags281;
    record.lookupFlag280 = obj.m_lookupFlag280;
    record.polarStretchPointCount = obj.m_polarStretchPointCount;
    record.polarStretchPointCoordinates.reserve(
        obj.m_polarStretchPoints.size() * 3);
    for (const auto &point : obj.m_polarStretchPoints) {
      record.polarStretchPointCoordinates.push_back(point.x);
      record.polarStretchPointCoordinates.push_back(point.y);
      record.polarStretchPointCoordinates.push_back(point.z);
    }
    record.polarStretchHandleCount = obj.m_polarStretchHandleCount;
    record.polarStretchHandles = obj.m_polarStretchHandles;
    record.polarStretchHandleShorts = obj.m_polarStretchHandleShorts;
    record.polarStretchCodeCount = obj.m_polarStretchCodeCount;
    record.polarStretchCodes = obj.m_polarStretchCodes;
    record.scaleOffsetX = obj.m_scaleOffset.x;
    record.scaleOffsetY = obj.m_scaleOffset.y;
    record.scaleOffsetZ = obj.m_scaleOffset.z;
    record.baseConnectionNames.reserve(obj.m_baseConnectionNames.size());
    for (const auto &n : obj.m_baseConnectionNames)
      record.baseConnectionNames.push_back(n);
    record.baseConnectionCodes = obj.m_baseConnectionCodes;
    record.scaleDependent = obj.m_scaleDependent;
    record.scaleBasePointX = obj.m_scaleBasePoint.x;
    record.scaleBasePointY = obj.m_scaleBasePoint.y;
    record.scaleBasePointZ = obj.m_scaleBasePoint.z;
    record.scaleConnectionNames.reserve(obj.m_scaleConnectionNames.size());
    for (const auto &n : obj.m_scaleConnectionNames)
      record.scaleConnectionNames.push_back(n);
    record.scaleConnectionCodes = obj.m_scaleConnectionCodes;
    record.stretchPointCount = obj.m_stretchPointCount;
    record.stretchPointCoordinates.reserve(obj.m_stretchPoints.size() * 2);
    for (const auto &p : obj.m_stretchPoints) {
      record.stretchPointCoordinates.push_back(p.x);
      record.stretchPointCoordinates.push_back(p.y);
    }
    record.handleReferenceCount = obj.m_handleReferenceCount;
    record.handleReferenceHandles = obj.m_handleReferenceHandles;
    record.handleReferenceIndexCounts = obj.m_handleReferenceIndexCounts;
    record.handleReferenceIndexes = obj.m_handleReferenceIndexes;
    record.codeReferenceCount = obj.m_codeReferenceCount;
    record.codeReferenceCodes = obj.m_codeReferenceCodes;
    record.codeReferenceIndexCounts = obj.m_codeReferenceIndexCounts;
    record.codeReferenceIndexes = obj.m_codeReferenceIndexes;
    record.adbVersion = obj.m_adbVersion;
    record.dimensionBaseVersion = obj.m_dimensionBaseVersion;
    record.dependencyName = obj.m_dependencyName;
    record.classVersion = obj.m_classVersion;
  ...
  void addField(const DRW_Field &field) {
    ...
    record.appData = field.appData;
    record.extData = copyTableEntryExtData(field.extData);
    record.reactorHandles = field.reactorHandles;
    record.xDictHandle = field.xDictHandle;
    record.numReactors = field.reactorCount();
    record.xDictFlag = field.extensionDictionaryFlag();
    record.hasDsData = field.hasDataStorageBinaryData();
    record.typedPayloadComplete = field.hasCompleteDwgPayload();
  ...
  const std::vector<DRW_DwgFramePublication> &dwgFramePublications() const {
    return m_dwgFramePublications;
  }
  const std::vector<DRW_DwgDictionaryMembership> &
  dwgDictionaryMemberships() const {
    return m_dwgDictionaryMemberships;
  }
  const std::vector<DRW_DwgDictionaryWithDefaultMembership> &
  dwgDictionaryWithDefaultMemberships() const {
    return m_dwgDictionaryWithDefaultMemberships;
  }
  const std::vector<DRW_DwgGroupMembership> &dwgGroupMemberships() const {
    return m_dwgGroupMemberships;
  }
  const std::vector<DRW_DwgSortEntsMembership> &dwgSortEntsMemberships() const {
    return m_dwgSortEntsMemberships;
  }
  const std::vector<DRW_DwgFieldListMembership> &
  dwgFieldListMemberships() const {
    return m_dwgFieldListMemberships;
  }
  const std::vector<DRW_DwgFieldPayloadReceipt> &
  dwgFieldPayloadReceipts() const {
    return m_dwgFieldPayloadReceipts;
  }
  const std::vector<DRW_DwgTypedReference> &dwgTypedReferences() const {
    return m_dwgTypedReferences;
  }
  const std::vector<DRW_DwgBlockReachability> &dwgBlockReachabilities() const {
    return m_dwgBlockReachabilities;
  }
  bool dwgFramePublicationCoverageComplete() const {
    return m_dwgFramePublicationCoverageComplete;
  }
  const std::vector<std::uint32_t> &dwgFramePublicationConflicts() const {
    return m_dwgFramePublicationConflicts;
  }
  const DRW_DwgFrameCoverageReport &dwgFrameCoverageReport() const {
    return m_dwgFrameCoverageReport;
  }
  const DRW_DwgClassCoverageReport &dwgClassCoverageReport() const {
    return m_dwgClassCoverageReport;
  }
  DwgSourceGraphResult buildDwgSourceGraph() const {
    return buildDwgSourceGraphInventory(
        m_dwgFrameCoverageReport, m_dwgClassCoverageReport,
        m_dwgFramePublications, m_dwgDictionaryMemberships,
        m_dwgGroupMemberships, m_dwgSortEntsMemberships,
        m_dwgFieldListMemberships, m_dwgTypedReferences,
        m_dwgBlockReachabilities, m_dwgFramePublicationConflicts);
  }
  bool hasDwgFramePublicationForHandle(std::uint32_t handle) const {
    return std::any_of(m_dwgFramePublications.cbegin(),
                       m_dwgFramePublications.cend(),
  ...
  authorizeDwgFieldListSource(const FieldListRecord &record) const {
    ...
    const auto membershipMatches =
        [&record](const DRW_DwgFieldListMembership &value) {
          return value.m_listHandle == record.handle;
        };
    const auto coverageMatches =
        [](const DRW_DwgFrameCoverageEntry &coverage,
           const DRW_DwgFramePublication &publication) {
          return coverage.m_handle == publication.m_handle &&
                 coverage.m_sourceOffset == publication.m_sourceOffset &&
                 coverage.m_sourceMapOrdinal ==
                     publication.m_sourceMapOrdinal &&
                 coverage.m_sourceOffsetSpace ==
                     publication.m_sourceOffsetSpace &&
                 coverage.m_disposition == DRW_DwgFrameDisposition::Published &&
                 coverage.m_publicationCount == 1u;
        };
    const auto hasExactCoverage =
        [&coverageMatches, this](const DRW_DwgFramePublication &publication) {
          return std::count_if(m_dwgFrameCoverageReport.m_entries.cbegin(),
                               m_dwgFrameCoverageReport.m_entries.cend(),
                               [&coverageMatches, &publication](
                                   const DRW_DwgFrameCoverageEntry &coverage) {
                                 return coverageMatches(coverage, publication);
                               }) == 1u;
        };

    std::vector<const DRW_DwgFramePublication *> publications;
    for (const DRW_DwgFramePublication &publication : m_dwgFramePublications) {
      if (publicationMatches(publication))
        publications.push_back(&publication);
    }
    std::vector<const DRW_DwgFieldListMembership *> memberships;
    for (const DRW_DwgFieldListMembership &membership :
         m_dwgFieldListMemberships) {
      if (membershipMatches(membership))
        memberships.push_back(&membership);
    }
    if (publications.empty()) {
      return {memberships.empty()
                  ? DwgFieldListSourceAuthorizationStatus::NotDwgSource
                  : DwgFieldListSourceAuthorizationStatus::OrphanMembership};
    }
    if (publications.size() != 1u) {
      return {DwgFieldListSourceAuthorizationStatus::DuplicateSourceFrame};
    }
    const DRW_DwgFramePublication &publication = *publications.front();
    if (!isDwgFieldListPublication(publication) ||
        publication.m_version < DRW::AC1015 ||
        !publication.m_hasSourceLocation ||
        !publication.m_classStreamOrdinal.has_value()) {
      return {DwgFieldListSourceAuthorizationStatus::InvalidSourceFrame};
    }
    const std::size_t sameHandleCoverage = std::count_if(
        m_dwgFrameCoverageReport.m_entries.cbegin(),
        m_dwgFrameCoverageReport.m_entries.cend(),
        [&publication](const DRW_DwgFrameCoverageEntry &coverage) {
          return coverage.m_handle == publication.m_handle;
        });
    if (sameHandleCoverage == 0u) {
      return {DwgFieldListSourceAuthorizationStatus::MissingSourceCoverage};
    }
    if (sameHandleCoverage != 1u || !hasExactCoverage(publication)) {
      return {DwgFieldListSourceAuthorizationStatus::MismatchedSourceCoverage};
    }
    if (memberships.empty()) {
      return {DwgFieldListSourceAuthorizationStatus::MissingMembership};
    }
    if (memberships.size() != 1u) {
      return {DwgFieldListSourceAuthorizationStatus::DuplicateMembership};
    }
    const DRW_DwgFieldListMembership &membership = *memberships.front();
    if (!membership.m_complete || !membership.m_hasSourceLocation) {
      return {DwgFieldListSourceAuthorizationStatus::IncompleteMembership};
    }
    if (membership.m_entries.size() > DRW_Field::kMaxItems ||
        membership.m_version < DRW::AC1015 ||
        !membership.m_classStreamOrdinal.has_value()) {
      return {DwgFieldListSourceAuthorizationStatus::InvalidMembership};
    }
    if (membership.m_version != publication.m_version ||
        membership.m_sourceOffset != publication.m_sourceOffset ||
        membership.m_sourceMapOrdinal != publication.m_sourceMapOrdinal ||
        membership.m_sourceOffsetSpace != publication.m_sourceOffsetSpace ||
        ...
        membership.m_classStreamOrdinal != publication.m_classStreamOrdinal ||
    ...
    for (std::size_t ordinal = 0; ordinal < membership.m_entries.size();
      ...
      const DRW_DwgFieldListMembership::Entry &entry =
          membership.m_entries[ordinal];
      if (entry.m_ordinal != static_cast<std::uint32_t>(ordinal)) {
        return {DwgFieldListSourceAuthorizationStatus::InvalidMembership};
      }
      if (entry.m_fieldHandle != record.fieldHandles[ordinal]) {
        return {DwgFieldListSourceAuthorizationStatus::MismatchedMembership};
      }
      if (entry.m_fieldHandle == DRW::NoHandle)
        continue;
      std::vector<const DRW_DwgFramePublication *> fields;
      for (const DRW_DwgFramePublication &candidate : m_dwgFramePublications) {
        if (candidate.m_handle == entry.m_fieldHandle)
      ...
      if (fields.size() != 1u) {
        return {DwgFieldListSourceAuthorizationStatus::DuplicateFieldEndpoint};
      }
      const DRW_DwgFramePublication &field = *fields.front();
      if (!isDwgFieldPublication(field) || !field.m_hasSourceLocation ||
          !field.m_classStreamOrdinal.has_value() || !hasExactCoverage(field)) {
        return {DwgFieldListSourceAuthorizationStatus::InvalidFieldEndpoint};
      }
    }
...
private:
  static bool sameDwgFieldVariant(const DRW_Variant &left,
    ...
    if (left.code() != right.code() || left.type() != right.type() ||
        left.isLayerRef() != right.isLayerRef()) {
      return false;
    }
    switch (left.type()) {
    ...
    case DRW_Variant::INTEGER64:
      return left.i64_val() == right.i64_val();
    case DRW_Variant::DOUBLE:
      return left.d_val() == right.d_val();
    case DRW_Variant::COORD: {
      const DRW_Coord *leftCoord = left.coord();
      const DRW_Coord *rightCoord = right.coord();
      return leftCoord != nullptr && rightCoord != nullptr &&
             leftCoord->x == rightCoord->x && leftCoord->y == rightCoord->y &&
             leftCoord->z == rightCoord->z;
    }
    case DRW_Variant::BINARY: {
      const auto *leftBinary = left.binary();
      const auto *rightBinary = right.binary();
      return leftBinary != nullptr && rightBinary != nullptr &&
  ...
  static bool sameDwgFieldCadValue(const CadValueRecord &left,
                                   const DRW_CadValue &right) {
    return left.formatFlags == right.m_formatFlags &&
           left.dataType == right.m_dataType &&
           left.dataSize == right.m_dataSize &&
           left.unitType == right.m_unitType &&
           sameDwgFieldVariant(left.value, right.m_value) &&
           left.formatString == right.m_formatString &&
           left.valueString == right.m_valueString &&
           left.handle == right.m_handle && left.rawData == right.m_rawData;
  }

  static bool
  sameDwgFieldAppData(const std::list<std::list<DRW_Variant>> &left,
                      const std::list<std::list<DRW_Variant>> &right) {
    if (left.size() != right.size())
      return false;
    auto leftGroup = left.cbegin();
    auto rightGroup = right.cbegin();
    for (; leftGroup != left.cend(); ++leftGroup, ++rightGroup) {
      if (leftGroup->size() != rightGroup->size())
        return false;
      auto leftValue = leftGroup->cbegin();
      auto rightValue = rightGroup->cbegin();
      for (; leftValue != leftGroup->cend(); ++leftValue, ++rightValue) {
        if (!sameDwgFieldVariant(*leftValue, *rightValue))
          return false;
      }
    }
    return true;
  ...
  static bool
  sameDwgFieldExtData(const std::vector<std::shared_ptr<DRW_Variant>> &left,
                      const std::vector<DRW_Variant *> &right) {
    if (left.size() != right.size())
      return false;
    for (std::size_t index = 0; index < left.size(); ++index) {
      if (static_cast<bool>(left[index]) != (right[index] != nullptr) ||
          (left[index] != nullptr &&
           !sameDwgFieldVariant(*left[index], *right[index]))) {
        return false;
      }
    }
    return true;
  ...
  static bool sameDwgFieldRecord(const FieldRecord &left,
                                 const DRW_Field &right) {
    if (left.handle != right.handle ||
        left.parentHandle != right.parentHandle ||
        left.evaluatorId != right.m_evaluatorId ||
        left.fieldCode != right.m_fieldCode ||
        left.formatString != right.m_formatString ||
        left.evaluationOptionFlags != right.m_evaluationOptionFlags ||
        left.filingOptionFlags != right.m_filingOptionFlags ||
        left.fieldStateFlags != right.m_fieldStateFlags ||
        left.evaluationStatusFlags != right.m_evaluationStatusFlags ||
        left.evaluationErrorCode != right.m_evaluationErrorCode ||
        left.evaluationErrorMessage != right.m_evaluationErrorMessage ||
        !sameDwgFieldCadValue(left.value, right.m_value) ||
        left.valueString != right.m_valueString ||
        left.valueStringLength != right.m_valueStringLength ||
        left.childHandles != right.m_childHandles ||
        left.objectHandles != right.m_objectHandles ||
        !sameDwgFieldAppData(left.appData, right.appData) ||
        !sameDwgFieldExtData(left.extData, right.extData) ||
        left.reactorHandles != right.reactorHandles ||
        left.xDictHandle != right.xDictHandle ||
        left.numReactors != right.reactorCount() ||
        left.xDictFlag != right.extensionDictionaryFlag() ||
        left.hasDsData != right.hasDataStorageBinaryData() ||
        !left.typedPayloadComplete ||
        left.childValues.size() != right.m_childValues.size()) {
      return false;
    }
    for (std::size_t index = 0; index < left.childValues.size(); ++index) {
      if (left.childValues[index].key != right.m_childValues[index].m_key ||
          !sameDwgFieldCadValue(left.childValues[index].value,
                                right.m_childValues[index].m_value)) {
        return false;
      }
    }
    return true;
  ...
  static bool sameDwgFieldListRecord(const FieldListRecord &left,
                                     const DRW_FieldList &right) {
    return left.handle == right.handle &&
           left.parentHandle == right.parentHandle &&
           left.unknown == right.m_unknown &&
           left.fieldHandles == right.m_fieldHandles &&
           sameDwgFieldAppData(left.appData, right.appData) &&
           sameDwgFieldExtData(left.extData, right.extData) &&
           left.reactorHandles == right.reactorHandles &&
           left.xDictHandle == right.xDictHandle &&
...
public:
  //! Exact typed FIELD comparison used to validate a frozen export payload.
  static bool matchesDwgFieldRecord(const FieldRecord &record,
                                    const DRW_Field &field) {
    return sameDwgFieldRecord(record, field);
  }

  //! Exact typed FIELDLIST comparison used to validate a frozen export payload.
  static bool matchesDwgFieldListRecord(const FieldListRecord &record,
                                        const DRW_FieldList &fieldList) {
    return sameDwgFieldListRecord(record, fieldList);
  }

  DwgFieldSourceAuthorization
  authorizeDwgFieldSource(const FieldRecord &record) const {
    const auto publicationMatches =
        [&record](const DRW_DwgFramePublication &value) {
          return value.m_handle == record.handle;
        };
    const auto receiptMatches =
        [&record](const DRW_DwgFieldPayloadReceipt &value) {
          return value.m_fieldHandle == record.handle;
        };
    const auto coverageMatches =
        [](const DRW_DwgFrameCoverageEntry &coverage,
           const DRW_DwgFramePublication &publication) {
    ...
    std::vector<const DRW_DwgFieldPayloadReceipt *> receipts;
    for (const DRW_DwgFieldPayloadReceipt &receipt :
         m_dwgFieldPayloadReceipts) {
    ...
    if (publications.empty()) {
      return {receipts.empty()
                  ? DwgFieldSourceAuthorizationStatus::NotDwgSource
                  : DwgFieldSourceAuthorizationStatus::OrphanPayload};
    }
    if (publications.size() != 1u) {
      return {DwgFieldSourceAuthorizationStatus::DuplicateSourceFrame};
    }
    if (!m_dwgFramePublicationCoverageComplete) {
      return {DwgFieldSourceAuthorizationStatus::FrameCoverageIncomplete};
    }
    const DRW_DwgFramePublication &publication = *publications.front();
    if (!isDwgFieldPublication(publication) ||
        publication.m_version < DRW::AC1015 ||
        !publication.m_hasSourceLocation ||
        !publication.m_classStreamOrdinal.has_value()) {
      return {DwgFieldSourceAuthorizationStatus::InvalidSourceFrame};
    }
    const std::size_t sameHandleCoverage = std::count_if(
        m_dwgFrameCoverageReport.m_entries.cbegin(),
        m_dwgFrameCoverageReport.m_entries.cend(),
    ...
    if (sameHandleCoverage == 0u) {
      return {DwgFieldSourceAuthorizationStatus::MissingSourceCoverage};
    }
    if (sameHandleCoverage != 1u || !hasExactCoverage(publication)) {
      return {DwgFieldSourceAuthorizationStatus::MismatchedSourceCoverage};
    }
    if (receipts.empty()) {
      return {DwgFieldSourceAuthorizationStatus::MissingPayload};
    }
    if (receipts.size() != 1u) {
      return {DwgFieldSourceAuthorizationStatus::DuplicatePayload};
    }
    const DRW_DwgFieldPayloadReceipt &receipt = *receipts.front();
    if (!receipt.m_complete || !receipt.m_hasSourceLocation ||
        !receipt.m_field.hasCompleteDwgPayload()) {
      return {DwgFieldSourceAuthorizationStatus::IncompletePayload};
    }
    if (receipt.m_version < DRW::AC1015 ||
        !receipt.m_classStreamOrdinal.has_value() ||
        !receipt.m_field.isDwgPayloadValid(receipt.m_version)) {
      return {DwgFieldSourceAuthorizationStatus::InvalidPayload};
    }
    if (receipt.m_version != publication.m_version ||
        receipt.m_sourceOffset != publication.m_sourceOffset ||
        receipt.m_sourceMapOrdinal != publication.m_sourceMapOrdinal ||
        receipt.m_sourceOffsetSpace != publication.m_sourceOffsetSpace ||
        receipt.m_encodedType != publication.m_encodedType ||
        receipt.m_resolvedType != publication.m_resolvedType ||
        ...
        receipt.m_field.handle != record.handle ||
        !sameDwgFieldRecord(record, receipt.m_field)) {
      return {DwgFieldSourceAuthorizationStatus::MismatchedPayload};
    }
    return {DwgFieldSourceAuthorizationStatus::Authorized};
  }

  DwgDictionarySourceAuthorization
  authorizeDwgDictionarySource(const DictionaryRecord &record) const {
    const auto publicationMatches =
        [&record](const DRW_DwgFramePublication &value) {
          return value.m_handle == record.handle;
        };
    const auto membershipMatches =
        [&record](const DRW_DwgDictionaryMembership &value) {
          return value.m_dictionaryHandle == record.handle;
        };
    const auto coverageMatches =
        [](const DRW_DwgFrameCoverageEntry &coverage,
           const DRW_DwgFramePublication &publication) {
    ...
    std::vector<const DRW_DwgDictionaryMembership *> memberships;
    for (const DRW_DwgDictionaryMembership &membership :
         m_dwgDictionaryMemberships) {
      if (membershipMatches(membership))
        memberships.push_back(&membership);
    }
    if (publications.empty()) {
      return {memberships.empty()
                  ? DwgDictionarySourceAuthorizationStatus::NotDwgSource
                  : DwgDictionarySourceAuthorizationStatus::OrphanMembership};
    }
    if (!m_dwgFramePublicationCoverageComplete) {
      return {DwgDictionarySourceAuthorizationStatus::FrameCoverageIncomplete};
    }
    if (publications.size() != 1u) {
      return {DwgDictionarySourceAuthorizationStatus::DuplicateSourceFrame};
    }
    const DRW_DwgFramePublication &publication = *publications.front();
    if (!isDwgDictionaryPublication(publication) ||
        !publication.m_hasSourceLocation) {
      return {DwgDictionarySourceAuthorizationStatus::InvalidSourceFrame};
    }
    const std::size_t sameHandleCoverage = std::count_if(
        m_dwgFrameCoverageReport.m_entries.cbegin(),
        m_dwgFrameCoverageReport.m_entries.cend(),
    ...
    if (sameHandleCoverage == 0u) {
      return {DwgDictionarySourceAuthorizationStatus::MissingSourceCoverage};
    }
    if (sameHandleCoverage != 1u || !hasExactCoverage(publication)) {
      return {DwgDictionarySourceAuthorizationStatus::MismatchedSourceCoverage};
    }
    if (memberships.empty()) {
      return {DwgDictionarySourceAuthorizationStatus::MissingMembership};
    }
    if (memberships.size() != 1u) {
      return {DwgDictionarySourceAuthorizationStatus::DuplicateMembership};
    }
    const DRW_DwgDictionaryMembership &membership = *memberships.front();
    if (!membership.m_complete || !membership.m_hasSourceLocation) {
      return {DwgDictionarySourceAuthorizationStatus::IncompleteMembership};
    }
    if (membership.m_entries.size() > DRW_Dictionary::kMaxEntries) {
      return {DwgDictionarySourceAuthorizationStatus::InvalidMembership};
    }
    if (membership.m_version != publication.m_version ||
        membership.m_sourceOffset != publication.m_sourceOffset ||
        membership.m_sourceMapOrdinal != publication.m_sourceMapOrdinal ||
        membership.m_sourceOffsetSpace != publication.m_sourceOffsetSpace ||
        membership.m_entries.size() != record.entries.size()) {
      return {DwgDictionarySourceAuthorizationStatus::MismatchedMembership};
    }
    std::set<std::string> names;
    std::set<std::uint32_t> handles;
    for (std::size_t ordinal = 0; ordinal < membership.m_entries.size();
         ++ordinal) {
      const DRW_DwgDictionaryMembership::Entry &entry =
          membership.m_entries[ordinal];
      if (entry.m_name.empty() || entry.m_handle == DRW::NoHandle ||
          !names.insert(entry.m_name).second ||
          !handles.insert(entry.m_handle).second) {
        return {DwgDictionarySourceAuthorizationStatus::InvalidMembership};
      }
      if (entry.m_name != record.entries[ordinal].name ||
          entry.m_handle != record.entries[ordinal].handle) {
        return {DwgDictionarySourceAuthorizationStatus::MismatchedMembership};
      }
    }
    return {DwgDictionarySourceAuthorizationStatus::Authorized};
  }
  DwgDictionaryWithDefaultSourceAuthorization
  authorizeDwgDictionaryWithDefaultSource(
      const DictionaryWithDefaultRecord &record) const {
    const auto publicationMatches =
        [&record](const DRW_DwgFramePublication &value) {
          return value.m_handle == record.handle;
        };
    const auto membershipMatches =
        [&record](const DRW_DwgDictionaryWithDefaultMembership &value) {
          return value.m_dictionaryHandle == record.handle;
        };
    const auto coverageMatches =
        [](const DRW_DwgFrameCoverageEntry &coverage,
    ...
    std::vector<const DRW_DwgDictionaryWithDefaultMembership *> memberships;
    for (const DRW_DwgDictionaryWithDefaultMembership &membership :
         m_dwgDictionaryWithDefaultMemberships) {
      if (membershipMatches(membership))
        memberships.push_back(&membership);
    }
    if (publications.empty()) {
      return {
          memberships.empty()
              ? DwgDictionaryWithDefaultSourceAuthorizationStatus::NotDwgSource
              : DwgDictionaryWithDefaultSourceAuthorizationStatus::
                    OrphanMembership};
    }
    if (!m_dwgFramePublicationCoverageComplete) {
      return {DwgDictionaryWithDefaultSourceAuthorizationStatus::
                  FrameCoverageIncomplete};
    }
    if (publications.size() != 1u) {
      return {DwgDictionaryWithDefaultSourceAuthorizationStatus::
    ...
    const DRW_DwgFramePublication &publication = *publications.front();
    if (!isDwgDictionaryWithDefaultPublication(publication) ||
        publication.m_version < DRW::AC1015 ||
        !publication.m_hasSourceLocation ||
        !publication.m_classStreamOrdinal.has_value()) {
      return {DwgDictionaryWithDefaultSourceAuthorizationStatus::
    ...
    if (memberships.empty()) {
      return {
          DwgDictionaryWithDefaultSourceAuthorizationStatus::MissingMembership};
    }
    if (memberships.size() != 1u) {
      return {DwgDictionaryWithDefaultSourceAuthorizationStatus::
                  DuplicateMembership};
    ...
    if (membership.m_entries.size() > DRW_Dictionary::kMaxEntries ||
        membership.m_version < DRW::AC1015 ||
        !membership.m_classStreamOrdinal.has_value() ||
        membership.m_hardOwner < 0 || membership.m_hardOwner > 1 ||
        membership.m_defaultEntryHandle == DRW::NoHandle) {
      return {
          DwgDictionaryWithDefaultSourceAuthorizationStatus::InvalidMembership};
    }
    if (membership.m_version != publication.m_version ||
        membership.m_sourceOffset != publication.m_sourceOffset ||
        membership.m_sourceMapOrdinal != publication.m_sourceMapOrdinal ||
        ...
        membership.m_className != publication.m_className ||
        membership.m_classStreamOrdinal != publication.m_classStreamOrdinal ||
        membership.m_cloning != record.cloning ||
        membership.m_hardOwner != record.hardOwner ||
        membership.m_defaultEntryHandle != record.defaultEntryHandle ||
        membership.m_entries.size() != record.entries.size()) {
      return {DwgDictionaryWithDefaultSourceAuthorizationStatus::
                  MismatchedMembership};
    }
    ...
    for (std::size_t ordinal = 0; ordinal < membership.m_entries.size();
         ++ordinal) {
      const DRW_DwgDictionaryWithDefaultMembership::Entry &entry =
          membership.m_entries[ordinal];
      if (entry.m_name.empty() || entry.m_handle == DRW::NoHandle ||
          !names.insert(entry.m_name).second ||
          !handles.insert(entry.m_handle).second) {
    ...
    return {DwgDictionaryWithDefaultSourceAuthorizationStatus::Authorized};
  }
  ...
  bool hasReplayableDataStorage(DRW::Version version) const {
    if (version <= DRW::AC1024)
      return false;
    for (const DataStorageRecord &storage : m_dataStorages) {
      if (storage.parseFailed || !storage.structurallyValid ||
          !storage.replayAllowed || storage.version != version)
        continue;
      for (const RawDwgSectionRecord &raw : m_rawDwgSections) {
        if (raw.name == storage.name && raw.version == version &&
            raw.replayState == ReplayState::ReplayAllowed &&
            !raw.data.empty()) {
          return true;
        }
      }
  ...
  bool hasReplayableDataStorage(DRW::Version version,
                                std::uint64_t handle) const {
    if (version <= DRW::AC1024 || handle == 0)
      return false;
    for (const DataStorageRecord &storage : m_dataStorages) {
      if (storage.parseFailed || !storage.structurallyValid ||
          !storage.replayAllowed || storage.version != version ||
          storage.full.findRecordByHandle(handle) == nullptr)
        continue;
      for (const RawDwgSectionRecord &raw : m_rawDwgSections) {
        if (raw.name == storage.name && raw.version == version &&
            raw.replayState == ReplayState::ReplayAllowed &&
  ...
  bool hasReplayableDataStorage(DRW::Version version,
                                ...
                                std::uint64_t handle) const {
    if (version <= DRW::AC1024 || sectionName.empty() || handle == 0)
      return false;
    for (const DataStorageRecord &storage : m_dataStorages) {
      if (storage.parseFailed || !storage.structurallyValid ||
          !storage.replayAllowed || storage.version != version ||
          storage.name != sectionName ||
          storage.full.findRecordByHandle(handle) == nullptr)
        continue;
      for (const RawDwgSectionRecord &raw : m_rawDwgSections) {
        if (raw.name == sectionName && raw.version == version &&
            raw.replayState == ReplayState::ReplayAllowed &&
            !raw.data.empty()) {
          return true;
        }
  ...
  const std::vector<SurfaceGeometryRecord> &surfaceGeometry() const {
    return m_surfaceGeometry;
  }
  const std::vector<DRW_RawDxfObject> &rawDxfObjects() const {
    return m_rawDxfObjects;
  }
  const std::vector<DRW_RawDxfObject> &rawDxfEntities() const {
    return m_rawDxfEntities;
  }
  const std::vector<DRW_RawDxfSection> &rawDxfSections() const {
    return m_rawDxfSections;
  }
  const std::vector<DRW_Class> &dxfClasses() const { return m_dxfClasses; }
  const std::map<std::uint32_t, DRW_LType> &lineTypes() const {
    return m_lineTypes;
  }
  const std::map<std::uint32_t, DRW_Textstyle> &textStyles() const {
    return m_textStyles;
  }
  const DRW_LType *findLineTypeTableEntryByName(const std::string &name) const {
    for (const auto &entry : m_lineTypeTableEntries) {
      if (tableNameEquals(entry.first, name))
        return &entry.second;
    }
    return nullptr;
  }
  const DRW_Textstyle *
  findTextStyleTableEntryByName(const std::string &name) const {
    for (const auto &entry : m_textStyleTableEntries) {
      if (tableNameEquals(entry.first, name))
        return &entry.second;
    }
    return nullptr;
  }
  const std::map<std::string, DRW_LType> &lineTypeTableEntries() const {
    return m_lineTypeTableEntries;
  }
  const std::map<std::string, DRW_Textstyle> &textStyleTableEntries() const {
    return m_textStyleTableEntries;
  }
  std::string lineTypeNameForHandle(std::uint32_t handle) const {
    const auto it = m_lineTypeNames.find(handle);
    return it == m_lineTypeNames.end() ? std::string{} : it->second;
  }
  std::string textStyleNameForHandle(std::uint32_t handle) const {
    const auto it = m_textStyleNames.find(handle);
    return it == m_textStyleNames.end() ? std::string{} : it->second;
  }
  const DRW_Layer *findLayerTableEntryByName(const std::string &name) const {
    const auto it = m_layerTableEntries.find(name);
    return it == m_layerTableEntries.end() ? nullptr : &it->second;
  }
  const DRW_Dimstyle *
  findDimStyleTableEntryByName(const std::string &name) const {
    for (const auto &entry : m_dimStyleTableEntries) {
      if (tableNameEquals(entry.first, name))
        return &entry.second;
    }
    return nullptr;
  }
  const DRW_AppId *findAppIdTableEntryByName(const std::string &name) const {
    for (const auto &entry : m_appIdTableEntries) {
      if (tableNameEquals(entry.first, name))
        return &entry.second;
    }
    return nullptr;
  }
  const std::map<std::string, DRW_AppId> &appIdTableEntries() const {
    return m_appIdTableEntries;
  }
  const std::vector<DRW_CsacDocumentOptions> &csacDocumentOptions() const {
    return m_csacDocumentOptions;
  }
  const std::vector<DRW_ContextDataManager> &contextDataManagers() const {
    return m_contextDataManagers;
  }
  const std::vector<DRW_ProxyEntity> &proxyEntities() const {
    return m_proxyEntities;
  }
  const std::vector<DRW_VbaProject> &vbaProjects() const {
    return m_vbaProjects;
  }
  const std::vector<DRW_PlotSettings> &plotSettings() const {
    return m_plotSettings;
  }
  const std::vector<DRW_ProxyObject> &proxyObjects() const {
    return m_proxyObjects;
  }
  ...
  // Mark records delivered by the DWG reader. Records added later through
  // the generic metadata API must not inherit imported-carrier identity.
  void markVportsImportedFromDwg() {
    for (VportRecord &record : m_vports)
      record.importedFromDwg = true;
  }
  bool isImportedVportSource(std::uint32_t handle,
                             const std::string &name) const {
    if (handle == 0)
      return false;
    return std::any_of(m_vports.cbegin(), m_vports.cend(),
                       [handle, &name](const VportRecord &record) {
                         return record.importedFromDwg &&
                                record.handle == handle &&
                                tableNameEquals(record.name, name);
                       });
  }
  ...
  const std::vector<ViewportEntityHeaderRecord> &viewportEntityHeaders() const {
    return m_viewportEntityHeaders;
  }
  const std::vector<VxControlRecord> &vxControls() const {
    return m_vxControls;
  }
  const std::vector<VxTableRecordRecord> &vxTableRecords() const {
    return m_vxTableRecords;
  }
  ...
  const std::vector<CameraRecord> &cameras() const { return m_cameras; }
  const std::vector<LightListRecord> &lightLists() const {
    return m_lightLists;
  }
  const std::vector<DataLinkRecord> &dataLinks() const { return m_dataLinks; }
  const std::vector<GeoMapImageRecord> &geoMapImages() const {
    return m_geoMapImages;
  }
  const std::vector<LayerFilterRecord> &layerFilters() const {
    return m_layerFilters;
  }
  const std::vector<GeoPositionMarkerRecord> &geoPositionMarkers() const {
    return m_geoPositionMarkers;
  }
  const std::vector<SectionObjectRecord> &sectionObjects() const {
    return m_sectionObjects;
  }
  const std::vector<TvDevicePropertiesRecord> &tvDeviceProperties() const {
    return m_tvDeviceProperties;
  }
  const std::vector<SunRecord> &suns() const { return m_suns; }
  const std::vector<BackgroundRecord> &backgrounds() const {
    return m_backgrounds;
  }
  const std::vector<MaterialRecord> &materials() const { return m_materials; }
  const std::vector<RenderSettingsRecord> &renderSettings() const {
    return m_renderSettings;
  }
  const std::vector<SunStudyRecord> &sunStudies() const { return m_sunStudies; }
  const std::vector<MotionPathRecord> &motionPaths() const {
    return m_motionPaths;
  }
  const std::vector<CurvePathRecord> &curvePaths() const {
    return m_curvePaths;
  }
  const std::vector<PointPathRecord> &pointPaths() const {
    return m_pointPaths;
  }
  const std::vector<ObjectPtrRecord> &objectPtrs() const {
    return m_objectPtrs;
  }
  const std::vector<PartialViewingIndexRecord> &partialViewingIndexes() const {
    return m_partialViewingIndexes;
  }
  const std::vector<DbColorRecord> &dbColors() const { return m_dbColors; }
  const std::vector<DimensionAssociationRecord> &dimensionAssociations() const {
    return m_dimensionAssociations;
  }
  const std::vector<EvaluationGraphRecord> &evaluationGraphs() const {
    return m_evaluationGraphs;
  }
  const std::vector<BlockRepresentationRecord> &
  blockRepresentationData() const {
    return m_blockRepresentationData;
  }
  const std::vector<SectionRecord> &sections() const { return m_sections; }
  const std::vector<ModelerGeometryRecord> &modelerGeometry() const {
    return m_modelerGeometry;
  }
  const std::vector<MeshRecord> &meshes() const { return m_meshes; }
  const std::vector<MeshSidecarRecord> &meshSidecars() const {
    return m_meshSidecars;
  }
  ...
  const std::vector<CellStyleMapRecord> &cellStyleMaps() const {
    return m_cellStyleMaps;
  }
  const std::vector<AssociativeRecord> &associativeObjects() const {
    return m_associativeObjects;
  }
  ...
  const std::vector<MLeaderStyleRecord> &mleaderStyles() const {
    return m_mleaderStyles;
  }
  const std::vector<DetailViewStyleRecord> &detailViewStyles() const {
    return m_detailViewStyles;
  }
  const std::vector<SectionViewStyleRecord> &sectionViewStyles() const {
    return m_sectionViewStyles;
  }
  const std::vector<BreakDataRecord> &breakData() const { return m_breakData; }
  const std::vector<BreakPointRefRecord> &breakPointRefs() const {
    return m_breakPointRefs;
  }
  const std::vector<GroupRecord> &groups() const { return m_groups; }
  const std::vector<ImageDefinitionReactorRecord> &
  imageDefinitionReactors() const {
    return m_imageDefinitionReactors;
  }
  const std::vector<RasterImageRecord> &rasterImages() const {
    return m_rasterImages;
  }
  ...
  const std::vector<PointCloudDefinitionRecord> &pointCloudDefinitions() const {
    return m_pointCloudDefinitions;
  }
  const std::vector<NavisworksModelDefinitionRecord> &
  navisworksModelDefinitions() const {
    return m_navisworksModelDefinitions;
  }
  const std::vector<NavisworksModelRecord> &navisworksModels() const {
    return m_navisworksModels;
  }
  const std::vector<PointCloudColorMapRecord> &pointCloudColorMaps() const {
    return m_pointCloudColorMaps;
  }
  ...
  const std::vector<Ole2FrameRecord> &ole2Frames() const {
    return m_ole2Frames;
  }
  const std::vector<OleFrameRecord> &oleFrames() const { return m_oleFrames; }
  const std::vector<SpatialFilterRecord> &spatialFilters() const {
    return m_spatialFilters;
  }
  const std::vector<GeoDataRecord> &geoData() const { return m_geoData; }
  const std::vector<TableGeometryRecord> &tableGeometry() const {
    return m_tableGeometry;
  }
  const std::vector<PlaceholderRecord> &placeholders() const {
    return m_placeholders;
  }
  const std::vector<DictionaryRecord> &dictionaries() const {
  ...
  const std::vector<MLineStyleRecord> &mlineStyles() const {
    return m_mlineStyles;
  }
  const std::vector<WipeoutVariablesRecord> &wipeoutVariables() const {
    return m_wipeoutVariables;
  }
  const std::vector<IDBufferRecord> &idBuffers() const { return m_idBuffers; }
  const std::vector<IndexRecord> &indexes() const { return m_indexes; }
  const std::vector<LayerIndexRecord> &layerIndexes() const {
    return m_layerIndexes;
  }
  const std::vector<SpatialIndexRecord> &spatialIndexes() const {
    return m_spatialIndexes;
  }
  const std::vector<DictionaryVarRecord> &dictionaryVars() const {
    return m_dictionaryVars;
  }
  // PR 8d.2b — four larger no-storage OBJECTS families.
  const std::vector<DictionaryWithDefaultRecord> &
  dictionariesWithDefault() const {
    return m_dictionariesWithDefault;
  }
  const std::vector<SortEntsTableRecord> &sortEntsTables() const {
    return m_sortEntsTables;
  }
  const std::vector<FieldListRecord> &fieldLists() const {
    return m_fieldLists;
  }
  const std::vector<DataTableRecord> &dataTables() const {
    return m_dataTables;
  }
  const std::vector<DynamicBlockRecord> &dynamicBlockObjects() const {
    return m_dynamicBlockObjects;
  }
  ...
  std::vector<const RawObjectRecord *>
  findRawObjectsByFamily(RawObjectFamily family) const {
  ...
  std::vector<const TableRecord *>
  findTableStylesReferencingHandle(std::uint32_t handle) const {
  ...
  std::vector<const TableRecord *>
  findTablesUsingStyle(std::uint32_t styleHandle) const {
    std::vector<const TableRecord *> result;
    if (styleHandle == 0)
      return result;
    for (const TableRecord &record : m_tables) {
  ...
  std::vector<const TableRecord *>
  findTablesReferencingHandle(std::uint32_t handle) const {
    std::vector<const TableRecord *> result;
    if (handle == 0)
      return result;
    for (const TableRecord &record : m_tables) {
  ...
  std::vector<const TableFallbackEntityRecord *>
  findTableFallbackEntities(std::uint32_t tableHandle) const {
  ...
  const TableRecord *
  findTableByFallbackEntityId(unsigned long long entityId) const {
    if (entityId == 0)
      return nullptr;
  ...
  const CellStyleMapRecord *
  findCellStyleMapByHandle(std::uint32_t handle) const {
    if (handle == 0)
      return nullptr;
  ...
  std::vector<const CellStyleMapRecord *>
  findCellStylesById(int styleId) const {
  ...
  std::vector<const CellStyleMapRecord *>
  findCellStyleMapsReferencingHandle(std::uint32_t handle) const {
    std::vector<const CellStyleMapRecord *> result;
    if (handle == 0)
      return result;
    for (const CellStyleMapRecord &record : m_cellStyleMaps) {
  ...
  std::vector<const TableRecord *>
  findTableStylesReferencingCellStyle(int styleId) const {
    std::vector<const TableRecord *> result;
    if (styleId == 0)
      return result;
    for (const TableRecord &record : m_tables) {
  ...
  TableNativeWriterEligibility
  tableNativeWriterEligibility(std::uint32_t handle,
    ...
    if (record == nullptr || isTableStyleRecord(*record)) {
      addTableNativeWriterBlocker(
          eligibility, TableNativeWriterBlocker::NoSemanticTableContent);
      return eligibility;
    }
    populateTableNativeWriterEligibility(*record, version, eligibility);
    return eligibility;
  }
  std::vector<TableNativeWriterEligibility>
  tableNativeWriterEligibilityForAll(DRW::Version version) const {
  ...
  TableNativeWriterBlockerCounts
  tableNativeWriterBlockerCounts(DRW::Version version) const {
  ...
  ModelerPayloadCounts modelerPayloadCounts() const {
    ...
    for (const ModelerGeometryRecord &record : m_modelerGeometry) {
      ...
      else if (record.markerSection == ModelerPayloadSection::DataStorage)
        ++counts.markerInDataStorage;
      for (const ModelerPayloadRangeRecord &range : record.payloadRanges) {
        ++counts.rangeCount;
        incrementModelerPayloadRangeKindCount(counts, range.kind);
        incrementModelerPayloadRangeConsistencyCount(counts, range.consistency);
      }
    }
    return counts;
  }
  MeshWriterBlockerCounts meshWriterBlockerCounts() const {
    ...
    for (const MeshRecord &record : m_meshes) {
      ...
      if ((record.edgeCount > 0 || record.creaseCount > 0 ||
           record.subdivisionLevel > 0) &&
          !record.hasCreaseData) {
  ...
  ExternalReferenceCounts externalReferenceCounts() const {
    ...
    for (const RasterImageRecord &record : m_rasterImages) {
      ...
      // WIPEOUT has IMAGE-derived geometry but no external raster asset.
      // A zero image-definition handle is therefore normal for it.
      if (!record.isWipeout && record.definitionHandle == 0)
  ...
  GraphReplayPolicyCounts graphReplayPolicyCounts() const {
    ...
    for (const AssociativeRecord &record : m_associativeObjects) {
      if (record.handle == 0 ||
          !hasRawGraphObjectForHandle(record.handle,
                                      GraphReplayFamily::AcDbAssoc)) {
    ...
    counts.parserPartial = prefixCounts.partial + prefixCounts.missing +
                           prefixCounts.unsupportedVersion +
                           prefixCounts.boundedCountOverflow;
    return counts;
  }
  std::vector<AdvancedEntityWriterReadiness>
  advancedEntityWriterLedger(DRW::Version version) const {
    ...
    for (const MLeaderRecord &mleader : m_mleaders)
      result.push_back(
          advancedEntityWriterReadinessFromMLeader(mleader, version));
    ...
    for (const OleFrameRecord &oleFrame : m_oleFrames)
      result.push_back(
          advancedEntityWriterReadinessFromOleFrame(oleFrame, version));
    return result;
  }

  AdvancedEntityWriterBlockerCounts
  advancedEntityWriterBlockerCounts(DRW::Version version) const {
  ...
  const TableCellRecord *findTableCell(std::uint32_t tableHandle, int row,
                                       int column) const {
  ...
  std::vector<const TableCellRecord *>
  findTableCellsReferencingHandle(std::uint32_t handle) const {
  ...
  const MLeaderStyleRecord *
  findMLeaderStyleByHandle(std::uint32_t handle) const {
    if (handle == 0)
      return nullptr;
  ...
  std::vector<const MLeaderRecord *>
  findMLeadersUsingStyle(std::uint32_t styleHandle) const {
  ...
  std::vector<const MLeaderRecord *>
  findMLeadersReferencingHandle(std::uint32_t handle) const {
    std::vector<const MLeaderRecord *> result;
    if (handle == 0)
      return result;
    for (const MLeaderRecord &record : m_mleaders) {
  ...
  std::vector<const MLeaderStyleRecord *>
  findMLeaderStylesReferencingHandle(std::uint32_t handle) const {
  ...
  const ModelerGeometryRecord *
  findModelerGeometryByHandle(std::uint32_t handle) const {
    if (handle == 0)
      return nullptr;
    for (const ModelerGeometryRecord &record : m_modelerGeometry) {
      if (record.handle == handle)
  ...
  std::vector<const ModelerGeometryRecord *>
  findModelerGeometryByHistoryHandle(std::uint32_t historyHandle) const {
  ...
  std::vector<const MeshSidecarRecord *>
  findMeshSidecarsBySourceHandle(std::uint32_t sourceHandle) const {
  ...
  const MeshSidecarRecord *
  findMeshSidecarByFallbackEntityId(unsigned long long fallbackEntityId) const {
  ...
  const RasterImageRecord *findRasterImageByHandle(std::uint32_t handle) const {
    ...
    // Some DXF IMAGE records omit code 5.  The filter then uses the
    // definition handle as the RS image handle, so retain a deterministic
    // fallback for that otherwise valid source shape.
    for (const RasterImageRecord &record : m_rasterImages) {
      if (record.handle == 0 && record.definitionHandle == handle)
        return &record;
    }
    return nullptr;
  }

  const ImageDefinitionRecord *
  findImageDefinitionByHandle(std::uint32_t handle) const {
    if (handle == 0)
      return nullptr;
    for (const ImageDefinitionRecord &record : m_imageDefinitions) {
      if (record.handle == handle)
  ...
  const ImageDefinitionReactorRecord *
  findImageDefinitionReactorByHandle(std::uint32_t handle) const {
    if (handle == 0)
      return nullptr;
  ...
  std::vector<const RasterImageRecord *>
  findRasterImagesByDefinitionHandle(std::uint32_t definitionHandle) const {
  ...
  findImageDefinitionReactorsByDefinitionHandle(
  ...
  const UnderlayDefinitionRecord *
  findUnderlayDefinitionByHandle(std::uint32_t handle) const {
    if (handle == 0)
      return nullptr;
    for (const UnderlayDefinitionRecord &record : m_underlayDefinitions) {
      if (record.handle == handle)
  ...
  std::vector<const UnderlayRecord *>
  findUnderlaysByDefinitionHandle(std::uint32_t definitionHandle) const {
  ...
  std::vector<const ImageDefinitionRecord *>
  findImageDefinitionsByPath(const std::string &path) const {
  ...
  std::vector<const UnderlayDefinitionRecord *>
  findUnderlayDefinitionsByPath(const std::string &path) const {
  ...
  std::vector<const ShapeRecord *>
  findShapesByShapeFileHandle(std::uint32_t shapeFileHandle) const {
  ...
  const AssociativeRecord *
  findAssociativeObjectByHandle(std::uint32_t handle) const {
    if (handle == 0)
      return nullptr;
    for (const AssociativeRecord &record : m_associativeObjects) {
      if (record.handle == handle)
  ...
  std::vector<const AssociativeRecord *>
  findAssociativeObjectsByKind(AssociativeKind kind) const {
  ...
  std::vector<const AssociativeRecord *>
  findAssociativeObjectsReferencingHandle(std::uint32_t handle) const {
    std::vector<const AssociativeRecord *> result;
    if (handle == 0)
      return result;
    for (const AssociativeRecord &record : m_associativeObjects) {
  ...
  std::vector<const AssociativeEdgeRecord *>
  findAssociativeEdgesFrom(std::uint32_t handle) const {
  ...
  std::vector<const AssociativeEdgeRecord *>
  findAssociativeEdgesTo(std::uint32_t handle) const {
    std::vector<const AssociativeEdgeRecord *> result;
    if (handle == 0)
      return result;
    for (const AssociativeEdgeRecord &edge : m_associativeEdges) {
  ...
  std::vector<const AssociativeRecord *>
  findAssociativeRecordsAffectedBy(std::uint32_t handle) const {
    std::vector<const AssociativeRecord *> result;
  ...
  std::vector<std::uint32_t> findAssociativeClosureFrom(std::uint32_t handle,
  ...
  std::vector<const AcShRecord *>
  findAcShObjectsByOwnerHandle(std::uint32_t ownerHandle) const {
  ...
  static const char *modelerPayloadSectionName(ModelerPayloadSection section) {
    ...
    case ModelerPayloadSection::DataStorage:
      return "datastore";
  ...
  static const char *
  modelerPayloadRangeConfidenceName(ModelerPayloadRangeConfidence confidence) {
    switch (confidence) {
  ...
  static ModelerRawByteSplit splitModelerRawBytes(std::uint32_t bodyBitSize,
                                                  size_t rawByteCount) {
    ...
    split.known = true;
    const size_t bodyByteCount = (static_cast<size_t>(bodyBitSize) + 7u) / 8u;
  ...
  static ModelerPayloadMarker
  scanModelerPayloadMarker(const std::vector<std::uint8_t> &bytes) {
  ...
  static ModelerPayloadKind
  classifyModelerPayload(const std::vector<std::uint8_t> &bytes) {
    return scanModelerPayloadMarker(bytes).kind;
  }

  static std::vector<ModelerPayloadRangeRecord>
  scanModelerPayloadRanges(const std::vector<std::uint8_t> &bytes,
    ...
    if (marker.kind == ModelerPayloadKind::Sab) {
      ...
      const std::vector<std::uint8_t> terminator = sabTerminatorBytes();
      const size_t terminatorOffset = findByteSequence(
          bytes, terminator, marker.offset + marker.length, bodyByteCount);
      size_t rangeEnd = bodyByteCount;
      ModelerPayloadRangeConsistency consistency = bodyConsistency;
      ModelerPayloadRangeConfidence confidence =
      ...
      appendModelerRange(ranges, ModelerPayloadRangeKind::Sab,
                         ...
                         rangeEnd - marker.offset, declaredBodyByteCount,
                         consistency, confidence, marker.text);
      if (rangeEnd < bodyByteCount) {
        appendModelerRange(
            ranges, ModelerPayloadRangeKind::UnknownTail,
            ModelerPayloadSection::Body, rangeEnd, bodyByteCount - rangeEnd,
            declaredBodyByteCount, ModelerPayloadRangeConsistency::Exact,
      ...
      appendModelerRange(ranges, kind, ModelerPayloadSection::Body,
                         ...
                         declaredBodyByteCount, bodyConsistency,
                         ModelerPayloadRangeConfidence::Marker, marker.text);
    } else if (bodyByteCount != 0u) {
      appendModelerRange(ranges,
    ...
    if (split.known && split.handleByteCount != 0u) {
      appendModelerRange(
          ranges, ModelerPayloadRangeKind::HandleStream,
          ModelerPayloadSection::HandleStream, split.bodyByteCount,
          split.handleByteCount, split.handleByteCount,
          ModelerPayloadRangeConsistency::Exact,
          ModelerPayloadRangeConfidence::DeclaredSize, std::string());
    }
    return ranges;
  }

private:
  struct DwgSourceGraphEdgeKey {
    std::size_t sourceNode = 0;
    std::size_t targetNode = 0;
    DwgSourceGraphEdgeKind kind = DwgSourceGraphEdgeKind::Owner;
    DRW_DwgTypedReferenceField typedReferenceField =
    ...
    bool operator<(const DwgSourceGraphEdgeKey &other) const noexcept {
      ...
      if (typedReferenceField != other.typedReferenceField) {
        return static_cast<unsigned>(typedReferenceField) <
               static_cast<unsigned>(other.typedReferenceField);
      }
      return membershipOrdinal < other.membershipOrdinal;
    }
  };

  struct DwgSourceGraphTypedReferenceKey {
    std::uint32_t sourceHandle = DRW::NoHandle;
    DRW_DwgTypedReferenceField field = DRW_DwgTypedReferenceField::Unknown;

    bool
    operator<(const DwgSourceGraphTypedReferenceKey &other) const noexcept {
      if (sourceHandle != other.sourceHandle)
        return sourceHandle < other.sourceHandle;
      return static_cast<unsigned>(field) < static_cast<unsigned>(other.field);
    }
  };

  struct DwgSourceGraphInventory {
    std::map<std::uint32_t, std::size_t> coverageByHandle;
    std::map<std::uint64_t, std::size_t> coverageByOrdinal;
    std::map<std::uint32_t, std::size_t> receiptByHandle;
    std::map<std::uint32_t, std::size_t> dictionaryMembershipByHandle;
    std::map<std::uint32_t, std::size_t> groupMembershipByHandle;
    std::map<std::uint32_t, std::size_t> sortEntsMembershipByHandle;
    std::map<std::uint32_t, std::size_t> fieldListMembershipByHandle;
    std::map<DwgSourceGraphTypedReferenceKey, std::size_t>
        typedReferenceBySource;
    std::map<std::uint32_t, std::size_t> blockReachabilityByRecord;
    std::map<std::uint64_t, std::size_t> classByOrdinal;
    std::map<std::uint32_t, std::size_t> frameNodeByHandle;
    std::map<std::uint64_t, std::size_t> classNodeByOrdinal;
    std::map<DwgSourceGraphEdgeKey, bool> edgeKeys;
    std::vector<std::vector<std::size_t>> hierarchy;
    std::vector<std::size_t> hierarchyInbound;
  };

  static bool
  sameDwgSourceGraphSource(const DwgSourceGraphSource &lhs,
                           const DwgSourceGraphSource &rhs) noexcept {
    return lhs.handle == rhs.handle && lhs.offset == rhs.offset &&
           lhs.mapOrdinal == rhs.mapOrdinal &&
           lhs.offsetSpace == rhs.offsetSpace;
  }

  static bool isDwgDictionaryWithDefaultPublication(
      const DRW_DwgFramePublication &publication) {
    return publication.m_isCustomClass && !publication.m_isEntity &&
           publication.m_className == "AcDbDictionaryWithDefault";
  }

  static bool
  isDwgDictionaryPublication(const DRW_DwgFramePublication &publication) {
    return !publication.m_isCustomClass && !publication.m_isEntity &&
           publication.m_encodedType == DRW_Dictionary::kDwgFixedType &&
           publication.m_resolvedType == DRW_Dictionary::kDwgFixedType;
  }

  static bool
  isDwgSortEntsTablePublication(const DRW_DwgFramePublication &publication) {
    return publication.m_isCustomClass && !publication.m_isEntity &&
  ...
  static bool
  isDwgFieldListPublication(const DRW_DwgFramePublication &publication) {
    return publication.m_isCustomClass && !publication.m_isEntity &&
  ...
  static bool
  isDwgFieldPublication(const DRW_DwgFramePublication &publication) {
    return publication.m_isCustomClass && !publication.m_isEntity &&
           publication.m_recordName == "FIELD" &&
           publication.m_className == "AcDbField";
  }

  static bool isDwgFieldPublication(const DwgSourceGraphNode &node) {
    return node.isCustomClass && !node.isEntity && node.recordName == "FIELD" &&
           node.className == "AcDbField";
  }

  static bool isDwgModernBlockRecordPublication(
      const DRW_DwgFramePublication &publication) {
    return publication.m_version >= DRW::AC1018 &&
           publication.m_resolvedType == DRW::DwgBlockRecordObjectType &&
           !publication.m_isEntity;
  }

  static DwgSourceGraphResult refuseDwgSourceGraph(
      DwgSourceGraphRefusal reason,
      const std::optional<DwgSourceGraphSource> &source = std::nullopt,
      const std::optional<std::uint64_t> &classStreamOrdinal = std::nullopt) {
    DwgSourceGraphResult result;
    result.status = DwgSourceGraphStatus::Refused;
    result.refusal = reason;
    result.source = source;
    result.classStreamOrdinal = classStreamOrdinal;
    return result;
  }

  static DwgSourceGraphResult buildDwgSourceGraphInventory(
      const DRW_DwgFrameCoverageReport &frameReport,
      const DRW_DwgClassCoverageReport &classReport,
      const std::vector<DRW_DwgFramePublication> &publications,
      const std::vector<DRW_DwgDictionaryMembership> &dictionaryMemberships,
      const std::vector<DRW_DwgGroupMembership> &groupMemberships,
      const std::vector<DRW_DwgSortEntsMembership> &sortEntsMemberships,
      const std::vector<DRW_DwgFieldListMembership> &fieldListMemberships,
      const std::vector<DRW_DwgTypedReference> &typedReferences,
      const std::vector<DRW_DwgBlockReachability> &blockReachabilities,
      const std::vector<std::uint32_t> &publicationConflicts) {
    try {
      if (frameReport.m_status !=
              DRW_DwgFrameCoverageStatus::FinalizedComplete ||
          !frameReport.m_complete) {
        return refuseDwgSourceGraph(
            DwgSourceGraphRefusal::FrameCoverageIncomplete);
      }
      if (!publicationConflicts.empty()) {
        return refuseDwgSourceGraph(
            DwgSourceGraphRefusal::FramePublicationConflict,
            DwgSourceGraphSource{publicationConflicts.front(), 0, 0,
                                 DRW_DwgFrameOffsetSpace::Unknown});
      }

      const bool classesUnavailable =
          classReport.m_status == DRW_DwgClassCoverageStatus::NotAvailable;
      const bool classesComplete =
          classReport.m_status ==
              DRW_DwgClassCoverageStatus::FinalizedComplete &&
          classReport.m_complete;
      if (!classesUnavailable && !classesComplete) {
        return refuseDwgSourceGraph(
            DwgSourceGraphRefusal::ClassCoverageIncomplete);
      }

      DwgSourceGraphInventory inventory;

      for (std::size_t index = 0; index < frameReport.m_entries.size();
           ++index) {
        const DRW_DwgFrameCoverageEntry &entry = frameReport.m_entries[index];
        const DwgSourceGraphSource source{entry.m_handle, entry.m_sourceOffset,
                                          entry.m_sourceMapOrdinal,
                                          entry.m_sourceOffsetSpace};
        if (entry.m_handle == DRW::NoHandle) {
          return refuseDwgSourceGraph(DwgSourceGraphRefusal::InvalidFrameHandle,
                                      source);
        }
        if (entry.m_disposition != DRW_DwgFrameDisposition::Published ||
            entry.m_publicationCount != 1) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::FrameCoverageIncomplete, source);
        }
        if (!inventory.coverageByHandle.emplace(entry.m_handle, index).second) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::DuplicateFrameHandle, source);
        }
        if (!inventory.coverageByOrdinal
                 .emplace(entry.m_sourceMapOrdinal, index)
                 .second) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::DuplicateFrameOrdinal, source);
        }
      }

      for (std::size_t index = 0; index < publications.size(); ++index) {
        const DRW_DwgFramePublication &publication = publications[index];
        if (!publication.m_hasSourceLocation) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MismatchedFramePublication,
              DwgSourceGraphSource{publication.m_handle, 0, 0,
                                   publication.m_sourceOffsetSpace});
        }
        const DwgSourceGraphSource source{
            publication.m_handle, publication.m_sourceOffset,
            publication.m_sourceMapOrdinal, publication.m_sourceOffsetSpace};
        if (publication.m_commonHandleDataValidated !=
            drwDwgCommonLinkEvidenceValidated(
                publication.m_commonLinkEvidence)) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::InconsistentCommonLinkEvidence, source);
        }
        const auto coverage =
            inventory.coverageByHandle.find(publication.m_handle);
        if (coverage == inventory.coverageByHandle.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::ExtraFramePublication, source);
        }
        const DRW_DwgFrameCoverageEntry &entry =
            frameReport.m_entries[coverage->second];
        const DwgSourceGraphSource expected{
            entry.m_handle, entry.m_sourceOffset, entry.m_sourceMapOrdinal,
            entry.m_sourceOffsetSpace};
        if (!sameDwgSourceGraphSource(source, expected)) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MismatchedFramePublication, source);
        }
        if (!inventory.receiptByHandle.emplace(publication.m_handle, index)
                 .second) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::ExtraFramePublication, source);
        }
      ...
      for (const DRW_DwgFrameCoverageEntry &entry : frameReport.m_entries) {
        if (inventory.receiptByHandle.find(entry.m_handle) ==
            inventory.receiptByHandle.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MissingFramePublication,
              DwgSourceGraphSource{entry.m_handle, entry.m_sourceOffset,
                                   entry.m_sourceMapOrdinal,
                                   entry.m_sourceOffsetSpace});
        }
      }

      for (std::size_t index = 0; index < dictionaryMemberships.size();
           ++index) {
        const DRW_DwgDictionaryMembership &membership =
            dictionaryMemberships[index];
        const DwgSourceGraphSource source{
            membership.m_dictionaryHandle, membership.m_sourceOffset,
            membership.m_sourceMapOrdinal, membership.m_sourceOffsetSpace};
        if (!membership.m_complete) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::IncompleteDictionaryMembership, source);
        }
        if (membership.m_dictionaryHandle == DRW::NoHandle ||
            !membership.m_hasSourceLocation) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::InvalidDictionaryMembership, source);
        }
        const auto receipt =
            inventory.receiptByHandle.find(membership.m_dictionaryHandle);
        if (receipt == inventory.receiptByHandle.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::ExtraDictionaryMembership, source);
        }
        const DRW_DwgFramePublication &publication =
            publications[receipt->second];
        const DwgSourceGraphSource expected{
            publication.m_handle, publication.m_sourceOffset,
            publication.m_sourceMapOrdinal, publication.m_sourceOffsetSpace};
        if (publication.m_resolvedType != DRW_Dictionary::kDwgFixedType ||
            membership.m_version != publication.m_version ||
            !sameDwgSourceGraphSource(source, expected)) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MismatchedDictionaryMembership, source);
        }
        if (!inventory.dictionaryMembershipByHandle
                 .emplace(membership.m_dictionaryHandle, index)
                 .second) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::ExtraDictionaryMembership, source);
        }
        std::map<UTF8STRING, bool> names;
        std::map<std::uint32_t, bool> members;
        for (const DRW_DwgDictionaryMembership::Entry &item :
             membership.m_entries) {
          if (item.m_name.empty() || item.m_handle == DRW::NoHandle) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::InvalidDictionaryMembership, source);
          }
          if (!names.emplace(item.m_name, true).second) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::DuplicateDictionaryKey, source);
          }
          if (!members.emplace(item.m_handle, true).second) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::DuplicateDictionaryMember, source);
          }
        }
      }
      ...
      for (const DRW_DwgFramePublication &publication : publications) {
        if (publication.m_resolvedType != DRW_Dictionary::kDwgFixedType)
          continue;
        if (inventory.dictionaryMembershipByHandle.find(publication.m_handle) ==
            inventory.dictionaryMembershipByHandle.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MissingDictionaryMembership,
              DwgSourceGraphSource{publication.m_handle,
      ...
      for (std::size_t index = 0; index < groupMemberships.size(); ++index) {
        const DRW_DwgGroupMembership &membership = groupMemberships[index];
        const DwgSourceGraphSource source{
            membership.m_groupHandle, membership.m_sourceOffset,
            membership.m_sourceMapOrdinal, membership.m_sourceOffsetSpace};
        if (!membership.m_complete) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::IncompleteGroupMembership, source);
        }
        if (membership.m_groupHandle == DRW::NoHandle ||
            !membership.m_hasSourceLocation) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::InvalidGroupMembership, source);
        }
        const auto receipt =
            inventory.receiptByHandle.find(membership.m_groupHandle);
        if (receipt == inventory.receiptByHandle.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::ExtraGroupMembership, source);
        }
        const DRW_DwgFramePublication &publication =
            publications[receipt->second];
        const DwgSourceGraphSource expected{
            publication.m_handle, publication.m_sourceOffset,
            publication.m_sourceMapOrdinal, publication.m_sourceOffsetSpace};
        if (publication.m_resolvedType != DRW_Group::kDwgFixedType ||
            publication.m_isEntity ||
            membership.m_version != publication.m_version ||
            membership.m_entries.size() > DRW_Group::kMaxEntityHandles ||
            !sameDwgSourceGraphSource(source, expected)) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MismatchedGroupMembership, source);
        }
        for (std::size_t entryIndex = 0;
             entryIndex < membership.m_entries.size(); ++entryIndex) {
          const DRW_DwgGroupMembership::Entry &entry =
              membership.m_entries[entryIndex];
          if (entry.m_handle == DRW::NoHandle ||
              entry.m_ordinal != static_cast<std::uint32_t>(entryIndex)) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::InvalidGroupMembership, source);
          }
        }
        if (!inventory.groupMembershipByHandle
                 .emplace(membership.m_groupHandle, index)
                 .second) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::DuplicateGroupMembership, source);
        }
      }
      ...
      for (const DRW_DwgFramePublication &publication : publications) {
        if (publication.m_resolvedType != DRW_Group::kDwgFixedType)
          continue;
        if (inventory.groupMembershipByHandle.find(publication.m_handle) ==
            inventory.groupMembershipByHandle.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MissingGroupMembership,
              DwgSourceGraphSource{publication.m_handle,
                                   publication.m_sourceOffset,
                                   publication.m_sourceMapOrdinal,
                                   publication.m_sourceOffsetSpace});
      ...
      for (std::size_t index = 0; index < sortEntsMemberships.size(); ++index) {
        const DRW_DwgSortEntsMembership &membership =
            sortEntsMemberships[index];
        const DwgSourceGraphSource source{
            membership.m_tableHandle, membership.m_sourceOffset,
            membership.m_sourceMapOrdinal, membership.m_sourceOffsetSpace};
        if (!membership.m_complete) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::IncompleteSortEntsMembership, source);
        }
        if (membership.m_tableHandle == DRW::NoHandle ||
            membership.m_blockOwnerHandle == DRW::NoHandle ||
            !membership.m_hasSourceLocation ||
            !membership.m_classStreamOrdinal.has_value() ||
            membership.m_entries.size() > DRW_SortEntsTable::kMaxEntries) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::InvalidSortEntsMembership, source);
        }
        const auto receipt =
            inventory.receiptByHandle.find(membership.m_tableHandle);
        if (receipt == inventory.receiptByHandle.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::ExtraSortEntsMembership, source);
        }
        const DRW_DwgFramePublication &publication =
            publications[receipt->second];
        const DwgSourceGraphSource expected{
            publication.m_handle, publication.m_sourceOffset,
            publication.m_sourceMapOrdinal, publication.m_sourceOffsetSpace};
        if (!isDwgSortEntsTablePublication(publication) ||
            membership.m_version != publication.m_version ||
            membership.m_encodedType != publication.m_encodedType ||
            membership.m_resolvedType != publication.m_resolvedType ||
            membership.m_recordName != publication.m_recordName ||
            ...
            !sameDwgSourceGraphSource(source, expected)) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MismatchedSortEntsMembership, source);
        }
        for (std::size_t entryIndex = 0;
             entryIndex < membership.m_entries.size(); ++entryIndex) {
        ...
        if (!inventory.sortEntsMembershipByHandle
                 .emplace(membership.m_tableHandle, index)
                 .second) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::DuplicateSortEntsMembership, source);
        }
      }
      ...
      for (const DRW_DwgFramePublication &publication : publications) {
        if (!isDwgSortEntsTablePublication(publication))
          continue;
        if (inventory.sortEntsMembershipByHandle.find(publication.m_handle) ==
            inventory.sortEntsMembershipByHandle.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MissingSortEntsMembership,
              DwgSourceGraphSource{publication.m_handle,
                                   publication.m_sourceOffset,
                                   publication.m_sourceMapOrdinal,
                                   publication.m_sourceOffsetSpace});
      ...
      for (std::size_t index = 0; index < fieldListMemberships.size();
           ++index) {
        const DRW_DwgFieldListMembership &membership =
            fieldListMemberships[index];
        const DwgSourceGraphSource source{
            membership.m_listHandle, membership.m_sourceOffset,
            membership.m_sourceMapOrdinal, membership.m_sourceOffsetSpace};
        if (!membership.m_complete) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::IncompleteFieldListMembership, source);
        }
        if (membership.m_version < DRW::AC1015 ||
            membership.m_listHandle == DRW::NoHandle ||
            !membership.m_hasSourceLocation ||
            !membership.m_classStreamOrdinal.has_value() ||
            membership.m_entries.size() > DRW_Field::kMaxItems) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::InvalidFieldListMembership, source);
        }
        const auto receipt =
            inventory.receiptByHandle.find(membership.m_listHandle);
        if (receipt == inventory.receiptByHandle.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::ExtraFieldListMembership, source);
        }
        const DRW_DwgFramePublication &publication =
            publications[receipt->second];
        const DwgSourceGraphSource expected{
        ...
        if (!isDwgFieldListPublication(publication) ||
            ...
            !sameDwgSourceGraphSource(source, expected)) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MismatchedFieldListMembership, source);
        }
        for (std::size_t entryIndex = 0;
             entryIndex < membership.m_entries.size(); ++entryIndex) {
          const DRW_DwgFieldListMembership::Entry &entry =
              membership.m_entries[entryIndex];
          if (entry.m_ordinal != static_cast<std::uint32_t>(entryIndex)) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::InvalidFieldListMembership, source);
          }
        }
        if (!inventory.fieldListMembershipByHandle
                 .emplace(membership.m_listHandle, index)
                 .second) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::DuplicateFieldListMembership, source);
        }
      }
      ...
      for (const DRW_DwgFramePublication &publication : publications) {
        if (!isDwgFieldListPublication(publication))
          continue;
        if (inventory.fieldListMembershipByHandle.find(publication.m_handle) ==
            inventory.fieldListMembershipByHandle.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MissingFieldListMembership,
              DwgSourceGraphSource{publication.m_handle,
                                   publication.m_sourceOffset,
                                   publication.m_sourceMapOrdinal,
                                   publication.m_sourceOffsetSpace});
      ...
      for (std::size_t index = 0; index < typedReferences.size(); ++index) {
        const DRW_DwgTypedReference &reference = typedReferences[index];
        const DwgSourceGraphSource source{
            reference.m_sourceHandle, reference.m_sourceOffset,
            reference.m_sourceMapOrdinal, reference.m_sourceOffsetSpace};
        if (!reference.m_complete) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::IncompleteTypedReference, source);
        }
        if (reference.m_sourceHandle == DRW::NoHandle ||
            !reference.m_hasSourceLocation ||
            reference.m_field == DRW_DwgTypedReferenceField::Unknown ||
            reference.m_referenceCode != DRW::DwgHardPointer) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::InvalidTypedReference, source);
        }
        const auto receipt =
            inventory.receiptByHandle.find(reference.m_sourceHandle);
        if (receipt == inventory.receiptByHandle.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::ExtraTypedReference, source);
        }
        const DRW_DwgFramePublication &publication =
            publications[receipt->second];
        const DwgSourceGraphSource expected{
            ...
            publication.m_sourceMapOrdinal, publication.m_sourceOffsetSpace};
        if (!isDwgDictionaryWithDefaultPublication(publication) ||
            reference.m_version != publication.m_version ||
            reference.m_encodedType != publication.m_encodedType ||
            reference.m_resolvedType != publication.m_resolvedType ||
            !reference.m_classStreamOrdinal.has_value() ||
            !publication.m_classStreamOrdinal.has_value() ||
            reference.m_classStreamOrdinal !=
                publication.m_classStreamOrdinal ||
            !sameDwgSourceGraphSource(source, expected)) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MismatchedTypedReference, source);
        }
        const DwgSourceGraphTypedReferenceKey key{reference.m_sourceHandle,
                                                  reference.m_field};
        if (!inventory.typedReferenceBySource.emplace(key, index).second) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::DuplicateTypedReference, source);
        }
      }
      ...
      for (const DRW_DwgFramePublication &publication : publications) {
        if (!isDwgDictionaryWithDefaultPublication(publication))
          continue;
        const DwgSourceGraphTypedReferenceKey key{
            publication.m_handle,
            DRW_DwgTypedReferenceField::DictionaryDefault};
        if (inventory.typedReferenceBySource.find(key) ==
            inventory.typedReferenceBySource.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MissingTypedReference,
              DwgSourceGraphSource{publication.m_handle,
                                   publication.m_sourceOffset,
                                   publication.m_sourceMapOrdinal,
                                   publication.m_sourceOffsetSpace});
      ...
      const auto reachabilitySource = [](const DRW_DwgSourceFrame &source) {
        return DwgSourceGraphSource{source.m_handle, source.m_sourceOffset,
                                    source.m_sourceMapOrdinal,
                                    source.m_sourceOffsetSpace};
      };
      const auto publicationForReachability =
          [&inventory, &publications,
           &reachabilitySource](const DRW_DwgSourceFrame &source)
          -> const DRW_DwgFramePublication * {
        if (!source.m_hasSourceLocation) {
          return nullptr;
        }
        const auto receipt = inventory.receiptByHandle.find(source.m_handle);
        if (receipt == inventory.receiptByHandle.cend()) {
          return nullptr;
        }
        const DRW_DwgFramePublication &publication =
        ...
        const DwgSourceGraphSource expected{
            publication.m_handle, publication.m_sourceOffset,
            publication.m_sourceMapOrdinal, publication.m_sourceOffsetSpace};
        return sameDwgSourceGraphSource(reachabilitySource(source), expected)
                   ? &publication
                   : nullptr;
      };
      for (std::size_t index = 0; index < blockReachabilities.size(); ++index) {
        const DRW_DwgBlockReachability &reachability =
            blockReachabilities[index];
        const DwgSourceGraphSource source =
            reachabilitySource(reachability.m_blockRecord);
        if (!reachability.m_complete) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::IncompleteBlockReachability, source);
        }
        if (reachability.m_version < DRW::AC1018 ||
            reachability.m_blockRecord.m_handle == DRW::NoHandle ||
            reachability.m_block.m_handle == DRW::NoHandle ||
            reachability.m_endBlock.m_handle == DRW::NoHandle) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::InvalidBlockReachability, source);
        }
        const DRW_DwgFramePublication *const record =
            publicationForReachability(reachability.m_blockRecord);
        const DRW_DwgFramePublication *const block =
            publicationForReachability(reachability.m_block);
        const DRW_DwgFramePublication *const endBlock =
            publicationForReachability(reachability.m_endBlock);
        if (record == nullptr || block == nullptr || endBlock == nullptr) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::ExtraBlockReachability, source);
        }
        if (!isDwgModernBlockRecordPublication(*record) ||
            record->m_version != reachability.m_version ||
            block->m_version != reachability.m_version ||
            block->m_resolvedType != DRW::DwgBlockEntityType ||
            !block->m_isEntity ||
            endBlock->m_version != reachability.m_version ||
            endBlock->m_resolvedType != DRW::DwgEndBlockEntityType ||
            !endBlock->m_isEntity ||
            reachability.m_blockRecord.m_handle ==
                reachability.m_block.m_handle ||
            reachability.m_blockRecord.m_handle ==
                reachability.m_endBlock.m_handle ||
            reachability.m_block.m_handle == reachability.m_endBlock.m_handle) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MismatchedBlockReachability, source);
        }
        std::map<std::uint32_t, bool> entities;
        for (const DRW_DwgSourceFrame &entity : reachability.m_entities) {
          const DRW_DwgFramePublication *const publication =
              publicationForReachability(entity);
          if (publication == nullptr || !publication->m_isEntity ||
              publication->m_version != reachability.m_version ||
              entity.m_handle == reachability.m_blockRecord.m_handle ||
              entity.m_handle == reachability.m_block.m_handle ||
              entity.m_handle == reachability.m_endBlock.m_handle ||
              !entities.emplace(entity.m_handle, true).second) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::InvalidBlockReachability, source);
          }
        }
        if (!inventory.blockReachabilityByRecord
                 .emplace(reachability.m_blockRecord.m_handle, index)
                 .second) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::DuplicateBlockReachability, source);
        }
      }

      for (const DRW_DwgFramePublication &publication : publications) {
        if (!isDwgModernBlockRecordPublication(publication))
          continue;
        if (inventory.blockReachabilityByRecord.find(publication.m_handle) ==
            inventory.blockReachabilityByRecord.cend()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::MissingBlockReachability,
              DwgSourceGraphSource{publication.m_handle,
                                   publication.m_sourceOffset,
                                   publication.m_sourceMapOrdinal,
                                   publication.m_sourceOffsetSpace});
      ...
      if (classesUnavailable && !classReport.m_entries.empty()) {
        return refuseDwgSourceGraph(
            DwgSourceGraphRefusal::ClassCoverageIncomplete);
      }
      if (classesComplete) {
        for (std::size_t index = 0; index < classReport.m_entries.size();
             ++index) {
          const DRW_DwgClassCoverageEntry &entry = classReport.m_entries[index];
          if (entry.m_state != DRW_DwgClassCoverageState::Published) {
            return refuseDwgSourceGraph(DwgSourceGraphRefusal::UnpublishedClass,
                                        std::nullopt, entry.m_streamOrdinal);
          }
          if (!inventory.classByOrdinal.emplace(entry.m_streamOrdinal, index)
                   .second) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::InvalidCustomClassOrdinal, std::nullopt,
                entry.m_streamOrdinal);
          }
        }
      }

      std::size_t edgeCapacity = 0;
      const auto addEdgeCapacity = [&edgeCapacity](std::size_t count) {
        if (count > std::numeric_limits<std::size_t>::max() - edgeCapacity) {
          return false;
        }
        edgeCapacity += count;
        return true;
      };
      for (const auto &coveragePair : inventory.coverageByOrdinal) {
        const DRW_DwgFrameCoverageEntry &entry =
            frameReport.m_entries[coveragePair.second];
        const DwgSourceGraphSource source{entry.m_handle, entry.m_sourceOffset,
                                          entry.m_sourceMapOrdinal,
                                          entry.m_sourceOffsetSpace};
        const auto receipt = inventory.receiptByHandle.find(entry.m_handle);
        const DRW_DwgFramePublication &publication =
            publications[receipt->second];

        if (publication.m_isCustomClass) {
          if (classesUnavailable) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::ClassCoverageIncomplete, source);
          }
          if (!publication.m_classStreamOrdinal.has_value()) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::MissingCustomClassOrdinal, source);
          }
          const auto classIt =
              inventory.classByOrdinal.find(*publication.m_classStreamOrdinal);
          if (classIt == inventory.classByOrdinal.cend()) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::InvalidCustomClassOrdinal, source,
                publication.m_classStreamOrdinal);
          }
          const DRW_DwgClassCoverageEntry &classEntry =
              classReport.m_entries[classIt->second];
          if (publication.m_encodedType < 0 ||
              static_cast<std::uint32_t>(publication.m_encodedType) !=
                  classEntry.m_classNumber ||
              publication.m_recordName != classEntry.m_recordName ||
              publication.m_className != classEntry.m_className ||
              publication.m_isEntity != (classEntry.m_entityFlag != 0)) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::CustomClassMismatch, source,
                publication.m_classStreamOrdinal);
          }
          if (!addEdgeCapacity(1)) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::AllocationFailure, source);
          }
        } else if (publication.m_classStreamOrdinal.has_value()) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::CustomClassMismatch, source,
              publication.m_classStreamOrdinal);
        }
        ...
        if (control != (publication.m_commonLinkEvidence ==
                        DRW_DwgCommonLinkEvidence::NotApplicable) ||
            (!control && !publication.m_controlHandles.empty())) {
          return refuseDwgSourceGraph(
              DwgSourceGraphRefusal::InvalidControlReceipt, source);
        }
        switch (publication.m_commonLinkEvidence) {
        case DRW_DwgCommonLinkEvidence::Validated:
          if (publication.m_numReactors < 0 ||
              static_cast<std::size_t>(publication.m_numReactors) !=
                  publication.m_reactorHandles.size()) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::ReactorCountMismatch, source);
          }
          if ((publication.m_parentHandle != DRW::NoHandle &&
               !addEdgeCapacity(1u)) ||
              !addEdgeCapacity(publication.m_reactorHandles.size()) ||
              (publication.m_xDictHandle != DRW::NoHandle &&
               !addEdgeCapacity(1u))) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::AllocationFailure, source);
          }
          break;
        case DRW_DwgCommonLinkEvidence::ValidatedAbsent:
          if (publication.m_parentHandle != DRW::NoHandle ||
              !publication.m_reactorHandles.empty() ||
              publication.m_numReactors != 0 ||
              publication.m_xDictHandle != DRW::NoHandle) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::InvalidValidatedAbsentLinks, source);
          }
          break;
        case DRW_DwgCommonLinkEvidence::NotApplicable:
          if (publication.m_carrier !=
                  DRW_DwgFramePublication::Carrier::Control ||
              publication.m_parentHandle != DRW::NoHandle ||
              !publication.m_reactorHandles.empty() ||
              publication.m_numReactors != 0 ||
              publication.m_xDictHandle != DRW::NoHandle) {
            return refuseDwgSourceGraph(
          ...
          if (!addEdgeCapacity(publication.m_controlHandles.size())) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::AllocationFailure, source);
          }
          break;
        case DRW_DwgCommonLinkEvidence::Unknown:
        default:
          return refuseDwgSourceGraph(DwgSourceGraphRefusal::UnknownCommonLinks,
                                      source);
        }
        if (publication.m_resolvedType == DRW_Dictionary::kDwgFixedType) {
          const auto membership =
              inventory.dictionaryMembershipByHandle.find(publication.m_handle);
          if (membership == inventory.dictionaryMembershipByHandle.cend() ||
              !addEdgeCapacity(
                  dictionaryMemberships[membership->second].m_entries.size())) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::AllocationFailure, source);
          }
        }
        if (publication.m_resolvedType == DRW_Group::kDwgFixedType) {
          const auto membership =
              inventory.groupMembershipByHandle.find(publication.m_handle);
          if (membership == inventory.groupMembershipByHandle.cend() ||
              !addEdgeCapacity(
                  groupMemberships[membership->second].m_entries.size())) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::AllocationFailure, source);
          }
        }
        if (isDwgSortEntsTablePublication(publication)) {
          const auto membership =
              inventory.sortEntsMembershipByHandle.find(publication.m_handle);
          if (membership == inventory.sortEntsMembershipByHandle.cend()) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::MissingSortEntsMembership, source);
          }
          const DRW_DwgSortEntsMembership &value =
              sortEntsMemberships[membership->second];
          std::size_t referenceCount = 1u + value.m_entries.size();
          for (const DRW_DwgSortEntsMembership::Entry &entry :
               value.m_entries) {
            if (entry.m_sortHandle != DRW::NoHandle) {
              if (referenceCount == std::numeric_limits<std::size_t>::max()) {
                return refuseDwgSourceGraph(
                    DwgSourceGraphRefusal::AllocationFailure, source);
              }
              ++referenceCount;
          ...
          if (!addEdgeCapacity(referenceCount)) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::AllocationFailure, source);
          }
        }
        if (isDwgFieldListPublication(publication)) {
          const auto membership =
              inventory.fieldListMembershipByHandle.find(publication.m_handle);
          if (membership == inventory.fieldListMembershipByHandle.cend()) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::MissingFieldListMembership, source);
          }
          std::size_t referenceCount = 0;
          for (const DRW_DwgFieldListMembership::Entry &entry :
               fieldListMemberships[membership->second].m_entries) {
            if (entry.m_fieldHandle != DRW::NoHandle)
              ++referenceCount;
          }
          if (!addEdgeCapacity(referenceCount)) {
        ...
        if (isDwgDictionaryWithDefaultPublication(publication)) {
          const DwgSourceGraphTypedReferenceKey key{
              publication.m_handle,
              DRW_DwgTypedReferenceField::DictionaryDefault};
          const auto reference = inventory.typedReferenceBySource.find(key);
          if (reference == inventory.typedReferenceBySource.cend() ||
              (typedReferences[reference->second].m_targetHandle !=
                   DRW::NoHandle &&
               !addEdgeCapacity(1u))) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::AllocationFailure, source);
      ...
      if (frameReport.m_entries.size() >
          std::numeric_limits<std::size_t>::max() -
              (classesComplete ? classReport.m_entries.size() : 0u)) {
        return refuseDwgSourceGraph(DwgSourceGraphRefusal::AllocationFailure);
      }
      DwgSourceGraphResult result;
      result.nodes.reserve(
          frameReport.m_entries.size() +
          (classesComplete ? classReport.m_entries.size() : 0u));
      result.edges.reserve(edgeCapacity);

      for (const auto &coveragePair : inventory.coverageByOrdinal) {
        const DRW_DwgFrameCoverageEntry &entry =
            frameReport.m_entries[coveragePair.second];
        const DRW_DwgFramePublication &publication =
            publications[inventory.receiptByHandle.find(entry.m_handle)
                             ->second];
        DwgSourceGraphNode node;
        node.kind = DwgSourceGraphNodeKind::Frame;
        node.source = {entry.m_handle, entry.m_sourceOffset,
                       entry.m_sourceMapOrdinal, entry.m_sourceOffsetSpace};
        node.encodedType = publication.m_encodedType;
        node.resolvedType = publication.m_resolvedType;
        node.isEntity = publication.m_isEntity;
        node.isCustomClass = publication.m_isCustomClass;
        node.commonLinkEvidence = publication.m_commonLinkEvidence;
        node.recordName = publication.m_recordName;
        node.className = publication.m_className;
        const std::size_t nodeIndex = result.nodes.size();
        result.nodes.push_back(std::move(node));
        inventory.frameNodeByHandle.emplace(entry.m_handle, nodeIndex);
      }
      if (classesComplete) {
        for (const auto &classPair : inventory.classByOrdinal) {
          const DRW_DwgClassCoverageEntry &entry =
              classReport.m_entries[classPair.second];
          DwgSourceGraphNode node;
          node.kind = DwgSourceGraphNodeKind::Class;
          node.classStreamOrdinal = entry.m_streamOrdinal;
          node.hasClassStreamOrdinal = true;
          node.classNumber = entry.m_classNumber;
          node.isEntity = entry.m_entityFlag != 0;
          node.recordName = entry.m_recordName;
          node.className = entry.m_className;
          const std::size_t nodeIndex = result.nodes.size();
          result.nodes.push_back(std::move(node));
          inventory.classNodeByOrdinal.emplace(entry.m_streamOrdinal,
                                               nodeIndex);
        }
      }

      inventory.hierarchy.resize(result.nodes.size());
      inventory.hierarchyInbound.assign(result.nodes.size(), 0u);
      const auto nodeForHandle =
          [&inventory](std::uint32_t handle) -> std::optional<std::size_t> {
        const auto it = inventory.frameNodeByHandle.find(handle);
        return it == inventory.frameNodeByHandle.cend()
                   ? std::nullopt
                   : std::optional<std::size_t>(it->second);
      };
      const auto addEdge =
          [&inventory, &result](std::size_t sourceNode, std::size_t targetNode,
                                DwgSourceGraphEdgeKind kind,
                                const DwgSourceGraphSource &source,
                                std::uint32_t membershipOrdinal =
                                    0) -> std::optional<DwgSourceGraphResult> {
        const bool hierarchy = kind == DwgSourceGraphEdgeKind::Owner ||
                               kind == DwgSourceGraphEdgeKind::ControlChild;
        if (hierarchy && sourceNode == targetNode) {
          return refuseDwgSourceGraph(DwgSourceGraphRefusal::HierarchyCycle,
                                      source);
        }
        if (!inventory.edgeKeys
                 .emplace(
                     DwgSourceGraphEdgeKey{sourceNode, targetNode, kind,
                                           DRW_DwgTypedReferenceField::Unknown,
                                           membershipOrdinal},
                     true)
                 .second) {
          return refuseDwgSourceGraph(DwgSourceGraphRefusal::DuplicateLink,
                                      source);
        }
        result.edges.push_back({sourceNode, targetNode, kind,
                                DRW_DwgTypedReferenceField::Unknown,
                                membershipOrdinal});
        if (hierarchy) {
          inventory.hierarchy[sourceNode].push_back(targetNode);
          ++inventory.hierarchyInbound[targetNode];
        }
        return std::nullopt;
      };
      ...
      for (const auto &coveragePair : inventory.coverageByOrdinal) {
        ...
        const std::size_t subject =
            inventory.frameNodeByHandle.find(entry.m_handle)->second;
        const auto addHandleEdge =
            [&](std::uint32_t handle, DwgSourceGraphEdgeKind kind,
                bool targetIsSubject =
                    false) -> std::optional<DwgSourceGraphResult> {
          const auto endpoint = nodeForHandle(handle);
          if (!endpoint.has_value()) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::MissingLinkEndpoint, source);
          }
          return targetIsSubject ? addEdge(*endpoint, subject, kind, source)
                                 : addEdge(subject, *endpoint, kind, source);
        };
        ...
        case DRW_DwgCommonLinkEvidence::Validated:
          if (publication.m_parentHandle != DRW::NoHandle) {
            if (const auto failure =
                    addHandleEdge(publication.m_parentHandle,
                                  DwgSourceGraphEdgeKind::Owner, true)) {
              return *failure;
            }
          }
          for (const std::uint32_t reactor : publication.m_reactorHandles) {
            if (const auto failure =
                    addHandleEdge(reactor, DwgSourceGraphEdgeKind::Reactor)) {
              return *failure;
            }
          }
          if (publication.m_xDictHandle != DRW::NoHandle) {
            if (const auto failure = addHandleEdge(
                    publication.m_xDictHandle,
                    DwgSourceGraphEdgeKind::ExtensionDictionary)) {
              return *failure;
            }
          }
          break;
        case DRW_DwgCommonLinkEvidence::NotApplicable:
          for (const std::uint32_t child : publication.m_controlHandles) {
            if (const auto failure = addHandleEdge(
                    child, DwgSourceGraphEdgeKind::ControlChild)) {
              return *failure;
            }
          }
          break;
        ...
        if (publication.m_resolvedType == DRW_Dictionary::kDwgFixedType) {
          ...
          if (membership == inventory.dictionaryMembershipByHandle.cend()) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::MissingDictionaryMembership, source);
          }
          for (const DRW_DwgDictionaryMembership::Entry &item :
               dictionaryMemberships[membership->second].m_entries) {
            if (const auto failure = addHandleEdge(
                    item.m_handle, DwgSourceGraphEdgeKind::DictionaryMember)) {
              return *failure;
            }
          }
        }
        if (publication.m_resolvedType == DRW_Group::kDwgFixedType) {
          ...
          if (membership == inventory.groupMembershipByHandle.cend()) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::MissingGroupMembership, source);
          }
          for (const DRW_DwgGroupMembership::Entry &item :
               groupMemberships[membership->second].m_entries) {
            const auto endpoint = nodeForHandle(item.m_handle);
            if (!endpoint.has_value()) {
              return refuseDwgSourceGraph(
                  DwgSourceGraphRefusal::MissingLinkEndpoint, source);
            }
            if (!result.nodes[*endpoint].isEntity) {
              return refuseDwgSourceGraph(
                  DwgSourceGraphRefusal::InvalidGroupMembership, source);
            }
            if (const auto failure = addEdge(
                    subject, *endpoint, DwgSourceGraphEdgeKind::GroupMember,
                    source, item.m_ordinal)) {
              return *failure;
            }
          }
        }
        if (isDwgSortEntsTablePublication(publication)) {
          ...
          const DRW_DwgSortEntsMembership &value =
              sortEntsMemberships[membership->second];
          const auto owner = nodeForHandle(value.m_blockOwnerHandle);
          if (!owner.has_value()) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::MissingLinkEndpoint, source);
          }
          if (result.nodes[*owner].isEntity ||
              result.nodes[*owner].resolvedType !=
                  DRW::DwgBlockRecordObjectType) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::InvalidSortEntsMembership, source);
          }
          if (const auto failure =
                  addEdge(subject, *owner,
                          DwgSourceGraphEdgeKind::SortEntsBlockOwner, source)) {
            return *failure;
          }
          for (const DRW_DwgSortEntsMembership::Entry &entry :
               value.m_entries) {
            const auto entity = nodeForHandle(entry.m_entityHandle);
            if (!entity.has_value()) {
              return refuseDwgSourceGraph(
                  DwgSourceGraphRefusal::MissingLinkEndpoint, source);
            }
            if (!result.nodes[*entity].isEntity) {
              return refuseDwgSourceGraph(
                  DwgSourceGraphRefusal::InvalidSortEntsMembership, source);
            }
            if (const auto failure = addEdge(
                    subject, *entity, DwgSourceGraphEdgeKind::SortEntsEntity,
                    source, entry.m_ordinal)) {
              return *failure;
            }
            if (entry.m_sortHandle == DRW::NoHandle)
              continue;
            const auto sortKey = nodeForHandle(entry.m_sortHandle);
            if (!sortKey.has_value()) {
              return refuseDwgSourceGraph(
                  DwgSourceGraphRefusal::MissingLinkEndpoint, source);
            }
            if (const auto failure = addEdge(
                    subject, *sortKey, DwgSourceGraphEdgeKind::SortEntsSortKey,
                    source, entry.m_ordinal)) {
              return *failure;
            }
          }
        ...
        if (isDwgFieldListPublication(publication)) {
          ...
          const DRW_DwgFieldListMembership &value =
              fieldListMemberships[membership->second];
          for (const DRW_DwgFieldListMembership::Entry &entry :
               value.m_entries) {
            if (entry.m_fieldHandle == DRW::NoHandle)
              continue;
            const auto field = nodeForHandle(entry.m_fieldHandle);
            if (!field.has_value()) {
              return refuseDwgSourceGraph(
                  DwgSourceGraphRefusal::MissingLinkEndpoint, source);
            }
            if (!isDwgFieldPublication(result.nodes[*field])) {
              return refuseDwgSourceGraph(
                  DwgSourceGraphRefusal::InvalidFieldListMembership, source);
            }
            if (const auto failure = addEdge(
                    subject, *field, DwgSourceGraphEdgeKind::FieldListMember,
                    source, entry.m_ordinal)) {
              return *failure;
            }
          }
        ...
        if (isDwgDictionaryWithDefaultPublication(publication)) {
          ...
          if (reference == inventory.typedReferenceBySource.cend()) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::MissingTypedReference, source);
          }
          const DRW_DwgTypedReference &value =
              typedReferences[reference->second];
          if (value.m_targetHandle != DRW::NoHandle) {
            const auto endpoint = nodeForHandle(value.m_targetHandle);
            if (!endpoint.has_value()) {
              return refuseDwgSourceGraph(
                  DwgSourceGraphRefusal::MissingLinkEndpoint, source);
            }
            if (!inventory.edgeKeys
                     .emplace(
                         DwgSourceGraphEdgeKey{
                             subject, *endpoint,
                             DwgSourceGraphEdgeKind::TypedReference,
                             value.m_field},
                         true)
                     .second) {
              return refuseDwgSourceGraph(DwgSourceGraphRefusal::DuplicateLink,
            ...
            result.edges.push_back({subject, *endpoint,
                                    DwgSourceGraphEdgeKind::TypedReference,
                                    value.m_field});
          }
        }
        if (isDwgModernBlockRecordPublication(publication)) {
          const auto reachability =
              inventory.blockReachabilityByRecord.find(publication.m_handle);
          if (reachability == inventory.blockReachabilityByRecord.cend()) {
            return refuseDwgSourceGraph(
                DwgSourceGraphRefusal::MissingBlockReachability, source);
          }
          const DRW_DwgBlockReachability &value =
              blockReachabilities[reachability->second];
          const auto addReachabilityEdge =
              [&addHandleEdge](
                  std::uint32_t handle) -> std::optional<DwgSourceGraphResult> {
            return addHandleEdge(handle,
                                 DwgSourceGraphEdgeKind::BlockReachability);
          };
          if (const auto failure =
                  addReachabilityEdge(value.m_block.m_handle)) {
            return *failure;
          }
          if (const auto failure =
                  addReachabilityEdge(value.m_endBlock.m_handle)) {
            return *failure;
          }
          for (const DRW_DwgSourceFrame &entity : value.m_entities) {
            if (const auto failure = addReachabilityEdge(entity.m_handle)) {
              return *failure;
            }
          }
        }
        if (publication.m_isCustomClass) {
          const std::size_t classNode =
              inventory.classNodeByOrdinal
                  .find(*publication.m_classStreamOrdinal)
                  ->second;
          if (const auto failure =
                  addEdge(subject, classNode, DwgSourceGraphEdgeKind::ClassUse,
                          source)) {
            return *failure;
          }
        }
      ...
      std::sort(
          ...
          [](const DwgSourceGraphEdge &lhs, const DwgSourceGraphEdge &rhs) {
            if (lhs.sourceNode != rhs.sourceNode)
              return lhs.sourceNode < rhs.sourceNode;
            if (lhs.targetNode != rhs.targetNode)
              return lhs.targetNode < rhs.targetNode;
            if (lhs.kind != rhs.kind) {
              return static_cast<unsigned>(lhs.kind) <
                     static_cast<unsigned>(rhs.kind);
            }
            if (lhs.typedReferenceField != rhs.typedReferenceField) {
              return static_cast<unsigned>(lhs.typedReferenceField) <
                     static_cast<unsigned>(rhs.typedReferenceField);
            }
            return lhs.membershipOrdinal < rhs.membershipOrdinal;
          });

      std::vector<std::size_t> inbound = inventory.hierarchyInbound;
      std::vector<std::size_t> queue;
      queue.reserve(result.nodes.size());
      ...
      for (std::size_t index = 0; index < queue.size(); ++index) {
        const std::size_t node = queue[index];
        ++visited;
        for (const std::size_t child : inventory.hierarchy[node]) {
          if (--inbound[child] == 0u)
            queue.push_back(child);
        }
      }
      if (visited != result.nodes.size()) {
        for (std::size_t index = 0; index < inbound.size(); ++index) {
          if (inbound[index] == 0u ||
              result.nodes[index].kind != DwgSourceGraphNodeKind::Frame) {
            continue;
          }
          return refuseDwgSourceGraph(DwgSourceGraphRefusal::HierarchyCycle,
                                      result.nodes[index].source);
        }
        return refuseDwgSourceGraph(DwgSourceGraphRefusal::HierarchyCycle);
      }

      result.status = DwgSourceGraphStatus::Complete;
      result.refusal = DwgSourceGraphRefusal::None;
      return result;
    } catch (...) {
      return refuseDwgSourceGraph(DwgSourceGraphRefusal::AllocationFailure);
    }
  ...
  static std::vector<std::shared_ptr<DRW_Variant>>
  copyTableEntryExtData(const std::vector<DRW_Variant *> &source) {
    std::vector<std::shared_ptr<DRW_Variant>> result;
    result.reserve(source.size());
    for (const DRW_Variant *value : source) {
      if (value != nullptr)
        result.push_back(std::make_shared<DRW_Variant>(*value));
    }
    return result;
  }

  static ModelerPayloadSection
  modelerPayloadSectionForMarker(const ModelerPayloadMarker &marker,
                                 const ModelerRawByteSplit &split,
                                 size_t rawByteCount) {
  ...
  static ModelerPayloadMarker
  findModelerPayloadMarker(const std::vector<std::uint8_t> &bytes,
                           const char *marker, ModelerPayloadKind kind) {
  ...
  static constexpr size_t npos() { return static_cast<size_t>(-1); }

  static ModelerPayloadMarker
  scanModelerPayloadMarkerInRange(const std::vector<std::uint8_t> &bytes,
                                  size_t start, size_t end) {
    const ModelerPayloadMarker sab = findModelerPayloadMarkerInRange(
        bytes, "ACIS BinaryFile", ModelerPayloadKind::Sab, start, end);
    if (sab.kind != ModelerPayloadKind::Unknown)
      return sab;
    const ModelerPayloadMarker satHistory = findModelerPayloadMarkerInRange(
        bytes, "Begin-of-ACIS-History", ModelerPayloadKind::Sat, start, end);
    if (satHistory.kind != ModelerPayloadKind::Unknown)
      return satHistory;
    return findModelerPayloadMarkerInRange(bytes, "ACIS",
                                           ModelerPayloadKind::Sat, start, end);
  }

  static ModelerPayloadMarker
  findModelerPayloadMarkerInRange(const std::vector<std::uint8_t> &bytes,
                                  const char *marker, ModelerPayloadKind kind,
                                  size_t start, size_t end) {
    ModelerPayloadMarker result;
  ...
  static std::vector<std::uint8_t> sabTerminatorBytes() {
    return {'E', 'n', 'd', 0x0Eu, 0x02u, 'o', 'f', 0x0Eu, 0x04u, 'A',
            'C', 'I', 'S', 0x0Du, 0x04u, 'd', 'a', 't',   'a'};
  }
  ...
  static void appendModelerRange(std::vector<ModelerPayloadRangeRecord> &ranges,
                                 ModelerPayloadRangeKind kind,
                                 ModelerPayloadSection section, size_t offset,
  ...
  static void
  appendUnknownModelerRange(std::vector<ModelerPayloadRangeRecord> &ranges,
                            size_t offset, size_t length,
                            size_t declaredByteSize,
                            ModelerPayloadRangeConsistency consistency) {
...
public:
  static bool tableNameEquals(const std::string &lhs, const std::string &rhs) {
  ...
  const DRW_Vport *findVportTableEntryByName(const std::string &name) const {
    for (const auto &entry : m_vportTableEntries) {
      if (tableNameEquals(entry.first, name))
        return &entry.second;
    }
    return nullptr;
  }
  const std::map<std::string, DRW_Vport> &vportTableEntries() const {
    return m_vportTableEntries;
  }
  ...
  std::vector<const ViewRecord *>
  findViewsReferencingHandle(std::uint32_t handle) const {
  ...
  const DocumentMappingRecord *
  findDocumentMappingBySourceHandle(std::uint32_t handle) const {
    if (handle == 0)
      return nullptr;
  ...
  std::vector<const DocumentMappingRecord *>
  findDocumentMappingsByName(DocumentMappingSource sourceType,
                             const std::string &name) const {
  ...
  std::vector<const DocumentMappingRecord *>
  findDocumentMappingsByOwner(std::uint32_t ownerHandle) const {
    std::vector<const DocumentMappingRecord *> result;
    if (ownerHandle == 0)
      return result;
    for (const DocumentMappingRecord &record : m_documentMappings) {
  ...
  std::vector<const DocumentMappingRecord *>
  findDocumentMappingsByLayout(std::uint32_t layoutHandle) const {
    std::vector<const DocumentMappingRecord *> result;
    if (layoutHandle == 0)
      return result;
    for (const DocumentMappingRecord &record : m_documentMappings) {
  ...
  void mapViewToDocumentItem(std::uint32_t handle, const std::string &name,
                             int documentItemIndex) {
    mapDocumentItem(DocumentMappingSource::View, handle, name,
                    documentItemIndex);
  }
  void mapUcsToDocumentItem(std::uint32_t handle, const std::string &name,
                            int documentItemIndex) {
    mapDocumentItem(DocumentMappingSource::Ucs, handle, name,
                    documentItemIndex);
  }
  bool invalidateDocumentMappingByItem(DocumentMappingSource sourceType,
  ...
  VisualMetadataSummaryCounts visualMetadataSummaryCounts() const {
    ...
    for (const VisualMetadataSummaryRecord &record :
      ...
      incrementVisualMetadataSpecCoverageCount(counts, record.specCoverage);
      if (record.replayState == ReplayState::ReplayInvalidated)
        ++counts.invalidated;
      if (record.replayState == ReplayState::ReplayReplaced)
        ++counts.replaced;
  ...
  VisualMetadataWriterBlockerCounts
  visualMetadataWriterBlockerCounts(DRW::Version version) const {
  ...
  VisualMetadataReplayEligibility
  visualMetadataReplayEligibility(std::uint32_t handle,
                                  DRW::Version version) const {
    ...
    for (const RawObjectRecord &record : m_rawObjects) {
      ...
      eligibility.rawReplayable = eligibility.rawBlocker == ReplayBlocker::None;
  ...
  std::vector<const LightRecord *>
  findLightsByParentHandle(std::uint32_t parentHandle) const {
  ...
  static ReplayBlocker rawReplayBlocker(const RawObjectRecord &record) {
    ...
    const std::uint32_t blockOwnerHandle =
        record.blockOwnerHandle != DRW::NoHandle ? record.blockOwnerHandle
                                                 : record.parentHandle;
    const bool replayableCustomEntity =
        record.isEntity && record.isCustomClass &&
        blockOwnerHandle != DRW::NoHandle &&
        (!record.recordName.empty() || !record.className.empty());
    if (record.isEntity && !isReplayableFixedModelerRawEntity(record) &&
        !isReplayableFixedEntityShellRawEntity(record) &&
        !isReplayableSurfaceRawEntity(record) && !replayableCustomEntity)
      return ReplayBlocker::EntityReplayUnsupported;
    if (record.rawBytes.empty())
      return ReplayBlocker::MissingRawBytes;
    // Keep malformed carriers out of the replay-eligible set at the
    // metadata boundary.  This is especially important for AC1024+,
    // where bodyBitSize describes only the trailing handle stream.
    if (record.objectSize != 0 &&
        static_cast<std::uint64_t>(record.objectSize) !=
            static_cast<std::uint64_t>(record.rawBytes.size())) {
      return ReplayBlocker::WriterRejected;
    }
    if (record.version > DRW::AC1021) {
      if (record.rawBytes.size() >
              std::numeric_limits<std::uint64_t>::max() / 8u ||
          static_cast<std::uint64_t>(record.bodyBitSize) >
              static_cast<std::uint64_t>(record.rawBytes.size()) * 8u) {
        return ReplayBlocker::WriterRejected;
      }
    }
    if (record.isCustomClass && record.recordName.empty() &&
  ...
  static bool isReplayableFixedModelerRawEntity(const RawObjectRecord &record) {
    return record.isEntity && !record.isCustomClass &&
           (record.objectType ==
                34 // VIEWPORT (paper-space; no typed RS entity)
            || record.objectType == 37 || record.objectType == 38 ||
            record.objectType == 39);
  }

  static bool
  isReplayableFixedEntityShellRawEntity(const RawObjectRecord &record) {
    return record.isEntity && !record.isCustomClass &&
           DRW_UnsupportedObject::isFixedEntityShellType(record.objectType);
  }

  static bool isReplayableSurfaceRawEntity(const RawObjectRecord &record) {
    if (!record.isEntity || !record.isCustomClass)
      return false;
    return record.recordName == "PLANESURFACE" ||
           record.recordName == "EXTRUDEDSURFACE" ||
           record.recordName == "REVOLVEDSURFACE" ||
           record.recordName == "SWEPTSURFACE" ||
           record.recordName == "LOFTEDSURFACE" ||
           record.recordName == "NURBSURFACE";
  }

  static RawObjectFamily
  rawObjectFamilyFromNames(const std::string &recordName,
    ...
    const bool isDynamicBlockPart =
        containsSubstring(recordName, "PARAMETER") ||
  ...
  static GraphReplayFamily
  graphReplayFamilyFromNames(const std::string &recordName,
                             const std::string &className) {
    if (recordName == "DIMASSOC" || recordName == "ACDBDIMASSOC" ||
  ...
  static GraphReplayFamily
  graphReplayFamilyFromRawObject(const RawObjectRecord &record) {
  ...
  static AdvancedEntityWriterFamily
  advancedEntityWriterFamilyFromNames(const std::string &recordName,
                                      const std::string &className) {
    ...
    if (recordName == "OLEFRAME" || className == "AcDbOleFrame")
      return AdvancedEntityWriterFamily::OleFrame;
    ...
    if (recordName == "PDFUNDERLAY" || recordName == "DGNUNDERLAY" ||
        ...
        className == "AcDbPdfReference" || className == "AcDbDgnReference" ||
  ...
  static const char *
  advancedEntityWriterFamilyName(AdvancedEntityWriterFamily family) {
    switch (family) {
    ...
    case AdvancedEntityWriterFamily::Ole2Frame:
      return "OLE2FRAME";
    case AdvancedEntityWriterFamily::OleFrame:
      return "OLEFRAME";
  ...
  static AssociativeKind
  associativeKindFromRecordName(const std::string &recordName) {
  ...
  static const char *
  associativePrefixParseStatusName(AssociativePrefixParseStatus status) {
    switch (status) {
  ...
  static size_t rawObjectFamilyCount(const RawObjectFamilyCounts &counts,
                                     RawObjectFamily family) {
    switch (family) {
    case RawObjectFamily::Associative:
  ...
  static size_t graphReplayFamilyCount(const GraphReplayFamilyCounts &counts,
                                       GraphReplayFamily family) {
    switch (family) {
    case GraphReplayFamily::DimensionAssociation:
  ...
  static size_t associativeShellKindCount(const AssociativeShellCounts &counts,
                                          AssociativeKind kind) {
    switch (kind) {
    case AssociativeKind::Network:
  ...
  size_t semanticOnlyRecordCount() const {
    return m_lights.size() + m_cameras.size() + m_tvDeviceProperties.size() +
           m_viewportEntityHeaders.size() + m_vxControls.size() +
           m_vxTableRecords.size() + m_suns.size() + m_modelerGeometry.size() +
           m_backgrounds.size() + m_materials.size() + m_renderSettings.size() +
           m_sunStudies.size() + m_motionPaths.size() + m_curvePaths.size() +
           m_pointPaths.size() + m_dbColors.size() + m_objectPtrs.size() +
           m_partialViewingIndexes.size() + m_dimensionAssociations.size() +
           m_evaluationGraphs.size() + m_sections.size() +
           m_sectionObjects.size() + m_tables.size() + m_cellStyleMaps.size() +
           ...
           m_breakPointRefs.size() + m_groups.size() + m_rasterImages.size() +
           m_imageDefinitions.size() + m_underlays.size() +
           m_underlayDefinitions.size() + m_pointCloudDefinitions.size() +
           m_navisworksModelDefinitions.size() + m_navisworksModels.size() +
           m_pointCloudColorMaps.size() + m_rasterVariables.size() +
           m_shapes.size() + m_ole2Frames.size() + m_oleFrames.size() +
  ...
  bool hasReplayableAdvancedObjects() const {
    return hasReplayable(m_rawObjects) || hasReplayable(m_lights) ||
           hasReplayable(m_cameras) || hasReplayable(m_tvDeviceProperties) ||
           hasReplayable(m_viewportEntityHeaders) ||
           hasReplayable(m_vxControls) || hasReplayable(m_vxTableRecords) ||
           hasReplayable(m_suns) || hasReplayable(m_backgrounds) ||
           hasReplayable(m_materials) || hasReplayable(m_renderSettings) ||
           hasReplayable(m_sunStudies) || hasReplayable(m_motionPaths) ||
           hasReplayable(m_curvePaths) || hasReplayable(m_pointPaths) ||
           hasReplayable(m_objectPtrs) ||
           hasReplayable(m_partialViewingIndexes) ||
           hasReplayable(m_dbColors) ||
           hasReplayable(m_dimensionAssociations) ||
           hasReplayable(m_evaluationGraphs) || hasReplayable(m_sections) ||
           hasReplayable(m_sectionObjects) ||
           hasReplayable(m_modelerGeometry) || hasReplayable(m_tables) ||
           hasReplayable(m_cellStyleMaps) ||
           hasReplayable(m_associativeObjects) ||
           hasReplayable(m_acshObjects) || hasReplayable(m_mleaders) ||
           hasReplayable(m_mleaderStyles) ||
           hasReplayable(m_detailViewStyles) ||
           hasReplayable(m_sectionViewStyles) || hasReplayable(m_breakData) ||
           hasReplayable(m_breakPointRefs) || hasReplayable(m_groups) ||
           hasReplayable(m_rasterImages) || hasReplayable(m_imageDefinitions) ||
           hasReplayable(m_underlays) || hasReplayable(m_underlayDefinitions) ||
           hasReplayable(m_pointCloudDefinitions) ||
           hasReplayable(m_navisworksModelDefinitions) ||
           hasReplayable(m_navisworksModels) ||
           hasReplayable(m_pointCloudColorMaps) ||
           hasReplayable(m_rasterVariables) || hasReplayable(m_shapes) ||
           hasReplayable(m_ole2Frames) || hasReplayable(m_oleFrames) ||
           hasReplayable(m_imageDefinitionReactors) ||
           hasReplayable(m_spatialFilters) || hasReplayable(m_geoData) ||
           hasReplayable(m_tableGeometry) || hasReplayable(m_placeholders);
  }
  ...
  void invalidateByOwner(std::uint32_t ownerHandle) {
    invalidateMatching(
        [ownerHandle](std::uint32_t, std::uint32_t parentHandle) {
  ...
  void invalidateVxGraphForHandle(std::uint32_t dependentHandle) {
    if (dependentHandle == 0)
      return;
    for (VxControlRecord &record : m_vxControls) {
      if (record.replayState != ReplayState::ReplayAllowed)
        continue;
      if (!vxControlRecordReferences(record, dependentHandle))
        continue;
      record.replayState = ReplayState::ReplayInvalidated;
      invalidateRawVxControlObject(record.handle);
    }
    for (VxTableRecordRecord &record : m_vxTableRecords) {
      if (record.replayState != ReplayState::ReplayAllowed)
        continue;
      if (!vxTableRecordReferences(record, dependentHandle))
        continue;
      record.replayState = ReplayState::ReplayInvalidated;
      invalidateRawVxTableRecordObject(record.handle);
    }
  }
  ...
  void invalidateViewGraphForHandle(std::uint32_t dependentHandle) {
    ...
    for (ViewRecord &record : m_views) {
      ...
      if (viewRecordReferences(record, dependentHandle)) {
        record.replayState = ReplayState::ReplayInvalidated;
        invalidateDocumentMappingForSource(DocumentMappingSource::View,
                                           record.handle);
    ...
    for (VportRecord &record : m_vports) {
      ...
      if (vportRecordReferences(record, dependentHandle)) {
        record.replayState = ReplayState::ReplayInvalidated;
        invalidateDocumentMappingForSource(DocumentMappingSource::Vport,
                                           record.handle);
        invalidateRawVisualMetadataObject(record.handle);
      }
    }
    for (ViewportEntityHeaderRecord &record : m_viewportEntityHeaders) {
      if (record.replayState != ReplayState::ReplayAllowed)
        continue;
      if (viewportEntityHeaderRecordReferences(record, dependentHandle)) {
        record.replayState = ReplayState::ReplayInvalidated;
        invalidateRawVisualMetadataObject(record.handle);
      }
    }
  ...
  void invalidateAllReplayable() {
    ...
    invalidateContainer(m_cameras);
    invalidateContainer(m_lightLists);
    invalidateContainer(m_geoPositionMarkers);
    invalidateContainer(m_sectionObjects);
    invalidateContainer(m_tvDeviceProperties);
    invalidateContainer(m_viewportEntityHeaders);
    invalidateContainer(m_vxControls);
    invalidateContainer(m_vxTableRecords);
    invalidateContainer(m_suns);
    invalidateContainer(m_backgrounds);
    invalidateContainer(m_materials);
    invalidateContainer(m_renderSettings);
    invalidateContainer(m_sunStudies);
    invalidateContainer(m_motionPaths);
    invalidateContainer(m_curvePaths);
    invalidateContainer(m_pointPaths);
    invalidateContainer(m_objectPtrs);
    invalidateContainer(m_partialViewingIndexes);
    invalidateContainer(m_dbColors);
    invalidateContainer(m_dimensionAssociations);
    invalidateContainer(m_evaluationGraphs);
    invalidateContainer(m_sections);
    ...
    invalidateContainer(m_pointCloudDefinitions);
    invalidateContainer(m_navisworksModelDefinitions);
    invalidateContainer(m_navisworksModels);
    invalidateContainer(m_pointCloudColorMaps);
    invalidateContainer(m_rasterVariables);
    invalidateContainer(m_shapes);
    invalidateContainer(m_ole2Frames);
    invalidateContainer(m_oleFrames);
...
private:
  ...
  static bool isTableStyleRecord(const TableRecord &record) {
    return record.tableStyleHandle == 0 && record.recordName != "ACAD_TABLE" &&
           record.recordName != "TABLECONTENT" && record.semanticParsed &&
           record.styleResolved;
  }

  static void
  collectTableStyleBorderHandle(TableRecord &record,
  ...
  static void
  collectTableStyleBorderSummary(TableRecord &record,
                                 const DRW_TableStyleBorder &border) {
  ...
  static void
  collectTableStyleContentSummary(TableRecord &record,
                                  const DRW_TableStyleContentFormat &format) {
  ...
  static void
  collectTableStyleCellHandles(TableRecord &record,
  ...
  static void
  collectTableStyleCellSummary(TableRecord &record,
                               const DRW_TableStyleCellStyle &cellStyle) {
    if (cellStyle.m_id != 0)
    ...
    if (cellStyle.m_verticalMargin != 0.0 ||
        ...
        cellStyle.m_bottomMargin != 0.0 || cellStyle.m_rightMargin != 0.0 ||
  ...
  static void
  collectTableStyleRowHandles(TableRecord &record,
  ...
  static void
  collectTableStyleRowSummary(TableRecord &record,
                              const DRW_TableStyleRowStyle &rowStyle) {
  ...
  static void
  collectCellStyleMapSummary(CellStyleMapRecord &record,
                             const DRW_TableStyleCellStyle &cellStyle) {
    if (cellStyle.m_id != 0)
  ...
  void populateTableNativeWriterEligibility(
    ...
    if (eligibility.storageMode == TableContentStorageMode::Unsupported ||
        eligibility.storageMode == TableContentStorageMode::LegacyDirectTable) {
      addTableNativeWriterBlocker(
          eligibility, TableNativeWriterBlocker::UnsupportedTableVersion);
    }
    if (version == DRW::AC1021) {
      addTableNativeWriterBlocker(
          eligibility, TableNativeWriterBlocker::AmbiguousTableContentStorage);
    }
    ...
    if (record.parentHandle == 0) {
      addTableNativeWriterBlocker(eligibility,
                                  TableNativeWriterBlocker::MissingOwnerHandle);
    }
    if (record.fallbackRendered &&
        findTableFallbackEntities(record.handle).empty()) {
      addTableNativeWriterBlocker(
          eligibility, TableNativeWriterBlocker::MissingFallbackAttachment);
    }
    if (record.fallbackInvalidated) {
      addTableNativeWriterBlocker(eligibility,
                                  TableNativeWriterBlocker::EditedFallback);
    }
    if (record.tableStyleHandle != 0 && !record.styleResolved) {
      addTableNativeWriterBlocker(
    ...
    if (tableHasUnresolvedCellStyleMap(record)) {
      addTableNativeWriterBlocker(
          eligibility, TableNativeWriterBlocker::UnresolvedCellStyleMap);
    }
    if (record.m_unknownRangeCount != 0) {
      addTableNativeWriterBlocker(
          eligibility, TableNativeWriterBlocker::UnknownSubrecordRange);
    }
    if (record.m_incompleteRangeCount != 0) {
      addTableNativeWriterBlocker(
          eligibility, TableNativeWriterBlocker::IncompleteSubrecordRange);
    }
    if (record.m_overrideMaskCount != 0 || record.overrideCellCount != 0) {
      addTableNativeWriterBlocker(eligibility,
                                  TableNativeWriterBlocker::OverrideMask);
    }
    if (record.m_breakRangeCount != 0) {
      addTableNativeWriterBlocker(eligibility,
                                  TableNativeWriterBlocker::BreakData);
    }
    if (record.m_tableGeometryTailRangeCount != 0 ||
        record.geometryCellCount != 0) {
      addTableNativeWriterBlocker(eligibility,
                                  TableNativeWriterBlocker::GeometryTail);
    }
    if (record.mergedRangeCount != 0 || tableHasMergedCell(record)) {
      addTableNativeWriterBlocker(eligibility,
                                  TableNativeWriterBlocker::MergedCell);
    }
    if (record.fieldContentCount != 0 || record.fieldHandleCount != 0) {
      addTableNativeWriterBlocker(eligibility,
                                  TableNativeWriterBlocker::FieldContent);
    }
    if (record.blockContentCount != 0 || record.blockHandleCount != 0 ||
        record.hasBlockContent) {
      addTableNativeWriterBlocker(eligibility,
                                  TableNativeWriterBlocker::BlockContent);
    }
    if (record.attributeCount != 0 || record.attributeHandleCount != 0) {
      addTableNativeWriterBlocker(eligibility,
                                  TableNativeWriterBlocker::AttributeContent);
    }
    if (record.unknownContentCount != 0) {
      addTableNativeWriterBlocker(eligibility,
                                  TableNativeWriterBlocker::UnknownCellContent);
    }
    if (record.valueHandleCount != 0) {
      addTableNativeWriterBlocker(
          eligibility, TableNativeWriterBlocker::IncompleteValuePayload);
    }
    if (record.fallbackRendered) {
      addTableNativeWriterBlocker(
          eligibility,
    ...
    if (record.lineTypeHandleCount != 0) {
      addTableNativeWriterBlocker(eligibility,
                                  TableNativeWriterBlocker::UnresolvedLineType);
    }
    if (record.replayState == ReplayState::ReplayInvalidated) {
      addTableNativeWriterBlocker(
    ...
    if (record.replayState == ReplayState::ReplayReplaced) {
      addTableNativeWriterBlocker(eligibility,
                                  TableNativeWriterBlocker::RawReplayReplaced);
    }
    eligibility.eligibleTextOnly = eligibility.blockers.empty();
  }

  static TableContentStorageMode
  tableContentStorageModeForVersion(DRW::Version version) {
  ...
  static void
  addTableNativeWriterBlocker(TableNativeWriterEligibility &eligibility,
  ...
  static void
  incrementTableStorageModeCount(TableNativeWriterBlockerCounts &counts,
  ...
  static void
  incrementTableNativeWriterBlockerCount(TableNativeWriterBlockerCounts &counts,
                                         TableNativeWriterBlocker blocker) {
    switch (blocker) {
    case TableNativeWriterBlocker::NoSemanticTableContent:
  ...
  static bool tableCellReferences(const TableCellRecord &cell,
                                  std::uint32_t handle) {
    if (handle == 0)
      return false;
    if (cell.valueHandle == handle || cell.textStyleHandle == handle ||
        cell.textStyleOverrideHandle == handle || cell.blockHandle == handle ||
  ...
  static bool tableRecordReferences(const TableRecord &record,
                                    std::uint32_t handle) {
    if (handle == 0)
      return false;
    if (record.tableStyleHandle == handle)
      return true;
    if (record.m_tableUnknownHandle == handle)
      return true;
  ...
  static bool mleaderRecordReferences(const MLeaderRecord &record,
                                      std::uint32_t handle) {
    if (handle == 0)
      return false;
    if (record.styleHandle == handle || record.leaderLineTypeHandle == handle ||
        record.arrowHeadHandle == handle || record.textStyleHandle == handle ||
    ...
    for (std::uint32_t attributeHandle :
         record.blockAttributeDefinitionHandles) {
      if (attributeHandle == handle)
        return true;
    }
    return false;
...
private:
  VisualMetadataSummaryRecord
  visualSummaryFromView(const ViewRecord &record) const {
  ...
  VisualMetadataSummaryRecord
  visualSummaryFromVport(const VportRecord &record) const {
    VisualMetadataSummaryRecord summary;
  ...
  static VisualMetadataSummaryRecord
  visualSummaryFromVisualStyle(const VisualStyleRecord &record) {
    VisualMetadataSummaryRecord summary;
    summary.sourceType = VisualMetadataSource::VisualStyle;
    summary.specCoverage = record.bodyDecoded
                               ? VisualMetadataSpecCoverage::OdaCovered
                               : VisualMetadataSpecCoverage::RawOnly;
    summary.displayName =
        !record.name.empty() ? record.name : record.description;
    summary.handle = record.handle;
    summary.ownerHandle = record.parentHandle;
  ...
  static VisualMetadataSummaryRecord
  visualSummaryFromLight(const LightRecord &record) {
    VisualMetadataSummaryRecord summary;
  ...
  static VisualMetadataSummaryRecord
  visualSummaryFromSun(const SunRecord &record) {
    VisualMetadataSummaryRecord summary;
  ...
  size_t visualSummaryUnresolvedReferenceCount(
    ...
    if (record.referencedVisualStyleHandle != 0 &&
        findVisualStyleByHandle(record.referencedVisualStyleHandle) ==
            nullptr) {
      ++count;
    }
  ...
  static void
  incrementVisualMetadataSourceCount(VisualMetadataSummaryCounts &counts,
                                     VisualMetadataSource source) {
    switch (source) {
  ...
  void
  accumulateVisualReferenceBlockers(VisualMetadataWriterBlockerCounts &counts,
  ...
  static bool
  visualMetadataNativeWriterAvailable(VisualMetadataSource sourceType,
                                      DRW::Version version) {
    if (version == DRW::UNKNOWNV)
      return false;
    switch (sourceType) {
  ...
  DocumentMappingRecord documentMappingFromView(const ViewRecord &record,
                                                int documentItemIndex) const {
  ...
  DocumentMappingRecord documentMappingFromUcs(const UcsRecord &record,
                                               int documentItemIndex) const {
    DocumentMappingRecord mapping;
    mapping.sourceHandle = record.handle;
    mapping.handle = record.handle;
  ...
  DocumentMappingRecord documentMappingFromVport(const VportRecord &record,
                                                 int documentItemIndex) const {
    DocumentMappingRecord mapping;
    mapping.sourceHandle = record.handle;
    mapping.handle = record.handle;
  ...
  void invalidateDocumentMappingForSource(DocumentMappingSource sourceType,
                                          std::uint32_t handle) {
    if (handle == 0)
      return;
    for (DocumentMappingRecord &record : m_documentMappings) {
  ...
  size_t documentMappingUnresolvedReferenceCount(
    ...
    appendUnresolvedUcsReference(unresolvedHandles, record.associatedUcsHandle);
    ...
    appendUnresolvedOpaqueReference(unresolvedHandles,
                                    record.visualStyleHandle);
    appendUnresolvedSunReference(unresolvedHandles, record.sunHandle);
    appendUnresolvedOpaqueReference(unresolvedHandles,
                                    record.liveSectionHandle);
    appendUnresolvedOpaqueReference(unresolvedHandles,
                                    record.viewportHeaderHandle);
    appendUnresolvedOpaqueReference(unresolvedHandles,
                                    record.layoutBlockRecordHandle);
  ...
  void
  appendUnresolvedUcsReference(std::vector<std::uint32_t> &unresolvedHandles,
  ...
  void
  appendUnresolvedSunReference(std::vector<std::uint32_t> &unresolvedHandles,
                               std::uint32_t handle) const {
  ...
  static void
  appendUnresolvedOpaqueReference(std::vector<std::uint32_t> &unresolvedHandles,
                                  std::uint32_t handle) {
    if (handle != 0)
      unresolvedHandles.push_back(handle);
  }

  static bool containsValue(const std::vector<std::uint32_t> &values,
                            std::uint32_t value, size_t limit) {
  ...
  static bool viewRecordReferences(const ViewRecord &record,
                                   std::uint32_t handle) {
    if (handle == 0)
      return false;
    return record.namedUcsHandle == handle || record.baseUcsHandle == handle ||
           record.backgroundHandle == handle ||
           record.visualStyleHandle == handle || record.sunHandle == handle ||
           record.liveSectionHandle == handle;
  }

  static bool vportRecordReferences(const VportRecord &record,
                                    std::uint32_t handle) {
    if (handle == 0)
      return false;
    return record.namedUcsHandle == handle || record.baseUcsHandle == handle ||
  ...
  static bool
  viewportEntityHeaderRecordReferences(const ViewportEntityHeaderRecord &record,
                                       std::uint32_t handle) {
    if (handle == 0)
      return false;
    if (record.parentHandle == handle || record.xrefBlockHandle == handle ||
        record.viewportEntityHandle == handle ||
        record.previousViewportEntityHeaderHandle == handle ||
        record.xDictHandle == handle) {
      return true;
    }
    return std::find(record.reactorHandles.cbegin(),
                     record.reactorHandles.cend(),
                     handle) != record.reactorHandles.cend();
  }

  static bool vxControlRecordReferences(const VxControlRecord &record,
                                        std::uint32_t handle) {
    if (handle == 0)
      return false;
    if (record.parentHandle == handle || record.xDictHandle == handle)
      return true;
    if (std::find(record.reactorHandles.cbegin(), record.reactorHandles.cend(),
                  handle) != record.reactorHandles.cend()) {
      return true;
    }
    return std::find(record.recordHandles.cbegin(), record.recordHandles.cend(),
                     handle) != record.recordHandles.cend();
  }

  static bool vxTableRecordReferences(const VxTableRecordRecord &record,
                                      std::uint32_t handle) {
    if (handle == 0)
      return false;
    if (record.parentHandle == handle || record.xDictHandle == handle)
  ...
  static bool mleaderStyleRecordReferences(const MLeaderStyleRecord &record,
    ...
    return record.leaderLineTypeHandle == handle ||
           ...
           record.textStyleHandle == handle || record.blockHandle == handle;
  }
  ...
  static bool associativeRecordReferences(const AssociativeRecord &record,
    ...
    if (record.owningNetworkHandle == handle ||
        ...
        record.rNodeHandle == handle || record.dNodeHandle == handle) {
      return true;
    }
    ...
    for (std::uint32_t refHandle : record.persistentSubentityHandles) {
      if (refHandle == handle)
        return true;
    }
    return false;
  ...
  static bool isAssociativeRawObject(const RawObjectRecord &record) {
    return record.family == RawObjectFamily::Associative ||
           associativeKindFromRecordName(record.recordName) !=
  ...
  static bool isVisualMetadataRawObject(const RawObjectRecord &record) {
    return record.recordName == "VIEW" || record.recordName == "VPORT" ||
           record.recordName == "UCS" || record.recordName == "VISUALSTYLE" ||
           record.recordName == "SUN" || record.recordName == "LIGHT" ||
           record.recordName == "CAMERA" ||
           record.recordName == "TVDEVICEPROPERTIES" ||
           ...
           record.className == "AcDbSun" || record.className == "AcDbLight" ||
           record.className == "AcDbCamera" ||
           record.className == "AcDbTvDeviceProperties" ||
           record.objectType == DRW_ViewportEntityHeader::kDwgType;
  }
  ...
  static bool isVxControlRawObject(const RawObjectRecord &record) {
    return record.objectType == DRW_VxControl::kDwgClassNum ||
           record.recordName == "VXCONTROL" ||
           record.recordName == "VX_CONTROL" ||
           record.className == "AcDbVxControl";
  }

  static bool isVxTableRecordRawObject(const RawObjectRecord &record) {
    return record.objectType == DRW_VxTableRecord::kDwgClassNum ||
           record.recordName == "VXTABLERECORD" ||
           record.recordName == "VX_TABLE_RECORD" ||
           record.className == "AcDbVxTableRecord";
  }

  void invalidateRawVxControlObject(std::uint32_t handle) {
    if (handle == 0)
      return;
    for (RawObjectRecord &record : m_rawObjects) {
      if (record.replayState == ReplayState::ReplayAllowed &&
          record.handle == handle && isVxControlRawObject(record)) {
        record.replayState = ReplayState::ReplayInvalidated;
      }
    }
  }

  void invalidateRawVxTableRecordObject(std::uint32_t handle) {
    if (handle == 0)
      return;
    for (RawObjectRecord &record : m_rawObjects) {
      if (record.replayState == ReplayState::ReplayAllowed &&
          record.handle == handle && isVxTableRecordRawObject(record)) {
        record.replayState = ReplayState::ReplayInvalidated;
      }
    }
  }
  ...
  // MLEADERSTYLE names are dictionary keys, not fields in the DWG object
  // body. Resolve them after either side arrives because OBJECTS traversal
  // order is not a name-resolution contract.
  void resolveDictionaryMLeaderStyleNames() {
    std::map<std::uint32_t, std::string> nameByHandle;
    for (const DictionaryRecord &dictionary : m_dictionaries) {
      for (const DictionaryEntryRecord &entry : dictionary.entries) {
        if (!entry.name.empty())
          nameByHandle.emplace(entry.handle, entry.name);
      }
    }
    if (nameByHandle.empty())
      return;

    for (MLeaderStyleRecord &style : m_mleaderStyles) {
      if (!style.name.empty())
        continue;
      const auto it = nameByHandle.find(style.handle);
      if (it != nameByHandle.end())
  ...
  static AssociativePrefixKind
  associativePrefixKindFromDrw(DRW_AssociativePrefixStatus::Kind kind) {
    switch (kind) {
  ...
  static AssociativePrefixStatusRecord
  makeAssociativePrefixStatus(const DRW_AssociativePrefixStatus &source) {
  ...
  void appendAssociativeEdges(const AssociativeRecord &record) {
    ...
    addAssociativeEdge(record.handle, record.recordName, record.kind,
                       AssociativeEdgeKind::ActionBody, record.actionBodyHandle,
                       AssociativeEdgeConfidence::ExplicitHandle);
    for (const DRW_AssociativeHandleRef &ref : record.dependencyRefs) {
      addAssociativeEdge(record.handle, record.recordName, record.kind,
    ...
    for (std::uint32_t handle : record.persistentSubentityHandles) {
      addAssociativeEdge(record.handle, record.recordName, record.kind,
                         AssociativeEdgeKind::UnknownHandleReference, handle,
                         AssociativeEdgeConfidence::ExplicitHandle);
    }
    addAssociativeEdge(record.handle, record.recordName, record.kind,
                       AssociativeEdgeKind::DependsOn, record.dependencyHandle,
                       AssociativeEdgeConfidence::ExplicitHandle);
    addAssociativeEdge(record.handle, record.recordName, record.kind,
  ...
  void appendAssociativeEdges(const AcShRecord &record) {
    addAssociativeEdge(record.handle, record.recordName,
                       AssociativeKind::Unknown,
                       AssociativeEdgeKind::HistoryNode, record.ownerHandle,
                       AssociativeEdgeConfidence::InferredFromClassLayout);
  }

  void addAssociativeEdge(std::uint32_t sourceHandle,
                          const std::string &sourceRecordName,
                          AssociativeKind sourceKind,
    ...
    for (const AssociativeEdgeRecord &edge : m_associativeEdges) {
      if (edge.sourceHandle == sourceHandle &&
          edge.targetHandle == targetHandle && edge.edgeKind == edgeKind) {
        return;
      }
    }
  ...
  static void incrementAssociativeEdgeKindCount(AssociativeEdgeCounts &counts,
                                                AssociativeEdgeKind kind) {
    switch (kind) {
  ...
  static void
  incrementModelerPayloadRangeKindCount(ModelerPayloadCounts &counts,
                                        ModelerPayloadRangeKind kind) {
    switch (kind) {
    case ModelerPayloadRangeKind::Sat:
      ++counts.satRanges;
  ...
  static void incrementRawObjectFamilyCount(RawObjectFamilyCounts &counts,
                                            RawObjectFamily family) {
    switch (family) {
    case RawObjectFamily::Associative:
      ++counts.associative;
  ...
  static AdvancedEntityWriterReadiness
  advancedEntityWriterReadinessFromRawObject(const RawObjectRecord &rawRecord,
                                             DRW::Version version) {
    AdvancedEntityWriterReadiness record;
    record.family = advancedEntityWriterFamilyFromNames(rawRecord.recordName,
                                                        rawRecord.className);
    ...
    record.fallbackAvailable = advancedEntityFallbackAvailable(record.family);
    record.editedFallbackInvalidated =
        ...
        rawRecord.isEntity;
    record.missingRequiredMetadata = rawRecord.isCustomClass &&
                                     rawRecord.recordName.empty() &&
    ...
    record.unsupportedAdvancedContent =
        ...
        !record.nativeWriterAvailable && !record.rawReplayAvailable;
    return record;
  }

  static AdvancedEntityWriterReadiness
  advancedEntityWriterReadinessFromMLeader(const MLeaderRecord &mleader,
                                           DRW::Version version) {
    AdvancedEntityWriterReadiness record;
    ...
    record.unsupportedAdvancedContent = mleader.effectiveContentType == 1 ||
                                        mleader.effectiveContentType == 3 ||
                                        mleader.hasBlockContent ||
                                        mleader.overrideFlags != 0;
    return record;
  }

  static AdvancedEntityWriterReadiness
  advancedEntityWriterReadinessFromMesh(const MeshRecord &mesh,
                                        DRW::Version version) {
    AdvancedEntityWriterReadiness record;
    ...
    record.unsupportedAdvancedContent =
        mesh.isSubDMesh || mesh.subdivisionLevel > 0 || mesh.edgeCount > 0 ||
        mesh.creaseCount > 0 ||
  ...
  static AdvancedEntityWriterReadiness
  advancedEntityWriterReadinessFromShape(const ShapeRecord &shape,
                                         DRW::Version version) {
    AdvancedEntityWriterReadiness record;
  ...
  static AdvancedEntityWriterReadiness
  advancedEntityWriterReadinessFromOle2Frame(const Ole2FrameRecord &ole2Frame,
                                             DRW::Version version) {
    AdvancedEntityWriterReadiness record;
    ...
    record.missingRequiredMetadata = !ole2Frame.payloadPresent ||
                                     ole2Frame.payloadTruncated ||
  ...
  static AdvancedEntityWriterReadiness
  advancedEntityWriterReadinessFromOleFrame(const OleFrameRecord &oleFrame,
                                            DRW::Version version) {
    AdvancedEntityWriterReadiness record;
    record.family = AdvancedEntityWriterFamily::OleFrame;
    record.odaCoverage = AdvancedEntityWriterOdaCoverage::Complete;
    record.handle = oleFrame.handle;
    record.recordName = "OLEFRAME";
    record.className = "AcDbOleFrame";
    record.nativeWriterAvailable =
        advancedEntityNativeWriterAvailable(record.family, version);
    record.rawReplayAvailable = false;
    record.fallbackAvailable = false;
    record.editedFallbackInvalidated =
        oleFrame.replayState != ReplayState::ReplayAllowed;
    record.missingRequiredMetadata = !oleFrame.payloadPresent ||
                                     oleFrame.payloadTruncated ||
                                     oleFrame.payloadTooLarge;
    record.missingPayloadBytes = oleFrame.rawByteCount == 0;
    record.unsupportedAdvancedContent = !record.nativeWriterAvailable;
    return record;
  }

  static bool
  advancedEntityNativeWriterAvailable(AdvancedEntityWriterFamily family,
                                      DRW::Version version) {
    if (version == DRW::UNKNOWNV)
      return false;
    switch (family) {
  ...
  static bool
  advancedEntityFallbackAvailable(AdvancedEntityWriterFamily family) {
    switch (family) {
    case AdvancedEntityWriterFamily::Mesh:
    case AdvancedEntityWriterFamily::RasterImage:
    case AdvancedEntityWriterFamily::Wipeout:
  ...
  static AdvancedEntityWriterOdaCoverage
  advancedEntityWriterOdaCoverage(AdvancedEntityWriterFamily family) {
    switch (family) {
    case AdvancedEntityWriterFamily::Shape:
    case AdvancedEntityWriterFamily::Ole2Frame:
    case AdvancedEntityWriterFamily::OleFrame:
  ...
  static void incrementGraphReplayFamilyCount(GraphReplayFamilyCounts &counts,
                                              GraphReplayFamily family) {
    switch (family) {
    case GraphReplayFamily::DimensionAssociation:
  ...
  static void incrementGraphReplayBlockerCounts(GraphReplayPolicyCounts &counts,
  ...
  bool hasRawGraphObjectForHandle(std::uint32_t handle,
                                  GraphReplayFamily family) const {
    if (handle == 0)
      return false;
    for (const RawObjectRecord &record : m_rawObjects) {
      if (record.handle == handle &&
  ...
  static MeshRawRangeStatus meshRawRangeStatus(int vertexCount, int faceCount,
                                               size_t preservedVertexCount) {
  ...
  static bool meshHasMalformedCounts(const MeshRecord &record) {
    if (record.vertexCount <= 0 || record.faceCount <= 0)
      return true;
    const size_t expectedVertexCount = static_cast<size_t>(record.vertexCount) *
  ...
  static ExternalReferencePathDiagnostic
  externalReferencePathDiagnostic(const std::string &path) {
  ...
  static ClipBoundaryStatus clipBoundaryStatus(int clip, size_t vertexCount,
                                               bool isWipeout) {
    if (clip == 0 && !isWipeout)
  ...
  static void incrementClipBoundaryCounts(ExternalReferenceCounts &counts,
                                          ClipBoundaryStatus status) {
    switch (status) {
  ...
  static void
  incrementPathStatusCounts(ExternalReferenceCounts &counts,
  ...
  template <typename Predicate> void invalidateMatching(Predicate predicate) {
    ...
    invalidateMatching(m_cameras, predicate);
    invalidateMatching(m_lightLists, predicate);
    invalidateMatching(m_geoPositionMarkers, predicate);
    invalidateMatching(m_sectionObjects, predicate);
    invalidateMatching(m_tvDeviceProperties, predicate);
    invalidateMatching(m_viewportEntityHeaders, predicate);
    invalidateMatching(m_vxControls, predicate);
    invalidateMatching(m_vxTableRecords, predicate);
    invalidateMatching(m_suns, predicate);
    invalidateMatching(m_backgrounds, predicate);
    invalidateMatching(m_materials, predicate);
    invalidateMatching(m_renderSettings, predicate);
    invalidateMatching(m_sunStudies, predicate);
    invalidateMatching(m_motionPaths, predicate);
    invalidateMatching(m_curvePaths, predicate);
    invalidateMatching(m_pointPaths, predicate);
    invalidateMatching(m_objectPtrs, predicate);
    invalidateMatching(m_partialViewingIndexes, predicate);
    invalidateMatching(m_dbColors, predicate);
    invalidateMatching(m_dimensionAssociations, predicate);
    invalidateMatching(m_evaluationGraphs, predicate);
    invalidateMatching(m_sections, predicate);
    ...
    invalidateMatching(m_pointCloudDefinitions, predicate);
    invalidateMatching(m_navisworksModelDefinitions, predicate);
    invalidateMatching(m_navisworksModels, predicate);
    invalidateMatching(m_pointCloudColorMaps, predicate);
    ...
    invalidateMatching(m_oleFrames, predicate);
  ...
  static void incrementAssociativeKindCount(AssociativeShellCounts &counts,
                                            AssociativeKind kind) {
    switch (kind) {
    case AssociativeKind::Network:
      ++counts.network;
  ...
  static bool sameDwgFrameIdentity(const DRW_DwgFramePublication &lhs,
                                   const DRW_DwgFramePublication &rhs) {
    return lhs.m_version == rhs.m_version && lhs.m_handle == rhs.m_handle &&
           lhs.m_sourceOffset == rhs.m_sourceOffset &&
           lhs.m_sourceMapOrdinal == rhs.m_sourceMapOrdinal &&
           lhs.m_sourceOffsetSpace == rhs.m_sourceOffsetSpace &&
           lhs.m_hasSourceLocation == rhs.m_hasSourceLocation &&
           lhs.m_encodedType == rhs.m_encodedType &&
           lhs.m_resolvedType == rhs.m_resolvedType &&
           lhs.m_isEntity == rhs.m_isEntity &&
           lhs.m_isCustomClass == rhs.m_isCustomClass &&
           lhs.m_recordName == rhs.m_recordName &&
           lhs.m_className == rhs.m_className &&
           lhs.m_classStreamOrdinal == rhs.m_classStreamOrdinal &&
           lhs.m_commonHandleDataValidated == rhs.m_commonHandleDataValidated &&
           lhs.m_commonLinkEvidence == rhs.m_commonLinkEvidence &&
           lhs.m_parentHandle == rhs.m_parentHandle &&
           lhs.m_reactorHandles == rhs.m_reactorHandles &&
           lhs.m_xDictHandle == rhs.m_xDictHandle &&
           lhs.m_numReactors == rhs.m_numReactors &&
           lhs.m_xDictFlag == rhs.m_xDictFlag &&
           lhs.m_controlHandles == rhs.m_controlHandles;
  }

  static bool canMergeDwgFrameCarriers(DRW_DwgFramePublication::Carrier lhs,
                                       DRW_DwgFramePublication::Carrier rhs) {
    using Carrier = DRW_DwgFramePublication::Carrier;
    if (lhs == Carrier::Control || rhs == Carrier::Control)
      return lhs == Carrier::Control && rhs == Carrier::Control;
    return true;
  }

  static DRW_DwgFramePublication::Carrier
  mergedDwgFrameCarrier(DRW_DwgFramePublication::Carrier lhs,
                        DRW_DwgFramePublication::Carrier rhs) {
    return lhs == rhs ? lhs : DRW_DwgFramePublication::Carrier::TypedAndRaw;
  }

  DRW::Version m_sourceDwgVersion = DRW::UNKNOWNV;
  std::vector<RawObjectRecord> m_rawObjects;
  std::vector<DRW_DwgFramePublication> m_dwgFramePublications;
  std::vector<DRW_DwgDictionaryMembership> m_dwgDictionaryMemberships;
  std::vector<DRW_DwgDictionaryWithDefaultMembership>
      m_dwgDictionaryWithDefaultMemberships;
  std::vector<DRW_DwgGroupMembership> m_dwgGroupMemberships;
  std::vector<DRW_DwgSortEntsMembership> m_dwgSortEntsMemberships;
  std::vector<DRW_DwgFieldListMembership> m_dwgFieldListMemberships;
  std::vector<DRW_DwgFieldPayloadReceipt> m_dwgFieldPayloadReceipts;
  std::vector<DRW_DwgTypedReference> m_dwgTypedReferences;
  std::vector<DRW_DwgBlockReachability> m_dwgBlockReachabilities;
  bool m_dwgFramePublicationCoverageComplete = false;
  std::vector<std::uint32_t> m_dwgFramePublicationConflicts;
  DRW_DwgFrameCoverageReport m_dwgFrameCoverageReport;
  ...
  std::map<std::uint32_t, std::string> m_lineTypeNames;
  std::map<std::uint32_t, std::string> m_textStyleNames;
  std::map<std::uint32_t, DRW_LType> m_lineTypes;
  std::map<std::uint32_t, DRW_Textstyle> m_textStyles;
  ...
  std::map<std::string, DRW_Vport> m_vportTableEntries;
  ...
  std::vector<CameraRecord> m_cameras;
  std::vector<LightListRecord> m_lightLists;
  std::vector<DataLinkRecord> m_dataLinks;
  std::vector<GeoMapImageRecord> m_geoMapImages;
  std::vector<LayerFilterRecord> m_layerFilters;
  std::vector<GeoPositionMarkerRecord> m_geoPositionMarkers;
  std::vector<SectionObjectRecord> m_sectionObjects;
  std::vector<TvDevicePropertiesRecord> m_tvDeviceProperties;
  std::vector<ViewportEntityHeaderRecord> m_viewportEntityHeaders;
  std::vector<VxControlRecord> m_vxControls;
  std::vector<VxTableRecordRecord> m_vxTableRecords;
  std::vector<SunRecord> m_suns;
  std::vector<BackgroundRecord> m_backgrounds;
  std::vector<MaterialRecord> m_materials;
  std::vector<RenderSettingsRecord> m_renderSettings;
  std::vector<SunStudyRecord> m_sunStudies;
  std::vector<MotionPathRecord> m_motionPaths;
  std::vector<CurvePathRecord> m_curvePaths;
  std::vector<PointPathRecord> m_pointPaths;
  std::vector<ObjectPtrRecord> m_objectPtrs;
  std::vector<PartialViewingIndexRecord> m_partialViewingIndexes;
  std::vector<DbColorRecord> m_dbColors;
  std::vector<DimensionAssociationRecord> m_dimensionAssociations;
  std::vector<EvaluationGraphRecord> m_evaluationGraphs;
  std::vector<BlockRepresentationRecord> m_blockRepresentationData;
  std::vector<SectionRecord> m_sections;
  std::vector<ModelerGeometryRecord> m_modelerGeometry;
  std::vector<SurfaceGeometryRecord> m_surfaceGeometry;
  ...
  std::vector<PointCloudDefinitionRecord> m_pointCloudDefinitions;
  std::vector<NavisworksModelDefinitionRecord> m_navisworksModelDefinitions;
  std::vector<NavisworksModelRecord> m_navisworksModels;
  std::vector<PointCloudColorMapRecord> m_pointCloudColorMaps;
  std::vector<RasterVariablesRecord> m_rasterVariables;
  std::vector<ShapeRecord> m_shapes;
  std::vector<Ole2FrameRecord> m_ole2Frames;
  std::vector<OleFrameRecord> m_oleFrames;
  ...
  std::vector<IndexRecord> m_indexes;
  ...
  std::vector<DRW_CsacDocumentOptions> m_csacDocumentOptions;
  std::vector<DRW_ContextDataManager> m_contextDataManagers;
  std::vector<DRW_ProxyEntity> m_proxyEntities;
  std::vector<DRW_VbaProject> m_vbaProjects;
  std::vector<DRW_PlotSettings> m_plotSettings;
  std::vector<DRW_ProxyObject> m_proxyObjects;
