...
std::ostream& operator <<(std::ostream& os, const RS_LineData& ld) {
    os << "RS_LINE: ((" << ld.startpoint << ")(" << ld.endpoint << "))";
    return os;
}
...
RS_Line::RS_Line(RS_EntityContainer* parent, const RS_Vector& pStart, const RS_Vector& pEnd)
    : LC_CachedLengthEntity(parent), m_data({pStart, pEnd}) {
    RS_Line::calculateBorders();
}
...
RS_Line::RS_Line(const RS_Vector& pStart, const RS_Vector& pEnd)
    : LC_CachedLengthEntity(nullptr), m_data({pStart, pEnd}) {
    RS_Line::calculateBorders();
}

RS_Entity* RS_Line::clone() const {
    const auto l = new RS_Line(*this);
    return l;
}

void RS_Line::calculateBorders() {
    m_minV = RS_Vector::minimum(m_data.startpoint, m_data.endpoint);
    m_maxV = RS_Vector::maximum(m_data.startpoint, m_data.endpoint);
    updateLength();
}
...
double RS_Line::getProjectionValueAlongLine(const RS_Vector& coord) const {
    const RS_Vector direction{m_data.endpoint - m_data.startpoint};

    const double directionMagnitude{direction.magnitude()};
    double v = 0.0;

    if (directionMagnitude > RS_TOLERANCE2) {
        const RS_Vector vpc{coord - m_data.startpoint};
        // find projection to the line
        v = RS_Vector::dotP(vpc, direction) / directionMagnitude;
    }
    return v;
}

RS_Vector RS_Line::doGetNearestPointOnEntity(const RS_Vector& coord, const bool onEntity, double* dist, RS_Entity** entity) const {
    if (entity != nullptr) {
        *entity = const_cast<RS_Line*>(this);
    }

    const RS_Vector direction{m_data.endpoint - m_data.startpoint};
    RS_Vector vpc{coord - m_data.startpoint};
...
RS_Vector RS_Line::doGetNearestMiddle(const RS_Vector& coord, double* dist, const int middlePoints) const {
    const RS_Vector dvp(getEndpoint() - getStartpoint());
    const double l = dvp.magnitude();
    if (l <= RS_TOLERANCE) {
        //line too short
        return this->getNearestCenter(coord, dist);
    }
    ...
    vp0 = getStartpoint() + dvp * (doubleI / counts);
...
RS_Vector RS_Line::doGetNearestDist(const double distance, const RS_Vector& coord, double* dist) const {
    const RS_Vector dv = RS_Vector::polar(distance, getAngle1());
    RS_Vector ret;
...
RS_Vector RS_Line::getNearestDistToEndpoint(const double distance, const bool startp) const {
    const double a1 = getAngle1();
    const RS_Vector dv = RS_Vector::polar(distance, a1);
    RS_Vector ret;

    if (startp) {
        ret = m_data.startpoint + dv;
    }
    else {
        ret = m_data.endpoint - dv;
    }
    return ret;
}
...
LC_Quadratic RS_Line::getQuadratic() const {
    ...
    const auto dvp = m_data.endpoint - m_data.startpoint;
    const RS_Vector normal(-dvp.y, dvp.x);
    ce[0] = normal.x;
    ce[1] = normal.y;
    ce[2] = -normal.dotP(m_data.endpoint);
...
double RS_Line::areaLineIntegral() const {
  ...
  return 0.5 * (m_data.endpoint.y - m_data.startpoint.y) * (m_data.startpoint.x + m_data.endpoint.x);
}

LC_FirstMoment RS_Line::firstMomentLineIntegral() const {
  const double x0 = m_data.startpoint.x;
  const double y0 = m_data.startpoint.y;
  const double x1 = m_data.endpoint.x;
  const double y1 = m_data.endpoint.y;
...
LC_SecondMoment RS_Line::secondMomentLineIntegral() const {
    const double x0 = m_data.startpoint.x, y0 = m_data.startpoint.y;
    const double x1 = m_data.endpoint.x,   y1 = m_data.endpoint.y;
    const double dy = y1 - y0;
    const double dx = x1 - x0;
    return {
...
RS2::Ending RS_Line::getTrimPoint(const RS_Vector& trimCoord, const RS_Vector& trimPoint) {
    const RS_Vector vp1 = getStartpoint() - trimCoord;
    const RS_Vector vp2 = trimPoint - trimCoord;
...
void RS_Line::reverse() {
    std::swap(m_data.startpoint, m_data.endpoint);
}
...
bool RS_Line::hasEndpointsWithinWindow(const RS_Vector& firstCorner, const RS_Vector& secondCorner) const {
    ...
    return m_data.startpoint.isInWindowOrdered(vLow, vHigh) || m_data.endpoint.isInWindowOrdered(vLow, vHigh);
}
...
bool RS_Line::offset(const RS_Vector& coord, const double distance) {
    ...
    const double ds(direction.magnitude());
    if (ds < RS_TOLERANCE) {
        return false;
    }
    direction /= ds;
    const RS_Vector vp(coord - getStartpoint());
...
RS_Vector RS_Line::getNormalVector() const {
    const RS_Vector vp = m_data.endpoint - m_data.startpoint; //direction vector
    const double r = vp.magnitude();
    if (r < RS_TOLERANCE) {
        return RS_Vector{false};
    }
...
std::vector<RS_Entity*> RS_Line::offsetTwoSides(const double distance) const {
    ...
    const RS_Vector& vp = getNormalVector() * distance;
    ret.push_back(new RS_Line{m_data.startpoint + vp, m_data.endpoint + vp});
    ret.push_back(new RS_Line{m_data.startpoint - vp, m_data.endpoint - vp});
    return ret;
}
...
void RS_Line::rotate(const double angle) {
    const RS_Vector rvp(angle);
    m_data.startpoint.rotate(rvp);
    m_data.endpoint.rotate(rvp);
    calculateBorders();
}

void RS_Line::rotate(const RS_Vector& center, const double angle) {
    const RS_Vector rvp(angle);
    m_data.startpoint.rotate(center, rvp);
    m_data.endpoint.rotate(center, rvp);
    calculateBorders();
}
...
void RS_Line::scale(const RS_Vector& factor) {
    m_data.startpoint.scale(factor);
    m_data.endpoint.scale(factor);
    calculateBorders();
}
...
RS_Entity& RS_Line::shear(const double k) {
    m_data.startpoint.shear(k);
    m_data.endpoint.shear(k);
    calculateBorders();
    return *this;
}
...
void RS_Line::moveRef(const RS_Vector& ref, const RS_Vector& offset) {
    if (std::abs(m_data.startpoint.x - ref.x) < 1.0e-4 && std::abs(m_data.startpoint.y - ref.y) < 1.0e-4) {
        moveStartpoint(m_data.startpoint + offset);
    }
    if (std::abs(m_data.endpoint.x - ref.x) < 1.0e-4 && std::abs(m_data.endpoint.y - ref.y) < 1.0e-4) {
        moveEndpoint(m_data.endpoint + offset);
    }
}
...
void RS_Line::draw(RS_Painter* painter) {
    if (isConstruction()) {
        ...
        painter->drawInfiniteWCS(m_data.startpoint, m_data.endpoint);
    }
    else {
        // Adjust dash offset
        painter->updateDashOffset(this);
        painter->drawLineWCS(m_data.startpoint, m_data.endpoint);
    }
}
