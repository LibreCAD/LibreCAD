...
LC_Hyperbola makeCanonicalHyperbola(double a, double b, double phi1, double phi2,
                                    bool reversed = false) {
    LC_HyperbolaData d;
...
TEST_CASE("LC_Hyperbola: getNearestPointOnEntity for unbounded uses parametric search (A7)",
...
TEST_CASE("LC_Hyperbola: getRefPoints includes both vertices for unbounded (A16)",
          "[hyperbola][refpoints][regression]")
{
    ...
    for (size_t i = 0; i < refs.size(); ++i) {
        ...
        if (vectorsApproxEqual(v, RS_Vector{ 2.0, 0.0})) sawPositive = true;
        if (vectorsApproxEqual(v, RS_Vector{-2.0, 0.0})) sawNegative = true;
    }
    REQUIRE(sawPositive);
    REQUIRE(sawNegative);
}
