          ...
          "[parabola][tangent][regression]")
{
    ...
    auto p = makeCanonicalParabola(1.0, 1.0);
    RS_Line normal{nullptr, RS_LineData{RS_Vector{0.0, 0.0}, RS_Vector{0.0, 1.0}}};
          ...
          "[parabola][reconstruction][regression]")
{
    ...
    std::vector<RS_Vector> samples = {
        bezierAt(cps, 0.0), bezierAt(cps, 0.3),
    ...
    for (const auto &cand : candidates) {
        ...
        if (cand.m_vertex.distanceTo(orig.getData().m_vertex) < 1e-3 &&
            std::abs(cand.m_axis.magnitude() -
                     orig.getData().m_axis.magnitude()) < 1e-3) {
            foundMatch = true;
            break;
        }
    }
          ...
          "[parabola][length]")
{
    ...
    auto speedAt = [&](double t) {
        const double mt = 1.0 - t;
        const RS_Vector dp = (cps[1] - cps[0]) * (2.0 * mt) +
                             (cps[2] - cps[1]) * (2.0 * t);
        return dp.magnitude();
    };
    const int n = 1000;
          ...
          "[parabola][revert]")
{
    ...
    REQUIRE(p2.areaLineIntegral() == Approx(-p1.areaLineIntegral()).margin(1e-12));
