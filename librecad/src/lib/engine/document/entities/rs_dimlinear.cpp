...
RS_DimLinearData::RS_DimLinearData() : extensionPoint1(false), extensionPoint2(false) {
}

RS_DimLinearData::RS_DimLinearData(const RS_DimLinearData& other) : extensionPoint1{other.extensionPoint1},
                                                                    extensionPoint2{other.extensionPoint2}, angle{other.angle},
                                                                    obliqueAngle{other.obliqueAngle} {
}

RS_DimLinearData::RS_DimLinearData(const RS_Vector& extensionPoint1, const RS_Vector& extensionPoint2, const double angle,
                                   const double oblique) : extensionPoint1(extensionPoint1), extensionPoint2(extensionPoint2), angle(angle),
                                                           obliqueAngle(oblique) {
}

RS_DimLinearData::~RS_DimLinearData() = default;

std::ostream& operator <<(std::ostream& os, const RS_DimLinearData& dd) {
    os << "(" << dd.extensionPoint1 << "," << dd.extensionPoint1 << ',' << dd.angle << ',' << dd.obliqueAngle << ',' << ")";
    return os;
}
...
RS_VectorSolutions RS_DimLinear::getRefPoints() const {
    return RS_VectorSolutions({
        m_dimLinearData.extensionPoint1,
        m_dimLinearData.extensionPoint2,
        m_dimGenericData.definitionPoint,
        m_dimGenericData.middleOfText
    });
}

void RS_DimLinear::setAngle(const double a) {
...
void RS_DimLinear::doUpdateDim() {
    ...
    const RS_ConstructionLine dimLine(m_dimGenericData.definitionPoint, m_dimGenericData.definitionPoint + dirDim);

    const RS_Vector dimP1 = dimLine.getNearestPointOnEntity(m_dimLinearData.extensionPoint1);
    const RS_Vector dimP2 = dimLine.getNearestPointOnEntity(m_dimLinearData.extensionPoint2);

    const auto extLineStyle = m_dimStyleTransient->extensionLine();
    const bool showExtLine1 = extLineStyle->suppressFirstLine() == LC_DimStyle::ExtensionLine::DONT_SUPPRESS;
    const bool showExtLine2 = extLineStyle->suppressSecondLine() == LC_DimStyle::ExtensionLine::DONT_SUPPRESS;
    ...
    const double extensionLine1Length = (m_dimLinearData.extensionPoint1 - dimP1).magnitude();
    const bool extensionLine1NonZero = extensionLine1Length < 1e-6;
    const bool extensionLine2NonZero = (m_dimLinearData.extensionPoint2 - dimP2).magnitude() < 1e-6;
    ...
    if (showExtLine1) {
        const RS_Vector vDimexe1 = RS_Vector::polar(dimexe, extAngle1);
        addDimExtensionLine(m_dimLinearData.extensionPoint1 + vDimexo1, dimP1 + vDimexe1, true);
    }

    if (showExtLine2) {
        const RS_Vector vDimexe2 = RS_Vector::polar(dimexe, extAngle2);
...
QString RS_DimLinear::getMeasuredLabel() {
    ...
    // construction line for dimension line
    const RS_ConstructionLine dimLine(nullptr,
                                      RS_ConstructionLineData(m_dimGenericData.definitionPoint, m_dimGenericData.definitionPoint + dirDim));

    const RS_Vector dimP1 = dimLine.getNearestPointOnEntity(m_dimLinearData.extensionPoint1);
    const RS_Vector dimP2 = dimLine.getNearestPointOnEntity(m_dimLinearData.extensionPoint2);

    // Definitive dimension line:
    const double distance = dimP1.distanceTo(dimP2);
...
bool RS_DimLinear::hasEndpointsWithinWindow(const RS_Vector& v1, const RS_Vector& v2) const {
    return m_dimLinearData.extensionPoint1.isInWindow(v1, v2) || m_dimLinearData.extensionPoint2.isInWindow(v1, v2);
}
...
void RS_DimLinear::getDimPoints(RS_Vector& dimP1, RS_Vector& dimP2) const {
    ...
    // construction line for dimension line
    const RS_ConstructionLine dimLine(
        nullptr, RS_ConstructionLineData(m_dimGenericData.definitionPoint, m_dimGenericData.definitionPoint + dirDim));
...
void RS_DimLinear::stretch(const RS_Vector& firstCorner, const RS_Vector& secondCorner, const RS_Vector& offset) {
    //e->calculateBorders();
