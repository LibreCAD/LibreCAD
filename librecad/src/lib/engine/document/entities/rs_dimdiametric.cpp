...
RS_DimDiametricData::RS_DimDiametricData() : definitionPoint(false) {
}
 ...
 */
RS_DimDiametricData::RS_DimDiametricData(const RS_Vector& definitionPoint, const double leader) : definitionPoint{definitionPoint},
    leader{leader} {
}
 ...
 */
RS_DimDiametric::RS_DimDiametric(RS_EntityContainer* parent, const RS_DimensionData& d, const RS_DimDiametricData& ed)
...
QString RS_DimDiametric::getMeasuredLabel() {
    ...
    const double distance = m_dimGenericData.definitionPoint.distanceTo(m_dimDiametricData.definitionPoint) * getGeneralFactor();
...
RS_VectorSolutions RS_DimDiametric::getRefPoints() const {
    return RS_VectorSolutions({m_dimDiametricData.definitionPoint, m_dimGenericData.definitionPoint, m_dimGenericData.middleOfText});
}
...
void RS_DimDiametric::doUpdateDim() {
    ...
    createDimensionLine(m_dimGenericData.definitionPoint, m_dimDiametricData.definitionPoint, true, true, true, true,
                        m_dimGenericData.autoText);
}
...
void RS_DimDiametric::moveRef(const RS_Vector& ref, const RS_Vector& offset) {
    if (ref.distanceTo(m_dimDiametricData.definitionPoint) < 1.0e-4) {
        const RS_Vector c = (m_dimDiametricData.definitionPoint + m_dimGenericData.definitionPoint) / 2.0;
        const double d = c.distanceTo(m_dimDiametricData.definitionPoint);
        const double a = c.angleTo(m_dimDiametricData.definitionPoint + offset);
        ...
        const RS_Vector c = (m_dimDiametricData.definitionPoint + m_dimGenericData.definitionPoint) / 2.0;
        const double d = c.distanceTo(m_dimGenericData.definitionPoint);
        const double a = c.angleTo(m_dimGenericData.definitionPoint + offset);
...
std::ostream& operator <<(std::ostream& os, const RS_DimDiametric& d) {
    os << " DimDiametric: " << d.getGenericData() << "\n" << d.getEData() << "\n";
    return os;
}
