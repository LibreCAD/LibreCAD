...
/**
 * @retval true the entity belongs to the drawing itself: it lies in a graphic or a block, directly
 * or inside polylines and groups, so an action may change or replace it.
 * The children of block references, dimensions, hatches, text and splines are generated by their
 * owner, which rebuilds them and so loses any edit, or frees an entity the undo history still holds.
 */
 ...
 * i.e. it is in a graphic or in a polyline, and is not generated by another entity (see isEditable()).
 */
bool RS_Information::isTrimmable(const RS_Entity* e) {
    return e != nullptr && e->getParent() != nullptr && isEditable(e);
}
