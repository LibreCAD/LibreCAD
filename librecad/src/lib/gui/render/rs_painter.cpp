...
namespace {
    const auto COLOR_BLACK = RS_Color(Qt::black);
    const auto COLOR_WHITE = RS_Color(Qt::white);
    const QColor Q_COLOR_BLACK = COLOR_BLACK.toQColor();
    const QColor Q_COLOR_WHITE = COLOR_WHITE.toQColor();
    ...
    public:
        explicit PainterGuard(QPainter& painter) : m_painter{&painter} {
            painter.save();
        }
    ...
    void drawPlus(QPainter& painter, const QPointF& uiPos, const int halfSize) {
        const QPointF left{uiPos.x() - halfSize, uiPos.y()};
        const QPointF right{uiPos.x() + halfSize, uiPos.y()};
        painter.drawLine(left, right);
        const QPointF bottom{uiPos.x(), uiPos.y() - halfSize};
        const QPointF top{uiPos.x(), uiPos.y() + halfSize};
        painter.drawLine(bottom, top);
    }

    // draw a plus sign by given halfSize
    void drawCross(QPainter& painter, const QPointF& uiPos, const int halfSize) {
        const auto half = QPoint(halfSize, halfSize).toPointF();
    ...
    void drawOctagon(QPainter& painter, const QPointF& uiPos, const int halfSize) {
        QPointF dr0(halfSize, std::sin(M_PI / 8.) * halfSize);
        ...
        QPolygonF octagon;
        std::transform(vertices.cbegin(), vertices.cend(), std::back_inserter(octagon), [&uiPos](const QPointF& vertex) {
            return vertex + uiPos;
        });
    ...
    void drawSquare(QPainter& painter, const QPointF& uiPos, const int halfSize) {
        const auto dr0 = QPoint(halfSize, halfSize).toPointF();
        const auto dr1 = QPoint(-halfSize, halfSize).toPointF();
        const QPolygonF square{{uiPos + dr0, uiPos + dr1, uiPos - dr0, uiPos - dr1}};
        PainterGuard guard(painter);
        painter.setBrush(Qt::NoBrush);
...
RS_Painter::RS_Painter(QPaintDevice* pd)
    : QPainter{pd}, m_cachedDpmm{getDpmm()} {
}
...
void RS_Painter::drawPointEntityWCS(const RS_Vector& wcsPos) {
    const RS_Vector uiPos = toGui(wcsPos);
    drawPointEntityUI(uiPos, m_pointsMode, m_screenPointsSize);
}

void RS_Painter::drawRefPointEntityWCS(const RS_Vector& wcsPos, const int pdMode, const double pdSize) {
    ...
    const int screenPDSize = determinePointScreenSize(pdSize);
    double uiX, uiY;
 ...
 */
void RS_Painter::drawPointEntityUI(const RS_Vector& uiPos, const int pdmode, const int pdsize) {
    ...
    */
    const QPointF uiCoords{uiPos.x, uiPos.y};
...
void RS_Painter::drawFilledPolygonWCS(const RS_Vector& wcsV1, const RS_Vector& wcsV2, const RS_Vector& wcsV3, const RS_Vector& wcsV4,
                                      const RS_Vector& wcsV5) {
    QPolygonF uiPolygon;
...
void RS_Painter::drawFilledCircleWCS(const RS_Vector& wcsCenter, const double radius) {
    fillEllipseUI(toGuiPointF(wcsCenter), toGuiDX(radius), toGuiDY(radius));
}

void RS_Painter::drawPolygonWCS(const RS_Vector& wcsV1, const RS_Vector& wcsV2, const RS_Vector& wcsV3, const RS_Vector& wcsV4,
                                const RS_Vector& wcsV5) {
    QPolygonF uiPolygon;
    uiPolygon.push_back(toGuiPointF(wcsV1));
    uiPolygon.push_back(toGuiPointF(wcsV2));
...
void RS_Painter::drawLineWCSScaled(const RS_Vector& wcsP1, const RS_Vector& wcsP2, const double lineWidthFactor) {
...
void RS_Painter::drawEntityCircle(const RS_Circle* circle) {
    ...
    if (uiRadiusX < m_minCircleDrawingRadius) {
        QPainter::drawPoint(QPointF(uiCenter.x, uiCenter.y));
    }
    else if (m_circleRenderSameAsArcs && m_arcRenderInterpolate) {
        QPainterPath path;
        drawArcInterpolatedByLines(uiCenter, uiRadiusX, 0., 360., path);
        QPainter::drawPath(path);
    }
    else if (uiRadiusX <= getMaximumArcNonErrorRadius()) {
        // draw arc using QT
        const double uiRadiusY = toGuiDY(data.radius);
...
void RS_Painter::drawArcEntity(RS_Arc* arc, QPainterPath& path) {
    ...
    if (uiRadii.x <= m_minArcDrawingRadius) {
        // draw just a point
        QPainter::drawPoint(QPointF{uiCenter.x, uiCenter.y});
    }
    else if (m_arcRenderInterpolate) {
        // draw arc interpolated by lines
        drawArcInterpolatedByLines(uiCenter, uiRadii.x, toUCSAngleDegrees(arc->getData().startAngleDegrees), arc->getData().angularLength,
                                   path);
    }
    else {
        // same as
        ...
        if (uiRadii.x <= getMaximumArcNonErrorRadius()) {
            ...
            if (visualArcIsVisible) {
                ...
                auto wcsBoundingMinP = wcsBoundingBox.minP();
                auto wcsBoundingMaxP = wcsBoundingBox.maxP();
                QPolygonF visualBox(QRectF(wcsBoundingMinP.x, wcsBoundingMinP.y, wcsBoundingMaxP.x - wcsBoundingMinP.x,
                                           wcsBoundingMaxP.y - wcsBoundingMinP.y));
                ...
                if (vpStart.isInWindowOrdered(wcsBoundingMinP, wcsBoundingMaxP)) {
                    crossPoints.push_back(0.);
                }
                if (vpEnd.isInWindowOrdered(wcsBoundingMinP, wcsBoundingMaxP)) {
                ...
                for (size_t i = 1; i < crossPoints.size(); ++i) {
                    ...
                    if (arcSegment.getMiddlePoint().isInWindowOrdered(wcsBoundingMinP, wcsBoundingMaxP)) {
                        drawArcSegmentBySplinePointsUI(uiCenter, uiRadii.x, toUCSAngle(arcSegment.getAngle1()), arcSegment.getAngleLength(),
                                                       path);
                    }

#ifdef DEBUG_ARC_RENDERING
                    arcSegment.calculateBorders(); RS_Vector uiCenter = toGui(arcSegment.getStartpoint()); drawCircleUI(uiCenter, 20);
                    uiCenter = toGui(arcSegment.getEndpoint()); drawCircleUI(uiCenter, 20);
#endif
                }
            }
        }
...
void RS_Painter::drawArcSegmentBySplinePointsUI(const RS_Vector& uiCenter, const double uiRadiusX, const double startAngleRad,
                                                const double angularLengthRad, QPainterPath& path) {
    // Issue #2035
    ...
    int arcPoints = static_cast<int>(ceil(std::abs(angularLengthRad) / dA));
...
#ifdef DEBUG_ARC_RENDERING
            drawPointEntityUI(cp1.x, cp1.y, 2, 15); drawPointEntityUI(cp2.x, cp2.y, 2, 15);
            drawPointEntityUI(uiControlPoints[n - 2].x, uiControlPoints[n - 2].y, 2, 15); drawPointEntityUI(
...
void RS_Painter::drawArcQT(const RS_Vector& uiCenter, const RS_Vector& uiRadii, const double uiStartAngleDegrees,
                           const double angularLength, QPainterPath& path) {
    ...
    const RS_Vector minCorner = uiCenter - uiRadii;
...
void RS_Painter::drawArcInterpolatedByLines(const RS_Vector& uiCenter, const double uiRadiusX, const double uiStartAngleDegrees,
                                            const double angularLength, QPainterPath& path) const {
    ...
    const double angularLengthRad = RS_Math::deg2rad(angularLength);
    ...
    if (m_arcRenderInterpolationAngleFixed) {
        ...
        stepsCount = static_cast<int>(angularLengthRad / m_arcRenderInterpolationAngleValue) + 2;
    }
    else {
        ...
        const double relativeError = 0.5 * std::abs(m_arcRenderInterpolationMaxSagitta) / uiRadiusX;
        ...
        const double stepsTolerance = std::abs(angularLengthRad) / lineSegmentAngle;
        stepsCount = static_cast<int>(ceil(stepsTolerance)) + 2;
    }
    //        LC_ERR << "ARC steps: " << stepsTol <<  " " << steps << " len " << angularLength << " start " << uiStartAngleDegrees;
    const double uiStartAngleRad = RS_Math::deg2rad(uiStartAngleDegrees);

    const double deltaAngleRad = angularLengthRad / stepsCount;
...
/**
 * Draws a circle.
 * @param wcsCenter Center point
 * @param wcsRadius Radius
 */
void RS_Painter::drawCircleWCS(const RS_Vector& wcsCenter, const double wcsRadius) {
    const RS_Vector uiCenter = toGui(wcsCenter);
    const double uiRadius = toGuiDX(wcsRadius);
    drawCircleUI(uiCenter, uiRadius);
}

void RS_Painter::drawCircleUI(const RS_Vector& uiCenter, const double uiRadius) {
    if (uiRadius < m_minCircleDrawingRadius) {
        QPainter::drawPoint(QPointF(uiCenter.x, uiCenter.y));
    }
    else {
        if (m_circleRenderSameAsArcs) {
            if (m_arcRenderInterpolate) {
                QPainterPath path;
...
void RS_Painter::drawCircleUIDirect(const RS_Vector& uiPos, const double uiRadius) {
    if (uiRadius < m_minCircleDrawingRadius) {
...
void RS_Painter::drawEllipseWCS(const RS_Vector& wcsCenter, const double wcsMajorRadius, const double ratio, const double wcsAngleDegrees) {
...
void RS_Painter::drawEllipseUI(const RS_Vector& uiCenter, const RS_Vector& uiRadii, const double uiAngleDegrees) {
    if (uiRadii.x < m_minEllipseMajorRadius) {
        ...
        const QPointF radii{uiRadii.x, uiRadii.y};

        if (uiRadii.y < m_minEllipseMinorRadius) {
            //ellipse too small
...
void RS_Painter::addEllipseArcToPath(QPainterPath& localPath, const RS_Vector& uiRadii, const double startAngleDeg,
                                     const double angularLengthDeg, const bool useSpline) const {
    if (useSpline) {
        const double startRad = RS_Math::deg2rad(toUCSAngleDegrees(startAngleDeg));
        const double lenRad = RS_Math::deg2rad(toUCSAngleDegrees(angularLengthDeg));
        drawEllipseSegmentBySplinePointsUI(uiRadii, startRad, lenRad, localPath, false);
    }
    else {
        const QRectF rect(-uiRadii.x, -uiRadii.y, 2 * uiRadii.x, 2 * uiRadii.y);
        localPath.arcMoveTo(rect, startAngleDeg);
        localPath.arcTo(rect, startAngleDeg, angularLengthDeg);
    }
}

void RS_Painter::drawEllipseSegmentBySplinePointsUI(const RS_Vector& uiRadii, const double startRad, const double lenRad,
                                                    QPainterPath& path, const bool closed) const {
        ...
        for (auto* e : *static_cast<RS_EntityContainer*>(loop)) {
            ...
            switch (e->rtti()) {
                ...
                case RS2::EntityCircle: {
                    ...
                    const double radius = toGuiDX(circle->getRadius());
                ...
                case RS2::EntityEllipse: {
                    ...
                    if (ellipse->isEllipticArc()) {
                        const double startAngle = toUcsDegrees(ellipse->getAngle1());
                        const double angularLength = RS_Math::rad2deg(
                            ellipse->isReversed() ? -ellipse->getAngleLength() : ellipse->getAngleLength());
...
void RS_Painter::debugOutPath(const QPainterPath& tmpPath) const {
    ...
    for (int i = 0; i < c; i++) {
        ...
        LC_ERR << "i " << i << "(" << element.x << "," << element.y << ") Line To " << element.isLineTo() << " Move To: " << element.
            isMoveTo() << " Is Curve:" << element.isCurveTo();
    }
}

void RS_Painter::drawSplinePointsWCS(const std::vector<RS_Vector>& wcsControlPoints, const bool closed) {
...
void RS_Painter::drawImgWCS(const QImage& img, const RS_Vector& wcsInsertionPoint, const RS_Vector& uVector, const RS_Vector& vVector) {
    //    if (viewport->hasUCS()) {
    const double wcsAngle = uVector.angle();
    const double ucsAngle = toUCSAngle(wcsAngle);

    auto ucsUVector = uVector;
    auto ucsVVector = vVector;

    const auto angleVector = RS_Vector(ucsAngle - wcsAngle);

    ucsUVector.rotate(angleVector);
    ucsVVector.rotate(angleVector);
    //    }

    const double magnitudeU = uVector.magnitude(); // fixme - sand - render - cache?
    const double magnitudeV = vVector.magnitude(); // fixme - sand - render - cache?
    const RS_Vector scale{toGuiDX(magnitudeU), toGuiDY(magnitudeV)};
    const RS_Vector uiInsert = toGui(wcsInsertionPoint);
    drawImgUI(img, uiInsert, ucsUVector, ucsVVector, scale);
}

void RS_Painter::drawImgUI(const QImage& img, const RS_Vector& uiInsert, const RS_Vector& uVector, const RS_Vector& vVector,
                           const RS_Vector& factor) {
    PainterGuard painterGuard(*this);
    ...
    if (std::signbit(RS_Vector::crossP(uVector, vVector).z)) {
        // mirrored
...
void RS_Painter::drawTextH(const int x1, const int y1, const int x2, const int y2, const QString& text) {
    QPainter::drawText(x1, y1, x2, y2, Qt::AlignRight | Qt::AlignVCenter, text);
}

void RS_Painter::drawTextV(const int x1, const int y1, const int x2, const int y2, const QString& text) {
    PainterGuard painterGuard{*this};
...
void RS_Painter::fillRect(const int x1, const int y1, const int w, const int h, const RS_Color& col) {
...
void RS_Painter::fillEllipseUI(const QPointF& point_f, const double radiusX, const double radiusY) {
    const QBrush brushSaved = brush();
    setBrushColor(RS_Color(pen().color()));
...
void RS_Painter::fillTriangleUI(const RS_Vector& uiP1, const RS_Vector& uiP2, const RS_Vector& uiP3) {
    QPolygonF arr;
    const QBrush brushSaved = brush();
...
double RS_Painter::getDpmm() const {
    const auto paintDevice = device();
    int mm(paintDevice->widthMM());
    if (mm <= 0) {
        mm = 400;
    }
    return static_cast<double>(paintDevice->width()) / mm;
}
...
void RS_Painter::setPen(const RS_Pen& pen) {
    ...
    switch (m_drawingMode) {
        case RS2::ModeBW:
            pColor = Q_COLOR_BLACK;
            break;

        case RS2::ModeWB:
            pColor = Q_COLOR_WHITE;
            break;
...
void RS_Painter::setPen(const RS_Color& color) {
    switch (m_drawingMode) {
        case RS2::ModeBW: {
            const auto& col = RS_Color(Qt::black);
            m_lpen.setColor(col);
            QPainter::setPen(col);
            break;
        }
        case RS2::ModeWB: {
            const auto& col = RS_Color(Qt::white);
            m_lpen.setColor(col);
            QPainter::setPen(col);
            break;
        }
        default:
            m_lpen.setColor(color);
...
void RS_Painter::setClipRect(const int x, const int y, const int w, const int h) {
...
void RS_Painter::drawText(const QRect& uiRect, const int flags, const QString& text, QRect* uiBoundingBox) {
    QPainter::drawText(uiRect, flags, text, uiBoundingBox);
}
...
void RS_Painter::setPenJoinStyle(const Qt::PenJoinStyle style) {
    m_penJoinStyle = style;
}

void RS_Painter::setPenCapStyle(const Qt::PenCapStyle style) {
    m_penCapStyle = style;
}

void RS_Painter::setMinCircleDrawingRadius(const double val) {
    m_minCircleDrawingRadius = val;
}

void RS_Painter::setMinArcDrawingRadius(const double val) {
    m_minArcDrawingRadius = val;
}

void RS_Painter::setMinEllipseMajorRadius(const double val) {
    m_minEllipseMajorRadius = val;
}

void RS_Painter::setMinEllipseMinorRadius(const double val) {
    m_minEllipseMinorRadius = val;
}

void RS_Painter::setMinLineDrawingLen(const double val) {
    m_minLineDrawingLen = val;
}

void RS_Painter::drawRectUI(const double uiX1, const double uiY1, const double uiX2, const double uiY2) {
    drawPolygon(QRect(static_cast<int>(uiX1 + 0.5), static_cast<int>(uiY1 + 0.5), static_cast<int>(uiX2 - uiX1 + 0.5),
                      static_cast<int>(uiY2 - uiY1 + 0.5)));
}

void RS_Painter::drawRectUI(const RS_Vector& p1, const RS_Vector& p2) {
    drawPolygon(QRect(static_cast<int>(p1.x + 0.5), static_cast<int>(p1.y + 0.5), static_cast<int>(p2.x - p1.x + 0.5),
                      static_cast<int>(p2.y - p1.y + 0.5)));
}

void RS_Painter::drawHandleWCS(const RS_Vector& wcsPos, const RS_Color& c, QPointF& previousUIPosition, const int size) {
    const QPointF uiPos = toGuiPointF(wcsPos);
    // here we try to optimize the drawing and don't drive handles if they are too close to each other
    if ((uiPos - previousUIPosition).manhattanLength() > m_minLineDrawingLen * 2) {
        fillRect(QRectF{uiPos - QPointF(size, size), QSize{size, size} * 2}, c);
    }
    previousUIPosition = uiPos;
}

void RS_Painter::setMinRenderableTextHeightInPx(const int i) {
    m_minRenderableTextHeightInPx = i;
}

void RS_Painter::updateDashOffset(const RS_Entity* e) {
    // Adjust dash offset
    if (m_lpen.getLineType() == RS2::SolidLine /*|| view.getGraphic() == nullptr*/) {
        return;
    }

    // factor from model space to GUI
    const double toMm = m_defaultWidthFactor;
    m_currenPatternOffset -= e->getLength() * toMm;
}

int RS_Painter::determinePointScreenSize(const double pdsize) const {
    const int deviceHeight = getHeight();
    if (!std::isnormal(pdsize)) {
        const int screenPointSize = deviceHeight / 20;
        return screenPointSize;
    }
    if (DXF_FORMAT_PDSize_isPercent(pdsize)) {
        const int screenPointSize = deviceHeight * DXF_FORMAT_PDSize_Percent(pdsize) / 100;
        return screenPointSize;
    }
    const int screenPointSize = toGuiDY(pdsize);
    return screenPointSize;
}

void RS_Painter::updatePointsScreenSize(const double pdSize) {
    m_screenPointsSize = determinePointScreenSize(pdSize);
}

void RS_Painter::drawInfiniteWCS(const RS_Vector& startpoint, const RS_Vector& endpoint) {
    const LC_Rect viewportRect = m_renderer->getBoundingClipRect();
    RS_Vector start(false);

    const double offsetX = toGuiDX(0.25); // todo - check why gui coordinates are used there -  while intersection is with WCS coordinates?
    const double offsetY = toGuiDY(0.25);

    const RS_Vector pLeft = LC_LineMath::getIntersectionInfiniteLineLineFast(startpoint, endpoint, viewportRect.minP(),
                                                                             RS_Vector(viewportRect.minP().x, viewportRect.maxP().y),
                                                                             offsetX, offsetY);
    if (pLeft.valid) {
        start = pLeft;
    }
    const RS_Vector pBottom = LC_LineMath::getIntersectionInfiniteLineLineFast(startpoint, endpoint, viewportRect.minP(),
                                                                               RS_Vector(viewportRect.maxP().x, viewportRect.minP().y),
                                                                               offsetX, offsetY);
    if (pBottom.valid) {
        if (start.valid) {
    ...
    const RS_Vector pRight = LC_LineMath::getIntersectionInfiniteLineLineFast(startpoint, endpoint,
                                                                              RS_Vector(viewportRect.maxP().x, viewportRect.minP().y),
                                                                              viewportRect.maxP(), offsetX, offsetY);
    if (pRight.valid) {
        if (start.valid) {
    ...
    if (start.valid) {
        const RS_Vector pTop = LC_LineMath::getIntersectionInfiniteLineLineFast(startpoint, endpoint,
                                                                                RS_Vector(viewportRect.minP().x, viewportRect.maxP().y),
                                                                                viewportRect.maxP(), offsetX, offsetY);
        if (pTop.valid) {
...
void RS_Painter::drawEntity(RS_Entity* entity) {
    m_renderer->renderEntity(this, entity);
}

void RS_Painter::drawAsChild(RS_Entity* entity) {
    m_renderer->renderEntityAsChild(this, entity);
}

bool RS_Painter::isTextLineNotRenderable(const double wcsLineHeight) const {
    const double uiHeight = toGuiDY(wcsLineHeight);
    return m_renderer->isTextLineNotRenderable(uiHeight);
}
...
void RS_Painter::setViewPort(LC_GraphicViewport* v) {
    ...
    m_viewPortOffsetX = v->getOffsetX();
    m_viewPortOffsetY = v->getOffsetY();
    m_viewPortOffset.set(m_viewPortOffsetX, m_viewPortOffsetY);
    m_viewPortHeight = v->getHeight();
}
...
void RS_Painter::toGui(const RS_Vector& wcsCoordinate, double& uiX, double& uiY) const {
    ...
    if (hasUCS()) {
        ...
        /*
                ...
                uiX = newPos.y;
        */
        const double ucsPositionX = wcsCoordinate.x - getUcsOrigin().x;
        const double ucsPositionY = wcsCoordinate.y - getUcsOrigin().y;

        const RS_Vector& ucsRotation = getUcsRotation();
        const double ucsX = ucsPositionX * ucsRotation.x - ucsPositionY * ucsRotation.y;
        const double ucsY = ucsPositionX * ucsRotation.y + ucsPositionY * ucsRotation.x;

        //        uiX = toGuiX(uiX);
        uiX = ucsX * m_viewPortFactorX + m_viewPortOffsetX;
        //        uiY = toGuiY(uiY);
        uiY = -ucsY * m_viewPortFactorY - m_viewPortOffsetY + m_viewPortHeight;
    }
    else {
        //        uiX = toGuiX(wcsCoordinate.x);
        uiX = wcsCoordinate.x * m_viewPortFactorX + m_viewPortOffsetX;
        //        uiY = toGuiY(wcsCoordinate.y);
        uiY = -wcsCoordinate.y * m_viewPortFactorY - m_viewPortOffsetY + m_viewPortHeight;
    }
}
...
RS_Vector RS_Painter::toGui(const RS_Vector& worldCoordinates) const {
    ...
    uiPosition.y = m_viewPortHeight - uiPosition.y;
...
#ifdef DEBUG_RENDERING_TOGUI
    {
        ...
        if (!(equal(uiX, uiPosition.x) && equal(uiY, uiPosition.y))) {
            LC_ERR << QString{" : (%1, %2) vs (%3, %4)"}.arg(uiPosition.x, 10, 'g', 10).arg(uiPosition.y, 10, 'g', 10).arg(uiX, 10, 'g', 10)
                                                        .arg(uiY, 10, 'g', 10);
...
QTransform RS_Painter::getToGuiTransform() const {
    ...
    std::transform(wcs.begin(), wcs.end(), std::back_inserter(gui), [this](const QPointF& wcsPoint) {
        const RS_Vector guiV = toGui({wcsPoint.x(), wcsPoint.y()});
...
double RS_Painter::toGuiDX(const double ucsDX) const {
    //    return viewport->toGuiDX(d);
    return ucsDX * m_viewPortFactor.x;
}

double RS_Painter::toGuiDY(const double ucsDY) const {
...
bool RS_Painter::isFullyWithinBoundingRect(const RS_Entity* e) const {
    ...
    return e->getMax().x < m_wcsBoundingRect.maxP().x && e->getMin().x > m_wcsBoundingRect.minP().x && e->getMax().y < m_wcsBoundingRect.maxP().y
        && e->getMin().y > m_wcsBoundingRect.minP().y;
}

bool RS_Painter::isFullyWithinBoundingRect(const LC_Rect& rect) const {
    return rect.maxP().x < m_wcsBoundingRect.maxP().x && rect.minP().x > m_wcsBoundingRect.minP().x && rect.maxP().y < m_wcsBoundingRect.maxP().y
        && rect.minP().y > m_wcsBoundingRect.minP().y;
}
