...
bool RS_FilterJWW::fileImport(RS_Graphic& g, const QString& file, RS2::FormatType /*type*/) {
    ...
    const bool success = jww.in((const char*)QFile::encodeName(file), this);
    RS_DEBUG->print("RS_FilterJWW::fileImport: reading file: OK");
    //graphic->setAutoUpdateBorders(true);

    if (success == false) {
        RS_DEBUG->print(RS_Debug::D_WARNING, "Cannot open JWW file '%s'.", (const char*)QFile::encodeName(file));
        return false;
    }
...
void RS_FilterJWW::addLayer(const DL_LayerData& data) {
    ...
    ////////////////////2006/06/05
    const auto layer = new RS_Layer(toNativeString(data.name.c_str(), getDXFEncoding()));
    ////////////////////
...
void RS_FilterJWW::addBlock(const DL_BlockData& data) {
    ...
    if (QString(data.name.c_str()).toLower() != "*paper_space0" && QString(data.name.c_str()).toLower() != "*paper_space" &&
        QString(data.name.c_str()).toLower() != "*model_space" && QString(data.name.c_str()).toLower() != "$paper_space0" &&
        QString(data.name.c_str()).toLower() != "$paper_space" && QString(data.name.c_str()).toLower() != "$model_space") {
#ifndef RS_NO_COMPLEX_ENTITIES
        if (QString(data.name.c_str()).startsWith("__CE")) {
            const auto ec = new RS_EntityContainer();
            ec->setLayer("0");
            currentContainer = ec;
...
#endif
            const RS_Vector bp(data.bpx, data.bpy);
            //////////////////////////////2006/06/05
            const QString enc = RS_System::getEncoding(variables.getString("$DWGCODEPAGE", "ANSI_1252"));
            // get the codec for Japanese
            QString blName = data.name.c_str();
            const std::optional<QStringConverter::Encoding> encoding = QStringConverter::encodingForName(enc.toLatin1());
            if (encoding) {
                blName = QStringEncoder{QStringEncoder::Utf8}(QString::fromStdString(data.name));
            }
            //////////////////////////////
            const auto block = new RS_Block(graphic, RS_BlockData(blName, bp, false));
...
void RS_FilterJWW::addPoint(const DL_PointData& data) {
    const RS_Vector v(data.x, data.y);

    const auto entity = new RS_Point(currentContainer, RS_PointData(v));
...
void RS_FilterJWW::addLine(const DL_LineData& data) {
    ...
    const auto entity = new RS_Line{currentContainer, {v1, v2}};
...
void RS_FilterJWW::addArc(const DL_ArcData& data) {
    ...
    //	   p1[0], p1[1], p1[2],
    //	   p2[0], p2[1], p2[2]);
    const RS_Vector v(data.cx, data.cy);
    const RS_ArcData d(v, data.radius, RS_Math::deg2rad(data.angle1), RS_Math::deg2rad(data.angle2), false);
    const auto entity = new RS_Arc(currentContainer, d);
    setEntityAttributes(entity, attributes);
...
void RS_FilterJWW::addEllipse(const DL_EllipseData& data) {
    ...
    const RS_Vector v1{data.cx, data.cy};
    const RS_Vector v2{data.mx, data.my};

    const auto entity = new RS_Ellipse{currentContainer, {v1, v2, data.ratio, data.angle1, data.angle2, false}};
    setEntityAttributes(entity, attributes);
...
void RS_FilterJWW::addCircle(const DL_CircleData& data) {
    ...
    const auto entity = new RS_Circle(currentContainer, {{data.cx, data.cy}, data.radius});
    setEntityAttributes(entity, attributes);
...
void RS_FilterJWW::addPolyline(const DL_PolylineData& data) {
    ...
    //RS_DEBUG->print("RS_FilterJWW::addPolyline()");
    const RS_PolylineData d(RS_Vector(false), RS_Vector(false), data.flags & 0x1);
...
void RS_FilterJWW::addVertex(const DL_VertexData& data) {
    RS_DEBUG->print("RS_FilterJWW::addVertex(): %f/%f bulge: %f", data.x, data.y, data.bulge);
...
void RS_FilterJWW::addSpline(const DL_SplineData& data) {
    ...
    if (data.degree == 2) {
        const LC_SplinePointsData d(((data.flags & 0x1) == 0x1), true);
    ...
    if (data.degree >= 1 && data.degree <= 3) {
        const RS_SplineData d(data.degree, ((data.flags & 0x1) == 0x1));
        ...
        RS_DEBUG->print(RS_Debug::D_WARNING, "RS_FilterJWW::addSpline: Invalid degree for spline: %d. " "Accepted values are 1..3.",
                        data.degree);
    }
}
...
void RS_FilterJWW::addInsert(const DL_InsertData& data) {
    ...
    const RS_Vector ip(data.ipx, data.ipy);
    const RS_Vector sc(data.sx, data.sy);
    const RS_Vector sp(data.colSp, data.rowSp);

    //cout << "Insert: " << name << " " << ip << " " << cols << "/" << rows << endl;

    const RS_InsertData d(data.name.c_str(), ip, sc, RS_Math::deg2rad(data.angle), data.cols, data.rows, sp, nullptr, RS2::NoUpdate);
    const auto entity = new RS_Insert(currentContainer, d);
    setEntityAttributes(entity, attributes);
...
QString RS_FilterJWW::getDXFEncoding() const {
    ...
    bool ok;
    const int version = acadver.toInt(&ok);
    ...
    // < ACAD2007
    const QString codePage = variables.getString("$DWGCODEPAGE", "ANSI_1252");
...
void RS_FilterJWW::addMText(const DL_MTextData& data) {
    ...
    const RS_MTextData d(ip, data.height, data.width, valign, halign, dir, lss, data.lineSpacingFactor, mtext, sty, data.angle,
                         RS2::NoUpdate);
    const auto entity = new RS_MText(currentContainer, d);
...
void RS_FilterJWW::addText(const DL_TextData& data) {
    ...
    if (data.vJustification != 0 || data.hJustification != 0) {
        switch (data.hJustification) {
            ...
            case 3: // aligned (TODO)
                attachmentPoint = 2;
                refPoint = RS_Vector((data.ipx + data.apx) / 2.0, (data.ipy + data.apy) / 2.0);
                angle = RS_Vector(data.ipx, data.ipy).angleTo(RS_Vector(data.apx, data.apy));
                break;
            case 4: // Middle (TODO)
                attachmentPoint = 2;
                refPoint = RS_Vector(data.apx, data.apy);
    ...
    constexpr int drawingDirection = 5;
    constexpr double width = 100.0;

    mtext = "";
    addMText(DL_MTextData(refPoint.x, refPoint.y, refPoint.z, data.height, width, attachmentPoint, drawingDirection, RS_MTextData::Exact,
                          1.0, data.text, data.style, angle));
}
 ...
 */
RS_DimensionData RS_FilterJWW::convDimensionData(const DL_DimensionData& data) {
    const RS_Vector defP(data.dpx, data.dpy);
    ...
    return RS_DimensionData(defP, midP, valign, halign, lss, data.lineSpacingFactor, t, sty, data.angle, 0.0, true, nullptr, false, false);
}
 ...
 */
void RS_FilterJWW::addDimAlign(const DL_DimensionData& data, const DL_DimAlignedData& edata) {
    RS_DEBUG->print("RS_FilterJWW::addDimAligned");

    const RS_DimensionData dimensionData = convDimensionData(data);

    const RS_Vector ext1(edata.epx1, edata.epy1);
    const RS_Vector ext2(edata.epx2, edata.epy2);

    const RS_DimAlignedData d(ext1, ext2);

    const auto entity = new RS_DimAligned(currentContainer, dimensionData, d);
    setEntityAttributes(entity, attributes);
    entity->update();
    currentContainer->addEntity(entity);
}
 ...
 */
void RS_FilterJWW::addDimLinear(const DL_DimensionData& data, const DL_DimLinearData& edata) {
    RS_DEBUG->print("RS_FilterJWW::addDimLinear");

    const RS_DimensionData dimensionData = convDimensionData(data);

    const RS_Vector dxt1(edata.dpx1, edata.dpy1);
    const RS_Vector dxt2(edata.dpx2, edata.dpy2);

    const RS_DimLinearData d(dxt1, dxt2, RS_Math::deg2rad(edata.angle), RS_Math::deg2rad(edata.oblique));

    const auto entity = new RS_DimLinear(currentContainer, dimensionData, d);
    setEntityAttributes(entity, attributes);
    entity->update();
    currentContainer->addEntity(entity);
}
 ...
 */
void RS_FilterJWW::addDimRadial(const DL_DimensionData& data, const DL_DimRadialData& edata) {
    RS_DEBUG->print("RS_FilterJWW::addDimRadial");

    const RS_DimensionData dimensionData = convDimensionData(data);
    const RS_Vector dp(edata.dpx, edata.dpy);

    const RS_DimRadialData d(dp, edata.leader);

    const auto entity = new RS_DimRadial(currentContainer, dimensionData, d);
 ...
 */
void RS_FilterJWW::addDimDiametric(const DL_DimensionData& data, const DL_DimDiametricData& edata) {
    RS_DEBUG->print("RS_FilterJWW::addDimDiametric");

    const RS_DimensionData dimensionData = convDimensionData(data);
    const RS_Vector dp(edata.dpx, edata.dpy);

    const RS_DimDiametricData d(dp, edata.leader);

    const auto entity = new RS_DimDiametric(currentContainer, dimensionData, d);
 ...
 */
void RS_FilterJWW::addDimAngular(const DL_DimensionData& data, const DL_DimAngularData& edata) {
    RS_DEBUG->print("RS_FilterJWW::addDimAngular");

    const RS_DimensionData dimensionData = convDimensionData(data);
    const RS_Vector dp1(edata.dpx1, edata.dpy1);
    const RS_Vector dp2(edata.dpx2, edata.dpy2);
    const RS_Vector dp3(edata.dpx3, edata.dpy3);
    const RS_Vector dp4(edata.dpx4, edata.dpy4);

    const RS_DimAngularData d(dp1, dp2, dp3, dp4);

    const auto entity = new RS_DimAngular(currentContainer, dimensionData, d);
 ...
 * angular dimensions (DIMENSION).
 */
void RS_FilterJWW::addDimAngular3P(const DL_DimensionData& data, const DL_DimAngular3PData& edata) {
    RS_DEBUG->print("RS_FilterJWW::addDimAngular3P");

    RS_DimensionData dimensionData = convDimensionData(data);
    const RS_Vector dp1(edata.dpx3, edata.dpy3);
    const RS_Vector dp2(edata.dpx1, edata.dpy1);
    const RS_Vector dp3(edata.dpx3, edata.dpy3);
...
void RS_FilterJWW::addLeader(const DL_LeaderData& data) {
    ...
    //RS_DEBUG->print("RS_FilterJWW::addPolyline()");
    const RS_LeaderData d(data.arrowHeadFlag == 1, "");
...
void RS_FilterJWW::addHatch(const DL_HatchData& data) {
    ...
    hatch = new RS_Hatch(currentContainer, RS_HatchData(data.solid, data.scale, data.angle, QString(data.pattern.c_str())));
...
void RS_FilterJWW::addHatchEdge(const DL_HatchEdgeData& data) {
    ...
    if (hatchLoop) {
        RS_Entity* e = nullptr;
        switch (data.type) {
            case 1: RS_DEBUG->print("RS_FilterJWW::addHatchEdge(): " "line: %f,%f %f,%f", data.x1, data.y1, data.x2, data.y2);
                e = new RS_Line{hatchLoop, {{data.x1, data.y1}, {data.x2, data.y2}}};
                break;
            case 2:
                if (data.ccw && data.angle1 < 1.0e-6 && data.angle2 > 2 * M_PI - 1.0e-6) {
                    e = new RS_Circle(hatchLoop, {{data.cx, data.cy}, data.radius});
                }
                else {
                    if (data.ccw) {
                        e = new RS_Arc(hatchLoop, RS_ArcData(RS_Vector(data.cx, data.cy), data.radius, RS_Math::correctAngle(data.angle1),
                                                             RS_Math::correctAngle(data.angle2), false));
                    }
                    else {
                        e = new RS_Arc(hatchLoop, RS_ArcData(RS_Vector(data.cx, data.cy), data.radius,
                                                             RS_Math::correctAngle(2 * M_PI - data.angle1),
                                                             RS_Math::correctAngle(2 * M_PI - data.angle2), true));
                    }
                }
                break;
            default:
...
void RS_FilterJWW::addImage(const DL_ImageData& data) {
    ...
    const RS_Vector ip(data.ipx, data.ipy);
    const RS_Vector uv(data.ux, data.uy);
    const RS_Vector vv(data.vx, data.vy);
    const RS_Vector size(data.width, data.height);

    const auto image = new RS_Image(currentContainer, RS_ImageData(QString(data.ref.c_str()).toInt(nullptr, 16), ip, uv, vv, size, QString(""),
                                                             data.brightness, data.contrast, data.fade));
...
void RS_FilterJWW::linkImage(const DL_ImageDefData& data) {
    ...
    const int handle = QString(data.ref.c_str()).toInt(nullptr, 16);
    QString sfile(data.file.c_str());
    const QFileInfo fiDxf(file);
    ...
    for (unsigned i = 0; i < graphic->countBlocks(); ++i) {
        const RS_Block* b = graphic->blockAt(i);
...
void RS_FilterJWW::endEntity() {
    ...
    if (hatch) {
        ...
        if (hatch->validate()) {
            ...
            RS_DEBUG->print(RS_Debug::D_ERROR, "RS_FilterJWW::endEntity(): updating hatch failed: invalid hatch area");
        }
        hatch = nullptr;
    }
}
 ...
 */
void RS_FilterJWW::setVariableVector(const char* key, const double v1, const double v2, const double v3, const int code) {
    RS_DEBUG->print("RS_FilterJWW::setVariableVector");

    // update document's variable list:
    if (currentContainer->rtti() == RS2::EntityGraphic) {
        ((RS_Graphic*)currentContainer)->addVariable(QString(key), RS_Vector(v1, v2, v3), code);
    }
}
 ...
 */
void RS_FilterJWW::setVariableString(const char* key, const char* value, const int code) {
    ...
    // update document's variable list:
    if (currentContainer->rtti() == RS2::EntityGraphic) {
        ((RS_Graphic*)currentContainer)->addVariable(QString(key), QString(value), code);
    }
}
 ...
 */
void RS_FilterJWW::setVariableInt(const char* key, const int value, const int code) {
 ...
 */
void RS_FilterJWW::setVariableDouble(const char* key, const double value, const int code) {
 ...
 */
bool RS_FilterJWW::fileExport(RS_Graphic& g, const QString& file, const RS2::FormatType type) {
    RS_DEBUG->print("RS_FilterJWW::fileExport: exporting file '%s'...", (const char*)QFile::encodeName(file));
    ...
    QString path = QFileInfo(file).path(); if (QFileInfo(path).isWritable() == false) {
        RS_DEBUG->print("RS_FilterJWW::fileExport: can't write file: " "no permission");
        return false;
    }
    //
#endif
    ...
    jww.writeDimStyle(*dw, graphic->getVariableDouble("$DIMASZ", 2.5), graphic->getVariableDouble("$DIMEXE", 1.25),
                      graphic->getVariableDouble("$DIMEXO", 0.625), graphic->getVariableDouble("$DIMGAP", 0.625),
    ...
    if (type == RS2::FormatJWW) {
        ...
        for (unsigned i = 0; i < graphic->countBlocks(); ++i) {
            const RS_Block* blk = graphic->blockAt(i);
            if (!blk->isDeleted())
                jww.writeBlockRecord(*dw, std::string((const char*)blk->getName().toLocal8Bit().data()));
            /*
            // v2.0.4.9..:
            //writeBlock(*dw, blk);
    ...
    if (type == RS2::FormatJWW) {
        RS_Block b1(graphic, RS_BlockData("*Model_Space", RS_Vector(0.0, 0.0), false));
        writeBlock(*dw, &b1);
        RS_Block b2(graphic, RS_BlockData("*Paper_Space", RS_Vector(0.0, 0.0), false));
        writeBlock(*dw, &b2);
        RS_Block b3(graphic, RS_BlockData("*Paper_Space0", RS_Vector(0.0, 0.0), false));
        writeBlock(*dw, &b3);
    }
...
void RS_FilterJWW::writeVariables(DL_WriterA& dw) {
        ...
        if (it.key() != "$ACADVER" && it.key() != "$HANDSEED") {
            ...
            switch (it.value().getType()) {
                ...
                case RS2::VariableString:
                    dw.dxfString(it.value().getCode(), it.value().getString().toLatin1().constData());
                    break;
    ...
    const QString dwgcode = variables.getString("$DWGCODEPAGE", "UNKNOWN");
    if (dwgcode == "UNKNOWN") {
...
void RS_FilterJWW::writeLayer(DL_WriterA& dw, RS_Layer* l) {
    if (!l) {
        RS_DEBUG->print(RS_Debug::D_WARNING, "RS_FilterJWW::writeLayer: layer is nullptr");
        return;
    }

    RS_DEBUG->print("RS_FilterJWW::writeLayer %s", l->getName().toLatin1().constData());

    jww.writeLayer(dw, DL_LayerData((const char*)l->getName().toLocal8Bit().data(), l->isFrozen() + (l->isLocked() << 2)), DL_Attributes(
                       std::string(""), colorToNumber(l->getPen().getColor()), widthToNumber(l->getPen().getWidth()),
                       (const char*)lineTypeToName(l->getPen().getLineType()).toLocal8Bit().data()));
 ...
 */
void RS_FilterJWW::writeLineType(DL_WriterA& dw, const RS2::LineType t) {
    jww.writeLineType(dw, DL_LineTypeData((const char*)lineTypeToName(t).toLocal8Bit().data(), 0));
}
...
void RS_FilterJWW::writeBlock(DL_WriterA& dw, RS_Block* blk) {
    if (!blk) {
        RS_DEBUG->print(RS_Debug::D_WARNING, "RS_FilterJWW::writeBlock: Block is nullptr");
        return;
    }

    RS_DEBUG->print("writing block: %s", (const char*)blk->getName().toLocal8Bit().data());

    jww.writeBlock(dw, DL_BlockData((const char*)blk->getName().toLocal8Bit().data(), 0, blk->getBasePoint().x, blk->getBasePoint().y,
                                    blk->getBasePoint().z));
    for (RS_Entity* e = blk->firstEntity(RS2::ResolveNone); e; e = blk->nextEntity(RS2::ResolveNone)) {
        writeEntity(dw, e);
    }
 ...
 * Writes the given entity to the JWW file.
 */
void RS_FilterJWW::writeEntity(DL_WriterA& dw, RS_Entity* e, const DL_Attributes& attrib) {
 ...
 */
void RS_FilterJWW::writePoint(DL_WriterA& dw, RS_Point* p, const DL_Attributes& attrib) {
    jww.writePoint(dw, DL_PointData(p->getPos().x, p->getPos().y, 0.0), attrib);
}

/**
 * Writes the given Line( entity to the file.
 */
void RS_FilterJWW::writeLine(DL_WriterA& dw, RS_Line* l, const DL_Attributes& attrib) {
    jww.writeLine(dw, DL_LineData(l->getStartpoint().x, l->getStartpoint().y, 0.0, l->getEndpoint().x, l->getEndpoint().y, 0.0), attrib);
}

/**
 * Writes the given polyline entity to the file.
 */
void RS_FilterJWW::writePolyline(DL_WriterA& dw, RS_Polyline* l, const DL_Attributes& attrib) {
    int count = l->count();
    if (l->isClosed() == false) {
        count++;
    }

    jww.writePolyline(dw, DL_PolylineData(count, 0, 0, l->isClosed() * 0x1), attrib);
    bool first = true;
    RS_Entity* nextEntity = nullptr;
    const RS_AtomicEntity* ae = nullptr;
    lc::LC_ContainerTraverser traverser{*l, RS2::ResolveNone};
    const RS_Entity* lastEntity = traverser.last();
    for (RS_Entity* v = traverser.first(); v != nullptr; v = nextEntity) {
        ...
        if (first) {
            ...
            jww.writeVertex(dw, DL_VertexData(ae->getStartpoint().x, ae->getStartpoint().y, 0.0, bulge));
            first = false;
        }
        ...
        if (l->isClosed() == false || v != lastEntity) {
            jww.writeVertex(dw, DL_VertexData(ae->getEndpoint().x, ae->getEndpoint().y, 0.0, bulge));
        }
    }
 ...
 */
void RS_FilterJWW::writeSpline(DL_WriterA& dw, RS_Spline* s, const DL_Attributes& attrib) {
    ...
    const size_t degree = static_cast<size_t>(s->getDegree());
    if (s->getNumberOfControlPoints() < degree + 1) {
        RS_DEBUG->print(RS_Debug::D_ERROR, "RS_FilterJWW::writeSpline: " "Discarding spline: not enough control points given.");
        return;
    }

    // Number of control points:
    const int numCtrl = s->getNumberOfControlPoints();

    // Number of knots (= number of control points + spline degree + 1)
    const int numKnots = numCtrl + s->getDegree() + 1;

    const int flags = s->isClosed() ? 11 : 8;

    // write spline header:
    jww.writeSpline(dw, DL_SplineData(s->getDegree(), numKnots, numCtrl, flags), attrib);

    const int k = s->getDegree() + 1;
    DL_KnotData kd;
 ...
 * Writes the given spline entity to the file.
 */
void RS_FilterJWW::writeSplinePoints(DL_WriterA& dw, LC_SplinePoints* s, const DL_Attributes& attrib) {
    // split spline into atomic entities for JWW R12:
    if (jww.getVersion() == VER_R12) {
    ...
    if (numCtrl < 3) {
        if (numCtrl > 1) {
            jww.writeLine(dw, DL_LineData(cp.at(0).x, cp.at(0).y, 0.0, cp.at(1).x, cp.at(1).y, 0.0), attrib);
        }
        return;
    }

    // Number of knots (= number of control points + spline degree + 1)
    const int numKnots = numCtrl + 3;
 ...
 */
void RS_FilterJWW::writeCircle(DL_WriterA& dw, RS_Circle* c, const DL_Attributes& attrib) {
    jww.writeCircle(dw, DL_CircleData(c->getCenter().x, c->getCenter().y, 0.0, c->getRadius()), attrib);
}

void RS_FilterJWW::writeArc(DL_WriterA& dw, RS_Arc* a, const DL_Attributes& attrib) {
    double a1, a2;
    if (a->isReversed()) {
    ...
    jww.writeArc(dw, DL_ArcData(a->getCenter().x, a->getCenter().y, 0.0, a->getRadius(), a1, a2), attrib);
}

void RS_FilterJWW::writeEllipse(DL_WriterA& dw, RS_Ellipse* s, const DL_Attributes& attrib) {
    if (s->isReversed()) {
        jww.writeEllipse(dw, DL_EllipseData(s->getCenter().x, s->getCenter().y, 0.0, s->getMajorP().x, s->getMajorP().y, 0.0, s->getRatio(),
                                            s->getAngle2(), s->getAngle1()), attrib);
    }
    else {
        jww.writeEllipse(dw, DL_EllipseData(s->getCenter().x, s->getCenter().y, 0.0, s->getMajorP().x, s->getMajorP().y, 0.0, s->getRatio(),
                                            s->getAngle1(), s->getAngle2()), attrib);
    }
}

void RS_FilterJWW::writeInsert(DL_WriterA& dw, RS_Insert* i, const DL_Attributes& attrib) {
    jww.writeInsert(dw, DL_InsertData(i->getName().toLatin1().constData(), i->getInsertionPoint().x, i->getInsertionPoint().y, 0.0,
                                      i->getScale().x, i->getScale().y, 0.0, RS_Math::rad2deg(i->getAngle()), i->getCols(), i->getRows(),
                                      i->getSpacing().x, i->getSpacing().y), attrib);
}

void RS_FilterJWW::writeText(DL_WriterA& dw, RS_MText* t, const DL_Attributes& attrib) {
    if (jww.getVersion() == VER_R12) {
        int hJust = 0;
        int vJust = 0;
        ...
        jww.writeText(dw, DL_TextData(t->getInsertionPoint().x, t->getInsertionPoint().y, 0.0, t->getInsertionPoint().x,
                                      t->getInsertionPoint().y, 0.0, t->getHeight(), 1.0, 0, hJust, vJust,
                                      (const char*)toDxfString(t->getText()).toLocal8Bit().data(),
                                      (const char*)t->getStyle().toLocal8Bit().data(), t->getAngle()), attrib);
    }
    else {
        ...
        jww.writeMText(dw, DL_MTextData(t->getInsertionPoint().x, t->getInsertionPoint().y, 0.0, t->getHeight(), t->getWidth(),
                                        attachmentPoint, t->getDrawingDirection(), t->getLineSpacingStyle(), t->getLineSpacingFactor(),
                                        (const char*)toDxfString(t->getText()).toLocal8Bit().data(),
                                        (const char*)t->getStyle().toLocal8Bit().data(), t->getAngle()), attrib);
    }
}

void RS_FilterJWW::writeDimension(DL_WriterA& dw, RS_Dimension* d, const DL_Attributes& attrib) {
    ...
    const DL_DimensionData dimData(d->getDefinitionPoint().x, d->getDefinitionPoint().y, 0.0, d->getMiddleOfText().x,
                                   d->getMiddleOfText().y, 0.0, type, attachmentPoint, d->getLineSpacingStyle(), d->getLineSpacingFactor(),
                                   (const char*)toDxfString(d->getText()).toLocal8Bit().data(),
                                   (const char*)d->getStyle().toLocal8Bit().data(), d->getTextAngle());

    if (d->rtti() == RS2::EntityDimAligned) {
        const RS_DimAligned* da = (RS_DimAligned*)d;

        const DL_DimAlignedData dimAlignedData(da->getExtensionPoint1().x, da->getExtensionPoint1().y, 0.0, da->getExtensionPoint2().x,
                                               da->getExtensionPoint2().y, 0.0);

        jww.writeDimAligned(dw, dimData, dimAlignedData, attrib);
    }
    else if (d->rtti() == RS2::EntityDimLinear) {
        // fixme - support of dim ordinate?
        const RS_DimLinear* dl = (RS_DimLinear*)d;

        const DL_DimLinearData dimLinearData(dl->getExtensionPoint1().x, dl->getExtensionPoint1().y, 0.0, dl->getExtensionPoint2().x,
                                             dl->getExtensionPoint2().y, 0.0, dl->getAngle(), dl->getOblique());

        jww.writeDimLinear(dw, dimData, dimLinearData, attrib);
    }
    else if (d->rtti() == RS2::EntityDimRadial) {
        const RS_DimRadial* dr = (RS_DimRadial*)d;

        const DL_DimRadialData dimRadialData(dr->getRadialDefinitionPoint().x, dr->getRadialDefinitionPoint().y, 0.0, dr->getLeader());

        jww.writeDimRadial(dw, dimData, dimRadialData, attrib);
    }
    else if (d->rtti() == RS2::EntityDimDiametric) {
        const RS_DimDiametric* dr = (RS_DimDiametric*)d;

        const DL_DimDiametricData dimDiametricData(dr->getDiametricDefinitionPoint().x, dr->getDiametricDefinitionPoint().y, 0.0, dr->getLeaderLength());

        jww.writeDimDiametric(dw, dimData, dimDiametricData, attrib);
    }
    else if (d->rtti() == RS2::EntityDimAngular) {
        const RS_DimAngular* da = (RS_DimAngular*)d;

        const DL_DimAngularData dimAngularData(da->getDefinitionPoint1().x, da->getDefinitionPoint1().y, 0.0, da->getDefinitionPoint2().x,
                                               da->getDefinitionPoint2().y, 0.0, da->getDefinitionPoint3().x, da->getDefinitionPoint3().y,
                                               0.0, da->getDefinitionPoint4().x, da->getDefinitionPoint4().y, 0.0);

        jww.writeDimAngular(dw, dimData, dimAngularData, attrib);
    }
}

void RS_FilterJWW::writeLeader(DL_WriterA& dw, RS_Leader* l, const DL_Attributes& attrib) {
    if (l->count() > 0) {
        jww.writeLeader(dw, DL_LeaderData(l->hasArrowHead(), 0, 3, 0, 0, 1.0, 10.0, l->count()), attrib);
        bool first = true;
        for (RS_Entity* v : lc::LC_ContainerTraverser{*l, RS2::ResolveNone}.entities()) {
            // Write line verties:
            if (v->rtti() == RS2::EntityLine) {
                const RS_Line* l = (RS_Line*)v;
                if (first) {
                    jww.writeLeaderVertex(dw, DL_LeaderVertexData(l->getStartpoint().x, l->getStartpoint().y, 0.0));
                    first = false;
                }
                jww.writeLeaderVertex(dw, DL_LeaderVertexData(l->getEndpoint().x, l->getEndpoint().y, 0.0));
            }
        }
    }
    else {
...
void RS_FilterJWW::writeHatch(DL_WriterA& dw, RS_Hatch* h, const DL_Attributes& attrib) {
    // split hatch into atomic entities:
    if (jww.getVersion() == VER_R12) {
    ...
    if (!writeIt) {
        RS_DEBUG->print(RS_Debug::D_WARNING, "RS_FilterJWW::writeHatch: Dropping Hatch");
    }
    else {
        const DL_HatchData data(h->countLoops(), h->isSolid(), h->getScale(), h->getAngle(),
        ...
        for (RS_Entity* l : lc::LC_ContainerTraverser{*h, RS2::ResolveNone}.entities()) {
            // Write hatch loops:
            if (l->isContainer() && !l->getFlag(RS2::FlagTemp)) {
                const RS_EntityContainer* loop = (RS_EntityContainer*)l;
                ...
                for (RS_Entity* ed : lc::LC_ContainerTraverser{*loop, RS2::ResolveNone}.entities()) {
                    ...
                    if (ed->rtti() == RS2::EntityLine) {
                        const RS_Line* ln = (RS_Line*)ed;
                        jww.writeHatchEdge(dw, DL_HatchEdgeData(ln->getStartpoint().x, ln->getStartpoint().y, ln->getEndpoint().x,
                                                                ln->getEndpoint().y));
                    }
                    else if (ed->rtti() == RS2::EntityArc) {
                        const RS_Arc* ar = (RS_Arc*)ed;
                        if (!ar->isReversed()) {
                            jww.writeHatchEdge(dw, DL_HatchEdgeData(ar->getCenter().x, ar->getCenter().y, ar->getRadius(), ar->getAngle1(),
                                                                    ar->getAngle2(), true));
                        }
                        else {
                            jww.writeHatchEdge(dw, DL_HatchEdgeData(ar->getCenter().x, ar->getCenter().y, ar->getRadius(),
                                                                    2 * M_PI - ar->getAngle1(), 2 * M_PI - ar->getAngle2(), false));
                        }
                    }
                    else if (ed->rtti() == RS2::EntityCircle) {
                        const RS_Circle* ci = (RS_Circle*)ed;
                        jww.writeHatchEdge(dw, DL_HatchEdgeData(ci->getCenter().x, ci->getCenter().y, ci->getRadius(), 0.0, 2 * M_PI,
                                                                true));
                    }
                }
...
void RS_FilterJWW::writeSolid(DL_WriterA& dw, RS_Solid* s, const DL_Attributes& attrib) {
    // split solid into line entities:
    //if (jww.getVersion()==VER_R12) {
    for (int i = 0; i < 3; ++i) {
        jww.writeLine(dw, DL_LineData(s->getCorner(i).x, s->getCorner(i).y, 0.0, s->getCorner((i + 1) % 3).x, s->getCorner((i + 1) % 3).y,
                                      0.0), attrib);
    }
    //return;
    //}
...
void RS_FilterJWW::writeImage(DL_WriterA& dw, RS_Image* i, const DL_Attributes& attrib) {
    const int handle = jww.writeImage(dw, DL_ImageData(std::string(""), i->getInsertionPoint().x, i->getInsertionPoint().y, 0.0,
                                                       i->getUVector().x, i->getUVector().y, 0.0, i->getVVector().x, i->getVVector().y, 0.0,
                                                       i->getWidth(), i->getHeight(), i->getBrightness(), i->getContrast(), i->getFade()),
                                      attrib);
    if (handle > 0) {
        i->setHandle(handle);
    }
...
void RS_FilterJWW::writeEntityContainer(DL_WriterA& dw, RS_EntityContainer* con, const DL_Attributes& /*attrib*/) {
    QString blkName;
    blkName = "__CE";
    ...
    //Block creation
    const RS_BlockData blkdata(blkName, RS_Vector(0, 0), false);

    const auto blk = new RS_Block(graphic, blkdata);

    for (const RS_Entity* e1 : lc::LC_ContainerTraverser{*con, RS2::ResolveNone}.entities()) {
        blk->addEntity(e1);
    }
    writeBlock(dw, blk);
    //delete e1;
 ...
 */
void RS_FilterJWW::writeAtomicEntities(DL_WriterA& dw, RS_EntityContainer* c, const DL_Attributes& attrib, const RS2::ResolveLevel level) {
...
void RS_FilterJWW::writeImageDef(DL_WriterA& dw, RS_Image* i) {
    ...
    jww.writeImageDef(dw, i->getHandle(), DL_ImageData((const char*)i->getFile().toLocal8Bit().data(), i->getInsertionPoint().x,
                                                       i->getInsertionPoint().y, 0.0, i->getUVector().x, i->getUVector().y, 0.0,
                                                       i->getVVector().x, i->getVVector().y, 0.0, i->getWidth(), i->getHeight(),
                                                       i->getBrightness(), i->getContrast(), i->getFade()));
}
 ...
 */
void RS_FilterJWW::setEntityAttributes(RS_Entity* entity, const DL_Attributes& attrib) {
    ...
    if (attrib.getLayer().empty()) {
        ...
        const auto encoding = QStringConverter::encodingForName(enc.toLatin1());
        if (encoding)
...
DL_Attributes RS_FilterJWW::getEntityAttributes(RS_Entity* entity) {
    ...
    // Color:
    const int color = colorToNumber(pen.getColor());
    ...
    // Width:
    const int width = widthToNumber(pen.getWidth());

    DL_Attributes attrib(layerName.toLocal8Bit().constData(), //2007-02-24 toLatin1().constData(),
                         color, width, lineType.toLocal8Bit().constData());
...
RS_Pen RS_FilterJWW::attributesToPen(const DL_Attributes& attrib) const {
    ...
    */
    ...
    RS_Pen pen(numberToColor(attrib.getColor()), numberToWidth(attrib.getWidth()), nameToLineType(attrib.getLineType().c_str()));
    return pen;
}
 ...
 */
RS_Color RS_FilterJWW::numberToColor(const int num, const bool comp) {
    ...
    if (comp) {
        ...
        if (num <= 255 && num >= 0) {
            return RS_Color((int)(dxfColors[num][0] * 255), (int)(dxfColors[num][1] * 255), (int)(dxfColors[num][2] * 255));
        }
        RS_DEBUG->print(RS_Debug::D_WARNING, "RS_FilterJWW::numberToColor: Invalid color number given.");
        return RS_Color(RS2::FlagByLayer);
    }
    return RS_Color();
}
...
int RS_FilterJWW::colorToNumber(const RS_Color& col) {
    ...
    for (int i = 1; i <= 255; i++) {
        const int d = abs(col.red() - (int)(dxfColors[i][0] * 255)) + abs(col.green() - (int)(dxfColors[i][1] * 255)) + abs(
            col.blue() - (int)(dxfColors[i][2] * 255));
 ...
 */
QString RS_FilterJWW::lineTypeToName(const RS2::LineType lineType) {
    // Standard linetypes for QCad II / AutoCAD
    switch (lineType) {
        case RS2::SolidLine:
            return "CONTINUOUS";
 ...
 */
int RS_FilterJWW::widthToNumber(const RS2::LineWidth width) {
    switch (width) {
...
QString RS_FilterJWW::toDxfString(const QString& string) {
    ...
    */
    ...
    for (int i = 0; i < string.length(); ++i) {
        const int c = string.at(i).unicode();
        switch (c) {
            case 0x0A:
                res += "\\P";
                break;
 ...
 */
RS2::AngleFormat RS_FilterJWW::numberToAngleFormat(const int num) {
    RS2::AngleFormat af;
 ...
 */
int RS_FilterJWW::angleFormatToNumber(const RS2::AngleFormat af) {
    int num;
 ...
 */
RS2::Unit RS_FilterJWW::numberToUnit(const int num) {
    switch (num) {
        default: case 0:
            return RS2::None;
            break;
 ...
 */
int RS_FilterJWW::unitToNumber(const RS2::Unit unit) {
    switch (unit) {
