...
bool LC_ParabolaSpline::isParabolaSpline(const DRW_Spline& s)
{
    ...
    if (!s.knotslist.empty()) {
        if (s.knotslist.size() != 6) return false;
    ...
    if (!s.weightlist.empty()) {
        if (s.weightlist.size() != 3) return false;
        for (double w : s.weightlist) {
            if (std::abs(w - 1.0) > kWeightTolerance) return false;
        }
    }
...
std::unique_ptr<LC_Parabola> LC_ParabolaSpline::splineToParabola(const DRW_Spline& s,
...
bool LC_ParabolaSpline::parabolaToSpline(const LC_ParabolaData& pd, DRW_Spline& spl)
{
    if (!pd.isValid()) {
        return false;
    }
