...
bool RS_FilterCXF::fileExport(RS_Graphic& g, const QString& file, RS2::FormatType /*type*/) {
    ...
    FILE* fp = fopen(file.toLocal8Bit(), "wt");
    ...
    if (fp != nullptr) {
        ...
        fprintf(fp, "# Creator:           %s\n", static_cast<const char*>(RS_SYSTEM->getAppName().toLocal8Bit()));
        fprintf(fp, "# Version:           %s\n", static_cast<const char*>(RS_SYSTEM->getAppVersion().toLocal8Bit()));
        ...
        const QString es = g.getVariableString("Encoding", "");
        if (!es.isEmpty()) {
                ...
                for (RS_Entity* e : lc::LC_ContainerTraverser{*blk, RS2::ResolveAll}.entities()) {
                    if (!e->isDeleted()) {
                        ...
                        if (e->rtti() == RS2::EntityLine) {
                            ...
                            fprintf(fp, "L %f,%f,%f,%f\n", l->getStartpoint().x, l->getStartpoint().y, l->getEndpoint().x,
                                    l->getEndpoint().y);
                        }
                        ...
                        // arcs:
                            ...
                            fprintf(fp, "%f,%f,%f,%f,%f\n", a->getCenter().x, a->getCenter().y, a->getRadius(),
                                    RS_Math::rad2deg(a->getAngle1()), RS_Math::rad2deg(a->getAngle2()));
                        }
 ...
 */
void RS_FilterCXF::stream(std::ofstream& fs, const double value) {
