...
void RS_FilterDXF::addBlock(const DL_BlockData& data) {
    ...
    if (QString(QString::fromUtf8(data.name.c_str())).toLower() != "*paper_space0" && QString(QString::fromUtf8(data.name.c_str())).
        toLower() != "*paper_space" && QString(QString::fromUtf8(data.name.c_str())).toLower() != "*model_space" &&
        QString(QString::fromUtf8(data.name.c_str())).toLower() != "$paper_space0" && QString(QString::fromUtf8(data.name.c_str())).
        toLower() != "$paper_space" && QString(QString::fromUtf8(data.name.c_str())).toLower() != "$model_space") {
        ...
        if (QString(QString::fromUtf8(data.name.c_str())).startsWith("__CE")) {
            ...
            RS_Block* block = new RS_Block(graphic, RS_BlockData(QString::fromUtf8(data.name.c_str()), bp, false));
...
void RS_FilterDXF::addArc(const DL_ArcData& data) {
    ...
    RS_ArcData d(v, data.radius, data.angle1 / ARAD, data.angle2 / ARAD, false);
    auto entity = new RS_Arc(currentContainer, d);
    setEntityAttributes(entity, attributes);
...
void RS_FilterDXF::addEllipse(const DL_EllipseData& data) {
    ...
    RS_EllipseData ed(v1, v2, data.ratio, data.angle1, data.angle2, false);
...
void RS_FilterDXF::addPolyline(const DL_PolylineData& data) {
    ...
    //RS_DEBUG->print("RS_FilterDXF::addPolyline()");
    RS_PolylineData d(RS_Vector(false), RS_Vector(false), data.flags & 0x1);
...
void RS_FilterDXF::addVertex(const DL_VertexData& data) {
    RS_DEBUG->print("RS_FilterDXF::addVertex(): %f/%f bulge: %f", data.x, data.y, data.bulge);
...
void RS_FilterDXF::addInsert(const DL_InsertData& data) {
    ...
    RS_InsertData d(data.name.c_str(), ip, sc, data.angle / ARAD, data.cols, data.rows, sp, NULL, RS2::NoUpdate);
...
void RS_FilterDXF::addMText(const DL_MTextData& data) {
    ...
    RS_TextData d(ip, data.height, data.width, valign, halign, dir, lss, data.lineSpacingFactor, mtext, sty, data.angle, RS2::NoUpdate);
...
void RS_FilterDXF::addText(const DL_TextData& data) {
    ...
    mtext = "";
    addMText(DL_MTextData(refPoint.x, refPoint.y,
#ifdef  RS_VECTOR2D
                          0.,
#else
                          refPoint.z,
#endif
                          data.height, width, attachmentPoint, drawingDirection, RS2::Exact, 1.0, data.text.c_str(), data.style, angle));
}
 ...
 */
RS_DimensionData RS_FilterDXF::convDimensionData(const DL_DimensionData& data) {
    ...
    return RS_DimensionData(defP, midP, valign, halign, lss, data.lineSpacingFactor, t, sty, data.angle);
}
 ...
 */
void RS_FilterDXF::addDimAlign(const DL_DimensionData& data, const DL_DimAlignedData& edata) {
    ...
    RS_DimAligned* entity = new RS_DimAligned(currentContainer, dimensionData, d);
    setEntityAttributes(entity, attributes);
    entity->update();
    currentContainer->addEntity(entity);
}
 ...
 */
void RS_FilterDXF::addDimLinear(const DL_DimensionData& data, const DL_DimLinearData& edata) {
    ...
    RS_DimLinearData d(dxt1, dxt2, RS_Math::deg2rad(edata.angle), RS_Math::deg2rad(edata.oblique));

    RS_DimLinear* entity = new RS_DimLinear(currentContainer, dimensionData, d);
    setEntityAttributes(entity, attributes);
    entity->update();
    currentContainer->addEntity(entity);
}
 ...
 */
void RS_FilterDXF::addDimRadial(const DL_DimensionData& data, const DL_DimRadialData& edata) {
    ...
    RS_DimRadial* entity = new RS_DimRadial(currentContainer, dimensionData, d);
 ...
 */
void RS_FilterDXF::addDimDiametric(const DL_DimensionData& data, const DL_DimDiametricData& edata) {
    ...
    RS_DimDiametric* entity = new RS_DimDiametric(currentContainer, dimensionData, d);
 ...
 */
void RS_FilterDXF::addDimAngular(const DL_DimensionData& data, const DL_DimAngularData& edata) {
    ...
    RS_DimAngular* entity = new RS_DimAngular(currentContainer, dimensionData, d);
 ...
 * angular dimensions (DIMENSION).
 */
void RS_FilterDXF::addDimAngular3P(const DL_DimensionData& data, const DL_DimAngular3PData& edata) {
...
void RS_FilterDXF::addHatch(const DL_HatchData& data) {
    ...
    hatch = new RS_Hatch(currentContainer,
                         RS_HatchData(data.solid, data.scale, data.angle, QString(QString::fromUtf8(data.pattern.c_str()))));
...
void RS_FilterDXF::addHatchEdge(const DL_HatchEdgeData& data) {
    ...
    if (hatchLoop != NULL && !omitHatchLoop) {
        RS_Entity* e = NULL;
        switch (data.type) {
            case 1: RS_DEBUG->print("RS_FilterDXF::addHatchEdge(): " "line: %f,%f %f,%f", data.x1, data.y1, data.x2, data.y2);
                e = new RS_Line(hatchLoop, RS_LineData(RS_Vector(data.x1, data.y1), RS_Vector(data.x2, data.y2)));
                break;
            case 2:
                if (data.ccw && data.angle1 < 1.0e-6 && data.angle2 > 2 * M_PI - 1.0e-6) {
                    e = new RS_Circle(hatchLoop, RS_CircleData(RS_Vector(data.cx, data.cy), data.radius));
                }
                else {
                    if (data.ccw) {
...
void RS_FilterDXF::addImage(const DL_ImageData& data) {
    ...
    RS_Image* image = new RS_Image(currentContainer,
                                   RS_ImageData(QString(data.ref.c_str()).toInt(NULL, 16), ip, uv, vv, size, QString(""), data.brightness,
...
void RS_FilterDXF::linkImage(const DL_ImageDefData& data) {
    ...
    for (RS_Entity* e = graphic->firstEntity(RS2::ResolveNone); e != NULL; e = graphic->nextEntity(RS2::ResolveNone)) {
...
bool RS_FilterDXF::fileExport(RS_Graphic& g, const QString& file, RS2::FormatType type) {
    RS_DEBUG->print("RS_FilterDXF::fileExport: exporting file '%s'...", (const char*)QFile::encodeName(file));
    ...
    dxf.writeDimStyle(*dw, graphic->getVariableDouble("$DIMASZ", 2.5), graphic->getVariableDouble("$DIMEXE", 1.25),
    ...
    if (type == RS2::FormatDXFOLD) {
        ...
        for (uint i = 0; i < graphic->countBlocks(); ++i) {
            ...
            dxf.writeBlockRecord(*dw, std::string(blk->getName().toLocal8Bit()));
            /*
            // v2.0.4.9..:
//writeBlock(*dw, blk);
...
dw->dxfHex(340, 0);
    ...
    if (type == RS2::FormatDXFOLD) {
        ...
        for (uint i = 0; i < graphic->countBlocks(); ++i) {
            ...
            for (RS_Entity* e = block->firstEntity(RS2::ResolveAll); e != NULL; e = block->nextEntity(RS2::ResolveAll)) {
                if (e->rtti() == RS2::EntityImage) {
                    RS_Image* img = ((RS_Image*)e);
        ...
        if (it.key() != "$ACADVER" && it.key() != "$HANDSEED") {
            ...
            switch (it.value().getType()) {
                ...
                case RS2::VariableString:
                    dw.dxfString(it.value().getCode(), it.value().getString().toLatin1());
                    break;
...
void RS_FilterDXF::writeLayer(DL_WriterA& dw, RS_Layer* l) {
    ...
    dxf.writeLayer(dw, DL_LayerData(toDxfString(l->getName()).toStdString(), //RLZ: verify layername whit locales
                                    l->isFrozen() + (l->isLocked() << 2), !l->isConstructionLayer()),
                   DL_Attributes(std::string(""), colorToNumber(l->getPen().getColor()), widthToNumber(l->getPen().getWidth()),
...
void RS_FilterDXF::writeBlock(DL_WriterA& dw, RS_Block* blk) {
    if (blk == NULL) {
        RS_DEBUG->print(RS_Debug::D_WARNING, "RS_FilterDXF::writeBlock: Block is NULL");
        return;
    }

    RS_DEBUG->print("writing block: %s", (const char*)blk->getName().toLocal8Bit());

    dxf.writeBlock(dw, DL_BlockData((const char*)blk->getName().toLocal8Bit(), 0, blk->getBasePoint().x, blk->getBasePoint().y,
#ifdef  RS_VECTOR2D
                                    0.));
#else
                                    blk->getBasePoint().z));
 ...
 */
void RS_FilterDXF::writePoint(DL_WriterA& dw, RS_Point* p, const DL_Attributes& attrib) {
    dxf.writePoint(dw, DL_PointData(p->getPos().x, p->getPos().y, 0.0), attrib);
}

/**
 * Writes the given Line( entity to the file.
 */
void RS_FilterDXF::writeLine(DL_WriterA& dw, RS_Line* l, const DL_Attributes& attrib) {
    dxf.writeLine(dw, DL_LineData(l->getStartpoint().x, l->getStartpoint().y, 0.0, l->getEndpoint().x, l->getEndpoint().y, 0.0), attrib);
}

/**
 * Writes the given polyline entity to the file.
 */
void RS_FilterDXF::writePolyline(DL_WriterA& dw, RS_Polyline* l, const DL_Attributes& attrib) {
    int count = l->count();
    if (l->isClosed() == false) {
        count++;
    }

    dxf.writePolyline(dw, DL_PolylineData(count, 0, 0, l->isClosed() * 0x1), attrib);
    bool first = true;
    RS_Entity* nextEntity = 0;
    RS_AtomicEntity* ae = NULL;
    RS_Entity* lastEntity = l->lastEntity(RS2::ResolveNone);
    for (RS_Entity* v = l->firstEntity(RS2::ResolveNone); v != NULL; v = nextEntity) {
        ...
        if (first) {
            ...
            dxf.writeVertex(dw, DL_VertexData(ae->getStartpoint().x, ae->getStartpoint().y, 0.0, bulge));
            first = false;
        }
        ...
        if (l->isClosed() == false || v != lastEntity) {
            dxf.writeVertex(dw, DL_VertexData(ae->getEndpoint().x, ae->getEndpoint().y, 0.0, bulge));
        }
    }
 ...
 * Writes the given spline entity to the file.
 */
void RS_FilterDXF::writeSplinePoints(DL_WriterA& dw, LC_SplinePoints* s, const DL_Attributes& attrib) {
    // split spline into atomic entities for DXF R12:
    if (dxf.getVersion() == VER_R12) {
    ...
    if (numCtrl < 3) {
        if (numCtrl > 1) {
            dxf.writeLine(dw, cp.at(0).x, cp.at(0).y, 0.0, cp.at(1).x, cp.at(1).y, 0.0), attrib
            )
            ;
        }
        return;
 ...
 */
void RS_FilterDXF::writeCircle(DL_WriterA& dw, RS_Circle* c, const DL_Attributes& attrib) {
    dxf.writeCircle(dw, DL_CircleData(c->getCenter().x, c->getCenter().y, 0.0, c->getRadius()), attrib);
}

void RS_FilterDXF::writeArc(DL_WriterA& dw, RS_Arc* a, const DL_Attributes& attrib) {
    double a1, a2;
    if (a->isReversed()) {
        a1 = a->getAngle2() * ARAD;
        a2 = a->getAngle1() * ARAD;
    ...
    dxf.writeArc(dw, DL_ArcData(a->getCenter().x, a->getCenter().y, 0.0, a->getRadius(), a1, a2), attrib);
}

void RS_FilterDXF::writeEllipse(DL_WriterA& dw, RS_Ellipse* s, const DL_Attributes& attrib) {
    if (s->isReversed()) {
        dxf.writeEllipse(dw, DL_EllipseData(s->getCenter().x, s->getCenter().y, 0.0, s->getMajorP().x, s->getMajorP().y, 0.0, s->getRatio(),
...
void RS_FilterDXF::writeInsert(DL_WriterA& dw, RS_Insert* i, const DL_Attributes& attrib) {
    dxf.writeInsert(dw, DL_InsertData(i->getName().toLatin1().data(), i->getInsertionPoint().x, i->getInsertionPoint().y, 0.0,
                                      i->getScale().x, i->getScale().y, 0.0, i->getAngle() * ARAD, i->getCols(), i->getRows(),
...
void RS_FilterDXF::writeText(DL_WriterA& dw, RS_Text* t, const DL_Attributes& attrib) {
    if (dxf.getVersion() == VER_R12) {
        ...
        dxf.writeText(dw, DL_TextData(t->getInsertionPoint().x, t->getInsertionPoint().y, 0.0, t->getInsertionPoint().x,
                                      t->getInsertionPoint().y, 0.0, t->getHeight(), 1.0, 0, hJust, vJust,
                                      (const char*)toDxfString(t->getText()).toLocal8Bit(), (const char*)t->getStyle().toLocal8Bit(),
                                      t->getAngle()), attrib);
    }
    else {
        ...
        dxf.writeMText(dw, DL_MTextData(t->getInsertionPoint().x, t->getInsertionPoint().y, 0.0, t->getHeight(), t->getWidth(),
...
void RS_FilterDXF::writeDimension(DL_WriterA& dw, RS_Dimension* d, const DL_Attributes& attrib) {
    ...
    DL_DimensionData dimData(d->getDefinitionPoint().x, d->getDefinitionPoint().y, 0.0, d->getMiddleOfText().x, d->getMiddleOfText().y, 0.0,
                             type, attachmentPoint, d->getLineSpacingStyle(), d->getLineSpacingFactor(),
                             (const char*)toDxfString(d->getText()).toLocal8Bit(), (const char*)d->getStyle().toLocal8Bit(), d->getAngle());

    if (d->rtti() == RS2::EntityDimAligned) {
        RS_DimAligned* da = (RS_DimAligned*)d;

        DL_DimAlignedData dimAlignedData(da->getExtensionPoint1().x, da->getExtensionPoint1().y, 0.0, da->getExtensionPoint2().x,
        ...
        DL_DimLinearData dimLinearData(dl->getExtensionPoint1().x, dl->getExtensionPoint1().y, 0.0, dl->getExtensionPoint2().x,
        ...
        DL_DimRadialData dimRadialData(dr->getDefinitionPoint().x, dr->getDefinitionPoint().y, 0.0, dr->getLeader());
        ...
        DL_DimDiametricData dimDiametricData(dr->getDefinitionPoint().x, dr->getDefinitionPoint().y, 0.0, dr->getLeader());
        ...
        DL_DimAngularData dimAngularData(da->getDefinitionPoint1().x, da->getDefinitionPoint1().y, 0.0, da->getDefinitionPoint2().x,
                                         da->getDefinitionPoint2().y, 0.0, da->getDefinitionPoint3().x, da->getDefinitionPoint3().y, 0.0,
                                         da->getDefinitionPoint4().x, da->getDefinitionPoint4().y, 0.0);

        dxf.writeDimAngular(dw, dimData, dimAngularData, attrib);
    }
}

void RS_FilterDXF::writeLeader(DL_WriterA& dw, RS_Leader* l, const DL_Attributes& attrib) {
    if (l->count() > 0) {
        dxf.writeLeader(dw, DL_LeaderData(l->hasArrowHead(), 0, 3, 0, 0, 1.0, 10.0, l->count()), attrib);
        bool first = true;
        for (RS_Entity* v = l->firstEntity(RS2::ResolveNone); v != NULL; v = l->nextEntity(RS2::ResolveNone)) {
            // Write line verties:
            if (v->rtti() == RS2::EntityLine) {
                RS_Line* l = (RS_Line*)v;
                if (first) {
                    dxf.writeLeaderVertex(dw, DL_LeaderVertexData(l->getStartpoint().x, l->getStartpoint().y, 0.0));
                    first = false;
                }
                dxf.writeLeaderVertex(dw, DL_LeaderVertexData(l->getEndpoint().x, l->getEndpoint().y, 0.0));
            }
        }
    }
    else {
...
void RS_FilterDXF::writeHatch(DL_WriterA& dw, RS_Hatch* h, const DL_Attributes& attrib) {
    // split hatch into atomic entities:
    if (dxf.getVersion() == VER_R12) {
    ...
    if (h->countLoops() > 0) {
        ...
        for (RS_Entity* l = h->firstEntity(RS2::ResolveNone); l != NULL; l = h->nextEntity(RS2::ResolveNone)) {
    ...
    if (!writeIt) {
        ...
        DL_HatchData data(h->countLoops(), h->isSolid(), h->getScale(), h->getAngle(), (const char*)h->getPattern().toLocal8Bit());
        ...
        for (RS_Entity* l = h->firstEntity(RS2::ResolveNone); l != NULL; l = h->nextEntity(RS2::ResolveNone)) {
            ...
            if (l->isContainer() && !l->getFlag(RS2::FlagTemp)) {
                ...
                for (RS_Entity* ed = loop->firstEntity(RS2::ResolveNone); ed != NULL; ed = loop->nextEntity(RS2::ResolveNone)) {
                    // Write hatch loop edges:
                    if (ed->rtti() == RS2::EntityLine) {
                        RS_Line* ln = (RS_Line*)ed;
                        dxf.writeHatchEdge(dw, DL_HatchEdgeData(ln->getStartpoint().x, ln->getStartpoint().y, ln->getEndpoint().x,
                                                                ln->getEndpoint().y));
                    }
                    else if (ed->rtti() == RS2::EntityArc) {
                        RS_Arc* ar = (RS_Arc*)ed;
                        if (!ar->isReversed()) {
                            dxf.writeHatchEdge(dw, DL_HatchEdgeData(ar->getCenter().x, ar->getCenter().y, ar->getRadius(), ar->getAngle1(),
                                                                    ar->getAngle2(), true));
                        }
                        else {
                            dxf.writeHatchEdge(dw, DL_HatchEdgeData(ar->getCenter().x, ar->getCenter().y, ar->getRadius(),
                                                                    2 * M_PI - ar->getAngle1(), 2 * M_PI - ar->getAngle2(), false));
                        }
                    }
                    else if (ed->rtti() == RS2::EntityCircle) {
                        RS_Circle* ci = (RS_Circle*)ed;
                        dxf.writeHatchEdge(dw, DL_HatchEdgeData(ci->getCenter().x, ci->getCenter().y, ci->getRadius(), 0.0, 2 * M_PI,
                                                                true));
                    }
                }
...
void RS_FilterDXF::writeSolid(DL_WriterA& dw, RS_Solid* s, const DL_Attributes& attrib) {
    // split solid into line entities:
    //if (dxf.getVersion()==VER_R12) {
    for (int i = 0; i < 3; ++i) {
        dxf.writeLine(dw, DL_LineData(s->getCorner(i).x, s->getCorner(i).y, 0.0, s->getCorner((i + 1) % 3).x, s->getCorner((i + 1) % 3).y,
                                      0.0), attrib);
    }
    //return;
    //}
...
void RS_FilterDXF::writeImage(DL_WriterA& dw, RS_Image* i, const DL_Attributes& attrib) {
    int handle = dxf.writeImage(dw, DL_ImageData(std::string(""), i->getInsertionPoint().x, i->getInsertionPoint().y, 0.0,
...
void RS_FilterDXF::writeEntityContainer(DL_WriterA& dw, RS_EntityContainer* con, const DL_Attributes& /*attrib*/) {
    QString blkName;
    blkName = "__CE";
    ...
    for (RS_Entity* e1 = con->firstEntity(); e1 != NULL; e1 = con->nextEntity()) {
        blk->addEntity(e1);
    }
    writeBlock(dw, blk);
    //delete e1;
 ...
 */
void RS_FilterDXF::writeAtomicEntities(DL_WriterA& dw, RS_EntityContainer* c, const DL_Attributes& attrib, RS2::ResolveLevel level) {
    for (RS_Entity* e = c->firstEntity(level); e != NULL; e = c->nextEntity(level)) {
...
void RS_FilterDXF::writeImageDef(DL_WriterA& dw, RS_Image* i) {
    ...
    dxf.writeImageDef(dw, i->getHandle(), DL_ImageData((const char*)i->getFile().toLocal8Bit(), i->getInsertionPoint().x,
 ...
 */
void RS_FilterDXF::setEntityAttributes(RS_Entity* entity, const DL_Attributes& attrib) {
...
DL_Attributes RS_FilterDXF::getEntityAttributes(RS_Entity* entity) {
    ...
    DL_Attributes attrib(toDxfString(layerName).toStdString(), color, width, lineType.toLatin1().data());
...
int RS_FilterDXF::colorToNumber(const RS_Color& col) {
    ...
    // All other colors
        ...
        for (int i = 1; i <= 255; i++) {
            int d = abs(col.red() - (int)(dxfColors[i][0] * 255)) + abs(col.green() - (int)(dxfColors[i][1] * 255)) + abs(
...
RS2::LineType RS_FilterDXF::nameToLineType(const QString& name) {
    ...
    else if (uName == "ACAD_ISO02W100" || uName == "ACAD_ISO03W100" || uName == "DASHED" || uName == "HIDDEN") {
        return RS2::DashLine;
    }
