...
TEST_CASE("A degenerate Z scale never survives", "[insert][zscale]") {
    // Anything that leaves the block transform singular or meaningless has to
    // become 1. An exact "!= 0.0" test passes all but the first two of these.
    CHECK(RS_InsertData::usableScale(0.0) == Catch::Approx(1.0));
    CHECK(RS_InsertData::usableScale(-0.0) == Catch::Approx(1.0));
    CHECK(RS_InsertData::usableScale(1.0e-300) == Catch::Approx(1.0));
    // Below ezdxf's own 1e-12 threshold, so it would rewrite the value anyway.
    CHECK(RS_InsertData::usableScale(1.0e-13) == Catch::Approx(1.0));
    ...
    // Usable factors are data and pass through untouched, mirrors included.
...
TEST_CASE("A degenerate X or Y scale never survives either", "[insert][zscale]") {
    ...
    CHECK(repaired.y == Catch::Approx(1.0));
