...
struct LC_ActionDrawCircleTangental3Entities::ActionData {
    std::vector<RS_AtomicEntity*> circles;
    std::shared_ptr<RS_CircleData> circleData{std::make_shared<RS_CircleData>()};
    RS_Vector coord;
    bool valid{false};
 ...
 *
 */
LC_ActionDrawCircleTangental3Entities::LC_ActionDrawCircleTangental3Entities(LC_ActionContext* actionContext)
    : LC_ActionDrawCircleBase("ActionDrawCircleTan3Entities", actionContext, RS2::ActionDrawCircleTan3Entities),
      m_actionData(std::make_unique<ActionData>()) {
}

LC_ActionDrawCircleTangental3Entities::~LC_ActionDrawCircleTangental3Entities() = default;

void LC_ActionDrawCircleTangental3Entities::drawSnapper() {
    // disable snapper
}

void LC_ActionDrawCircleTangental3Entities::init(const int status) {
...
void LC_ActionDrawCircleTangental3Entities::doInitialInit() {
    m_actionData->circles.clear();
}

void LC_ActionDrawCircleTangental3Entities::doInitWithContextEntity(RS_Entity* contextEntity, [[maybe_unused]] const RS_Vector& clickPos) {
    if (g_enTypeList.contains(contextEntity->rtti())) {
        setCircle(contextEntity, SetCircle1);
    }
}

void LC_ActionDrawCircleTangental3Entities::finish() {
...
RS_Entity* LC_ActionDrawCircleTangental3Entities::doTriggerCreateEntity() {
    const auto circle = new RS_Circle(m_document, *m_actionData->circleData);
...
void LC_ActionDrawCircleTangental3Entities::doTriggerCompletion([[maybe_unused]] bool success) {
    m_actionData->circles.clear();
    setStatus(SetCircle1);
}

void LC_ActionDrawCircleTangental3Entities::onMouseMoveEvent(const int status, const LC_MouseEvent* e) {
    ...
    switch (status) {
        case SetCircle1:
        case SetCircle2: {
            const auto* c = catchCircle(e, true); // fixme - sand - CHECK! Modifiable catch for preview?
            if (c != nullptr) {
                highlightHover(c);
            }
            break;
        ...
        case SetCircle3: {
            ...
            if (c != nullptr) {
                const bool canBuildCircle = getData(c);
                if (canBuildCircle) {
                    highlightHover(c);
                }
            }
        ...
        case SetCenter: {
            ...
            if (preparePreview()) {
                ...
                for (const auto& c : m_actionData->candidates) {
...
bool LC_ActionDrawCircleTangental3Entities::getData(RS_Entity* testThirdEntity) const {
    ...
    size_t i = 0;
    const size_t countLines = std::count_if(circlesList.begin(), circlesList.end(), [](const RS_AtomicEntity* e) -> bool {
    ...
    if (i < circlesList.size() && isLine(circlesList[i])) {
        ...
        switch (countLines) {
            ...
            case 1: {
                ...
                for (unsigned k = 0; k < 4; ++k) {
                    ...
                    auto lc2 = LC_Quadratic(circlesList[i], circlesList[i2], k & 2U);
            ...
            case 2: {
                ...
                for (unsigned k = 0; k < 4; ++k) {
                    ...
                    auto lc2 = LC_Quadratic(circlesList[i2], circlesList[i1], k & 2U);
                    sol.push_back(LC_Quadratic::getIntersection(lc1, lc2));
                }
                break;
            }
            case 3: {
                ...
                auto line0 = static_cast<RS_Line*>(circlesList[i]);
                auto line1 = static_cast<RS_Line*>(circlesList[i1]);
                auto line2 = static_cast<RS_Line*>(circlesList[i2]);
        ...
        for (int j = 1; j <= 2; j++) {
            if (circlesList[(i + j) % 3]->rtti() == RS2::EntityCircle) {
                ...
                circlesList[i]->getNearestPointOnEntity(circlesList[(i + j) % 3]->getCenter(), false, &d, nullptr);
        ...
        for (const auto& v : sol1) {
            double d = RS_MAXDOUBLE;
            circlesList[i]->getNearestPointOnEntity(v, false, &d, nullptr);
            ...
            for (const auto& circleData : data) {
                if (circlesList[(i + 1) % 3]->isTangent(*circleData) && circlesList[(i + 2) % 3]->isTangent(*circleData)) {
...
bool LC_ActionDrawCircleTangental3Entities::preparePreview() const {
    ...
    const auto& candidates = m_actionData->candidates;
    //find the nearest circle
    ...
    for (size_t i = 0; i < candidates.size(); ++i) {
        ...
        double d = RS_MAXDOUBLE;
        RS_Circle(nullptr, *candidates.at(i)).getNearestPointOnEntity(m_actionData->coord, false, &d);
        double dCenter = m_actionData->coord.distanceTo(candidates.at(i)->center);
        d = std::min<double>(d, dCenter);
        if (d < dist) {
            dist = d;
            index = i;
        }
    ...
    if (index < candidates.size()) {
        m_actionData->circleData = candidates.at(index);
...
void LC_ActionDrawCircleTangental3Entities::setCircle(RS_Entity* en, const int status) {
...
void LC_ActionDrawCircleTangental3Entities::onMouseLeftButtonRelease(const int status, const LC_MouseEvent* e) {
    switch (status) {
        case SetCircle1:
        case SetCircle2: {
...
void LC_ActionDrawCircleTangental3Entities::onMouseRightButtonRelease(const int status, [[maybe_unused]] const LC_MouseEvent* e) {
...
void LC_ActionDrawCircleTangental3Entities::updateActionPrompt() {
    switch (getStatus()) {
        case SetCircle1:
        ...
        case SetCircle2:
            ...
            break;
        case SetCircle3:
            updatePromptTRBack(tr("Specify the third line/arc/circle"));
            break;
        case SetCenter:
...
RS2::CursorType LC_ActionDrawCircleTangental3Entities::doGetMouseCursor([[maybe_unused]] int status) {
    return RS2::SelectCursor;
}

RS_Vector LC_ActionDrawCircleTangental3Entities::getTangentPoint(const RS_Vector& creatingCircleCenter, const double creatingCircleRadius,
                                                   const RS_AtomicEntity* const circle) {
    const bool calcTangentFromOriginalCircle = (creatingCircleCenter.distanceTo(circle->getCenter()) < circle->getRadius()) && (
