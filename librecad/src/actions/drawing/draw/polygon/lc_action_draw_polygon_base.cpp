...
LC_ActionDrawPolygonBase::LC_ActionDrawPolygonBase( const QString& name, LC_ActionContext *actionContext, const RS2::ActionType actionType)
...
LC_ActionDrawPolygonBase::~LC_ActionDrawPolygonBase() = default;

void LC_ActionDrawPolygonBase::doSaveOptions() {
    save("Number", m_edgesNumber);
    save("Polyline", m_createPolyline);
    save("Rounded", m_roundedCorners);
    save("Radius", m_roundingRadius);
}

void LC_ActionDrawPolygonBase::doLoadOptions() {
    m_edgesNumber = loadInt("Number", 5);
    m_createPolyline = loadBool("Polyline", false);
    m_roundedCorners = loadBool("Rounded", false);
...
void LC_ActionDrawPolygonBase::doTriggerCompletion([[maybe_unused]]bool success) {
    if (m_completeActionOnTrigger) {
        setStatus(-1);
    }
}

bool LC_ActionDrawPolygonBase::doUpdateDistanceByInteractiveInput(const QString& tag, const double distance) {
    if (tag == "radius") {
...
void LC_ActionDrawPolygonBase::onMouseMoveEvent(const int status, const LC_MouseEvent* e) {
...
void LC_ActionDrawPolygonBase::onMouseLeftButtonRelease(const int status, const LC_MouseEvent* e) {
...
void LC_ActionDrawPolygonBase::onMouseRightButtonRelease(const int status, [[maybe_unused]] const LC_MouseEvent* e) {
    deletePreview();
    initPrevious(status);
}

void LC_ActionDrawPolygonBase::onCoordinateEvent(const int status,  [[maybe_unused]]bool isZero, const RS_Vector &coord) {
    switch (status) {
        case SetPoint1: {
...
bool LC_ActionDrawPolygonBase::doProcessCommand(int status, const QString &command) {
    bool accept = false;
...
QStringList LC_ActionDrawPolygonBase::getAvailableCommands() {
    QStringList cmd;
    switch (getStatus()) {
        case SetPoint1:
        case SetPoint2:
...
void LC_ActionDrawPolygonBase::updateActionPrompt() {
    switch (getStatus()) {
        case SetPoint1:
            updatePromptTRCancel(getPoint1Hint(), MOD_SHIFT_RELATIVE_ZERO);
            break;
        case SetPoint2:
            updatePromptTRBack(getPoint2Hint(), MOD_SHIFT_ANGLE_SNAP);
            break;
        case SetNumber:
            updatePromptTRBack(tr("Enter number:"));
            break;
        case SetRadius:
            updatePromptTRBack(tr("Enter rounding radius:"));
            break;
        default:
            updatePrompt();
            break;
...
QString LC_ActionDrawPolygonBase::getPoint1Hint() const { return tr("Specify center"); }

RS2::CursorType LC_ActionDrawPolygonBase::doGetMouseCursor([[maybe_unused]] int status){
    return RS2::CadCursor;
}

LC_ActionOptionsWidget* LC_ActionDrawPolygonBase::createOptionsWidget(){
    return new LC_PolygonOptionsWidget();
}

LC_ActionOptionsPropertiesFiller* LC_ActionDrawPolygonBase::createOptionsFiller() {
    return new LC_PolygonOptionsFiller();
}

#define DRAW_JOIN_POINTS_ON_PREVIEW true

void LC_ActionDrawPolygonBase::createPolygonPreview(const RS_Vector &mouse) {
    PolygonInfo polygonInfo;
...
RS_Polyline *LC_ActionDrawPolygonBase::createShapePolyline(PolygonInfo &polygonInfo, bool preview) {
