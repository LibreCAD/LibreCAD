...
LC_ActionDrawStar::LC_ActionDrawStar(LC_ActionContext* actionContext)
    : LC_AbstractActionWithPreview("ActionDrawStar", actionContext, RS2::ActionDrawStar) {
}

void LC_ActionDrawStar::doSaveOptions() {
    save("Number", m_raysNumber);
    save("InnerRound", m_innerRadiusRounded);
    save("InnerRadius", m_innerRadius);
    save("OuterRound", m_outerRadiusRounded);
    save("OuterRadius", m_outerRadius);
    save("Symmetric", m_symmetric);
    save("Polyline", m_createPolyline);
}

void LC_ActionDrawStar::doLoadOptions() {
    m_raysNumber = loadInt("Number", 5);
    m_innerRadiusRounded = loadBool("InnerRound", false);
    m_innerRadius = loadDouble("InnerRadius", 10.0);
    m_outerRadiusRounded = loadBool("OuterRound", false);
    m_outerRadius = loadDouble("OuterRadius", 10.0);
    m_symmetric = loadBool("Symmetric", true);
 ...
 * @param status
 */
void LC_ActionDrawStar::doPreparePreviewEntities([[maybe_unused]] const LC_MouseEvent* e, RS_Vector& snap, QList<RS_Entity*>& list,
                                                 const int status) {
...
bool LC_ActionDrawStar::doProcessCommand([[maybe_unused]] int status, const QString& c) {
    bool processed = true;
    if (checkCommand("nopoly", c)) {
        // don't create polyline
        m_createPolyline = false;
        updateOptions();
    }
    else if (checkCommand("usepoly", c)) {
        // draw star by single polyline
        ...
        // setting rounding radiuses
        setStatus(SetRadiuses);
    }
    else if (checkCommand("sym", c)) {
        // draw symmetric star
        m_symmetric = true;
        updateOptions();
    }
    else if (checkCommand("nosym", c)) {
        // draw asymmetric star
        m_symmetric = false;
        updateOptions();
    }
    else {
...
RS_Polyline* LC_ActionDrawStar::createShapePolyline(RS_Vector& snap, QList<RS_Entity*>& list, int status, bool preview) {
    ...
    if (m_outerRadiusRounded) {
        ...
        RS_Vector outerIntersection = LC_LineMath::getIntersectionLineLine(par1data.startpoint, par1data.endpoint, par2data.startpoint,
                                                                           par2data.endpoint);
        ...
        if (mayDrawOuterRound) {
            ...
            if (preview) {
                ...
                RS_Arc* outerArc = new RS_Arc(nullptr, RS_ArcData(outerIntersection, outerRadius, outerAng2, outerAng1, outerReversed));
    ...
    if (m_innerRadiusRounded) {
        ...
        RS_Vector innerIntersection = LC_LineMath::getIntersectionLineLine(par1data.startpoint, par1data.endpoint, par2data.startpoint,
                                                                           par2data.endpoint);
        ...
        if (mayDrawInnerRound) {
            ...
            if (preview) {
                ...
                RS_Arc* innerArc = new RS_Arc(nullptr, RS_ArcData(innerIntersection, innerRadius, innerAng2, innerAng1, innerReversed));
    ...
    for (int i = 0; i < m_raysNumber; ++i) {
        ...
        if (mayDrawInnerRound) {
            // add bulge and second rounding join
...
void LC_ActionDrawStar::updateActionPrompt() {
    switch (getStatus()) {
        case SetCenter:
            updatePromptTRCancel(tr("Set center point"), MOD_SHIFT_RELATIVE_ZERO);
            break;
        case SetOuterPoint:
            updatePromptTRBack(tr("Set outer point"), MOD_SHIFT_ANGLE_SNAP);
            break;
        case SetInnerPoint:
            updatePromptTRBack(tr("Set inner point"), MOD_SHIFT_ANGLE_SNAP);
            break;
        case SetRadiuses:
            updatePromptTRBack(tr("Set rounding radiuses (outer, inner)"));
            break;
        case SetRays:
            updatePromptTRBack(tr("Set rays number"));
            break;
        default:
...
LC_ActionOptionsWidget* LC_ActionDrawStar::createOptionsWidget() {
    return new LC_StarOptionsWidget();
}

LC_ActionOptionsPropertiesFiller* LC_ActionDrawStar::createOptionsFiller() {
    return new LC_StarOptionsFiller();
}
