...
LC_ActionModifyBreakDivide::LC_ActionModifyBreakDivide(LC_ActionContext *actionContext)
   :LC_AbstractActionWithPreview("ActionModifyBreakDivide",actionContext, RS2::ActionModifyBreakDivide){
}

void LC_ActionModifyBreakDivide::doSaveOptions() {
    save("RemoveSegments", m_removeSegments);
    save("RemoveSelected", m_removeSelected);
}

void LC_ActionModifyBreakDivide::doLoadOptions() {
    m_removeSegments = loadBool("RemoveSegments", true);
    m_removeSelected = loadBool("RemoveSelected", true);
}
...
void LC_ActionModifyBreakDivide::updateActionPrompt(){
    updatePromptTRCancel(tr("Select line, arc or circle"), m_removeSegments ? MOD_SHIFT_LC(tr("Proceed even if no intersections")):MOD_NONE);
}

LC_ActionOptionsWidget* LC_ActionModifyBreakDivide::createOptionsWidget(){
    return new LC_BreakDivideOptionsWidget();
}

LC_ActionOptionsPropertiesFiller* LC_ActionModifyBreakDivide::createOptionsFiller() {
    return new LC_BreakDivideOptionsFiller();
}
