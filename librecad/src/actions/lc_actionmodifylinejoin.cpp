...
RS_Line *LC_ActionModifyLineJoin::catchLine(QMouseEvent *e){
    ...
    // Lines generated by a block reference, dimension, hatch, text or spline are not
    // joinable: removing the original would leave it inside its parent, and the pick
    // is kept across clicks, while such a parent rebuilds (frees) its children on
    // undo. This is the same rule master applies.
