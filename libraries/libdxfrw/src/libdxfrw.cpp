...
void dxfRW::resetDxfWriteSession() {
    ...
    m_handleRemap = m_explicitHandleRemap;
    ...
    for (std::size_t i = 0; i < normalized.segments.size(); ++i) {
        ...
        if (version > DRW::AC1009) {
            ...
            if (segment.shapeFlags != 0) {
                ...
                const std::uint32_t styleHandle =
                    resolveReference(segment.styleHandle.ref);
                if (styleHandle != 0)
                    write(writer->writeString(340, toHexStr(styleHandle)));
...
bool dxfRW::writeVport(DRW_Vport *ent){
    ...
    if (version > DRW::AC1014) {
        ...
        const std::uint32_t namedUcs = resolveReference(ent->namedUcsHandle);
        const std::uint32_t baseUcs = resolveReference(ent->baseUcsHandle);
        if (namedUcs != 0)
            writer->writeString(345, toHexStr(namedUcs));
        if (baseUcs != 0)
            writer->writeString(346, toHexStr(baseUcs));
        if (version > DRW::AC1018) {
            ...
            const std::uint32_t background =
                resolveReference(ent->backgroundHandle);
            const std::uint32_t visualStyle =
                resolveReference(ent->visualStyleHandle);
            const std::uint32_t sun = resolveReference(ent->m_sunHandle);
            if (background != 0)
                writer->writeString(332, toHexStr(background));
            if (visualStyle != 0)
                writer->writeString(348, toHexStr(visualStyle));
            if (sun != 0)
                writer->writeString(361, toHexStr(sun));
    }
    }
...
bool dxfRW::writeView(DRW_View *ent){
    ...
    if (version > DRW::AC1018) {
        ...
        const std::uint32_t background =
            resolveReference(ent->m_backgroundHandle);
        const std::uint32_t visualStyle =
            resolveReference(ent->m_visualStyleHandle);
        const std::uint32_t sun = resolveReference(ent->m_sunHandle);
        const std::uint32_t liveSection =
            resolveReference(ent->m_liveSectionHandle);
        if (background != 0)
            writer->writeString(332, toHexStr(background));
        if (visualStyle != 0)
            writer->writeString(348, toHexStr(visualStyle));
        if (sun != 0)
            writer->writeString(361, toHexStr(sun));
        if (liveSection != 0)
            writer->writeString(334, toHexStr(liveSection));
    }
    if (version > DRW::AC1009 && ent->hasUCS) {
        const std::uint32_t baseUcs = resolveReference(ent->baseUCS_ID);
        const std::uint32_t namedUcs = resolveReference(ent->namedUCS_ID);
        if (baseUcs != 0)
            writer->writeString(346, toHexStr(baseUcs));
        if (namedUcs != 0)
            writer->writeString(345, toHexStr(namedUcs));
...
bool dxfRW::writeUCS(DRW_UCS *ent){
    ...
    if (version > DRW::AC1014) {
        ...
        const std::uint32_t baseUcs =
            resolveReference(ent->baseUcsHandle.ref);
        const std::uint32_t namedUcs =
            resolveReference(ent->namedUcsHandle.ref);
        if (baseUcs != 0)
            write(writer->writeString(346, toHexStr(baseUcs)));
        if (namedUcs != 0)
            write(writer->writeString(345, toHexStr(namedUcs)));
    }
    if (!writeTableEntryAppData(*ent))
        result = false;
    if (!ent->extData.empty())
...
bool dxfRW::writeTables() {
    ...
    // VPORT records may refer to named UCS table records, so emit UCS first.
    ...
    if (hasWriteFailure())
        return false;
    iface->writeUCSs();
    if (hasWriteFailure() || !writeEndTable())
        return false;
...
bool dxfRW::writeLayout(DRW_Layout *ent) {
    ...
    const std::uint32_t shadePlot = resolveReference(ent->shadePlotHandle.ref);
    const std::uint32_t paperSpace =
        resolveReference(ent->paperSpaceBlockRecordHandle.ref);
    const std::uint32_t activeViewport =
        resolveReference(ent->lastActiveViewportHandle.ref);
    const std::uint32_t namedUcs = resolveReference(ent->namedUcsHandle.ref);
    const std::uint32_t baseUcs = resolveReference(ent->baseUcsHandle.ref);
    if (shadePlot != 0)
        writer->writeString(333, toHexStr(shadePlot));
    if (paperSpace != 0)
        writer->writeString(330, toHexStr(paperSpace));
    if (activeViewport != 0)
        writer->writeString(331, toHexStr(activeViewport));
    if (namedUcs != 0)
        writer->writeString(345, toHexStr(namedUcs));
    if (baseUcs != 0)
        writer->writeString(346, toHexStr(baseUcs));
...
bool dxfRW::writeGeoData(DRW_GeoData *ent) {
    ...
    const std::uint32_t hostBlock = resolveReference(ent->m_hostBlockHandle);
    if (hostBlock != 0)
        writer->writeString(330, toHexStr(hostBlock));
...
bool dxfRW::writeField(DRW_Field *ent) {
    ...
    const std::vector<std::uint32_t> childHandles =
        resolveReferences(ent->m_childHandles);
    const std::vector<std::uint32_t> objectHandles =
        resolveReferences(ent->m_objectHandles);
    written = written && writer->writeInt32(
        90, static_cast<int>(childHandles.size()));
    for (std::uint32_t child : childHandles)
        written = written && writeString(360, toHexStr(child));
    written = written && writer->writeInt32(
        97, static_cast<int>(objectHandles.size()));
    for (std::uint32_t object : objectHandles)
    ...
    auto writeChildValue = [this](const DRW_Field::ChildValue &child) {
        DRW_CadValue value = child.m_value;
        if (value.m_dataType == 64) {
            const std::uint32_t sourceHandle = value.m_handle != 0
                ? value.m_handle
                : static_cast<std::uint32_t>(value.m_value.type()
                        == DRW_Variant::INTEGER
                    ? value.m_value.i_val() : 0);
            value.m_handle = resolveReference(sourceHandle);
            if (value.m_value.type() == DRW_Variant::INTEGER)
                value.m_value.addInt(value.m_value.code(),
                                     static_cast<std::int32_t>(value.m_handle));
        }
        return writer->writeUtf8String(6, child.m_key)
            && writeDxfFieldValue(writer.get(), version, value);
    };
    ...
    if (dxfFieldValueHasData(ent->m_value)) {
        DRW_CadValue value = ent->m_value;
        if (value.m_dataType == 64) {
            const std::uint32_t sourceHandle = value.m_handle != 0
                ? value.m_handle
                : static_cast<std::uint32_t>(value.m_value.type()
...
bool dxfRW::writeFieldList(DRW_FieldList *ent) {
    ...
    fieldHandles.reserve(ent->m_fieldHandles.size());
    for (const std::uint32_t field : ent->m_fieldHandles) {
        const std::uint32_t resolved = resolveReference(field);
        if (field == 0 || resolved != 0)
            fieldHandles.push_back(resolved);
    }
    written = written && writeString(100, "AcDbIdSet")
        && writer->writeInt32(90, static_cast<int>(fieldHandles.size()))
        && writer->writeBool(290, ent->m_unknown != 0);
    for (std::uint32_t field : fieldHandles)
...
bool dxfRW::writeMLeaderStyle(DRW_MLeaderStyle *ent) {
    ...
    writer->writeInt32(91, ent->leaderColor);
    const std::uint32_t leaderLineType =
        resolveReference(ent->leaderLineTypeHandle.ref);
    if (leaderLineType != 0)
        writer->writeString(340, toHexStr(leaderLineType));
    ...
    const std::uint32_t arrowHeadBlock =
        resolveReference(ent->arrowHeadBlockHandle.ref);
    if (arrowHeadBlock != 0)
        writer->writeString(341, toHexStr(arrowHeadBlock));
    writer->writeDouble(44, ent->arrowHeadSize);
    writer->writeUtf8String(300, ent->textDefault);
    const std::uint32_t textStyle =
        resolveReference(ent->textStyleHandle.ref);
    if (textStyle != 0)
        writer->writeString(342, toHexStr(textStyle));
    ...
    const std::uint32_t block = resolveReference(ent->blockHandle.ref);
    if (block != 0)
        writer->writeString(343, toHexStr(block));
